...
use rexux_realtime_webrtc::Message;
use rexux_realtime_webrtc::VoiceHost;
use rexux_realtime_webrtc::decode_frame;
use rexux_realtime_webrtc::encode_frame;
...
fn spawn() -> Result<Child> {
    Ok(Command::new(cargo_bin("rexux-voice-host")?)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .kill_on_drop(true)
...
async fn build_commit() -> Result<String> {
    let output = timeout(
        DEADLINE,
        Command::new(cargo_bin("rexux-voice-host")?)
            .arg("--build-commit")
            .kill_on_drop(true)
            .output(),
    )
...
async fn installed_client_rejects_mixed_builds_and_missing_helper() -> Result<()> {
    ...
    let helper_dir = directory.path().join("rexux-resources/voice/bin");
    fs::create_dir_all(&bin)?;
    fs::create_dir_all(&helper_dir)?;
    fs::write(directory.path().join("rexux-package.json"), "{}")?;
    let app = bin.join(if cfg!(windows) { "codex.exe" } else { "codex" });
    fs::write(&app, [])?;
    let source = cargo_bin("rexux-voice-host")?;
...
async fn installed_client_accepts_non_utf8_package_path() -> Result<()> {
    ...
    let helper_dir = root.join("rexux-resources/voice/bin");
    fs::create_dir_all(&bin)?;
    fs::create_dir_all(&helper_dir)?;
    fs::write(root.join("rexux-package.json"), "{}")?;
