...
pub enum Command {
    /// Submit a new Rexux Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Rexux Cloud task.
    Status(StatusCommand),
    /// List Rexux Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Rexux Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Rexux Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Rexux Cloud.
    ...
    /// Git branch to run in Rexux Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Rexux Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Rexux Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Rexux Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
