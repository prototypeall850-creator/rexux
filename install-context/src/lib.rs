...
const CODE_MODE_HOST_EXECUTABLE_NAME: &str = if cfg!(windows) {
    "rexux-code-mode-host.exe"
} else {
    "rexux-code-mode-host"
};
const PACKAGE_METADATA_FILENAME: &str = "rexux-package.json";
const PATH_DIRNAME: &str = "rexux-path";
const RELEASES_DIRNAME: &str = "releases";
const RESOURCES_DIRNAME: &str = "rexux-resources";
...
/// Version metadata recorded in a bundled Rexux runtime package.
#[derive(Clone, Debug, Deserialize, Eq, PartialEq)]
pub struct RexuxPackageManifest {
    pub version: Version,
}
...
pub struct InstallContext {
    ...
    pub package_layout: Option<RexuxPackageLayout>,
}
...
pub enum InstallMethod {
    Standalone {
        ...
        /// such as
        /// `~/.rexux/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        /// Package-layout installs use the package root that contains `bin/`,
        /// `rexux-resources/`, and `rexux-path/`.
    ...
    /// A Rexux binary launched through the npm-managed `codex.js` shim.
    Npm,
    /// A Rexux binary launched through the bun-managed `codex.js` shim.
    Bun,
    /// A Rexux binary launched through the pnpm-managed `codex.js` shim.
    Pnpm,
    /// A Rexux binary launched through the Vite+-managed `codex.js` shim.
    VitePlus,
    /// A Rexux binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled Rexux binaries, custom
    /// internal launchers, and tests that execute Rexux from an arbitrary path.
    Other,
}
...
impl InstallContext {
    pub fn from_exe(
        ...
        let rexux_home = rexux_utils_home_dir::find_rexux_home().ok();
        Self::from_exe_with_rexux_home(
            is_macos,
            current_exe,
            method_override,
            rexux_home.as_deref(),
    ...
    fn from_exe_with_rexux_home(
        is_macos: bool,
        current_exe: Option<&Path>,
        method_override: Option<InstallMethod>,
        rexux_home: Option<&Path>,
    ) -> Self {
        let package_layout = current_exe.and_then(RexuxPackageLayout::from_exe);
        let method = if let Some(method) = method_override {
            method
        } else if let Some(exe_path) = current_exe {
            install_method_from_exe(exe_path, rexux_home, package_layout.as_ref(), is_macos)
        } else {
            InstallMethod::Other
        };
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let method_override = if std::env::var_os("REXUX_MANAGED_BY_VITE_PLUS").is_some() {
                Some(InstallMethod::VitePlus)
            } else if std::env::var_os("REXUX_MANAGED_BY_PNPM").is_some() {
                Some(InstallMethod::Pnpm)
            } else if std::env::var_os("REXUX_MANAGED_BY_NPM").is_some() {
                Some(InstallMethod::Npm)
            } else if std::env::var_os("REXUX_MANAGED_BY_BUN").is_some() {
    ...
    pub fn package_manifest(&self) -> Option<RexuxPackageManifest> {
    ...
    pub fn code_mode_host_program(&self) -> PathBuf {
        // prefer the one packed under rexux-resources
        ...
        match exe_dir.file_name() {
            ...
            Some(name) if name == OsStr::new("MacOS") => {
                ...
                if contents.file_name()? != OsStr::new("Contents")
                    || bundle.file_name()? != OsStr::new("RexuxCLI.app")
                {
                    return None;
                }
...
fn install_method_from_exe(
    ...
    if let Some(standalone_method) = standalone_install_method(exe_path, rexux_home, package_layout)
    {
...
fn standalone_install_method(
    ...
    let canonical_rexux_home = canonical_absolute_path(rexux_home?)?;
    ...
    let releases_root = canonical_rexux_home
        .join("packages")
...
mod tests {
    ...
    const TEST_RESOURCE_NAME: &str = "rexux-test-helper";
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        ...
        let context = InstallContext::from_exe_with_rexux_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(&exe_path),
            /*method_override*/ None,
            /*rexux_home*/ Some(rexux_home.path()),
        );
        assert_eq!(
            context,
            InstallContext {
...
"#,
        ...
        let package_layout = RexuxPackageLayout {
    ...
    #[test]
    fn standalone_package_rg_falls_back_when_rexux_path_is_missing() -> std::io::Result<()> {
        let package_dir = tempfile::tempdir()?;
        let bin_dir = package_dir.path().join(BIN_DIRNAME);
        fs::create_dir_all(&bin_dir)?;
        fs::write(package_dir.path().join(PACKAGE_METADATA_FILENAME), "{}")?;
