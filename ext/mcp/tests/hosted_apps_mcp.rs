...
async fn runtime_overlay_preserves_disabled_server() -> TestResult {
    ...
    let config = ConfigBuilder::default()
        ...
        .cli_overrides(vec![
            ("features.apps".to_string(), true.into()),
            (
                "mcp_servers.rexux_apps.url".to_string(),
                "https://example.com/mcp".into(),
            ),
            ("mcp_servers.rexux_apps.enabled".to_string(), false.into()),
        ])
        .build()
        .await?;
    let auth = RexuxAuth::create_dummy_chatgpt_auth_for_testing();
...
async fn later_extension_can_remove_same_name_registration() -> TestResult {
    ...
    builder.mcp_server_contributor(Arc::new(RemoveRexuxApps));
    ...
    assert!(!servers.contains_key(REXUX_APPS_MCP_SERVER_NAME));
    Ok(())
}
...
async fn hosted_apps_mcp_requires_chatgpt_auth() -> TestResult {
    ...
    let config = ConfigBuilder::default()
        ...
        .build()
        .await?;
    let auth = RexuxAuth::from_api_key("test");
    let manager = installed_manager(&config, Some(auth.clone()));
...
fn installed_manager(config: &Config, auth: Option<RexuxAuth>) -> McpManager {
    let mut builder = ExtensionRegistryBuilder::new();
    rexux_mcp_extension::install(&mut builder);
...
struct RemoveRexuxApps;

impl McpServerContributor<Config> for RemoveRexuxApps {
    ...
    ) -> rexux_extension_api::ExtensionFuture<'a, Vec<McpServerContribution>> {
        Box::pin(async move {
