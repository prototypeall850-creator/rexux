...
use rexux_extension_api::TurnAbortInput;
...
pub struct GoalExtension<C> {
    state_dbs: Arc<rexux_state::StateRuntime>,
    ...
    C: Send + Sync + 'static,
{
    fn on_turn_start<'a>(&'a self, input: TurnStartInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            if matches!(
                input.collaboration_mode.mode,
                rexux_protocol::config_types::ModeKind::Plan
            ) {
    ...
    fn on_turn_error<'a>(&'a self, input: TurnErrorInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            let reason = match input.error {
                RexuxErrorInfo::UsageLimitExceeded => ActiveGoalStopReason::UsageLimit,
