...
use rexux_extension_api::JsonToolOutput;
use rexux_extension_api::ToolCall;
use rexux_extension_api::ToolExecutor;
use rexux_extension_api::ToolName;
use rexux_extension_api::ToolOutput;
use rexux_extension_api::ToolSpec;
use rexux_protocol::ThreadId;
use rexux_protocol::protocol::ThreadGoal;
use rexux_protocol::protocol::ThreadGoalStatus;
use rexux_protocol::protocol::validate_thread_goal_objective;
...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<rexux_state::StateRuntime>,
...
impl<'call> ToolExecutor<ToolCall<'call>> for GoalToolExecutor {
    ...
    ) -> rexux_extension_api::ToolExecutorFuture<'a>
    where
        'call: 'a,
    {
        Box::pin(async move {
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => rexux_state::GoalAccountingMode::ActiveOrComplete,
                ThreadGoalStatus::Blocked => rexux_state::GoalAccountingMode::ActiveOrStopped,
    ...
    async fn account_active_goal_progress(
        &self,
        mode: rexux_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            rexux_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
                self.analytics
            ...
            rexux_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<rexux_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &rexux_state::StateRuntime,
    thread_id: ThreadId,
...
pub(crate) fn protocol_goal_from_state(goal: rexux_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn protocol_status_from_state(status: rexux_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(
    status: ThreadGoalStatus,
) -> rexux_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => rexux_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => rexux_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::Blocked => rexux_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => rexux_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => rexux_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => rexux_state::ThreadGoalStatus::Complete,
    }
}
