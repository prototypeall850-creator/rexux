...
use rexux_extension_api::ThreadResumeInput;
use rexux_extension_api::ThreadStartInput;
use rexux_extension_api::ThreadStopInput;
use rexux_extension_api::ToolCall;
use rexux_extension_api::ToolCallOutcome;
use rexux_extension_api::ToolCallSource;
use rexux_extension_api::ToolExecutor;
use rexux_extension_api::ToolFinishInput;
use rexux_extension_api::ToolPayload;
use rexux_extension_api::TurnErrorInput;
use rexux_extension_api::TurnStartInput;
use rexux_extension_api::TurnStopInput;
use rexux_goal_extension::GoalExtensionConfig;
use rexux_goal_extension::GoalObjectiveUpdate;
use rexux_goal_extension::GoalRuntimeHandle;
use rexux_goal_extension::GoalService;
use rexux_goal_extension::GoalSetRequest;
use rexux_goal_extension::GoalTokenBudgetUpdate;
use rexux_goal_extension::install_with_backend;
...
use rexux_protocol::protocol::ThreadGoalStatus;
...
async fn spawned_descendant_usage_exhausts_root_goal_budget_once() -> anyhow::Result<()> {
    ...
    assert_eq!(rexux_state::ThreadGoalStatus::BudgetLimited, goal.status);
    Ok(())
}
...
async fn turn_error_usage_limit_accounts_progress_and_clears_accounting() -> anyhow::Result<()> {
    ...
    harness
        ...
        .await;
    harness
        .notify_turn_error("turn-1", RexuxErrorInfo::UsageLimitExceeded)
        .await;
    ...
    let goal = runtime
        ...
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(23, goal.tokens_used);
    assert_eq!(rexux_state::ThreadGoalStatus::UsageLimited, goal.status);
    assert_eq!(
        vec![
            CapturedGoalEvent {
...
async fn turn_error_blocks_goal() -> anyhow::Result<()> {
    ...
    harness
        .notify_turn_error("turn-1", RexuxErrorInfo::Other)
        .await;
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(rexux_state::ThreadGoalStatus::Blocked, goal.status);
    Ok(())
}
        ...
        [(None, None, 3), (Some(2), None, 5), (None, Some(2), 4)]
    {
        ...
        for turn in 1..=blocking_turn {
            ...
            let expected = if turn == blocking_turn {
                ...
                rexux_state::ThreadGoalStatus::Active
            };
...
async fn usage_limit_plan_turn_does_not_stop_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(rexux_state::ThreadGoalStatus::Active, goal.status);
...
async fn installed_tools(
    runtime: Arc<rexux_state::StateRuntime>,
    thread_id: ThreadId,
...
struct GoalExtensionHarness {
    registry: Arc<rexux_extension_api::ExtensionRegistry<()>>,
...
impl GoalExtensionHarness {
    ...
    async fn notify_tool_finish_with_outcome(
        ...
        let turn_store = ExtensionData::new(turn_id);
        let tool_name = rexux_extension_api::ToolName::plain(tool_name);
    ...
    async fn notify_turn_error(&self, turn_id: &str, error: RexuxErrorInfo) {
        let turn_store = ExtensionData::new(turn_id);
        for contributor in self.registry.turn_lifecycle_contributors() {
            contributor
...
fn tool_call(tool_name: &str, call_id: &str, arguments: serde_json::Value) -> ToolCall<'static> {
    ToolCall {
        ...
        tool_name: rexux_extension_api::ToolName::plain(tool_name),
        model: "gpt-test".to_string(),
        rexux_turn_metadata: None,
        truncation_policy: TruncationPolicy::Bytes(1024),
        source: ToolCallSource::Direct,
        conversation_history: rexux_extension_api::ConversationHistory::default(),
...
async fn test_runtime() -> anyhow::Result<Arc<rexux_state::StateRuntime>> {
    let tempdir = TempDir::new()?;
    rexux_state::StateRuntime::init(
        rexux_state::SqliteConfig::new_for_testing(tempdir.keep().as_path().abs()),
...
async fn seed_thread_metadata(
    runtime: &rexux_state::StateRuntime,
    thread_id: ThreadId,
) -> anyhow::Result<()> {
    let builder = rexux_state::ThreadMetadataBuilder::new(
        thread_id,
        runtime
            .sqlite()
            .home()
...
fn protocol_status(status: rexux_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
        rexux_state::ThreadGoalStatus::Active => ThreadGoalStatus::Active,
        rexux_state::ThreadGoalStatus::Paused => ThreadGoalStatus::Paused,
        rexux_state::ThreadGoalStatus::Blocked => ThreadGoalStatus::Blocked,
        rexux_state::ThreadGoalStatus::UsageLimited => ThreadGoalStatus::UsageLimited,
        rexux_state::ThreadGoalStatus::BudgetLimited => ThreadGoalStatus::BudgetLimited,
        rexux_state::ThreadGoalStatus::Complete => ThreadGoalStatus::Complete,
    }
}
