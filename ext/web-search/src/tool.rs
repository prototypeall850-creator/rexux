...
use rexux_api::SearchClient;
use rexux_api::SearchCommands;
use rexux_api::SearchQuery;
use rexux_api::SearchRequest;
use rexux_api::SearchSettings;
use rexux_core::X_REXUX_TURN_METADATA_HEADER;
use rexux_core::web_search_action_detail;
...
use rexux_extension_api::parse_tool_input_schema_without_compaction;
use rexux_extension_items::ExtensionItem;
use rexux_extension_items::web_search::WebSearchAction;
...
impl WebSearchTool {
    async fn handle_call(
        ...
        let extra_headers = search_request_headers(
            ...
            call.rexux_turn_metadata.as_deref(),
        );
        ...
        if let Some(results) = results.as_ref()
            && let Some(metrics) = rexux_otel::global()
...
fn search_request_headers(originator: Option<&str>, turn_metadata: Option<&str>) -> HeaderMap {
    ...
    {
        headers.insert(X_REXUX_TURN_METADATA_HEADER, header_value);
    }
