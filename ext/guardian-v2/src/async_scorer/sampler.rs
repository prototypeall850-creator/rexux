...
use rexux_api::ResponsesWebsocketConnection;
...
const RESPONSES_LITE_METADATA_KEY: &str =
    "ws_request_header_x_openai_internal_rexux_responses_lite";
...
impl LunaSampler {
    ...
    async fn responses_endpoint(&self) -> ResponsesEndpoint {
        ...
        if self.config.free_guardian
            ...
            && provider.supports_rexux_backend_routes()
        ...
        if !request.trusted_review_evidence.is_empty() {
            input.push(ResponseItem::Message {
                ...
                content: std::iter::once(ContentItem::InputText {
                    text: "Trusted synchronous Guardian reviews supplied by Rexux. Decisions \
