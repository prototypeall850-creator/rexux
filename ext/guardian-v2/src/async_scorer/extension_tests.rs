...
use rexux_core::context::ContextualUserFragment;
use rexux_core::context::InternalContextSource;
use rexux_core::context::InternalModelContextFragment;
...
use rexux_protocol::openai_models::GuardianV2ModelConfig;
use rexux_protocol::openai_models::GuardianV2TranscriptModelConfig;
...
use rexux_protocol::security_risk::SecurityRiskScore;
...
impl ExternalAuth for RefreshableAuth {
    fn resolve(&self) -> ExternalAuthFuture<'_, RexuxAuth> {
        Box::pin(async { Ok(RexuxAuth::from_api_key(*self.0.lock().expect("auth"))) })
    }

    fn refresh(&self, _: ExternalAuthRefreshContext) -> ExternalAuthFuture<'_, RexuxAuth> {
...
async fn installed_extension_reconnects_after_auth_refresh() -> Result<()> {
    ...
    let auth_manager = AuthManager::from_auth_for_testing(RexuxAuth::from_api_key("original"));
    auth_manager
...
) -> Result<(serde_json::Value, TestRexux, ExtensionRegistry<Config>)> {
...
async fn sample_configured_conversation_history_with_source(
    ...
    let builder = test_rexux()
        ...
        .with_model_info_override("rexux-auto-review", |model_info| {
            model_info
                .model_messages
                .as_mut()
...
async fn contributor_samples_tool_calls_with_the_existing_luna_pool() -> Result<()> {
    ...
    assert_eq!(
        ...
        json!([
            ...
            {
                "type": "input_text",
                "text": "The Rexux agent has requested the following action:\n"
            },
