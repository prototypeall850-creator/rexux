...
fn renders_verified_skill_paths() {
    assert_eq!(
        rendered_paths(vec!["/home/user/.rexux/skills/demo/SKILL.md".to_owned()]),
        vec!["/home/user/.rexux/skills/demo/SKILL.md"],
    );
}
...
fn bounds_escaped_skill_paths_without_corrupting_json_or_utf8() {
    let paths = (0..MAX_TRUSTED_SKILLS)
        .map(|index| {
            format!(
                "/home/user/.rexux/skills/{index:03}/{}SKILL.md",
                "\u{0001}é".repeat(80)
            )
        })
        .collect::<Vec<_>>();
...
fn preserves_multiple_invoked_skill_paths() {
    assert_eq!(
        rendered_paths(vec![
            "/home/user/.rexux/skills/first/SKILL.md".to_owned(),
            "/home/user/.rexux/skills/second/SKILL.md".to_owned(),
        ]),
        vec![
            "/home/user/.rexux/skills/first/SKILL.md",
            "/home/user/.rexux/skills/second/SKILL.md",
        ],
    );
}
...
async fn trusts_only_user_owned_skill_roots() -> Result<()> {
    ...
    let rexux_skill = test.home.path().join("skills/trusted/SKILL.md");
    ...
    for path in [&rexux_skill, &agents_skill, &repo_skill] {
    ...
    for trusted_path in [&rexux_skill, &agents_skill] {
        assert_eq!(
...
fn invoked_skill_paths_are_deduplicated_and_bounded() {
    ...
    for index in 0..MAX_TRUSTED_SKILLS.saturating_mul(2) {
        let path = format!("/home/user/.rexux/skills/{index:03}/SKILL.md");
    ...
    assert_eq!(
        ...
        (0..MAX_TRUSTED_SKILLS)
            .map(|index| format!("/home/user/.rexux/skills/{index:03}/SKILL.md"))
            .collect::<Vec<_>>()
    );
