...
use rexux_api::ImageResponse;
use rexux_api::ImagesClient;
...
const X_REXUX_IMAGE_TURN_ID_HEADER: &str = "x-codex-image-turn-id";
...
impl ImageBackendError {
    fn from_api(error: rexux_api::ApiError) -> Self {
        let message = error.to_string();
        Self {
            message,
            rexux_error: map_api_error(error),
        }
    }

    fn from_message(message: String) -> Self {
        Self {
            rexux_error: RexuxErr::Stream(message.clone()),
            message,
        }
    }
    ...
    pub(crate) fn rexux_error(&self) -> &RexuxErr {
        &self.rexux_error
    }
}

#[derive(Clone)]
pub(crate) struct RexuxImagesBackend {
    provider: SharedModelProvider,
    originator: Option<String>,
}

impl RexuxImagesBackend {
...
fn image_request_headers(originator: Option<&str>, turn_id: &str) -> HeaderMap {
    ...
    if let Ok(turn_id) = HeaderValue::from_str(turn_id) {
        headers.insert(X_REXUX_IMAGE_TURN_ID_HEADER, turn_id);
    }
