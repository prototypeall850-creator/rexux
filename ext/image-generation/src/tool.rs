...
use rexux_api::ImageBackground;
use rexux_api::ImageEditRequest;
use rexux_api::ImageGenerationRequest;
use rexux_api::ImageQuality;
use rexux_api::ImageUrl;
use rexux_exec_server::CreateDirectoryOptions;
use rexux_exec_server::LOCAL_FS;
use rexux_extension_api::ExtensionTurnItem;
use rexux_extension_api::FunctionCallError;
use rexux_extension_api::ToolCall;
use rexux_extension_api::ToolEnvironment;
...
use rexux_extension_api::parse_tool_input_schema;
...
use rexux_utils_image::PromptImageMode;
use rexux_utils_image::load_for_prompt_bytes;
...
use crate::backend::RexuxImagesBackend;
...
impl<'call> ToolExecutor<ToolCall<'call>> for ImageGenerationTool {
    ...
    fn handle<'a>(&'a self, call: ToolCall<'call>) -> rexux_extension_api::ToolExecutorFuture<'a>
    where
        'call: 'a,
    {
    ...
    async fn handle_call(
        ...
        .map_err(|error| {
            (
                ...
                usage_limit_failure(error.rexux_error()),
            )
        })
...
fn usage_limit_failure(error: &RexuxErr) -> Option<ImageGenerationFailure> {
    let RexuxErrorDetails::UsageLimitReached(usage_limit) = error.details() else {
        return None;
    };
