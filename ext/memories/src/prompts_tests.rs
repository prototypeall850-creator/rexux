...
async fn build_memory_tool_developer_instructions_renders_embedded_template() {
    ...
    let rexux_home = AbsolutePathBuf::from_absolute_path(temp.path()).unwrap();
    let memories_dir = rexux_home.join("memories");
    ...
    let instructions = build_memory_tool_developer_instructions(&rexux_home)
        .await
        .unwrap();
