...
use rexux_exec_server::CapabilityRootDiscovery;
use rexux_exec_server::ExecutorCapabilityDiscoverySnapshot;
use rexux_exec_server::LOCAL_FS;
use rexux_extension_api::ConversationHistory;
use rexux_extension_api::ExtensionData;
use rexux_extension_api::ExtensionEventSink;
use rexux_extension_api::ExtensionMetrics;
use rexux_extension_api::ExtensionRegistryBuilder;
use rexux_extension_api::ExtensionWarning;
use rexux_extension_api::FunctionCallError;
use rexux_extension_api::NoopTurnItemEmitter;
use rexux_extension_api::PreviousWorldStateSection;
use rexux_extension_api::RenderedWorldStateFragment;
use rexux_extension_api::SkillInvocationInput;
use rexux_extension_api::SkillInvocationKind;
use rexux_extension_api::ThreadStartInput;
use rexux_extension_api::ToolCall;
use rexux_extension_api::ToolCallSource;
use rexux_extension_api::ToolPayload;
use rexux_extension_api::TurnInputContext;
use rexux_extension_api::WorldStateContributionInput;
use rexux_extension_api::WorldStateSectionContribution;
use rexux_models_manager::model_info::model_info_from_slug;
use rexux_otel::MetricsClient;
use rexux_otel::MetricsConfig;
use rexux_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use rexux_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use rexux_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use rexux_otel::THREAD_SKILLS_TRUNCATED_METRIC;
...
use rexux_protocol::protocol::SKILLS_INSTRUCTIONS_CLOSE_TAG;
use rexux_protocol::protocol::SKILLS_INSTRUCTIONS_OPEN_TAG;
use rexux_protocol::protocol::SessionSource;
use rexux_protocol::protocol::SkillScope;
use rexux_protocol::protocol::TruncationPolicy;
use rexux_protocol::protocol::TurnEnvironmentSelection;
use rexux_protocol::user_input::UserInput;
use rexux_skills::SkillMetadata;
...
use rexux_skills_extension::HostSkillsSnapshot;
use rexux_skills_extension::InjectedHostSkillPrompts;
use rexux_skills_extension::SkillLoadOutcome;
...
use rexux_skills_extension::install_with_providers_and_metrics;
use rexux_skills_extension::provider::SkillListQuery;
use rexux_skills_extension::provider::SkillProvider;
...
static NEXT_REXUX_HOME_ID: AtomicUsize = AtomicUsize::new(0);
...
async fn skill_world_state_fragments(
    registry: &rexux_extension_api::ExtensionRegistry<TestConfig>,
    ...
    let sections = registry.context_contributors()[0]
        .contribute_world_state(WorldStateContributionInput {
            thread_id: rexux_protocol::ThreadId::new(),
            turn_id,
            environments: &[],
...
async fn installed_extension_uses_host_service_snapshot() -> TestResult {
    let rexux_home = test_rexux_home();
    let skill_path = rexux_home.join("skills").join("demo").join("SKILL.md");
    ...
    std::fs::remove_dir_all(rexux_home)?;
    Ok(())
}
...
async fn host_world_state_records_catalog_metrics_on_publish_and_change() -> TestResult {
    ...
    let skill_path = AbsolutePathBuf::try_from(test_rexux_home().join("skills/demo/SKILL.md"))?;
    let mut outcome = SkillLoadOutcome::default();
    outcome.skills.push(SkillMetadata {
        name: "demo".to_string(),
        description: "Demo skill.".to_string(),
    ...
    let second_skill_path =
        AbsolutePathBuf::try_from(test_rexux_home().join("skills/other/SKILL.md"))?;
    outcome.skills.push(SkillMetadata {
...
async fn host_catalog_compacts_shared_paths_under_budget_pressure() -> TestResult {
    ...
    let rexux_home = AbsolutePathBuf::try_from(test_root.clone())?;
    ...
    let snapshot = service
        .snapshot_for_config(
            &HostSkillsLoadInput::new(rexux_home, Vec::new(), config_layer_stack),
...
fn test_rexux_home() -> PathBuf {
    let id = NEXT_REXUX_HOME_ID.fetch_add(1, Ordering::Relaxed);
    std::env::temp_dir().join(format!(
        "rexux-skills-extension-test-{}-{id}",
        std::process::id(),
    ))
}
