...
use rexux_extension_api::ThreadIdleInput;
...
use rexux_queue_extension::QueueServiceError;
...
impl ThreadLifecycleContributor<rexux_core::config::Config> for InstalledQueue {
...
fn registered_queue_extensions() -> (
    Arc<InstalledQueue>,
    Arc<ExtensionRegistry<rexux_core::config::Config>>,
) {
...
fn loaded_thread_queue(test: &TestRexux) -> anyhow::Result<Arc<dyn QueueStore>> {
...
async fn starting_a_selected_item_while_active_leaves_it_queued() -> anyhow::Result<()> {
    ...
    .await;
    let test = test_rexux().build_with_streaming_server(&server).await?;
    let thread_id = test.session_configured.thread_id;
...
-> anyhow::Result<()> {
    ...
    let mut watcher_extensions = ExtensionRegistryBuilder::<rexux_core::config::Config>::new();
    rexux_queue_extension::install(&mut watcher_extensions, Arc::clone(&queue));
...
-> anyhow::Result<()> {
    ...
    let hook_log = std::fs::read_to_string(test.rexux_home_path().join("queue_prompt_hook.log"))?;
    assert_eq!(
        vec!["A", "blocked", "C"],
