...
use rexux_mcp::ResolvedMcpServer;
use rexux_protocol::mcp::CallToolResult;
use rexux_tools::ToolCallSource;
...
pub enum McpToolSource {
    /// A connector routed through the host-owned Rexux Apps MCP server.
    Connector,
    /// An MCP server whose frozen registration matches the active Rexux configuration.
    Config,
