...
`rexux-network-proxy` is Rexux's local network policy enforcement proxy. It runs:
...
`rexux-network-proxy` reads from Rexux's merged `config.toml` (via `rexux-core` config loading).
...
# bundle env vars pointing at immutable public files under $REXUX_HOME/proxy/ so common HTTPS
...
```bash
cargo run -p rexux-network-proxy --
```
...
`rexux-network-proxy` can be embedded as a library with a thin API:

```rust
use rexux_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `rexux-network-proxy` is embedded in managed Rexux runtime, policy decisions emit structured
OTEL-compatible events with `target=rexux_otel.network_proxy`.

Event name:

- `rexux.network_proxy.policy_decision`
...
This section documents the protections implemented by `rexux-network-proxy`, and the boundaries of
