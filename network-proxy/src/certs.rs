...
fn managed_ca_dir() -> Result<PathBuf> {
    let rexux_home =
        find_rexux_home().context("failed to resolve REXUX_HOME for managed MITM CA")?;
    Ok(rexux_home.join(MANAGED_MITM_CA_DIR).to_path_buf())
}
...
/// Returns whether `path` points at a current Rexux-generated MITM CA bundle.
