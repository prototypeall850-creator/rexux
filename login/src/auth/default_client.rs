//! Default Rexux HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! `HttpClient` construction.
//!
//! Use [`crate::default_client`] or [`rexux_login::default_client`] from other crates in this
//! workspace.

use rexux_http_client::BuildRouteAwareHttpClientError;
...
/// Parenthesis will be added by Rexux. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "codex_cli_rs";
pub const REXUX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "REXUX_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-openai-internal-codex-residency";

pub use rexux_config::ResidencyRequirement;
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(REXUX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(REXUX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        ...
        || originator_value.starts_with("Rexux ")
}
...
pub fn get_rexux_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Rexux user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Rexux user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Rexux originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
/// Rexux's route-aware system/PAC resolution.
...
/// Builds the default Rexux HTTP client for a concrete outbound route.
///
/// When route-aware proxy handling is disabled, or the client is running inside the Rexux
...
/// Builds the default Rexux HTTP client for a concrete outbound route without blocking the
...
/// Builds an HTTP client for an auth endpoint without Rexux default headers.
...
/// Builds the default Rexux HTTP client wrapper for an auth endpoint.
...
pub fn default_headers() -> HeaderMap {
    ...
    if let Ok(user_agent) = HeaderValue::from_str(&get_rexux_user_agent()) {
...
fn is_sandboxed() -> bool {
    std::env::var("REXUX_SANDBOX").as_deref() == Ok("seatbelt")
}
