...
fn bedrock_api_key_debug_redacts_secret() {
    ...
    assert_eq!(
        format!("{:?}", RexuxAuth::BedrockApiKey(auth)),
...
async fn login_with_bedrock_api_key_replaces_openai_auth() -> anyhow::Result<()> {
    ...
    assert_eq!(
        auth_manager.auth_cached().and_then(|auth| match auth {
            RexuxAuth::BedrockApiKey(auth) => Some(auth),
            RexuxAuth::ApiKey(_)
            | RexuxAuth::Chatgpt(_)
            ...
            | RexuxAuth::BedrockAccessKeys(_) => None,
        }),
        Some(bedrock_auth())
    );
    Ok(())
...
async fn access_keys_auth_round_trips_and_logs_out() -> anyhow::Result<()> {
    ...
    assert_eq!(
        auth_manager.auth_cached().and_then(|auth| match auth {
            RexuxAuth::BedrockAccessKeys(auth) => Some(auth),
            RexuxAuth::ApiKey(_)
            | RexuxAuth::Chatgpt(_)
            | RexuxAuth::ChatgptAuthTokens(_)
            | RexuxAuth::Headers(_)
            | RexuxAuth::AgentIdentity(_)
            | RexuxAuth::PersonalAccessToken(_)
            | RexuxAuth::BedrockApiKey(_) => None,
        }),
