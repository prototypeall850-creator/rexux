...
use rexux_agent_identity::AgentIdentityJwtClaims;
use rexux_agent_identity::decode_agent_identity_jwt;
use rexux_config::types::AuthCredentialsStoreMode;
pub use rexux_config::types::AuthKeyringBackendKind;
use rexux_keyring_store::DefaultKeyringStore;
use rexux_keyring_store::KeyringStore;
use rexux_protocol::account::PlanType as AccountPlanType;
use rexux_protocol::auth::AuthMode;
use rexux_secrets::LocalSecretsNamespace;
use rexux_secrets::SecretName;
use rexux_secrets::SecretScope;
use rexux_secrets::SecretsBackendKind;
use rexux_secrets::SecretsManager;
use once_cell::sync::Lazy;

/// Expected structure for $REXUX_HOME/auth.json.
...
pub(super) fn get_auth_file(rexux_home: &Path) -> PathBuf {
    rexux_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(rexux_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(rexux_home: PathBuf) -> Self {
        Self { rexux_home }
    }

    /// Attempt to read and parse the `auth.json` file in the given `REXUX_HOME` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.rexux_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.rexux_home)
    }
}

static REXUX_AUTH_SECRET_NAME: Lazy<SecretName> =
    Lazy::new(|| match SecretName::new("REXUX_AUTH") {
        Ok(name) => name,
        Err(err) => unreachable!("REXUX_AUTH should be a valid secret name: {err}"),
    });
const KEYRING_SERVICE: &str = "Rexux Auth";

// turns rexux_home path into a stable, short key string
fn compute_store_key(rexux_home: &Path) -> std::io::Result<String> {
    let canonical = rexux_home
        .canonicalize()
        .unwrap_or_else(|_| rexux_home.to_path_buf());
...
impl DirectKeyringAuthStorage {
    fn new(rexux_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            rexux_home,
            keyring_store,
        }
...
impl AuthStorageBackend for DirectKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.rexux_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.rexux_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.rexux_home)?;
...
impl SecretsKeyringAuthStorage {
    fn new(rexux_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        let direct_storage =
            DirectKeyringAuthStorage::new(rexux_home.clone(), Arc::clone(&keyring_store));
        let secrets_manager = SecretsManager::new_with_keyring_store_and_namespace(
            rexux_home.clone(),
            SecretsBackendKind::Local,
            keyring_store,
            LocalSecretsNamespace::RexuxAuth,
        );
        Self {
            rexux_home,
            direct_storage,
...
impl AuthStorageBackend for SecretsKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        match self
            .secrets_manager
            .get(&SecretScope::Global, &REXUX_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        self.secrets_manager
            .set(&SecretScope::Global, &REXUX_AUTH_SECRET_NAME, &serialized)
            .map_err(|err| {
                let message =
    ...
    fn delete(&self) -> std::io::Result<bool> {
        let keyring_removed = self
            .secrets_manager
            .delete(&SecretScope::Global, &REXUX_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
...
impl AutoAuthStorage {
    fn new(
        ...
        Self {
            ...
            file_storage: Arc::new(FileAuthStorage::new(rexux_home)),
        }
    }
}
...
// A global in-memory store for mapping rexux_home -> AuthDotJson.
...
impl EphemeralAuthStorage {
    fn new(rexux_home: PathBuf) -> Self {
        Self { rexux_home }
    }
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_store(rexux_home, mode, keyring_store, keyring_backend_kind)
}
...
fn create_auth_storage_with_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(rexux_home)),
        AuthCredentialsStoreMode::Keyring => {
            create_keyring_auth_storage(rexux_home, keyring_store, keyring_backend_kind)
        }
        ...
        AuthCredentialsStoreMode::Ephemeral => Arc::new(EphemeralAuthStorage::new(rexux_home)),
    }
}
...
fn create_keyring_auth_storage(
    ...
    match keyring_backend_kind {
        AuthKeyringBackendKind::Direct => {
            Arc::new(DirectKeyringAuthStorage::new(rexux_home, keyring_store))
        }
        AuthKeyringBackendKind::Secrets => {
            Arc::new(SecretsKeyringAuthStorage::new(rexux_home, keyring_store))
        }
    }
}
