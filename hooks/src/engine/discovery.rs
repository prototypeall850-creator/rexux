...
use rexux_config::version_for_toml;
use rexux_plugin::PluginHookSource;
use rexux_plugin::is_allowlisted_bundled_cleanup_hook;
...
fn config_toml_source_path(layer: &ConfigLayerEntry) -> AbsolutePathBuf {
    match &layer.name {
        ...
        ConfigLayerSource::Project { dot_rexux_folder } => layer
            .hooks_config_folder()
            .unwrap_or_else(|| dot_rexux_folder.clone())
...
fn append_matcher_groups(
    ...
    event_name: rexux_protocol::protocol::HookEventName,
            ...
            let normalized = match handler {
                HookHandlerConfig::Command {
                    ...
                    let additional_context_limit = if matches!(
                        event_name,
                        rexux_protocol::protocol::HookEventName::PreToolUse
                            | rexux_protocol::protocol::HookEventName::PostToolUse
                            | rexux_protocol::protocol::HookEventName::SessionStart
                            | rexux_protocol::protocol::HookEventName::UserPromptSubmit
                            | rexux_protocol::protocol::HookEventName::SubagentStart
                    ) {
                        additional_context_limit
                    } else {
...
mod tests {
    ...
    fn hook_metadata_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.rexux/config.toml").abs();
        let dot_rexux_folder = test_path_buf("/tmp/worktree/.codex").abs();
