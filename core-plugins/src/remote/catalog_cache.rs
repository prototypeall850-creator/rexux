...
pub(crate) fn load_cached_directory_plugins(
    ...
    let cache_path = cache_path(rexux_home, &cache_key);
...
pub(crate) fn write_cached_directory_plugins(
    ...
    let _ = rexux_utils_path::write_atomically(&cache_path, &contents);
}
...
fn cache_path(rexux_home: &Path, cache_key: &RemotePluginCatalogCacheKey) -> PathBuf {
