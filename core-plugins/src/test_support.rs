...
pub(crate) fn test_plugins_manager(rexux_home: PathBuf) -> PluginsManager {
    PluginsManager::new(
        rexux_home,
...
pub(crate) fn test_auth_manager(auth_mode: Option<AuthMode>) -> Arc<AuthManager> {
    auth_manager_from_optional_auth(test_rexux_auth(auth_mode))
}

pub(crate) async fn set_test_auth_mode(auth_manager: &AuthManager, auth_mode: Option<AuthMode>) {
    set_test_auth(auth_manager, test_rexux_auth(auth_mode)).await;
}

pub(crate) async fn set_test_auth(auth_manager: &AuthManager, auth: Option<RexuxAuth>) {
...
fn test_rexux_auth(auth_mode: Option<AuthMode>) -> Option<RexuxAuth> {
    auth_mode.map(|auth_mode| match auth_mode {
        AuthMode::ApiKey => RexuxAuth::from_api_key("test-api-key"),
        AuthMode::Chatgpt => RexuxAuth::create_dummy_chatgpt_auth_for_testing(),
        AuthMode::ChatgptAuthTokens => RexuxAuth::from_external_chatgpt_tokens(
            "header.e30.test",
            "test-account",
            /*chatgpt_plan_type*/ None,
        )
        .expect("test ChatGPT tokens should parse"),
        AuthMode::Headers => RexuxAuth::Headers(AuthHeaders::new(http::HeaderMap::new())),
        AuthMode::BedrockApiKey => RexuxAuth::BedrockApiKey(BedrockApiKeyAuth {
            api_key: "test-api-key".to_string(),
            region: "us-east-1".to_string(),
        }),
        AuthMode::BedrockAccessKeys => RexuxAuth::BedrockAccessKeys(BedrockAccessKeysAuth {
        ...
        AuthMode::AgentIdentity | AuthMode::PersonalAccessToken => {
            panic!("test auth mode requires a purpose-built RexuxAuth")
        }
    })
}
...
pub(crate) fn write_curated_plugin_sha_with(rexux_home: &Path, sha: &str) {
    write_file(&rexux_home.join(".tmp/plugins.sha"), &format!("{sha}\n"));
}

pub(crate) async fn load_plugins_config(rexux_home: &Path, cwd: &Path) -> PluginsConfigInput {
    let rexux_home = AbsolutePathBuf::try_from(rexux_home).expect("codex home should be absolute");
