    ...
    assert_eq!(
        ...
        ResolvedMarketplacePlugin {
            ...
            source: MarketplacePluginSource::Npm {
                package: "@acme/rexux-plugin".to_string(),
...
fn list_marketplaces_supports_repo_root_local_plugin_sources() {
    for path in [".", "./"] {
        ...
        fs::create_dir_all(repo_root.join(".git")).unwrap();
        fs::create_dir_all(repo_root.join(".agents/plugins")).unwrap();
        fs::create_dir_all(repo_root.join(".rexux-plugin")).unwrap();
        fs::write(
            repo_root.join(".agents/plugins/marketplace.json"),
            format!(
                r#"{{
  ...
  "plugins": [
    {{
      ...
      "source": {{
        ...
        .unwrap();
        fs::write(
            repo_root.join(".rexux-plugin/plugin.json"),
            r#"{
  "name":"repo-root-plugin",
  "interface": {
...
fn list_marketplaces_resolves_plugin_interface_paths_to_absolute() {
  ...
  "interface": {
    ...
    assert_eq!(
        ...
        Some(vec![Product::Rexux, Product::Chatgpt, Product::Atlas])
    );
    assert_eq!(
