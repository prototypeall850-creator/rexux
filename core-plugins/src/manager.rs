...
use crate::loader::configured_curated_plugin_ids_from_rexux_home;
...
use rexux_config::clear_user_plugin;
use rexux_config::set_user_plugin_enabled;
use rexux_config::skill_config_rules_from_stack;
use rexux_config::types::PluginConfig;
use rexux_config::types::ToolSuggestDisabledTool;
use rexux_config::types::ToolSuggestDiscoverableType;
use rexux_hooks::plugin_hook_declarations;
...
use rexux_plugin::PluginIdError;
use rexux_plugin::PluginTelemetryMetadata;
use rexux_plugin::app_connector_ids_from_declarations;
use rexux_plugin::prompt_safe_plugin_description;
...
use rexux_skills::SkillRootLoader;
...
use rexux_utils_plugins::PluginIdentity;
...
impl RemoteInstalledPluginsAuthIdentity {
    fn from_auth(auth: Option<&RexuxAuth>) -> Self {
        Self {
            auth_mode: auth.map(RexuxAuth::api_auth_mode),
            account_id: auth.and_then(RexuxAuth::get_account_id),
            chatgpt_user_id: auth.and_then(RexuxAuth::get_chatgpt_user_id),
            is_workspace_account: auth.map(RexuxAuth::is_workspace_account),
        }
    }
}
...
impl RemoteCatalogCacheRefreshRequest {
    fn has_same_cache_identity(&self, other: &Self) -> bool {
        self.service_config == other.service_config
            && self.auth.as_ref().and_then(RexuxAuth::get_account_id)
                == other.auth.as_ref().and_then(RexuxAuth::get_account_id)
            && self.auth.as_ref().and_then(RexuxAuth::get_chatgpt_user_id)
                == other.auth.as_ref().and_then(RexuxAuth::get_chatgpt_user_id)
            && self.auth.as_ref().map(RexuxAuth::is_workspace_account)
                == other.auth.as_ref().map(RexuxAuth::is_workspace_account)
    }
}
...
fn featured_plugin_ids_cache_key(
    ...
    FeaturedPluginIdsCacheKey {
        ...
        account_id: auth.and_then(RexuxAuth::get_account_id),
        chatgpt_user_id: auth.and_then(RexuxAuth::get_chatgpt_user_id),
        is_workspace_account: auth.is_some_and(RexuxAuth::is_workspace_account),
    }
}
...
fn target_curated_marketplace(auth_mode: Option<AuthMode>) -> TargetCuratedMarketplace {
    if auth_mode.is_some_and(AuthMode::uses_rexux_backend) {
...
impl PluginsManager {
    ...
    pub fn new_with_options(
        ...
        // Product restrictions are enforced at marketplace admission time for a given REXUX_HOME:
        ...
        // contents of that REXUX_HOME and does not re-filter configured plugins by product, so
        // already-admitted plugins may continue exposing MCP servers/tools from shared local state.
        //
        // This assumes a single REXUX_HOME is only used by one product.
        let remote_installed_plugin_bundle_sync_gate =
            crate::remote::remote_installed_plugin_bundle_sync_gate(&rexux_home);
        Self {
            rexux_home: rexux_home.clone(),
            store: PluginStore::new(rexux_home),
    ...
    fn remote_global_catalog_active(&self, config: &PluginsConfigInput) -> bool {
        config.remote_plugin_enabled && self.auth_mode().is_some_and(AuthMode::uses_rexux_backend)
    }
    ...
    pub fn cached_global_remote_discoverable_plugins_for_config(
        ...
        let Some(auth) = auth.filter(|auth| auth.uses_rexux_backend()) else {
            return Vec::new();
        };
    ...
    fn begin_remote_installed_plugins_reconcile(&self, auth: Option<&RexuxAuth>) -> Option<u64> {
    ...
    async fn install_resolved_plugin(
        ...
        let plugin_version =
            if is_openai_curated_marketplace_name(&resolved.plugin_id.marketplace_name) {
                let curated_plugin_version = read_curated_plugins_sha(self.rexux_home.as_path())
                    .ok_or_else(|| {
        ...
        let rexux_home = self.rexux_home.clone();
        ...
        let result: StorePluginInstallResult = tokio::task::spawn_blocking(move || {
            let materialized =
                materialize_marketplace_plugin_source(rexux_home.as_path(), &resolved.source)
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        ...
        clear_user_plugin(&self.rexux_home, plugin_id.as_key())
            .await
            .map_err(anyhow::Error::from)?;
    ...
    pub async fn read_plugin_detail_for_marketplace_plugin(
        ...
        let source_path = if plugin.source.is_install_materialized() && plugin.installed {
            ...
            let materialized = tokio::task::spawn_blocking(move || {
                materialize_marketplace_plugin_source(rexux_home.as_path(), &source)
            })
            .await
            .map_err(|err| {
                MarketplaceError::InvalidPlugin(format!(
        ...
        let loaded_manifest =
            if rexux_utils_plugins::find_plugin_manifest_path(source_path.as_path()).is_some() {
    ...
    fn start_curated_repo_sync(
        ...
        if let Err(err) = std::thread::Builder::new()
            ...
            .spawn(move || {
                match sync_openai_plugins_repo(rexux_home.as_path(), http_client_factory) {
                    Ok(curated_plugin_version) => {
                        let configured_curated_plugin_ids =
                            configured_curated_plugin_ids_from_rexux_home(rexux_home.as_path());
    ...
    fn configured_plugin_states(&self, config: &PluginsConfigInput) -> ConfiguredPluginStates {
        let configured_plugins =
            configured_plugins_from_stack(&config.config_layer_stack, self.rexux_home.as_path());
        ...
        let curated_marketplace_path = if include_openai_curated {
            match target_curated_marketplace(self.auth_mode()) {
                TargetCuratedMarketplace::OpenAi | TargetCuratedMarketplace::OpenAiWithRemote => {
                    let curated_repo_root = curated_plugins_repo_path(self.rexux_home.as_path());
                    curated_repo_root.is_dir().then_some(curated_repo_root)
                }
                TargetCuratedMarketplace::OpenAiApi => {
                    let api_marketplace_path =
                        curated_plugins_api_marketplace_path(self.rexux_home.as_path());
