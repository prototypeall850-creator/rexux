...
use rexux_http_client::RouteAwareRequestError;
...
const REXUX_PRODUCT_SKU: &str = "codex";
...
pub(crate) async fn fetch_and_cache_remote_plugin_catalog(
    ...
    catalog_cache::write_cached_directory_plugins(
        rexux_home, config, auth, scope, /*collection*/ None, &plugins,
    );
    Ok(())
}
...
pub async fn fetch_and_cache_global_remote_plugin_catalog(
    ...
    fetch_and_cache_remote_plugin_catalog(rexux_home, config, auth, RemotePluginScope::Global).await
}
...
pub fn invalidate_cached_remote_plugin_catalog_scopes(
    ...
    for scope in scopes {
        catalog_cache::remove_cached_directory_plugins(
            rexux_home, config, auth, *scope, /*collection*/ None,
        );
...
pub async fn uninstall_remote_plugin(
    ...
    tokio::task::spawn_blocking(move || {
        remove_remote_plugin_cache(rexux_home, marketplace_name, plugin_name, legacy_plugin_id)
    })
    .await
    .map_err(|err| {
...
fn remove_remote_plugin_cache(
    ...
    let store = PluginStore::try_new(rexux_home.clone())
    ...
    let legacy_remote_plugin_cache_root = rexux_home
...
async fn fetch_directory_plugins_for_scope_with_cache(
    ...
    if cache_mode != RemotePluginCatalogCacheMode::ForceRefetch
        && let Some(rexux_home) = rexux_home
        && let Some(cached) = catalog_cache::load_cached_directory_plugins(
            rexux_home, config, auth, scope, collection,
        )
    ...
    if let Some(rexux_home) = rexux_home {
        catalog_cache::write_cached_directory_plugins(
            rexux_home, config, auth, scope, collection, &plugins,
        );
    }
...
fn ensure_chatgpt_auth(auth: Option<&RexuxAuth>) -> Result<&RexuxAuth, RemotePluginCatalogError> {
...
fn authenticated_request(
    ...
    request
        ...
        .headers(rexux_model_provider::auth_provider_from_auth(auth).to_auth_headers())
        .header(OAI_PRODUCT_SKU_HEADER, REXUX_PRODUCT_SKU)
}
