...
pub async fn fetch_remote_featured_plugin_ids(
    ...
    url.query_pairs_mut().append_pair(
        "platform",
        product.unwrap_or(Product::Rexux).to_app_platform(),
    );
    ...
    if let Some(auth) = auth.filter(|auth| auth.uses_rexux_backend()) {
        request =
            request.headers(rexux_model_provider::auth_provider_from_auth(auth).to_auth_headers());
    }
...
fn ensure_rexux_backend_auth(
    auth: Option<&RexuxAuth>,
) -> Result<&RexuxAuth, RemotePluginMutationError> {
...
async fn post_remote_plugin_mutation(
    ...
    let auth = ensure_rexux_backend_auth(auth)?;
