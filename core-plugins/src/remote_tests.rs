...
#[tokio::test]
async fn recommended_plugins_requests_rexux_suggestions_endpoint() {
    let server = MockServer::start().await;
    Mock::given(method("GET"))
...
async fn remote_catalog_cache_modes_control_refresh_and_persist_fetched_results() {
    ...
    for (mode, expire_cache, expected_refresh_needed, expected_cache_used) in [
        ...
        if expire_cache {
            let cache_path =
                std::fs::read_dir(rexux_home.path().join("cache/remote_plugin_catalog"))
...
fn catalog_cache_invalidation_clears_global_collections_and_preserves_other_scopes() {
    ...
    assert_eq!(
        cached_remote_plugin_catalog_scopes(rexux_home.path(), &config, Some(&auth)),
...
#[test]
fn recommended_plugins_accept_rexux_suggestion_items() {
