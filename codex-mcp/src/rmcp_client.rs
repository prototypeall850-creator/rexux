...
//! and exposing cached Rexux Apps tools while a client is still connecting.
...
use crate::rexux_apps::normalize_rexux_apps_callable_name;
use crate::rexux_apps::normalize_rexux_apps_callable_namespace;
use crate::rexux_apps::normalize_rexux_apps_tool_title;
use crate::rexux_apps::prepare_openai_file_params_for_model;
use crate::elicitation::ElicitationRequestManager;
use crate::executor_environment_http_client::ExecutorEnvironmentHttpClient;
use crate::mcp::REXUX_APPS_MCP_SERVER_NAME;
...
use rexux_async_utils::CancelErr;
...
use rexux_protocol::protocol::McpStartupStatus;
use rexux_protocol::protocol::McpStartupUpdateEvent;
use rexux_rmcp_client::ExecutorStdioServerLauncher;
use rexux_rmcp_client::LocalStdioServerLauncher;
use rexux_rmcp_client::McpOAuthRefreshMode;
use rexux_rmcp_client::McpProtocolMode;
use rexux_rmcp_client::RmcpClient;
use rexux_rmcp_client::StdioServerLauncher;
use rexux_rmcp_client::StreamableHttpBearerToken;
use rexux_rmcp_client::StreamableHttpRedirectMode;
use rexux_rmcp_client::ToolWithConnectorId;
use rexux_rmcp_client::is_authentication_required_error;
...
/// MCP server capability indicating that Rexux should include [`SandboxState`]
...
pub(crate) const REXUX_APPS_REFRESH_DURATION_METRIC: &str = "codex.apps.refresh.duration_ms";
pub(crate) const DEFAULT_STARTUP_TIMEOUT: Duration = Duration::from_secs(30);
pub(crate) const DEFAULT_TOOL_TIMEOUT: Duration = Duration::from_secs(300);

pub(crate) const REXUX_APPS_RECONNECT_INITIAL_BACKOFF: Duration = Duration::from_secs(1);
const REXUX_APPS_RECONNECT_MAX_BACKOFF: Duration = Duration::from_secs(30);
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) rexux_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
}
...
impl ManagedClient {
    pub(crate) fn listed_tools(&self) -> Vec<ToolInfo> {
        ...
        if let Some(tools) = self
            .rexux_apps_tools_cache_context
            .as_ref()
        ...
        if self.rexux_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
#[derive(Default)]
struct RexuxAppsStartupReconnectState {
...
#[derive(Clone)]
struct RexuxAppsStartupStatusContext {
    submit_id: String,
    server_name: String,
    tx_event: Sender<Event>,
}

pub(crate) struct RexuxAppsStartupReconnect {
    factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>,
    state: StdMutex<RexuxAppsStartupReconnectState>,
    startup_status_context: Option<RexuxAppsStartupStatusContext>,
}

impl RexuxAppsStartupReconnect {
    pub(crate) fn new(factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>) -> Self {
        Self {
            factory,
            state: StdMutex::new(RexuxAppsStartupReconnectState::default()),
    ...
    fn with_startup_status_context(
        ...
        self.startup_status_context = tx_event.map(|tx_event| RexuxAppsStartupStatusContext {
            submit_id,
            server_name,
            tx_event,
        });
            ...
            let recovered = {
                ...
                match result {
                    ...
                    Err(error) => {
                        ...
                        let retry_after = rexux_apps_reconnect_backoff(state.consecutive_failures);
...
fn rexux_apps_reconnect_backoff(consecutive_failures: u32) -> Duration {
    let exponent = consecutive_failures.saturating_sub(1).min(5);
    REXUX_APPS_RECONNECT_INITIAL_BACKOFF
        .saturating_mul(1 << exponent)
        .min(REXUX_APPS_RECONNECT_MAX_BACKOFF)
}
...
struct ManagedClientStartup {
    ...
    rexux_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
...
impl ManagedClientStartup {
    fn start(&self) -> ManagedClientFuture {
        ...
        let is_rexux_apps_mcp_server = server_name == REXUX_APPS_MCP_SERVER_NAME;
        ...
        async move {
            ...
            let refresh_start = is_rexux_apps_mcp_server.then(Instant::now);
            ...
            {
                emit_duration(
                    REXUX_APPS_REFRESH_DURATION_METRIC,
...
pub(crate) struct AsyncManagedClient {
    ...
    pub(crate) is_rexux_apps_mcp_server: bool,
    ...
    pub(crate) startup_reconnect: Option<Arc<RexuxAppsStartupReconnect>>,
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        let cached_server_info = if is_rexux_apps_mcp_server {
            rexux_apps_tools_cache_context
                .as_ref()
        ...
        let startup = Arc::new(ManagedClientStartup {
            ...
            tx_event,
            elicitation_requests,
            rexux_apps_tools_cache_context: rexux_apps_tools_cache_context.clone(),
        ...
        let startup_reconnect = is_rexux_apps_mcp_server.then(|| {
            let startup = Arc::clone(&startup);
            Arc::new(
                RexuxAppsStartupReconnect::new(Arc::new(move || startup.start()))
    ...
    pub(crate) fn has_cached_tools(&self) -> bool {
        self.rexux_apps_tools_cache_context
            .as_ref()
    ...
    pub(crate) async fn listed_tools(&self) -> Option<Vec<ToolInfo>> {
            ...
            && let Some(startup_tools) = self.cached_tools()
        {
            ...
            match self.client().await {
                ...
                Err(_) if self.is_rexux_apps_mcp_server => self.cached_tools(),
                Err(_) => None,
            }
        }
    }
...
pub(crate) async fn list_tools_for_client_uncached(
    server_name: &str,
    is_rexux_apps_mcp_server: bool,
    rexux_apps_refresh_trigger: &'static str,
    ...
    .collect();
    if is_rexux_apps_mcp_server {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
            fetch_start.elapsed(),
            &[("trigger", rexux_apps_refresh_trigger)],
        );
    } else {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
...
/// Presents declared Rexux Apps file parameters to the model as local-path inputs and adds plugin
/// names to each tool. Plugin membership is resolved by connector ID, falling back to the MCP
/// server when absent.
pub(crate) fn prepare_rexux_apps_tools_for_model(
...
fn tool_info_from_listed_tool(
    ...
    if is_rexux_apps_mcp_server {
        rexux_apps_tool_info_from_listed_tool(server_name, server_instructions, tool)
    } else {
        regular_mcp_tool_info_from_listed_tool(server_name, server_instructions, tool)
    }
}

/// Converts a Rexux Apps tool by preserving connector fields, removing connector prefixes from
/// model-visible names and titles, and using the connector description for its tool namespace.
fn rexux_apps_tool_info_from_listed_tool(
    server_name: &str,
    server_instructions: Option<&str>,
    tool: ToolWithConnectorId,
) -> ToolInfo {
    ...
    let callable_name = normalize_rexux_apps_callable_name(
        &tool_def.name,
        connector_id.as_deref(),
        connector_name.as_deref(),
    );
    let callable_namespace =
        normalize_rexux_apps_callable_namespace(server_name, connector_name.as_deref());
    if let Some(title) = tool_def.title.as_deref() {
        let normalized_title = normalize_rexux_apps_tool_title(connector_name.as_deref(), title);
...
async fn start_server_task(
    ...
    let fetch_ticket = rexux_apps_tools_cache_context
        .as_ref()
    ...
    let client_tools = list_tools_for_client_uncached(
        &server_name,
        is_rexux_apps_mcp_server,
        /*rexux_apps_refresh_trigger*/ "initial",
        &client,
        startup_timeout,
        catalog_item_limit,
    ...
    let shared_tools = match (rexux_apps_tools_cache_context.as_ref(), fetch_ticket) {
        ...
        _ => unreachable!("Rexux Apps fetch ticket requires cache context"),
    };
    let has_shared_tool_catalog = is_rexux_apps_mcp_server || tool_catalog_cache_context.is_some();
...
fn record_protocol_discovery_metrics(
    ...
    if is_rexux_apps_mcp_server {
        tags.push(("server_kind", "openai_rexux_apps"));
    }
...
pub(crate) fn mcp_initialize_request_params(
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("rexux-mcp-client", env!("CARGO_PKG_VERSION")).with_title("Rexux"),
    )
...
pub(crate) async fn make_rmcp_client(
    ...
    match transport {
        McpServerTransportConfig::Stdio {
            ...
            let cwd = cwd.map(rexux_utils_path_uri::LegacyAppPathString::into_string);
                ...
                && let Some(env_var) = bearer_token_env_var.as_ref()
            {
                (
                    Arc::new(ExecutorEnvironmentHttpClient {
                        ...
                        http_client,
                    }) as Arc<dyn rexux_exec_server::HttpClient>,
...
mod tests {
    ...
    #[test]
    fn rexux_apps_connector_metadata_is_preserved() {
        ...
        let expected = ToolInfo {
            server_name: REXUX_APPS_MCP_SERVER_NAME.to_string(),
            supports_parallel_tool_calls: false,
            server_origin: None,
            callable_name: "capture_file_upload".to_string(),
            callable_namespace: "rexux_apps__gmail".to_string(),
