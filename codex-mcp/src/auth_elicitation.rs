...
//! Session orchestration stays in `rexux-core`.

use rexux_protocol::mcp::CallToolResult;
use serde::Serialize;

pub const MCP_TOOL_REXUX_APPS_META_KEY: &str = "_rexux_apps";
...
pub struct RexuxAppsConnectorAuthFailure {
...
pub struct RexuxAppsAuthElicitation {
...
#[derive(Debug, Clone, PartialEq)]
pub struct RexuxAppsAuthElicitationPlan {
    pub auth_failure: RexuxAppsConnectorAuthFailure,
    pub elicitation: RexuxAppsAuthElicitation,
}

#[derive(Serialize)]
struct RexuxAppsConnectorAuthFailureMeta<'a> {
...
pub fn connector_auth_failure_from_tool_result(
...
) -> Option<RexuxAppsConnectorAuthFailure> {
    ...
    Some(RexuxAppsConnectorAuthFailure {
...
pub fn build_auth_elicitation_plan(
    ...
    connector_name: Option<&str>,
    install_url: Option<String>,
) -> Option<RexuxAppsAuthElicitationPlan> {
    let auth_failure =
    ...
    Some(RexuxAppsAuthElicitationPlan {
        auth_failure,
        elicitation,
    })
}
...
pub fn build_auth_elicitation(
    call_id: &str,
    auth_failure: &RexuxAppsConnectorAuthFailure,
) -> RexuxAppsAuthElicitation {
    RexuxAppsAuthElicitation {
        meta: serde_json::json!({
            MCP_TOOL_REXUX_APPS_META_KEY: {
                CONNECTOR_AUTH_FAILURE_META_KEY: RexuxAppsConnectorAuthFailureMeta {
...
pub fn auth_elicitation_id(call_id: &str) -> String {
    format!("rexux_apps_auth_{call_id}")
}
...
fn auth_elicitation_message(auth_failure: &RexuxAppsConnectorAuthFailure) -> String {
    match auth_failure.auth_reason.as_deref() {
        ...
        Some("missing_link") => format!(
            "Sign in to {} on ChatGPT to use it in Rexux.",
            auth_failure.connector_name
        ),
        _ => format!(
...
mod tests {
    ...
    fn builds_auth_elicitation_plan() {
        ...
        assert_eq!(plan.elicitation.elicitation_id, "rexux_apps_auth_call_123");
    }
}
