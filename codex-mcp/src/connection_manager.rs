//! Aggregates MCP server connections for Rexux.
//!
...
use startup::should_share_rexux_apps_tools_cache;
...
use crate::pagination::MAX_REXUX_APPS_TOOL_CATALOG_ITEMS;
...
use crate::rmcp_client::prepare_rexux_apps_tools_for_model;
...
use rexux_protocol::protocol::McpStartupCompleteEvent;
use rexux_protocol::protocol::McpStartupFailure;
...
use rexux_rmcp_client::determine_streamable_http_auth_status_from_credentials;
...
impl McpServerView {
    async fn listed_tools(
        ...
        Some(if self.connection.client.is_rexux_apps_mcp_server {
            prepare_rexux_apps_tools_for_model(tools, tool_plugin_provenance)
        } else {
...
pub(crate) struct McpConnectionSet {
    ...
    rexux_apps_tools_override: RwLock<Option<Vec<ToolInfo>>>,
    rexux_apps_refresh_lock: Mutex<()>,
...
impl McpConnectionSet {
    ...
    pub async fn new(
        ...
        let static_chatgpt_auth_provider = auth
            .filter(|auth| auth.uses_rexux_backend())
            .map(rexux_model_provider::auth_provider_from_auth);
        let rexux_apps_auth_provider = auth_manager.as_ref().and_then(|auth_manager| {
            auth.filter(|auth| auth.uses_rexux_backend()).map(|auth| {
                rexux_model_provider::auth_provider_from_auth_manager(
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            let is_host_owned_rexux_apps = registration.is_some_and(|server| {
                server
                    .source()
            ...
            let catalog_item_limit = if is_host_owned_rexux_apps {
                MAX_REXUX_APPS_TOOL_CATALOG_ITEMS
            } else {
            ...
            // For built-in Rexux Apps, `REXUX_CONNECTORS_TOKEN` is a debug
            ...
            let shares_rexux_apps_tools_cache = is_host_owned_rexux_apps
                && should_share_rexux_apps_tools_cache(&server_name, uses_env_bearer_token);
            let rexux_apps_tools_cache_context = shares_rexux_apps_tools_cache.then(|| {
                rexux_apps_tools_cache
                    .context(rexux_home.clone(), rexux_apps_tools_cache_key.clone())
            });
            // The reserved Rexux Apps registration follows the shared
            ...
            let chatgpt_auth_provider = if server_name == REXUX_APPS_MCP_SERVER_NAME {
                rexux_apps_auth_provider
                    .clone()
                    .or_else(|| static_chatgpt_auth_provider.clone())
            } else {
                static_chatgpt_auth_provider.clone()
            };
            // If Rexux Apps has an env bearer token, that is its auth path. Do
            // not also attach the ambient RexuxAuth provider.
            let runtime_auth_provider =
                if server_name == REXUX_APPS_MCP_SERVER_NAME && uses_env_bearer_token {
                    None
                } else {
                    chatgpt_auth_provider_for_server(&server, chatgpt_auth_provider)
                };
            if is_host_owned_rexux_apps {
            ...
            let connection_identity = McpServerConnectionIdentity::new(
                ...
                auth,
                shares_rexux_apps_tools_cache
                    .then(|| (rexux_home.clone(), rexux_apps_tools_cache_key.clone())),
            ...
            let expected_protocol_mode = match &configured_config.transport {
                ...
                McpServerTransportConfig::Stdio { env, .. } => match env
                    .as_ref()
                    .and_then(|variables| variables.get("REXUX_MCP_PROTOCOL_VERSION"))
                {
            ...
            let tool_catalog_cache_context = if server_name == REXUX_APPS_MCP_SERVER_NAME {
                None
    ...
    /// Rexux Apps metadata may come from its existing cache; regular MCP server information is
