...
use rexux_rmcp_client::StoredOAuthCredentialSnapshot;
...
pub(crate) struct McpServerConnectionIdentity {
    ...
    runtime_auth: Option<RexuxAuth>,
    runtime_auth_token: Option<String>,
    rexux_apps_cache_identity: Option<(PathBuf, ConnectorRuntimeContextKey)>,
...
impl McpServerConnectionIdentity {
    ...
    pub(crate) fn has_same_connection_config(&self, other: &Self) -> bool {
        let same_runtime_auth = match (&self.runtime_auth, &other.runtime_auth) {
            (Some(RexuxAuth::AgentIdentity(left)), Some(RexuxAuth::AgentIdentity(right))) => {
        ...
        self.auth == other.auth
            ...
            && self.rexux_apps_cache_identity == other.rexux_apps_cache_identity
