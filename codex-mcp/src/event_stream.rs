...
use crate::REXUX_APPS_MCP_SERVER_NAME;
...
impl McpEventStreamOpener {
    ...
    fn matches_auth(&self, current: Option<&RexuxAuth>) -> bool {
        match (self.connection.auth.as_ref(), current) {
            (Some(RexuxAuth::AgentIdentity(expected)), Some(RexuxAuth::AgentIdentity(current))) => {
                expected.record() == current.record()
            }
            (Some(RexuxAuth::AgentIdentity(_)), _) | (_, Some(RexuxAuth::AgentIdentity(_))) => {
                false
            }
