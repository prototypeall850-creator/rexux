...
use crate::rmcp_client::REXUX_APPS_REFRESH_DURATION_METRIC;
...
impl McpConnectionSet {
    pub(crate) async fn stable_catalog_revision(&self) -> Option<u64> {
        for (server_name, view) in &self.servers {
            ...
            let Some(client) = view.connection.client.ready_transport() else {
                if !view.connection.client.is_rexux_apps_mcp_server
    ...
    pub async fn list_all_tools(&self) -> Vec<ToolInfo> {
        ...
        let server_results = join_all(self.servers.iter().map(|(server_name, view)| async move {
            ...
            let startup_complete = view
                ...
                .startup_complete
                .load(Ordering::Acquire);
            let catalog_override = if server_name == REXUX_APPS_MCP_SERVER_NAME {
                self.rexux_apps_tools_override.read().await.clone()
            } else {
                None
            };
            let server_tools = async {
                match catalog_override {
                    Some(tools) => {
                        ...
                        Some(prepare_rexux_apps_tools_for_model(
                            tools,
    ...
    pub(crate) async fn capture_binding_with_metadata(
        ...
        let server_snapshots = join_all(self.servers.iter().map(|(server_name, view)| async move {
                ...
                .load(Ordering::Acquire)
            {
                ...
                let cached_tools = view.connection.client.cached_tools().filter(|tools| {
                    view.connection.client.is_rexux_apps_mcp_server || !tools.is_empty()
                });
                ...
                let must_wait_for_startup = (required
                    ...
                    || (server_name == REXUX_APPS_MCP_SERVER_NAME && !has_cached_tools);
        ...
        let server_results = join_all(server_snapshots.into_iter().map(|(server_name, view, cached_tools)| async move {
            ...
            let server_tools = if server_name == REXUX_APPS_MCP_SERVER_NAME {
                prepare_rexux_apps_tools_for_model(server_tools, &self.tool_plugin_provenance)
            } else {
    ...
    /// Force-refresh Rexux Apps tools and publish one new exact catalog revision.
    #[expect(
        clippy::await_holding_invalid_type,
        reason = "catalog publication must remain serialized with captured tool calls"
    )]
    pub async fn hard_refresh_rexux_apps_tools_cache(&self) -> Result<Vec<ToolInfo>> {
        let _refresh = self.rexux_apps_refresh_lock.lock().await;
        let refresh_start = Instant::now();
        let view = self
            .servers
            .get(REXUX_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{REXUX_APPS_MCP_SERVER_NAME}'"))?;
        ...
        let client_tools = list_tools_for_client_uncached(
            ...
            /*rexux_apps_refresh_trigger*/ "explicit",
        ...
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{REXUX_APPS_MCP_SERVER_NAME}'")
        })?;

        let mut tool_catalog_revision = self.tool_catalog_revision.write().await;
        let tools = match (
            managed_client.rexux_apps_tools_cache_context.as_ref(),
            fetch_ticket,
        ) {
        ...
        *self.rexux_apps_tools_override.write().await = Some(client_tools);
        *tool_catalog_revision += 1;
        ...
        let tools = prepare_rexux_apps_tools_for_model(
