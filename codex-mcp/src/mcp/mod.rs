...
pub const REXUX_APPS_MCP_SERVER_NAME: &str = "rexux_apps";
const DEFAULT_REXUX_APPS_MCP_PRODUCT_SKU: &str = "codex";
const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
const REXUX_CONNECTORS_TOKEN_ENV_VAR: &str = "REXUX_CONNECTORS_TOKEN";
...
/// MCP runtime settings derived from `rexux_core::config::Config`.
///
...
pub struct McpConfig {
    ...
    /// Rexux home directory used for MCP OAuth state and app-tool cache files.
    ...
    /// Optional path to `rexux-linux-sandbox` for sandboxed MCP tool execution.
...
pub fn host_owned_rexux_apps_enabled(config: &McpConfig, auth: Option<&RexuxAuth>) -> bool {
    config.apps_enabled && auth.is_some_and(RexuxAuth::uses_rexux_backend)
}
...
fn is_trusted_chatgpt_mcp_server(
    ...
    if url::Url::parse(CHATGPT_REXUX_BASE_URL)
        .ok()
...
pub fn effective_mcp_servers_from_configured(
    ...
    let mut servers = configured_servers
        ...
        .collect::<HashMap<_, _>>();
    if !host_owned_rexux_apps_enabled(config, auth) {
        servers.remove(REXUX_APPS_MCP_SERVER_NAME);
    }
    servers
}
...
pub async fn read_mcp_resource(
    ...
    rexux_apps_tools_cache: ConnectorRuntimeManager<ToolInfo>,
    ...
    let manager = McpConnectionSet::new(
        ...
        McpRuntimeInput {
            ...
            tool_catalog_cache,
            rexux_apps_tools_cache_key: connector_runtime_context_key(auth),
...
fn rexux_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(REXUX_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => Some(REXUX_CONNECTORS_TOKEN_ENV_VAR.to_string()),
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(REXUX_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}

fn normalize_rexux_apps_base_url(base_url: &str) -> String {
...
fn rexux_apps_mcp_url_for_base_url(base_url: &str) -> String {
    let base_url = normalize_rexux_apps_base_url(base_url);
...
pub fn rexux_apps_mcp_server_config(
    chatgpt_base_url: &str,
    apps_mcp_product_sku: Option<&str>,
    originator: Option<&str>,
) -> McpServerConfig {
    mcp_server_config_for_url(
        rexux_apps_mcp_url_for_base_url(chatgpt_base_url),
...
pub fn hosted_plugin_runtime_mcp_server_config(
    ...
    rexux_apps_mcp_server_config(chatgpt_base_url, apps_mcp_product_sku, originator)
}
...
fn mcp_server_config_for_url(
    ...
    let product_sku = apps_mcp_product_sku.unwrap_or(DEFAULT_REXUX_APPS_MCP_PRODUCT_SKU);
    ...
    McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url,
            bearer_token_env_var: rexux_apps_mcp_bearer_token_env_var(),
