...
pub(crate) fn test_mcp_config(rexux_home: PathBuf) -> McpConfig {
    McpConfig {
        ...
        config_layer_stack: rexux_config::ConfigLayerStack::default(),
        approvals_reviewer: rexux_config::types::ApprovalsReviewer::default(),
        ...
        connector_snapshot: rexux_connectors::ConnectorSnapshot::default(),
    }
}
...
fn mcp_server_permissions_handle_unattached_and_threadless_servers() {
    ...
    let mut missing_server = rexux_apps_mcp_server_config(
        "https://example.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    );
...
#[test]
fn rexux_apps_mcp_url_for_base_url_uses_plugin_service_paths() {
    assert_eq!(
        rexux_apps_mcp_url_for_base_url("https://chatgpt.com/backend-api"),
        "https://chatgpt.com/backend-api/ps/mcp"
    );
    assert_eq!(
        rexux_apps_mcp_url_for_base_url("https://chat.openai.com"),
        "https://chat.openai.com/backend-api/ps/mcp"
    );
    assert_eq!(
        rexux_apps_mcp_url_for_base_url("http://localhost:8080/api/codex"),
        "http://localhost:8080/api/codex/ps/mcp"
    );
    assert_eq!(
        rexux_apps_mcp_url_for_base_url("http://localhost:8080"),
        "http://localhost:8080/api/codex/ps/mcp"
    );
}

#[test]
fn rexux_apps_server_config_uses_plugin_service_path() {
    let config = rexux_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    );
...
#[test]
fn rexux_apps_server_config_forwards_thread_originator_header() {
    let config = rexux_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
...
#[test]
fn rexux_apps_server_config_sets_product_sku_header() {
...
#[test]
fn rexux_apps_server_config_forwards_originator_and_configured_product_sku_headers() {
    let config = rexux_apps_mcp_server_config(
        "https://chatgpt.com",
        Some("tpp"),
        Some("thread_originator"),
...
fn effective_mcp_servers_preserve_chatgpt_auth_for_staging() {
    for url in [
        ...
        let server = rexux_apps_mcp_server_config(
...
async fn effective_mcp_servers_preserve_runtime_servers() {
    ...
    let mut config = test_mcp_config(rexux_home.path().to_path_buf());
