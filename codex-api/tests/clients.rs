...
use rexux_api::ResponsesClient;
use rexux_api::ResponsesEndpoint;
use rexux_api::ResponsesOptions;
use rexux_client::HttpTransport;
use rexux_client::Request;
use rexux_client::RequestBody;
use rexux_client::Response;
use rexux_client::StreamResponse;
use rexux_client::TransportError;
...
struct FlakyTransportState {
    attempts: i64,
    requests: Vec<(RequestBody, HeaderMap, rexux_client::RequestCompression)>,
}
...
impl FlakyTransport {
    ...
    fn requests(&self) -> Vec<(RequestBody, HeaderMap, rexux_client::RequestCompression)> {
        self.state
            .lock()
            .expect("flaky transport state mutex should not be poisoned")
            .requests
...
async fn streaming_client_retries_on_transport_error() -> Result<()> {
    ...
    assert_eq!(requests[0].2, rexux_client::RequestCompression::None);
    Ok(())
}
