...
use chrono::DateTime;
use chrono::Utc;
use rexux_protocol::auth::PlanType;
use rexux_protocol::error::RexuxErr;
use rexux_protocol::error::RexuxErrorDetails;
use rexux_protocol::error::ConnectionFailedError;
use rexux_protocol::error::RetryLimitReachedError;
use rexux_protocol::error::UnexpectedResponseError;
use rexux_protocol::error::UsageLimitReachedError;
use rexux_protocol::protocol::MisalignmentErrorDetails;
use http::HeaderMap;
use serde::Deserialize;
use serde_json::Value;

pub fn map_api_error(err: ApiError) -> RexuxErr {
    match err {
        ApiError::ContextWindowExceeded => RexuxErr::ContextWindowExceeded,
        ApiError::QuotaExceeded => RexuxErr::QuotaExceeded,
        ApiError::UsageNotIncluded => RexuxErr::UsageNotIncluded,
        ApiError::Retryable { message, delay } => {
            let error = RexuxErr::Stream(message);
            match delay {
        ...
        ApiError::RateLimitExceeded { message, delay } => {
            let error = RexuxErr::new(RexuxErrorDetails::RateLimitExceeded(message));
            match delay {
                Some(delay) => error.with_retry_delay(delay),
                None => error,
            }
        }
        ApiError::Stream(msg) => RexuxErr::Stream(msg),
        ApiError::ServerOverloaded => RexuxErr::ServerOverloaded,
        ApiError::Api { status, message } => {
            let user_message = api_error_user_message(status, &message);
            RexuxErr::UnexpectedStatus(UnexpectedResponseError {
                status,
                body: message,
                user_message,
                url: None,
        ...
        ApiError::InvalidRequest { message } => RexuxErr::InvalidRequest(message),
        ApiError::CyberPolicy { message } => {
            RexuxErr::new(RexuxErrorDetails::CyberPolicy { message })
        }
        ApiError::MisalignmentPolicyViolation {
            message,
            misalignment,
        } => RexuxErr::new(RexuxErrorDetails::MisalignmentPolicyViolation {
            message,
            misalignment,
        }),
        ApiError::Transport(transport) => match transport {
            TransportError::Http {
                ...
                {
                    return RexuxErr::ServerOverloaded;
                }
                        ...
                        == Some(MISALIGNMENT_POLICY_VIOLATION_ERROR_CODE)
                {
                    ...
                    return RexuxErr::new(RexuxErrorDetails::MisalignmentPolicyViolation {
                        message,
                ...
                if status == http::StatusCode::BAD_REQUEST {
                    ...
                    {
                        RexuxErr::InvalidImageRequest()
                    } else {
                        RexuxErr::InvalidRequest(body_text)
                    }
                } else if status == http::StatusCode::INTERNAL_SERVER_ERROR {
                    RexuxErr::InternalServerError
                    ...
                    if let Ok(err) = serde_json::from_str::<UsageErrorResponse>(&body_text) {
                        if err.error.error_type.as_deref() == Some("usage_limit_reached") {
                            ...
                            return RexuxErr::UsageLimitReached(UsageLimitReachedError {
                            ...
                            return RexuxErr::UsageNotIncluded;
                        }
                    }

                    RexuxErr::RetryLimit(RetryLimitReachedError {
                        status,
            ...
            TransportError::RetryLimit => RexuxErr::RetryLimit(RetryLimitReachedError {
                status: http::StatusCode::INTERNAL_SERVER_ERROR,
                request_id: None,
            }),
            TransportError::Timeout => RexuxErr::RequestTimeout,
            TransportError::Connection(source) => {
                RexuxErr::ConnectionFailed(ConnectionFailedError { source })
            }
            TransportError::Network(msg) | TransportError::Build(msg) => RexuxErr::Stream(msg),
        },
        ApiError::RateLimit(msg) => RexuxErr::Stream(msg),
    }
}
