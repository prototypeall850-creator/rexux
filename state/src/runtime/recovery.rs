//! Backup-and-rebuild support for Rexux runtime SQLite databases.
//!
//! Rexux keeps several independent runtime SQLite databases under one SQLite
...
/// Move one Rexux runtime SQLite database out of the way so that database can
    ...
    match tokio::fs::metadata(sqlite_home).await {
        ...
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
            ...
            Err(std::io::Error::other(format!(
                "no Rexux runtime database files were found to back up for {}",
                db_path.display()
            )))
        }
        Err(err) => Err(err),
...
async fn backup_sqlite_paths(
    ...
    if backups.is_empty() {
        ...
        return Err(std::io::Error::other(
            "no Rexux runtime database files were found to back up",
        ));
    }
