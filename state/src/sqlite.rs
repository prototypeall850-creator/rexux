...
impl RuntimeDbSpec {
    fn path(self, rexux_home: &Path) -> PathBuf {
        rexux_home.join(self.filename)
    }
}
...
/// Resolved configuration shared by all Rexux SQLite connections.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl SqliteConfig {
    ...
    /// Open a writable Rexux SQLite database, creating it if necessary.
    ...
    /// Open an existing Rexux SQLite database without creating or modifying it.
