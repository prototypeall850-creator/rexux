...
use rexux_backend_client::ConfigBundleResponse;
use rexux_backend_client::DeliveredTomlFragment;
use rexux_config::AbsolutePathBuf;
use rexux_config::CloudConfigFragment;
use rexux_config::CloudConfigTomlBundle;
use rexux_config::CloudRequirementsFragment;
use rexux_config::CloudRequirementsTomlBundle;
...
fn write_auth_json(rexux_home: &Path, value: serde_json::Value) -> std::io::Result<()> {
    std::fs::write(rexux_home.join("auth.json"), serde_json::to_string(&value)?)?;
    Ok(())
}

fn create_test_cache(rexux_home: &Path) -> CloudConfigBundleCache {
    CloudConfigBundleCache::new(AbsolutePathBuf::resolve_path_against_base(rexux_home, "/"))
}
...
async fn auth_manager_with_agent_identity_business_plan() -> Arc<AuthManager> {
    ...
    AuthManager::from_auth_for_testing(RexuxAuth::AgentIdentity(
...
impl BundleClient for StaticBundleClient {
    async fn get_bundle(&self, _auth: &RexuxAuth) -> Result<CloudConfigBundle, BundleRequestError> {
...
impl BundleClient for TokenBundleClient {
    async fn get_bundle(&self, auth: &RexuxAuth) -> Result<CloudConfigBundle, BundleRequestError> {
        self.request_count.fetch_add(1, Ordering::SeqCst);
        if matches!(
...
async fn get_bundle_ignores_invalid_cache_and_refetches() {
    let rexux_home = tempdir().expect("tempdir");
    let cache = create_test_cache(rexux_home.path());
    cache
        .save(
...
async fn get_bundle_refreshes_external_auth_after_unauthorized() {
    ...
    let initial_auth = RexuxAuth::from_external_chatgpt_tokens(
    ...
    let refreshed_auth = RexuxAuth::from_external_chatgpt_tokens(
        &refreshed_token,
        "account-12345",
        Some("enterprise"),
    )
...
async fn production_loader_recovers_startup_timeouts_in_background() {
    ...
    assert_eq!(
        ...
        rexux_protocol::protocol::AskForApproval::Never
    );
...
async fn dropping_loader_cancels_in_flight_startup_and_refresh() {
    for starts_from_cache in [false, true] {
        ...
        if starts_from_cache {
            create_test_cache(rexux_home.path())
                .save(
                    Some("user-12345".to_string()),
                    Some("account-12345".to_string()),
                    test_bundle(),
...
fn bundle_response_conversion_preserves_fragment_order() {
    let response = ConfigBundleResponse {
        config_toml: Some(Some(Box::new(rexux_backend_client::DeliveredConfigToml {
        ...
        requirements_toml: Some(Some(Box::new(
            rexux_backend_client::DeliveredRequirementsToml {
