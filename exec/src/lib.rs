...
use rexux_app_server_client::EnvironmentManager;
use rexux_app_server_client::ExecServerRuntimePaths;
use rexux_app_server_client::InProcessAppServerClient;
use rexux_app_server_client::InProcessClientStartArgs;
use rexux_app_server_client::InProcessServerEvent;
...
use rexux_app_server_protocol::Thread as AppServerThread;
...
use rexux_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use rexux_config::format_config_error_with_source;
use rexux_core::StateDbHandle;
use rexux_core::check_execpolicy_for_warnings;
use rexux_core::config::Config;
use rexux_core::config::ConfigBuilder;
use rexux_core::config::ConfigOverrides;
use rexux_core::config::ConfigTomlLoadResult;
...
use rexux_core::config::resolve_oss_provider;
use rexux_core::config::resolve_profile_v2_config_path;
use rexux_core::find_thread_meta_by_name_str;
use rexux_core::format_exec_policy_error_with_source;
use rexux_core::path_utils;
use rexux_core::read_session_meta_line;
...
use rexux_login::default_client::set_default_client_residency_requirement;
use rexux_login::default_client::set_default_originator;
use rexux_login::enforce_login_restrictions;
...
use rexux_otel::set_parent_from_context;
use rexux_otel::traceparent_context_from_env;
...
use rexux_utils_oss::ensure_oss_provider_ready;
use rexux_utils_oss::get_default_model_for_oss_provider;
use event_processor_with_human_output::EventProcessorWithHumanOutput;
pub use event_processor_with_jsonl_output::RexuxStatus;
...
struct ExecRunArgs {
    ...
    config: Config,
    resume_approvals_reviewer_override: Option<rexux_app_server_protocol::ApprovalsReviewer>,
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("rexux_exec".to_string()) {
    ...
    #[allow(clippy::print_stderr)]
    let rexux_home = match find_rexux_home() {
    ...
    let user_config_path = config_profile_v2
        .as_ref()
        .map(|profile_v2| resolve_profile_v2_config_path(&rexux_home, profile_v2));
    ...
    let overrides = ConfigOverrides {
        ...
        service_tier: None,
        rexux_self_exe: arg0_paths.rexux_self_exe.clone(),
        rexux_linux_sandbox_exe: arg0_paths.rexux_linux_sandbox_exe.clone(),
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        rexux_core::otel_init::build_provider(
            &config,
    ...
    rexux_core::otel_init::record_process_start(otel.as_ref(), "rexux_exec");
    rexux_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "rexux_exec");
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        arg0_paths.rexux_self_exe.clone(),
        arg0_paths.rexux_linux_sandbox_exe.clone(),
    )?;
    let state_db = rexux_core::init_state_db(&config).await;
    let environment_manager = if run_loader_overrides.ignore_user_config {
        ...
        EnvironmentManager::from_rexux_home(
    ...
    let in_process_start_args = InProcessClientStartArgs {
        ...
        client_name: "rexux_exec".to_string(),
...
async fn load_bootstrap_config_or_exit(
    ...
    cli_kv_overrides: Vec<(String, rexux_config::TomlValue)>,
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = rexux_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what Rexux
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    if !json_mode
        && let Some(message) =
            rexux_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        event_processor.process_warning(message);
    }

    info!("Rexux initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(notification) => {
                ...
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        rexux_app_server_protocol::TurnStatus::Failed
                            | rexux_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
...
fn thread_resume_params_from_config(
    config: &Config,
    thread_id: String,
    approvals_reviewer_override: Option<rexux_app_server_protocol::ApprovalsReviewer>,
) -> ThreadResumeParams {
    let permissions = permissions_selection_from_config(config);
    let sandbox = permissions.is_none().then(|| {
        sandbox_mode_from_permission_profile(
...
fn session_configured_from_thread_response(
    ...
    thread_source: Option<rexux_protocol::protocol::ThreadSource>,
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<rexux_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
...
fn should_backfill_turn_completed_items(
    ...
    !thread_ephemeral && payload.turn.items_view != rexux_app_server_protocol::TurnItemsView::Full
}
...
async fn parse_latest_turn_context_cwd(path: &Path) -> Option<PathBuf> {
    ...
    tokio::task::spawn_blocking(move || {
        let reader = rexux_rollout::open_rollout_seekable_reader(&path).ok()?;
        let mut scanner = rexux_rollout::ReverseJsonlScanner::new(reader).ok()?;
        while let Some(outcome) = scanner.scan_next::<RolloutLine>().ok()? {
            if let rexux_rollout::ScanOutcome::Parsed(RolloutLine {
...
async fn resolve_resume_thread_id(
    ...
    if let Some(state_db) = state_db {
        ...
        if let Some((_, session_meta)) =
            find_thread_meta_by_name_str(&config.rexux_home, session_id, Some(state_db.as_ref()))
                .await?
