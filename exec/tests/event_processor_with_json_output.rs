use rexux_app_server_protocol::CollabAgentState as ApiCollabAgentState;
use rexux_app_server_protocol::CollabAgentStatus as ApiCollabAgentStatus;
use rexux_app_server_protocol::CollabAgentTool;
use rexux_app_server_protocol::CollabAgentToolCallStatus as ApiCollabAgentToolCallStatus;
use rexux_app_server_protocol::CommandAction;
use rexux_app_server_protocol::CommandExecutionSource;
use rexux_app_server_protocol::CommandExecutionStatus as ApiCommandExecutionStatus;
use rexux_app_server_protocol::ErrorNotification;
use rexux_app_server_protocol::FileUpdateChange as ApiFileUpdateChange;
use rexux_app_server_protocol::ItemCompletedNotification;
use rexux_app_server_protocol::ItemStartedNotification;
use rexux_app_server_protocol::McpToolCallError;
use rexux_app_server_protocol::McpToolCallResult;
use rexux_app_server_protocol::McpToolCallStatus as ApiMcpToolCallStatus;
use rexux_app_server_protocol::PatchApplyStatus as ApiPatchApplyStatus;
use rexux_app_server_protocol::PatchChangeKind as ApiPatchChangeKind;
...
use rexux_app_server_protocol::WebSearchAction as ApiWebSearchAction;
use rexux_app_server_protocol::WebSearchItem as ApiWebSearchItem;
...
use rexux_exec::AgentMessageItem;
use rexux_exec::RexuxStatus;
use rexux_exec::CollabAgentState;
use rexux_exec::CollabAgentStatus;
use rexux_exec::CollabTool;
use rexux_exec::CollabToolCallItem;
use rexux_exec::CollabToolCallStatus;
use rexux_exec::CollectedThreadEvents;
use rexux_exec::CommandExecutionItem;
use rexux_exec::CommandExecutionStatus;
use rexux_exec::ErrorItem;
use rexux_exec::EventProcessorWithJsonOutput;
use rexux_exec::ExecThreadItem;
use rexux_exec::FileChangeItem;
use rexux_exec::FileUpdateChange as ExecFileUpdateChange;
use rexux_exec::ItemCompletedEvent;
use rexux_exec::ItemStartedEvent;
use rexux_exec::ItemUpdatedEvent;
use rexux_exec::McpToolCallItem;
use rexux_exec::McpToolCallItemError;
use rexux_exec::McpToolCallItemResult;
use rexux_exec::McpToolCallStatus;
use rexux_exec::PatchApplyStatus;
use rexux_exec::PatchChangeKind;
use rexux_exec::ReasoningItem;
use rexux_exec::ThreadErrorEvent;
use rexux_exec::ThreadEvent;
use rexux_exec::ThreadItemDetails;
use rexux_exec::ThreadStartedEvent;
use rexux_exec::TodoItem;
use rexux_exec::TodoListItem;
use rexux_exec::TurnCompletedEvent;
use rexux_exec::TurnFailedEvent;
use rexux_exec::TurnStartedEvent;
use rexux_exec::Usage;
use rexux_exec::WebSearchItem;
...
fn session_configured_produces_thread_started_event() {
    ...
    let session_configured = SessionConfiguredEvent {
        ...
        thread_source: None,
        thread_name: None,
        model: "rexux-mini-latest".to_string(),
        model_provider_id: "test-provider".to_string(),
        service_tier: None,
        approval_policy: AskForApproval::Never,
        approvals_reviewer: rexux_protocol::config_types::ApprovalsReviewer::User,
...
fn plan_update_emits_started_then_updated_then_completed() {
    ...
    assert_eq!(
        ...
        CollectedThreadEvents {
            ...
            status: RexuxStatus::InitiateShutdown,
        }
    );
}
...
fn token_usage_update_is_emitted_on_turn_completion() {
    ...
    let usage_update =
        processor.collect_thread_events(ServerNotification::ThreadTokenUsageUpdated(
            rexux_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
...
fn failed_turn_clears_stale_final_message() {
    ...
    assert_eq!(collected.status, RexuxStatus::Running);
    ...
    assert_eq!(collected.status, RexuxStatus::InitiateShutdown);
...
fn model_reroute_surfaces_as_error_item() {
    ...
    let collected = processor.collect_thread_events(ServerNotification::ModelRerouted(
        rexux_app_server_protocol::ModelReroutedNotification {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            from_model: "gpt-5".to_string(),
            to_model: "gpt-5-mini".to_string(),
            reason: rexux_app_server_protocol::ModelRerouteReason::HighRiskCyberActivity,
        },
    ));
