#![cfg(unix)]
use rexux_core::spawn::StdioPolicy;
...
pub(super) async fn spawn_command_under_sandbox(
    ...
    use rexux_core::exec::build_exec_request;
    ...
    let rexux_linux_sandbox_exe = Some(
        core_test_support::find_rexux_linux_sandbox_exe()
...
#[tokio::test]
async fn sandbox_blocks_first_time_dot_rexux_creation() {
    core_test_support::skip_if_sandbox!();
    #[cfg(target_os = "linux")]
    let sandbox_env = match linux_sandbox_test_env().await {
        Some(env) => env,
    ...
    let dot_rexux_metadata = tokio::fs::symlink_metadata(&dot_codex).await;
    if let Ok(metadata) = dot_rexux_metadata {
        assert!(
            !metadata.is_dir(),
            "{} should not be creatable as a directory",
            dot_codex.display()
        );
    } else if let Err(err) = &dot_rexux_metadata {
        assert_eq!(
            err.kind(),
