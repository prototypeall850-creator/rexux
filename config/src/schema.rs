...
use rexux_features::legacy_feature_keys;
...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == rexux_features::Feature::Artifact {
            continue;
        }
        if feature.id == rexux_features::Feature::CodeMode {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<rexux_features::FeatureToml<
                    rexux_features::CodeModeConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == rexux_features::Feature::CodeModeHost {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<rexux_features::FeatureToml<
                    rexux_features::CodeModeHostConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == rexux_features::Feature::NonPrefixedMcpToolNames {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<rexux_features::FeatureToml<
                    rexux_features::NonPrefixedMcpToolNamesConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == rexux_features::Feature::GuardianV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<rexux_features::FeatureToml<
                    rexux_features::GuardianV2ConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == rexux_features::Feature::MultiAgentV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<rexux_features::FeatureToml<
                    rexux_features::MultiAgentV2ConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == rexux_features::Feature::TokenBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<rexux_features::FeatureToml<
                    rexux_features::TokenBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == rexux_features::Feature::ContextManagement {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<rexux_features::FeatureToml<
                    rexux_features::ContextManagementConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == rexux_features::Feature::RolloutBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<rexux_features::FeatureToml<
                    rexux_features::RolloutBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == rexux_features::Feature::CurrentTimeReminder {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<rexux_features::FeatureToml<
                    rexux_features::CurrentTimeReminderConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == rexux_features::Feature::SleepTool {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<rexux_features::FeatureToml<
                    rexux_features::SleepToolConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == rexux_features::Feature::AppsMcpPathOverride {
            validation.properties.insert(
                feature.key.to_string(),
        ...
        if feature.id == rexux_features::Feature::NetworkProxy {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<rexux_features::FeatureToml<
                    rexux_features::NetworkProxyConfigToml,
                >>(),
            );
            continue;
        }
    ...
    validation.properties.insert(
        ...
        schema_gen.subschema_for::<rexux_features::ToolRegistryConfigToml>(),
    );
