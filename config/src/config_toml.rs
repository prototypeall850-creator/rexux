//! Schema-heavy configuration TOML types used by Rexux.
...
use rexux_model_provider_info::AMAZON_BEDROCK_RUNTIME_PROVIDER_ID;
...
/// Base config deserialized from ~/.rexux/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by Rexux will likely degrade model performance.
    ...
    /// file: Use a file in the Rexux home directory.
    ...
    /// Directory where Rexux stores the SQLite state DB.
    /// Defaults to `$REXUX_SQLITE_HOME` when set. Otherwise uses `$REXUX_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where Rexux writes log files. Setting this value explicitly
    /// also enables the TUI text log in this directory.
    /// Defaults to `$REXUX_HOME/log`.
    ...
    /// Optional product SKU forwarded on host-owned Rexux Apps MCP requests.
...
pub use rexux_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
pub use rexux_protocol::protocol::RealtimeVoice;
