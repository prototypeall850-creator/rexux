...
pub struct LoaderOverrides {
    /// Optional configuration file supplied with the installed Rexux package.
...
impl LoaderOverrides {
    ...
    pub fn without_managed_config_for_tests() -> Self {
        let base = std::env::temp_dir().join("rexux-config-tests");
        Self {
    ...
    pub fn user_config_path(&self, rexux_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
impl ConfigLayerEntry {
    ...
    pub fn config_folder(&self) -> Option<AbsolutePathBuf> {
        match &self.name {
            ...
            ConfigLayerSource::Project { dot_rexux_folder } => Some(dot_rexux_folder.clone()),
...
impl ConfigLayerStack {
    ...
    /// the base `$REXUX_HOME/config.toml` layer because the active layer is the
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<()> {
    ...
    // user config.
    let mut previous_project_dot_rexux_folder: Option<&AbsolutePathBuf> = None;
    for layer in layers {
        if let ConfigLayerSource::Project {
            dot_rexux_folder: current_project_dot_rexux_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_rexux_folder {
                ...
                if previous == current_project_dot_rexux_folder
                    || !current_project_dot_rexux_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_rexux_folder = Some(current_project_dot_rexux_folder);
        }
    }
