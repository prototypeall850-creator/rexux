...
use rexux_file_system::CopyOptions;
use rexux_file_system::CreateDirectoryOptions;
use rexux_file_system::ExecutorFileSystemFuture;
use rexux_file_system::FileMetadata;
use rexux_file_system::FileSystemReadStream;
use rexux_file_system::FileSystemSandboxContext;
use rexux_file_system::GetMetadataOptions;
use rexux_file_system::ReadDirectoryEntry;
use rexux_file_system::ReadFileOptions;
use rexux_file_system::RemoveOptions;
use rexux_file_system::WalkOptions;
use rexux_file_system::WalkOutcome;
use rexux_file_system::WriteFileOptions;
...
fn project_environment_filters_preserve_only_trusted_credential_host_bindings() {
    ...
    for project_policy in [
        ...
        let actual = rexux_protocol::shell_environment::populate_env(
            [
                ("GH_HOST", host),
...
async fn default_windows_managed_config_is_ignored_with_warning() {
    ...
    let managed_config_path = rexux_home.join("managed_config.toml");
    std::fs::write(
        &managed_config_path,
        r#"
model = "legacy-model"
...
"#,
    ...
    std::fs::write(rexux_home.join(CONFIG_TOML_FILE), r#"model = "user-model""#)
    ...
    let expected_warnings = vec![format!(
        "Ignoring deprecated managed config file at {}; REXUX_HOME/managed_config.toml is no longer supported on Windows. Use %ProgramData%\\OpenAI\\Rexux\\requirements.toml for enforced settings or config.toml for defaults.",
...
fn windows_local_managed_configuration_ignores_legacy_file_but_detects_requirements() {
    ...
    let rexux_home = tmp.path().join("rexux-home");
    std::fs::create_dir_all(&rexux_home).expect("create codex home");
    std::fs::write(rexux_home.join("managed_config.toml"), "")
        .expect("write default legacy managed config");
...
async fn local_layers_keep_raw_paths_order_and_legacy_requirements() {
    ...
    for dir in [&rexux_home, &dot_codex, &system_dir, &managed_dir] {
    ...
    let user_file = rexux_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &user_file,
        format!(
