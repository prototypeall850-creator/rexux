...
#[cfg(unix)]
const REXUX_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/codex/managed_config.toml";
...
pub(super) async fn load_config_layers_internal(
    ...
    let managed_config_path = AbsolutePathBuf::from_absolute_path(
        managed_config_path.unwrap_or_else(|| managed_config_default_path(rexux_home)),
    )?;
...
pub(super) fn managed_config_default_path(rexux_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = rexux_home;
        PathBuf::from(REXUX_MANAGED_CONFIG_SYSTEM_PATH)
    }
