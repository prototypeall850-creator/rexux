...
use rexux_protocol::config_types::ToolExposureSurface;
...
/// OAuth client settings used when Rexux launches an MCP OAuth flow.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema)]
#[schemars(deny_unknown_fields)]
pub struct McpServerOAuthConfig {
    ...
    /// Fixed callback port that takes precedence over Rexux's global OAuth callback port.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub callback_port: Option<u16>,
}

/// Authentication flow Rexux attempts after resolving an HTTP MCP server's
...
pub struct McpServerConfig {
    ...
    /// Effective environment id for where Rexux should start this MCP server.
    pub environment_id: String,

    /// When `false`, Rexux skips initializing this MCP server.
