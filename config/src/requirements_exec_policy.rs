...
use rexux_execpolicy::RuleRef;
use rexux_execpolicy::rule::PatternToken;
use rexux_execpolicy::rule::PrefixPattern;
use rexux_execpolicy::rule::PrefixRule;
...
pub enum RequirementsExecPolicyParseError {
    ...
    #[error(
        "rules prefix_rule at index {rule_index} has decision 'allow', which is not permitted in requirements.toml: Rexux merges these rules with other config and uses the most restrictive result (use 'prompt' or 'forbidden')"
    )]
...
impl RequirementsExecPolicyToml {
    ...
    /// representation used by `rexux-execpolicy`.
