//! Types used to define loaded and effective Rexux configuration values.
...
pub use rexux_protocol::config_types::ModeKind;
pub use rexux_protocol::config_types::Personality;
pub use rexux_protocol::config_types::ServiceTier;
...
pub enum ResumeCwdMode {
    /// Use the directory where Rexux was launched.
    Current,
...
/// Determine where Rexux should store CLI auth credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in REXUX_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in REXUX_HOME.
    Auto,
    /// Store credentials in memory only for the current process.
    Ephemeral,
}

/// Determine where Rexux should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
...
pub struct AnalyticsConfigToml {
    /// When `false`, disables analytics across Rexux product surfaces in this profile.
...
pub struct FeedbackConfigToml {
    /// When `false`, disables the feedback flow across Rexux product surfaces.
    pub enabled: Option<bool>,
}
...
pub struct MemoriesToml {
    ...
    /// Minimum remaining percentage required in Rexux rate-limit windows before memory startup runs.
...
pub struct AppConfig {
    /// When `false`, Rexux does not surface this app.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct OtelConfigToml {
    ...
    #[serde(default)]
    pub tool_result: rexux_protocol::config_types::ToolResultLogConfig,
...
/// Fallback resize-reflow row cap when Rexux cannot identify a terminal-specific scrollback size.
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$REXUX_HOME/themes` for custom themes.
    #[serde(default)]
    pub theme: Option<String>,

    /// Pet id to preselect in the terminal pet picker.
    ///
    /// Custom pet ids resolve against REXUX_HOME/pets/<pet-id>/pet.json.
    #[serde(default)]
    ...
    /// transcript exceeds this cap. Omit to use Rexux's terminal-specific default. Set to `0` to
...
/// (primarily the Rexux IDE extension). NOTE: these are different from
...
pub struct Notice {
    ...
    /// Tracks whether the user opted out of Rexux-managed fast defaults.
...
pub struct PluginMcpServerConfig {
    /// When `false`, Rexux skips initializing this plugin MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct MarketplaceConfig {
    /// Last time Rexux successfully added or refreshed this marketplace.
    #[serde(default)]
    pub last_updated: Option<String>,
    /// Git revision Rexux last successfully activated for this marketplace.
    #[serde(default)]
