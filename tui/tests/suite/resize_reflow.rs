...
async fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let codex = rexux_binary(&repo_root)?;
    ...
    write_config(rexux_home.path(), &repo_root)?;
    write_auth(rexux_home.path())?;

    let session_name = format!("rexux-resize-reflow-smoke-{}", std::process::id());
    ...
    let start_output = checked_output(
        Command::new("tmux")
            ...
            .arg("--")
            .arg("env")
            .arg(format!("REXUX_HOME={}", rexux_home.path().display()))
    ...
    let rexux_pane = stdout_text(&start_output).trim().to_string();
    anyhow::ensure!(!rexux_pane.is_empty(), "tmux did not report a pane id");
    ...
    let baseline_capture =
        wait_for_capture_contains(&rexux_pane, draft, Duration::from_secs(/*secs*/ 15))?;
    ...
    let final_capture = capture_pane(&rexux_pane)?;
    let final_row =
...
async fn tmux_width_resize_restore_keeps_visible_content_anchored() -> Result<()> {
    ...
    let session_name = format!("rexux-resize-width-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    sleep(Duration::from_millis(/*millis*/ 1_000));
    let restored_capture = capture_pane(&rexux_pane)?;
    let restored_row =
...
async fn tmux_scrolled_composer_resize_preserves_visible_draft_text() -> Result<()> {
    ...
    let session_name = format!("rexux-resize-scrolled-composer-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    let baseline =
        wait_for_capture_contains(&rexux_pane, &final_line, Duration::from_secs(/*secs*/ 15))?;
        ...
        [("narrowed", "28", "9", 2), ("restored", "44", "14", 6)]
    {
        ...
        let capture = capture_pane(&rexux_pane)?;
...
async fn run_repeated_resize_smoke() -> Result<()> {
    ...
    let session_name = format!("rexux-resize-repeat-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
...
fn rexux_binary(repo_root: &Path) -> Result<PathBuf> {
    ...
    anyhow::ensure!(
        fallback.is_file(),
        "codex binary is unavailable; run `cargo build -p rexux-cli` first"
    );
    Ok(fallback)
}

fn write_config(rexux_home: &Path, repo_root: &Path) -> Result<()> {
...
"#
    ...
    std::fs::write(rexux_home.join("config.toml"), config)?;
    Ok(())
}

fn write_auth(rexux_home: &Path) -> Result<()> {
    std::fs::write(
