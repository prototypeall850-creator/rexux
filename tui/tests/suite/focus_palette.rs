...
fn focus_gained_with_unanswered_palette_queries_preserves_immediate_input() -> Result<()> {
    ...
    write_test_config(rexux_home.path(), &repo_root)?;

    let mut terminal = PtyRexux::start(&repo_root, rexux_home)?;
...
impl PtyRexux {
    pub(super) fn start(repo_root: &Path, rexux_home: TempDir) -> Result<Self> {
        let mut master_fd = -1;
        let mut slave_fd = -1;
        ...
        let codex = rexux_utils_cargo_bin::cargo_bin("codex-tui")
            .or_else(|_| rexux_utils_cargo_bin::cargo_bin("codex"))?;
        let child = Command::new(codex)
            ...
            .stderr(slave)
            .spawn()
            .context("start Rexux in focus-test pseudo-terminal")?;
    ...
    pub(super) fn wait_for_startup(&mut self) -> Result<()> {
        ...
        while Instant::now() < deadline {
            ...
            if self.palette_answered && self.screen_contains("OpenAI Rexux") {
                return Ok(());
            }

            if let Some(status) = self.child.try_wait()? {
                bail!(
                    "Rexux exited before the focus test started ({status}); screen:\n{}",
        ...
        bail!(
            "Rexux did not initialize within {:?}; screen:\n{}",
            STARTUP_TIMEOUT,
            self.screen_contents(),
        );
    }
...
pub(super) fn write_test_config(rexux_home: &Path, repo_root: &Path) -> Result<()> {
    ...
    std::fs::write(rexux_home.join("config.toml"), config)
        .context("write focus-test Rexux configuration")?;
    std::fs::write(
