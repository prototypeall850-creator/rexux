...
fn startup_delays_composer_for_homes_without_authentication_state() -> std::io::Result<()> {
    ...
    let rexux_home = temporary_directory.path().join("rexux-home");
    ...
    assert!(should_delay_startup_composer_for_first_login(
        ...
        Ok(system_config_path.clone()),
        || Ok(false),
        |name| (name == rexux_login::REXUX_ACCESS_TOKEN_ENV_VAR).then(|| "  ".into()),
    ));

    std::fs::create_dir_all(rexux_home.join("tmp").join("arg0"))?;
    assert!(should_delay_startup_composer_for_first_login(
        &rexux_home,
        Ok(system_config_path.clone()),
        || Ok(false),
    ...
    let helper_directory = rexux_home
        .join("tmp")
        .join("arg0")
        .join("rexux-arg0-session");
    ...
    assert!(should_delay_startup_composer_for_first_login(
        ...
        Ok(system_config_path.clone()),
        || Ok(false),
        |name| (name == "REXUX_HOME").then(|| "/custom/home".into()),
    ));
    assert!(!should_delay_startup_composer_for_first_login(
        &rexux_home,
        Ok(system_config_path.clone()),
        || Ok(false),
        |name| { (name == rexux_login::REXUX_ACCESS_TOKEN_ENV_VAR).then(|| "access-token".into()) },
    ));
    for disabled_credential in [
        rexux_login::OPENAI_API_KEY_ENV_VAR,
        rexux_login::REXUX_API_KEY_ENV_VAR,
    ] {
        assert!(should_delay_startup_composer_for_first_login(
            &rexux_home,
            Ok(system_config_path.clone()),
    ...
    for existing_state in ["history.jsonl", "log", "sessions"] {
        let state_path = rexux_home.join(existing_state);
    ...
    assert!(!should_delay_startup_composer_for_first_login(
        ...
        |name| match name {
            "REXUX_HOME" => Some("/custom/home".into()),
            rexux_login::REXUX_ACCESS_TOKEN_ENV_VAR => Some("access-token".into()),
            _ => None,
        },
    ));
    ...
    let daemon_directory = rexux_home.join("app-server-control");
    ...
    let additional_temporary_state = rexux_home.join("tmp").join("other");
...
fn startup_keeps_composer_when_home_state_cannot_be_confirmed() -> std::io::Result<()> {
    ...
    #[cfg(unix)]
    {
        ...
        assert!(!should_delay_startup_composer_for_first_login(
            &blocking_parent.join("rexux-home"),
            Ok(system_config_path),
...
fn startup_keeps_composer_for_workload_identity_markers() -> std::io::Result<()> {
    ...
    for marker in [
        rexux_protocol::shell_environment::OPENAI_FEDERATION_RULE_ID_ENV_VAR,
        rexux_protocol::shell_environment::OPENAI_IDENTITY_TOKEN_FILE_ENV_VAR,
    ] {
    ...
    #[cfg(unix)]
    {
        ...
        assert!(!should_delay_startup_composer_for_first_login(
            ...
            || panic!("workload identity should not probe managed configuration"),
            |name| {
                (name == rexux_protocol::shell_environment::OPENAI_IDENTITY_TOKEN_FILE_ENV_VAR)
