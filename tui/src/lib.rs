...
use rexux_app_server_client::RemoteAppServerClient;
use rexux_app_server_client::RemoteAppServerConnectArgs;
pub use rexux_app_server_client::RemoteAppServerEndpoint;
use rexux_app_server_protocol::Account as AppServerAccount;
...
use rexux_app_server_protocol::ThreadSortKey as AppServerThreadSortKey;
...
use rexux_rollout::StateDbHandle;
use rexux_rollout::state_db;
use rexux_state::log_db;
...
pub(crate) use rexux_app_server_client::legacy_core;
...
async fn init_state_db_for_app_server_target(
    ...
    match app_server_target {
        AppServerTarget::Embedded => state_db::try_init(config).await.map(Some).map_err(|err| {
            let database_path = rexux_state::runtime_db_path_for_corruption_error(&err)
...
fn remove_legacy_tui_log_file(rexux_home: &Path) {
    ...
    let _ = std::fs::remove_file(rexux_home.join("log").join(TUI_LOG_FILE_NAME));
}
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let rexux_home = find_rexux_home().wrap_err("failed to resolve REXUX_HOME")?;
            rexux_app_server_client::app_server_control_socket_path(&rexux_home)
...
#[cfg(unix)]
async fn maybe_probe_default_daemon_socket(rexux_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path = rexux_app_server_client::app_server_control_socket_path(rexux_home).ok()?;
...
#[cfg(not(unix))]
async fn maybe_probe_default_daemon_socket(_rexux_home: &Path) -> Option<AbsolutePathBuf> {
    None
}
...
async fn resolve_startup_resume_or_fork_cwd(
    tui: &mut Tui,
    config: &Config,
    state_db: Option<&rexux_state::StateRuntime>,
...
async fn cloud_config_bundle_for_app_server_target(
    ...
    cloud_config_bundle_loader_for_storage(
        app_server_target
            .auth_config_for_cloud_loader(bootstrap_auth_config(rexux_home, bootstrap_config)?),
        /*enable_rexux_api_key_env*/ false,
    )
    .await
}
...
async fn run_ratatui_app(
    ...
    if let Some(w) = crate::render::highlight::set_theme_override(
        ...
        find_rexux_home().ok().map(AbsolutePathBuf::into_path_buf),
    ) {
    ...
    let required_elevated_sandbox_needs_setup = windows_sandbox_level
        ...
        && config
            ...
            .source
            .is_some()
        && !crate::windows_sandbox::sandbox_setup_is_complete(config.rexux_home.as_path());
    #[cfg(target_os = "windows")]
...
async fn load_bootstrap_config_or_exit(
    ...
    match load_config_toml_with_layer_stack(
        ...
        cwd,
        cli_kv_overrides,
        rexux_config::ConfigLoadOptions {
            loader_overrides,
            strict_config,
            cloud_config_bundle,
        },
...
mod tests {
    ...
    async fn bedrock_setup_wizard_requires_eligible_onboarding() -> color_eyre::Result<()> {
        ...
        for (label, config_toml, login_status, target, expected) in [
            ...
            std::fs::write(rexux_home.path().join("config.toml"), config_toml)?;
    ...
    fn write_session_rollout(
        ...
        let session_meta = rexux_protocol::protocol::SessionMeta {
            ...
            source: rexux_protocol::protocol::SessionSource::Cli,
            model_provider: Some(model_provider.to_string()),
            ..Default::default()
        };
        let session_meta = serde_json::to_value(rexux_protocol::protocol::SessionMetaLine {
            meta: session_meta,
            git: None,
        })?;
        let lines = [
    ...
    async fn startup_services_use_final_cloud_managed_http_policy() -> color_eyre::Result<()> {
            ...
            [(true, false), (false, true)]
        {
            ...
            let prepared_environment_manager =
                EnvironmentManager::prepare_from_rexux_home(rexux_home.path()).await?;
            ...
            let cloud_config_bundle =
                rexux_config::test_support::CloudConfigBundleFixture::loader_with_enterprise_requirement(
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: rexux_app_server_client::app_server_control_socket_path(&rexux_home)?,
            }
        );
        Ok(())
    }
    ...
    async fn default_daemon_auto_connect_skips_missing_socket() -> color_eyre::Result<()> {
        let rexux_home = TempDir::new()?;
        assert!(
            maybe_probe_default_daemon_socket(rexux_home.path())
                .await
                .is_none()
        );
        Ok(())
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        let rexux_home = TempDir::new()?;
        let socket_path =
            rexux_app_server_client::app_server_control_socket_path(rexux_home.path())?;
        ...
        assert_eq!(
            maybe_probe_default_daemon_socket(rexux_home.path()).await,
            Some(socket_path)
        );
        Ok(())
    }
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        ...
        let mut app_server = AppServerSession::new(
            rexux_app_server_client::AppServerClient::InProcess(
    ...
    async fn embedded_state_db_failure_is_typed_for_cli_recovery() -> color_eyre::Result<()> {
        ...
        let sqlite =
            rexux_state::SqliteConfig::new_for_testing(occupied_sqlite_home.as_path().abs());
    ...
    -> color_eyre::Result<()> {
        ...
        let sqlite = rexux_state::SqliteConfig::new_for_testing(sqlite_home.as_path().abs());
        ...
        assert!(
            rexux_state::sqlite_error_detail_is_corruption(startup_error.detail()),
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let rexux_home = temp_dir.path().to_path_buf();
