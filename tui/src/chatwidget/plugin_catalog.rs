...
impl ChatWidget {
    ...
    pub(super) fn marketplace_remove_confirmation_popup_params(
        ...
        header.push(Line::from(
            "This removes the configured marketplace from Rexux.".dim(),
        ));
                ...
                if let Some((location, plugin_name)) = plugin_detail_request {
                    vec![Box::new(move |tx| {
                        ...
                        tx.send(AppEvent::FetchPluginDetail {
                            cwd: cwd.clone(),
                            params: rexux_app_server_protocol::PluginReadParams {
                                marketplace_path,
...
fn plugin_hook_summary(plugin: &PluginDetail) -> String {
    if plugin.hooks.is_empty() {
        ...
        let mut event_counts = Vec::<(rexux_app_server_protocol::HookEventName, usize)>::new();
