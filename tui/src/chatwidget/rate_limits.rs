...
pub(super) fn app_server_rate_limit_error_kind(
    info: &AppServerRexuxErrorInfo,
) -> Option<RateLimitErrorKind> {
    match info {
        AppServerRexuxErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        AppServerRexuxErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        AppServerRexuxErrorInfo::ResponseTooManyFailedAttempts {
...
pub(super) fn is_app_server_cyber_policy_error(info: &AppServerRexuxErrorInfo) -> bool {
    matches!(info, AppServerRexuxErrorInfo::CyberPolicy)
}
...
impl ChatWidget {
    ...
    pub(crate) fn on_rolling_rate_limit_snapshot(&mut self, snapshot: RateLimitSnapshot) {
        if let (Some(previous), Some(current)) = (
            self.rexux_rate_limit_reached_type,
    ...
    fn on_rate_limit_snapshot_from(
        ...
        if let Some(mut snapshot) = snapshot {
            ...
            let is_rexux_limit = limit_id.eq_ignore_ascii_case("codex");
            if is_rexux_limit
                && (matches!(source, RateLimitSnapshotSource::AccountUsage)
                    || snapshot.spend_control_reached.is_some())
            {
                self.rexux_spend_control_reached = snapshot.spend_control_reached;
            }
            if (is_rexux_limit && matches!(source, RateLimitSnapshotSource::AccountUsage))
                || snapshot.rate_limit_reached_type.is_some()
            {
                self.rexux_rate_limit_reached_type = snapshot.rate_limit_reached_type;
            }
            let workspace_limit_reached = self.rexux_spend_control_reached == Some(true)
                || matches!(
                    self.rexux_rate_limit_reached_type,
                    Some(
            ...
            if is_rexux_limit && has_workspace_credits {
            ...
            let should_warn_about_rate_limit_usage = is_rexux_limit && !has_workspace_credits;
            ...
            self.rexux_rate_limit_reached_type = None;
            self.rexux_spend_control_reached = None;
        }
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::RexuxOp(AppCommand::override_turn_context(
                /*cwd*/ None,
