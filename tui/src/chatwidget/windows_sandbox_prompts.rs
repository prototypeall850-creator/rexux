...
impl ChatWidget {
    ...
    pub(super) fn elevated_windows_sandbox_setup_required(&self) -> bool {
        crate::windows_sandbox::level_from_config(&self.config) == WindowsSandboxLevel::Elevated
            && self
                ...
                .source
                .is_some()
            && !crate::windows_sandbox::sandbox_setup_is_complete(self.config.rexux_home.as_path())
    }
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        ...
        let result = rexux_windows_sandbox::apply_world_writable_scan_and_denies_for_permissions(
            self.config.rexux_home.as_path(),
            cwd.as_path(),
            &env_map,
            &permissions,
            Some(self.config.rexux_home.as_path()),
        );
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(
        ...
        header.push(*Box::new(
            Paragraph::new(if allow_unelevated {
                vec![
                    line!["Set up the Rexux agent sandbox to protect your files and control network access. Learn more <https://developers.openai.com/codex/windows>"],
                ]
            } else {
                vec![
                    line!["Your organization requires the default Rexux agent sandbox to continue. Set it up to protect your files and control network access."],
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(
        ...
        if allow_unelevated {
            lines.push(line![
                "You can still use Rexux in a non-admin sandbox. It carries greater risk if prompt injected."
            ]);
        } else {
            lines.push(line![
                "Your organization requires the default sandbox before Rexux can continue."
            ]);
        }
        lines.push(line![
        ...
        if allow_unelevated {
            items.push(SelectionItem {
                name: "Use Rexux with non-admin sandbox".to_string(),
                description: None,
                actions: vec![Box::new({
                    let otel = self.session_telemetry.clone();
