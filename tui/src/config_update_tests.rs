...
async fn remote_project_trust_guards_thread_start_and_preserves_repository_decisions() -> Result<()>
{
    ...
    std::fs::create_dir(project_cwd.join(".rexux"))?;
    ...
    std::fs::write(rexux_home.join("config.toml"), &undecided_config)?;
    std::fs::write(
    ...
    assert_eq!(
        std::fs::read_to_string(rexux_home.join("config.toml"))?,
        undecided_config
    );
    ...
    std::fs::remove_dir(project_cwd.join(".rexux"))?;
