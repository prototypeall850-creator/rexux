...
const GOAL_FILE_PREFIX: &str = "Read the Rexux goal objective file at ";
...
pub(crate) async fn materialize_goal_draft(
    ...
    rexux_home: Option<&GoalFilePath>,
    draft: GoalDraft,
    ...
    for (placeholder, text) in draft.pending_pastes.iter() {
        ...
        let path = ensure_goal_output_dir(app_server, rexux_home, &mut output_dir)
            .await?
...
pub(crate) async fn objective_text_for_edit(
    ...
    let Some(path) = objective_file_path(objective, rexux_home) else {
...
async fn ensure_goal_output_dir(
    ...
    let rexux_home = rexux_home
        .context("App server did not report $REXUX_HOME; cannot materialize goal files")?;
    let path = rexux_home
        .join(GOAL_ATTACHMENT_DIR)
