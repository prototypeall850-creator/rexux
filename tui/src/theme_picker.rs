...
//! under `{REXUX_HOME}/themes/`.  It provides:
//!
...
fn theme_picker_subtitle(rexux_home: Option<&Path>, terminal_width: Option<u16>) -> String {
    let themes_dir = rexux_home.map(|home| home.join("themes"));
...
pub(crate) fn build_theme_picker_params(
    ...
    let entries = highlight::list_available_themes(rexux_home);
    let rexux_home_owned = rexux_home.map(Path::to_path_buf);
    ...
    let preview_home = rexux_home_owned.clone();
...
mod tests {
    ...
    fn theme_picker_uses_half_width_with_stacked_fallback_preview() {
        let params = build_theme_picker_params(
            /*current_name*/ None, /*rexux_home*/ None, /*terminal_width*/ None,
        );
    ...
    #[test]
    fn subtitle_uses_tilde_path_when_rexux_home_under_home_directory() {
        let home = dirs::home_dir().expect("home directory should be available");
        let rexux_home = home.join(".rexux");

        let subtitle = theme_picker_subtitle(Some(&rexux_home), Some(200));
    ...
    fn subtitle_falls_back_when_tilde_path_subtitle_is_too_wide() {
        ...
        let rexux_home = home.join(long_segment).join(".rexux");

        let subtitle = theme_picker_subtitle(Some(&rexux_home), Some(140));
    ...
    fn subtitle_falls_back_to_preview_instructions_without_tilde_path() {
        let subtitle =
            theme_picker_subtitle(/*rexux_home*/ None, /*terminal_width*/ None);
        assert_eq!(subtitle, PREVIEW_FALLBACK_SUBTITLE);
    }
    ...
    fn subtitle_falls_back_for_94_column_terminal_side_by_side_layout() {
        ...
        let subtitle = theme_picker_subtitle(Some(&rexux_home), Some(94));
