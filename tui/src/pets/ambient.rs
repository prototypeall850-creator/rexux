//! Ambient terminal rendering for the Rexux companion.
//!
...
impl AmbientPet {
    ...
    /// REXUX_HOME cache, and records the terminal protocol support snapshot used
    ...
    pub(crate) fn load(
        ...
        let pet = Pet::load_with_rexux_home(
            ...
            /*rexux_home*/ Some(rexux_home),
        )
        .with_context(|| "load ambient pet")?;
        let cache_dir = rexux_home
            .join("cache")
            .join("tui-pets")
            .join("frame-cache")
            .join(&pet.id)
...
mod tests {
    ...
    #[test]
    fn notification_labels_match_rexux_app_vocabulary() {
