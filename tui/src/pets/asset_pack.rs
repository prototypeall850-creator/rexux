...
//! local spritesheets. The TUI resolves them from the public Rexux pets CDN on
//! first use, verifies that the downloaded file has the expected spritesheet
//! geometry, and installs it into a versioned cache under REXUX_HOME.
//!
...
pub(crate) fn builtin_spritesheet_path(rexux_home: &Path, file: &str) -> PathBuf {
    pack_dir(rexux_home).join("assets").join(file)
}
...
pub(crate) async fn ensure_builtin_pet(
    ...
    let destination = builtin_spritesheet_path(rexux_home, pet.spritesheet_file);
...
fn pack_dir(rexux_home: &Path) -> PathBuf {
    rexux_home.join(PET_PACK_DIR).join(PET_PACK_VERSION)
}
...
#[cfg(test)]
pub(crate) fn write_test_pack(rexux_home: &Path) {
    let assets_dir = pack_dir(rexux_home).join("assets");
