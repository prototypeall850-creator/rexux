...
impl EditorPaths {
    fn new() -> Self {
        ...
        let rexux_home = canonical_root.join("rexux-home");
        let cwd = canonical_root.join("workspace");
        fs::create_dir(&rexux_home).expect("create Rexux home");
...
#[test]
fn editor_directory_is_inside_isolated_rexux_home() {
    let paths = EditorPaths::new();
    let policy = workspace_write_policy(&[]);

    let directory = editor_directory(&[&paths.rexux_home], &policy, &paths.cwd)
        .expect("create isolated editor directory");

    assert_eq!(directory, paths.rexux_home.join("editor"));
...
fn editor_directory_rejects_writable_home_editor_and_parent() {
    let paths = EditorPaths::new();
    let editor = paths.rexux_home.join("editor");
    fs::create_dir(&editor).expect("create editor directory");
    let parent = paths.rexux_home.parent().expect("Rexux home parent");

    for writable_root in [paths.rexux_home.as_path(), editor.as_path(), parent] {
        let policy = workspace_write_policy(&[writable_root]);

        assert!(
            editor_directory(&[&paths.rexux_home], &policy, &paths.cwd).is_err(),
...
fn editor_directory_rejects_writable_descendant() {
    let paths = EditorPaths::new();
    let writable_descendant = paths.rexux_home.join("editor").join("nested");
    ...
    assert!(editor_directory(&[&paths.rexux_home], &policy, &paths.cwd).is_err());
}
...
fn editor_directory_rejects_read_only_carveout_with_writable_parent() {
    ...
    let policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry::new(
            AbsolutePathBuf::from_absolute_path(&paths.rexux_home)
                .expect("absolute Rexux home")
                .into(),
...
fn editor_directory_rejects_preexisting_symlink() {
    ...
    symlink(&outside, paths.rexux_home.join("editor")).expect("create editor directory symlink");
...
#[test]
#[cfg(unix)]
fn editor_directory_rejects_writable_rexux_home_alias() {
    use std::os::unix::fs::symlink;

    let paths = EditorPaths::new();
    let aliased_home = paths.cwd.join("rexux-home-link");
    symlink(&paths.rexux_home, &aliased_home).expect("create Rexux home symlink");
    let policy = workspace_write_policy(&[]);

    assert!(policy.can_write_path_with_cwd(&aliased_home.join("editor"), &paths.cwd));
    assert!(!policy.can_write_path_with_cwd(&paths.rexux_home.join("editor"), &paths.cwd));
    assert!(editor_directory(&[&aliased_home], &policy, &paths.cwd).is_err());
}

#[test]
#[cfg(unix)]
fn editor_directory_rejects_writable_rexux_home_alias_target() {
    use std::os::unix::fs::symlink;
    ...
    let alias_parent = paths
        .rexux_home
        .parent()
        .expect("Rexux home parent")
        .join("aliases");
    fs::create_dir(&alias_parent).expect("create protected alias parent");
    let aliased_home = alias_parent.join("rexux-home-link");
    symlink(&paths.rexux_home, &aliased_home).expect("create Rexux home symlink");
    let policy = workspace_write_policy(&[&paths.rexux_home]);
...
fn editor_directory_uses_protected_workspace_fallback_with_default_temporary_grants() {
    let root = tempfile::tempdir().expect("create editor test root");
    let rexux_home = root.path().join("rexux-home");
    let cwd = root.path().join("workspace");
    fs::create_dir(&rexux_home).expect("create Rexux home");
    fs::create_dir(&cwd).expect("create workspace");
    let workspace_rexux_home = cwd.join(".rexux");
    ...
    assert!(!workspace_rexux_home.exists());
    assert!(policy.can_write_path_with_cwd(&rexux_home, &cwd));
    assert!(!policy.can_write_path_with_cwd(&workspace_rexux_home, &cwd));
    assert!(!policy.can_write_path_with_cwd(&workspace_rexux_home.join("editor"), &cwd));

    let directory = editor_directory(&[&rexux_home, &workspace_rexux_home], &policy, &cwd)
        .expect("use protected workspace metadata directory");

    assert_eq!(
        directory,
        dunce::canonicalize(&workspace_rexux_home)
...
fn editor_directory_rejects_explicitly_writable_workspace_fallback() {
    ...
    fs::create_dir(&cwd).expect("create workspace");
    let workspace_rexux_home = cwd.join(".rexux");
    let writable_workspace_rexux_home = AbsolutePathBuf::from_absolute_path(&workspace_rexux_home)
        .expect("absolute workspace metadata directory");
    let policy = FileSystemSandboxPolicy::workspace_write(
        &[writable_workspace_rexux_home],
        /*exclude_tmpdir_env_var*/ false,
        /*exclude_slash_tmp*/ false,
    );
    ...
    assert!(policy.can_write_path_with_cwd(&workspace_rexux_home, &cwd));
    assert!(
        editor_directory(&[&rexux_home, &workspace_rexux_home], &policy, &cwd).is_err(),
...
fn editor_directory_rejects_workspace_fallback_symlink_to_writable_target() {
    ...
    let paths = EditorPaths::new();
    let workspace_rexux_home = paths.cwd.join(".rexux");
    symlink(&paths.rexux_home, &workspace_rexux_home).expect("create workspace metadata symlink");
    let policy = workspace_write_policy(&[&paths.rexux_home]);

    assert!(!policy.can_write_path_with_cwd(&workspace_rexux_home, &paths.cwd));
    assert!(policy.can_write_path_with_cwd(&paths.rexux_home, &paths.cwd));
    assert!(
        editor_directory(
            &[&paths.rexux_home, &workspace_rexux_home],
            &policy,
            &paths.cwd,
        )
        .is_err()
...
fn editor_directory_uses_next_protected_candidate_after_creation_error() {
    ...
    fs::write(&unavailable_home, "not a directory").expect("create unavailable Rexux home");
    let policy = workspace_write_policy(&[]);

    let directory = editor_directory(&[&unavailable_home, &paths.rexux_home], &policy, &paths.cwd)
...
fn editor_directory_rejects_windows_temporary_directory_outside_tmpdir_policy_root() {
    ...
    assert!(!paths.rexux_home.join("editor").exists());
}
...
#[tokio::test]
#[cfg(unix)]
async fn editor_process_receives_buffer_in_isolated_rexux_home() {
    let paths = EditorPaths::new();
    let policy = workspace_write_policy(&[]);
    let editor_directory = paths.rexux_home.join("editor");
...
async fn editor_process_uses_protected_workspace_fallback_with_default_temporary_grants() {
    ...
    fs::create_dir(&rexux_home).expect("create Rexux home");
    fs::create_dir(&cwd).expect("create workspace");
    let default_rexux_home = dirs::home_dir().expect("home directory").join(".rexux");
    let writable_default_rexux_home = AbsolutePathBuf::from_absolute_path(&default_rexux_home)
        .expect("absolute default Rexux home");
    let policy = FileSystemSandboxPolicy::workspace_write(
        &[writable_default_rexux_home],
        /*exclude_tmpdir_env_var*/ false,
        /*exclude_slash_tmp*/ false,
    );
    let workspace_rexux_home = cwd.join(".rexux");
    ...
    assert!(!workspace_rexux_home.exists());
    assert!(policy.can_write_path_with_cwd(&rexux_home, &cwd));
    assert!(policy.can_write_path_with_cwd(&default_rexux_home, &cwd));
    assert!(!policy.can_write_path_with_cwd(&workspace_rexux_home, &cwd));

    let content = run_editor("seed", &editor_command, &rexux_home, &policy, &cwd)
        .await
