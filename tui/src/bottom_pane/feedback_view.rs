...
use rexux_feedback::DOCTOR_REPORT_ATTACHMENT_FILENAME;
use rexux_feedback::FEEDBACK_DIAGNOSTICS_ATTACHMENT_FILENAME;
use rexux_feedback::FeedbackDiagnostics;
...
const REXUX_FEEDBACK_INTERNAL_URL: &str = "http://go/rexux-feedback-internal";
...
pub(crate) fn feedback_success_cell(
    ...
    let mut lines = vec![Line::from(match issue_url.as_ref() {
        Some(_) if feedback_audience == FeedbackAudience::OpenAiEmployee => {
            format!("{prefix} Please report this in #rexux-feedback:")
        }
    ...
    match issue_url {
        Some(url) if feedback_audience == FeedbackAudience::OpenAiEmployee => {
            lines.extend([
                ...
                Line::from(vec![
                    "    ".into(),
                    format!("https://go/rexux-feedback/{thread_id}").bold(),
                ]),
            ]);
        }
        Some(url) => {
...
fn slack_feedback_url(_thread_id: &str) -> String {
    REXUX_FEEDBACK_INTERNAL_URL.to_string()
}
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::render::renderable::Renderable>> = vec![
        ...
        Line::from(vec!["  • ".into(), "rexux-logs.log".into()]).into(),
        Line::from(vec![
            "  • ".into(),
        ...
        Line::from(vec![
            "  • ".into(),
            format!("{REXUX_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME} (if available)").into(),
        ])
        .into(),
        Line::from(vec![
            "  • ".into(),
            format!("{REXUX_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME} (if available)").into(),
        ])
        .into(),
    ];
    ...
    super::SelectionViewParams {
        ...
        items: vec![
            super::SelectionItem {
                name: "Yes".to_string(),
                description: Some(
                    "Share the current Rexux session logs and diagnostics with the team for troubleshooting."
                        .to_string(),
                ),
...
mod tests {
    ...
    use crate::app_event::AppEvent;
    use crate::app_event_sender::AppEventSender;
    use rexux_feedback::FeedbackDiagnostic;
    ...
    fn issue_url_available_for_bug_bad_result_safety_check_and_other() {
        ...
        let expected_slack_url = "http://go/rexux-feedback-internal".to_string();
    ...
    fn feedback_success_cell_matches_employee_bug_copy() {
        ...
        assert_eq!(
            rendered,
            "• Feedback uploaded. Please report this in #rexux-feedback:\n\n  http://go/rexux-feedback-internal\n\n  Share this and add some info about your problem:\n    https://go/rexux-feedback/thread-2"
        );
    }
