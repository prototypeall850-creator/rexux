...
use rexux_config::types::MAX_FUNCTION_KEY;
...
Set unique keys in `~/.rexux/config.toml` and retry. \
See the Rexux keymap documentation for supported actions and examples."
                    ));
                }
            }
        }
...
Set a different key in `~/.rexux/config.toml` and retry. \
See the Rexux keymap documentation for supported actions and examples."
                ));
            }
        }
