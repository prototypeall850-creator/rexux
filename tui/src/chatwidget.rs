//! The main Rexux TUI chat surface.
//!
...
use crate::version::REXUX_CLI_VERSION;
...
use rexux_app_server_protocol::RexuxErrorInfo as AppServerRexuxErrorInfo;
...
use rexux_app_server_protocol::CommandExecutionSource as ExecCommandSource;
use rexux_app_server_protocol::CreditsSnapshot;
...
use rexux_app_server_protocol::ModelVerification as AppServerModelVerification;
use rexux_app_server_protocol::RateLimitReachedType;
...
use rexux_app_server_protocol::SkillMetadata;
use rexux_app_server_protocol::SkillsListResponse;
use rexux_app_server_protocol::ThreadGoal as AppThreadGoal;
use rexux_app_server_protocol::ThreadGoalStatus as AppThreadGoalStatus;
...
use rexux_config::ConstraintResult;
...
#[cfg(test)]
use rexux_git_utils::CommitLogEntry;
use rexux_git_utils::current_branch_name;
use rexux_git_utils::get_git_repo_root;
use rexux_git_utils::local_git_branches;
use rexux_git_utils::recent_commits;
use rexux_otel::RuntimeMetricsSummary;
use rexux_otel::SessionTelemetry;
use rexux_plugin::PluginCapabilitySummary;
use rexux_protocol::ThreadId;
use rexux_protocol::account::PlanType;
use rexux_protocol::approvals::GuardianAssessmentAction;
use rexux_protocol::approvals::GuardianAssessmentDecisionSource;
use rexux_protocol::approvals::GuardianAssessmentEvent;
use rexux_protocol::approvals::GuardianAssessmentStatus;
...
use rexux_protocol::plan_tool::PlanItemArg as UpdatePlanItemArg;
use rexux_protocol::plan_tool::StepStatus as UpdatePlanItemStatus;
...
use rexux_terminal_detection::Multiplexer;
use rexux_terminal_detection::TerminalInfo;
use rexux_terminal_detection::TerminalName;
use rexux_terminal_detection::terminal_info;
use rexux_utils_absolute_path::AbsolutePathBuf;
use rexux_utils_cli::resume_hint;
...
use rexux_file_search::FileMatch;
...
use rexux_utils_approval_presets::ApprovalPreset;
use rexux_utils_approval_presets::builtin_approval_presets;
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) has_rexux_backend_auth: bool,
    pub(crate) model_catalog: Arc<ModelCatalog>,
    pub(crate) feedback: rexux_feedback::RexuxFeedback,
...
/// and by sending requests back to rexux-core.
///
...
pub(crate) struct ChatWidget {
    ...
    rexux_rate_limit_reached_type: Option<RateLimitReachedType>,
    rexux_spend_control_reached: Option<bool>,
    ...
    pub(crate) pet_http_client: rexux_http_client::RouteAwareClientPool,
    ...
    feedback: rexux_feedback::RexuxFeedback,
...
fn request_permissions_from_params(
    params: rexux_app_server_protocol::PermissionsRequestApprovalParams,
...
impl ChatWidget {
    ...
    pub(crate) fn open_feedback_consent(&mut self, category: crate::app_event::FeedbackCategory) {
        ...
        let include_windows_sandbox_log =
            rexux_windows_sandbox::current_log_file_path_for_rexux_home(&self.config.rexux_home)
                .is_file();
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.rexux_op_target {
            RexuxOpTarget::Direct(rexux_op_tx) => {
                crate::session_log::log_outbound_op(&op);
                if let Err(e) = rexux_op_tx.send(op) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            RexuxOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::RexuxOp(op));
            }
        }
        true
    }
...
const PLACEHOLDER: &str = "Ask Rexux to do anything";
