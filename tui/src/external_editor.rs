...
pub(crate) async fn run_editor(
    ...
    let default_rexux_home = dirs::home_dir().map(|home| home.join(".rexux"));
    #[cfg(any(target_os = "macos", target_os = "linux"))]
    let project_rexux_home = cwd.join(".rexux");
    let mut candidate_homes = vec![rexux_home];
    if let Some(default_rexux_home) = default_rexux_home.as_deref() {
        candidate_homes.push(default_rexux_home);
    }
    #[cfg(any(target_os = "macos", target_os = "linux"))]
    candidate_homes.push(&project_rexux_home);
