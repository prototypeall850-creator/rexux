...
-> Result<()> {
    ...
    app.config.rexux_home = home.path().to_path_buf().abs();
    app.config.chatgpt_base_url = server.uri();
    app.config.sqlite = rexux_state::SqliteConfig::new_for_testing(home.path().abs());
    ...
    app.chat_widget.handle_server_notification(
        ServerNotification::Error(ErrorNotification {
            error: AppServerTurnError {
                ...
                rexux_error_info: Some(RexuxErrorInfo::UsageLimitExceeded),
    ...
    rolling.rate_limit_reached_type =
        Some(rexux_app_server_protocol::RateLimitReachedType::WorkspaceMemberCreditsDepleted);
    app.handle_app_server_event(
        &session,
        rexux_app_server_client::AppServerEvent::ServerNotification(Box::new(
            ServerNotification::AccountRateLimitsUpdated(
                rexux_app_server_protocol::AccountRateLimitsUpdatedNotification {
    ...
    app.handle_app_server_event(
        ...
        rexux_app_server_client::AppServerEvent::ServerNotification(Box::new(
            ServerNotification::AccountUpdated(
                rexux_app_server_protocol::AccountUpdatedNotification {
                    auth_mode: None,
                    plan_type: None,
                },
            ),
...
async fn backend_banner_reset_redemption_rejects_pre_reset_content() -> Result<()> {
    ...
    app.handle_event(
        ...
        AppEvent::RateLimitResetCreditConsumed {
            ...
            credit_id: None,
            result: Ok(
                rexux_app_server_protocol::ConsumeAccountRateLimitResetCreditResponse {
                    outcome:
                        rexux_app_server_protocol::ConsumeAccountRateLimitResetCreditOutcome::Reset,
                },
            ),
        },
    )
    ...
    recovered.rate_limit_reset_credits =
        Some(rexux_app_server_protocol::RateLimitResetCreditsSummary {
            available_count: 0,
            credits: None,
        });
    app.handle_event(
