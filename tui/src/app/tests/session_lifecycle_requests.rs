...
use rexux_app_server_protocol::ClientNotification;
...
use rexux_protocol::items::ExitedReviewModeItem;
...
async fn start_recording_app_server_with_history(
    ...
    let embedded = crate::start_embedded_app_server(
        rexux_arg0::Arg0DispatchPaths::default(),
        config.clone(),
        Vec::new(),
        rexux_config::LoaderOverrides::default(),
        /*strict_config*/ false,
        rexux_config::CloudConfigBundleLoader::default(),
        rexux_feedback::RexuxFeedback::new(),
        /*log_db*/ None,
        state_db,
        Arc::new(rexux_exec_server::EnvironmentManager::default_for_tests()),
    )
    .await?;
    let rexux_home = config.rexux_home.display().to_string();
...
async fn make_history_test_app() -> Result<(App, tempfile::TempDir)> {
    ...
    app.config.sqlite = SqliteConfig::new_for_testing(rexux_home.path().abs());
    Ok((app, rexux_home))
}
...
async fn removing_remote_thread_omits_disconnect_guidance() -> Result<()> {
    for event in [
        ...
        let (mut app, rexux_home) = make_history_test_app().await?;
...
async fn external_transport_registers_dynamic_tools_and_finds_task_mentions() -> Result<()> {
    let (app, _rexux_home) = make_history_test_app().await?;
    ...
    for params in starts {
        ...
        assert_eq!(params["dynamicTools"][0]["name"], "rexux_tui");
        assert_eq!(
...
async fn archive_current_thread_reports_success_only_after_archiving() -> Result<()> {
    let (mut app, _rexux_home) = make_history_test_app().await?;
    let thread_id = ThreadId::from_string(
        &create_fake_rollout(
            &app.config.rexux_home,
            "2026-08-25T01-00-00",
...
async fn local_daemon_registers_approval_gated_mcp_tools_for_both_start_paths() -> Result<()> {
    ...
    app.config
        .web_search_mode
        .set(rexux_protocol::config_types::WebSearchMode::Live)?;
    std::fs::write(
    ...
    let inventory: rexux_app_server_protocol::ListMcpServerStatusResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::McpServerStatusList {
            request_id: AppServerRequestId::String("tui-tool-inventory".to_string()),
            params: rexux_app_server_protocol::ListMcpServerStatusParams {
                cursor: None,
                limit: None,
                detail: Some(rexux_app_server_protocol::McpServerStatusDetail::ToolsAndAuthOnly),
    ...
    let tools = &inventory
        .data
        .iter()
        .find(|server| server.name == "rexux_tui")
    ...
    for params in &starts {
        ...
        let server = &params["config"]["mcp_servers.rexux_tui"];
        assert!(
            server["url"]
                .as_str()
    ...
    let mcp_url = starts[0]["config"]["mcp_servers.rexux_tui"]["url"]
        .as_str()
        .expect("MCP server URL");
    let unauthorized = rexux_http_client::HttpClientBuilder::new()
        .build_direct()?
        .post(mcp_url)
        .send()
        .await?;
    ...
    app.config
        .web_search_mode
        .set(rexux_protocol::config_types::WebSearchMode::Disabled)?;
    ...
    assert_eq!(
        resumed["config"]["mcp_servers.rexux_tui"],
        starts[0]["config"]["mcp_servers.rexux_tui"]
    );
    app_server
        .resume_thread(
            app.config.clone(),
    ...
    assert_eq!(
        reattached["config"]["mcp_servers.rexux_tui"],
        starts[0]["config"]["mcp_servers.rexux_tui"]
    );
    app_server
    ...
    assert_eq!(
        forked["config"]["mcp_servers.rexux_tui"],
        starts[0]["config"]["mcp_servers.rexux_tui"]
    );
    let authorization =
        starts[0]["config"]["mcp_servers.rexux_tui"]["http_headers"]["Authorization"]
            .as_str()
            .expect("MCP bearer token");
    let client = rexux_http_client::HttpClientBuilder::new().build_direct()?;
    ...
    assert_eq!(
        child["config"]["mcp_servers.rexux_tui"],
        starts[0]["config"]["mcp_servers.rexux_tui"]
    );
...
async fn local_mcp_respects_configured_servers_and_managed_requirements() -> Result<()> {
    for scenario in ["conflicting", "blocked", "mismatched", "allowed"] {
        ...
        if scenario == "conflicting" {
            let raw = serde_json::from_value::<rexux_config::RawMcpServerConfig>(
            ...
            servers.insert(
                ...
                rexux_config::McpServerConfig::try_from(raw)
            ...
            if matches!(scenario, "mismatched" | "allowed") {
                let requirement = if scenario == "allowed" {
                    rexux_config::McpServerRequirement::Url(
                        rexux_protocol::mcp_policy::McpServerValueMatcher::Prefix {
                            value: "http://127.0.0.1:".to_string(),
                        },
                    )
                } else {
                    rexux_config::McpServerRequirement::Identity {
                        identity: rexux_config::McpServerIdentity::Url {
            ...
            let requirements = rexux_config::ConfigRequirements {
                mcp_servers: Some(rexux_config::Sourced::new(
                    allowed_servers,
                    rexux_config::RequirementSource::Unknown,
                )),
                ..Default::default()
            };
            app.config.config_layer_stack = rexux_config::ConfigLayerStack::new(
                Vec::new(),
                requirements,
        ...
        if scenario == "allowed" {
            ...
            assert!(start["config"]["mcp_servers.rexux_tui"].is_object());
        } else {
            assert_eq!(
                start["dynamicTools"][0]["tools"].as_array().map(Vec::len),
                Some(6)
            );
            assert!(start["config"]["mcp_servers.rexux_tui"].is_null());
        }
        app_server.shutdown().await?;
        proxy.await??;
    }
...
async fn embedded_server_rejects_unowned_dynamic_tool_calls() -> Result<()> {
    ...
    app.handle_app_server_event(
        &app_server,
        rexux_app_server_client::AppServerEvent::ServerRequest(Box::new(
            ServerRequest::DynamicToolCall {
                ...
                params: rexux_app_server_protocol::DynamicToolCallParams {
                    ...
                    namespace: Some("rexux_app".to_string()),
...
async fn dynamic_tool_requests_ignore_other_namespaces_and_dispatch_tui_namespace() -> Result<()> {
    ...
    for namespace in [Some("rexux_app"), None] {
        app.handle_app_server_event(
            &app_server,
            rexux_app_server_client::AppServerEvent::ServerRequest(Box::new(
                ServerRequest::DynamicToolCall {
        ...
        rexux_app_server_client::AppServerEvent::ServerRequest(Box::new(
            ServerRequest::DynamicToolCall {
                ...
                params: rexux_app_server_protocol::DynamicToolCallParams {
                    ...
                    namespace: Some("rexux_tui".to_string()),
                    tool: "list_threads".to_string(),
                    arguments: serde_json::json!({}),
                },
            },
    ...
    app_server
        ...
        .await?;
    let project: rexux_app_server_protocol::ProjectCreateResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::ProjectCreate {
            request_id: AppServerRequestId::String("create-source-project".to_string()),
            params: rexux_app_server_protocol::ProjectCreateParams {
                name: "Source project".to_string(),
                roots: vec![rexux_app_server_protocol::ProjectRoot {
        ...
        .await?;
    let _: rexux_app_server_protocol::ThreadMetadataUpdateResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::ThreadMetadataUpdate {
            request_id: AppServerRequestId::String("assign-source-project".to_string()),
            params: rexux_app_server_protocol::ThreadMetadataUpdateParams {
        ...
        .await?;
    let source_settings: rexux_app_server_protocol::ThreadResumeResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::ThreadResume {
            request_id: AppServerRequestId::String("read-source-sandbox".to_string()),
            params: rexux_app_server_protocol::ThreadResumeParams {
                thread_id: creation_source.to_string(),
                ..rexux_app_server_protocol::ThreadResumeParams::default()
            },
        })
        .await?;
    ...
    spawn_approved_task_tool_call(
        ...
        rexux_app_server_protocol::DynamicToolCallParams {
            thread_id: creation_source.to_string(),
            turn_id: "turn-1".to_string(),
...
async fn transcript_home_loads_every_older_history_page() -> Result<()> {
    ...
    let export_path = rexux_home.path().join("complete-export.md");
    app.chat_widget
...
async fn remote_legacy_history_start_negotiates_once_for_resume_and_fork() -> Result<()> {
    ...
    let response = crate::dynamic_tools::execute(
        ...
        rexux_app_server_protocol::ThreadStartParams::default(),
        status_updates,
        /*app_event_tx*/ None,
    )
    .await;
...
async fn underfilled_scrollback_fetches_older_pages_without_opening_the_transcript() -> Result<()> {
    ...
    if let Some(Overlay::Transcript(overlay)) = app.overlay.as_mut() {
        ...
        assert!(!partial.contains("OpenAI Rexux"));
        ...
        assert!(!loading.contains("OpenAI Rexux"));
                ...
                app.handle_app_server_event(
                    ...
                    AppServerEvent::ServerNotification(Box::new(
                        ServerNotification::ThreadStatusChanged(
                            rexux_app_server_protocol::ThreadStatusChangedNotification {
                                thread_id: started.session.thread_id.to_string(),
...
async fn cold_paginated_subagent_transcript_excludes_inherited_parent_history() -> Result<()> {
    let (app, rexux_home) = make_history_test_app().await?;
    let parent_thread_id = create_history_rollout(
        &app.config,
        ThreadHistoryMode::Paginated,
...
async fn changing_directory_preserves_project_trust_permissions_history_and_hooks() -> Result<()> {
    use rexux_protocol::config_types::TrustLevel as T;
    use serde_json::json;
    use std::fs;
    ...
    let [current, trusted, unknown, untrusted, mismatch, failed] =
        names.map(|name| rexux_home.path().join(name));
    fs::create_dir_all(&current)?;
    for directory in [&trusted, &unknown, &untrusted, &mismatch, &failed] {
        fs::create_dir_all(directory.join(".rexux"))?;
    ...
    let requirements = rexux_home.path().join("requirements.toml");
    ...
    for dir in [&trusted, &untrusted, &mismatch, &failed] {
        ...
        crate::legacy_core::config::set_project_trust_level(rexux_home.path(), dir, trust)
    ...
    let meta = rexux_rollout::read_session_meta_line(&forked_rollout).await?;
    ...
    let source = rexux_utils_path_uri::PathUri::from_abs_path(&agents.abs());
