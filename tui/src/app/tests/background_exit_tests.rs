...
async fn daemon_ctrl_c_shows_background_exit_menu_and_escape_dismisses_it() -> Result<()> {
    ...
    › 1. Cancel task        Stop the current task and stay in Rexux
      2. Run in background  Exit Rexux and leave the task running
      3. Exit               Stop the current task and exit Rexux
...
async fn exit_interrupts_before_requesting_shutdown() -> Result<()> {
    ...
    let goal = app_server
        .thread_goal_set(
            ...
            Some(rexux_app_server_protocol::ThreadGoalStatus::Paused),
            /*token_budget*/ None,
        )
        .await?
        .goal;
...
async fn daemon_ctrl_c_hides_background_exit_for_running_background_side_thread() -> Result<()> {
    ...
    › 1. Cancel task  Stop the current task and stay in Rexux
      2. Exit         Stop the current task and exit Rexux
