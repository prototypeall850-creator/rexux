...
use rexux_app_server_client::AppServerPath;
use rexux_app_server_protocol::AdditionalFileSystemPermissions;
use rexux_app_server_protocol::AdditionalNetworkPermissions;
use rexux_app_server_protocol::AdditionalPermissionProfile;
use rexux_app_server_protocol::AgentMessageDeltaNotification;
...
use rexux_app_server_protocol::FileUpdateChange;
use rexux_app_server_protocol::ItemStartedNotification;
use rexux_app_server_protocol::JSONRPCErrorError;
use rexux_app_server_protocol::McpServerElicitationRequest;
...
use rexux_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
use rexux_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
use rexux_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
use rexux_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
use rexux_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
use rexux_app_server_protocol::PatchChangeKind;
use rexux_app_server_protocol::PermissionsRequestApprovalParams;
use rexux_app_server_protocol::RequestId as AppServerRequestId;
...
use rexux_app_server_protocol::TokenUsageBreakdown;
...
use rexux_app_server_protocol::UserInput as AppServerUserInput;
...
use rexux_protocol::config_types::CollaborationModeMask;
...
use rexux_protocol::models::NetworkPermissions;
use rexux_protocol::models::PermissionProfile;
use rexux_protocol::openai_models::MODEL_SPECIALTY_CYBER;
use rexux_protocol::protocol::EventMsg;
use rexux_protocol::protocol::MAX_THREAD_GOAL_OBJECTIVE_CHARS;
use rexux_protocol::protocol::MultiAgentVersion;
use rexux_protocol::protocol::SessionMeta;
use rexux_protocol::protocol::SessionSource as RolloutSessionSource;
...
use rexux_protocol::user_input::TextElement;
...
async fn next_thread_settings_updated(
    ...
    for _ in 0..20 {
        ...
        if let rexux_app_server_client::AppServerEvent::ServerNotification(notification) = event
...
async fn handle_mcp_inventory_result_respects_origin_thread() {
    ...
    app.handle_mcp_inventory_result(
        Ok(vec![McpServerStatus {
            ...
            auth_status: rexux_app_server_protocol::McpAuthStatus::Unsupported,
        }]),
...
async fn external_editor_writable_directory_rejected_snapshot() -> Result<()> {
    ...
    let (mut app, mut app_event_rx, _op_rx) = make_test_app_with_channels().await;
    let rexux_home = app.chat_widget.config_ref().rexux_home.clone();
    let fallback_home = dirs::home_dir()
        .expect("home directory")
        .join(".rexux")
        .abs();
    let workspace_rexux_home = app.chat_widget.config_ref().cwd.join(".rexux");
    let permission_profile = PermissionProfile::workspace_write_with(
        &[rexux_home, fallback_home, workspace_rexux_home],
        rexux_protocol::permissions::NetworkSandboxPolicy::Restricted,
...
async fn enqueue_primary_thread_session_replays_buffered_approval_after_attach() -> Result<()> {
    ...
    app.handle_app_server_event(
        &app_server,
        rexux_app_server_client::AppServerEvent::ServerNotification(Box::new(
            ServerNotification::ServerRequestResolved(
                rexux_app_server_protocol::ServerRequestResolvedNotification {
...
async fn enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit() -> Result<()> {
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        match event {
            ...
            AppEvent::RexuxOp(Op::UserTurn { items, .. }) => {
                assert!(
...
async fn history_lookup_response_is_routed_to_requesting_thread() -> Result<()> {
    ...
    let cursor = rexux_message_history::HistoryBatchCursor::new(/*end_offset*/ 10);
...
async fn active_history_batch_is_delivered_without_replay_buffering() -> Result<()> {
    ...
    let cursor = rexux_message_history::HistoryBatchCursor::new(/*end_offset*/ 1);
    let event = HistoryLookupResponse::Batch {
        ...
        next_older_cursor: Some(rexux_message_history::HistoryBatchCursor::new(
            /*end_offset*/ 0,
        )),
    };
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(Box::new(
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::CollabAgentToolCall {
                id: "wait-1".to_string(),
                tool: rexux_app_server_protocol::CollabAgentTool::Wait,
                status: rexux_app_server_protocol::CollabAgentToolCallStatus::InProgress,
...
async fn collab_receiver_notification_does_not_cache_not_found_thread() {
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(Box::new(
        ServerNotification::ItemCompleted(rexux_app_server_protocol::ItemCompletedNotification {
            thread_id: ThreadId::new().to_string(),
            turn_id: "turn-1".to_string(),
            completed_at_ms: 0,
            item: ThreadItem::CollabAgentToolCall {
                id: "send-1".to_string(),
                tool: rexux_app_server_protocol::CollabAgentTool::SendInput,
                status: rexux_app_server_protocol::CollabAgentToolCallStatus::Failed,
                sender_thread_id: ThreadId::new().to_string(),
                receiver_thread_ids: vec![receiver_thread_id.to_string()],
                ...
                agents_states: HashMap::from([(
                    ...
                    rexux_app_server_protocol::CollabAgentState {
                        status: rexux_app_server_protocol::CollabAgentStatus::NotFound,
                        message: None,
                    },
                )]),
            },
    ...
    runtime.block_on(async {
        ...
        assert!(
            std::iter::from_fn(|| app_event_rx.try_recv().ok())
                .any(|event| matches!(event, AppEvent::RexuxOp(Op::UserTurn { .. })))
        );
...
async fn update_memory_settings_persists_and_updates_widget_config() -> Result<()> {
    ...
    app.config.rexux_home = rexux_home.path().to_path_buf().abs();
    ...
    let config = std::fs::read_to_string(rexux_home.path().join("config.toml"))?;
...
fn update_memory_settings_updates_current_thread_memory_mode() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let rexux_home = tempdir()?;
        app.config.rexux_home = rexux_home.path().to_path_buf().abs();
        app.config.sqlite = rexux_state::SqliteConfig::new_for_testing(rexux_home.path().abs());
        ...
        let state_db = rexux_state::StateRuntime::init(
            rexux_state::SqliteConfig::new_for_testing(rexux_home.path().abs()),
...
async fn reset_memories_clears_local_memory_directories() -> Result<()> {
    Box::pin(async {
        ...
        let memory_root = rexux_home.path().join("memories");
...
"#,
    ...
    let op = match app_event_rx.try_recv() {
        Ok(AppEvent::RexuxOp(op)) => op,
        other => panic!("expected RexuxOp event, got {other:?}"),
    };
    assert_eq!(
        op,
...
-> Result<()> {
    ...
    let rexux_home = tempdir()?;
    app.config.rexux_home = rexux_home.path().to_path_buf().abs();
    let config_toml_path = rexux_home.path().join("config.toml").abs();
...
async fn side_defers_subagent_approval_overlay_until_side_exits() -> Result<()> {
    ...
    app.enqueue_thread_request(
        ...
        ServerRequest::DynamicToolCall {
            ...
            params: rexux_app_server_protocol::DynamicToolCallParams {
...
async fn inactive_thread_exec_approval_preserves_context() {
    ...
    assert_eq!(
        ...
        vec![
            rexux_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            rexux_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession,
            rexux_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                network_policy_amendment: AppServerNetworkPolicyAmendment {
                    host: "example.com".to_string(),
                    action: AppServerNetworkPolicyRuleAction::Allow,
                },
            },
            rexux_app_server_protocol::CommandExecutionApprovalDecision::Cancel,
        ]
    );
}
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::FileChange {
                ...
                status: rexux_app_server_protocol::PatchApplyStatus::InProgress,
            },
        }),
    )
    .await
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: rexux_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: AppServerRequestId::Integer(10),
                decision: rexux_app_server_protocol::McpServerElicitationAction::Decline,
                content: None,
                meta: None,
            },
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                ...
                updated_at: 2,
                recency_at: Some(2),
                status: rexux_app_server_protocol::ThreadStatus::Idle,
                path: Some(rollout_path.clone()),
                cwd: test_path_buf("/tmp/agent").abs(),
                cli_version: "0.0.0".to_string(),
                source: rexux_app_server_protocol::SessionSource::Unknown,
...
async fn background_side_cleanup_removes_local_state_and_ignores_late_events() -> Result<()> {
    ...
    app.handle_app_server_event(
        &app_server,
        rexux_app_server_client::AppServerEvent::ServerRequest(Box::new(exec_approval_request(
            side_thread_id,
            "turn-1",
            "item-1",
            Some("approval-1"),
    ...
    let resolution = app
        ...
        .take_resolution(
            ...
            Op::ExecApproval {
                ...
                turn_id: None,
                decision: rexux_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            },
        )
...
async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()> {
    ...
    assert_eq!(
        app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
            rexux_app_server_protocol::SkillsChangedNotification {},
        )),
        None
    );
    Ok(())
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget.set_service_tier(Some(
        rexux_protocol::config_types::ServiceTier::Fast
            .request_value()
            .to_string(),
    ));
...
async fn set_thread_goal_draft_materializes_long_objective_and_confirms_before_paste() -> Result<()>
{
    ...
    let rexux_home = app_server
        .rexux_home_path(&app.chat_widget.config_ref().rexux_home)
        .expect("codex home");
    assert!(goal_files::objective_file_path(&goal.objective, Some(&rexux_home)).is_some());
    assert_eq!(
        goal_files::objective_text_for_edit(&mut app_server, Some(&rexux_home), &goal.objective)
            .await
    ...
    for path in [
        format!("/tmp/{suffix}"),
        format!("{rexux_home}/../other/{suffix}"),
        format!("{rexux_home}/other/{suffix}"),
    ] {
        assert!(!is_managed(&rexux_home, &path));
    }
    assert!(!is_managed(
    ...
    let attachments_dir = app.chat_widget.config_ref().rexux_home.join("attachments");
...
fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
    let turn_error = AppServerTurnError {
        ...
        rexux_error_info: Some(AppServerRexuxErrorInfo::ActiveTurnNotSteerable {
...
async fn remote_resume_current_cwd_rejection_snapshot() -> Result<()> {
    let (mut app, mut app_event_rx, _op_rx) = make_test_app_with_channels().await;
    std::fs::write(
        app.config.rexux_home.join("config.toml"),
        "[tui]\nresume_cwd = \"current\"\n",
    )?;
...
async fn remote_exec_resume_current_cwd_is_rejected() -> Result<()> {
    ...
    app.environment_manager = Arc::new(
        EnvironmentManager::create_for_tests(
            ...
            Some(rexux_exec_server::ExecServerRuntimePaths::new(
                std::env::current_exe()?,
                /*rexux_linux_sandbox_exe*/ None,
            )?),
        )
        .await,
    );
...
async fn in_app_resume_uses_configured_or_explicit_cwd() -> Result<()> {
    ...
    for (configured_mode, has_explicit_cwd, has_remote_exec, has_runtime_cwd, expected_directory) in [
        ...
        std::fs::write(
            rexux_home.join("config.toml"),
            format!("[tui]\nresume_cwd = \"{configured_mode}\"\n"),
        )?;
        let config = ConfigBuilder::default()
            .rexux_home(rexux_home.clone())
            .loader_overrides(LoaderOverrides::without_managed_config_for_tests())
        ...
        .expect("materialized rollout should be created");
        let rollout_path =
            app_test_support::rollout_path(&rexux_home, filename_timestamp, &thread_id);
...
async fn remembered_current_cwd_stays_at_launch_across_in_app_resumes() -> Result<()> {
    ...
    crate::legacy_core::config::edit::ConfigEditsBuilder::for_config(&config)
        .set_resume_cwd(rexux_config::types::ResumeCwdMode::Current)
        .apply()
        .await
...
async fn prompt_edit_forks_before_selected_prompt_and_preserves_source() -> Result<()> {
    ...
    .expect("materialized rollout should be created");
    let source_path =
        app_test_support::rollout_path(config.rexux_home.as_path(), filename_ts, &source_thread_id);
    ...
    for (turn_id, message, images, local_images) in [
        ...
        for item in [
            ...
            rexux_rollout::append_rollout_item_to_path(&source_path, &item).await?;
        }
    }
        ...
        .await?;
    {
        ...
        for item in selected_turn.items {
            store.push_notification(ServerNotification::ItemCompleted(
                rexux_app_server_protocol::ItemCompletedNotification {
...
async fn replace_chat_widget_reseeds_collab_agent_metadata_for_replay() {
    ...
    let replacement = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        has_rexux_backend_auth: app.chat_widget.has_rexux_backend_auth(),
        model_catalog: app.model_catalog.clone(),
        ...
        ThreadEventSnapshot {
            ...
            events: vec![ThreadBufferedEvent::Notification(Box::new(
                ServerNotification::ItemStarted(
                    rexux_app_server_protocol::ItemStartedNotification {
                        ...
                        item: ThreadItem::CollabAgentToolCall {
                            ...
                            tool: rexux_app_server_protocol::CollabAgentTool::Wait,
                            status:
                                rexux_app_server_protocol::CollabAgentToolCallStatus::InProgress,
                            sender_thread_id: ThreadId::new().to_string(),
                            receiver_thread_ids: vec![receiver_thread_id.to_string()],
                            prompt: None,
                            model: None,
    ...
    Box::pin(async {
        ...
        let op = AppCommand::override_turn_context(
            ...
            Some(ActivePermissionProfile::new(
                rexux_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE,
            )),
            /*windows_sandbox_level*/ None,
...
async fn changing_cyber_model_reasoning_preserves_selected_permissions() {
    Box::pin(async {
        ...
        assert!(
            app.apply_permission_profile_selection(PermissionProfileSelection {
                profile_id: rexux_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY
                    .to_string(),
        ...
        for effort in [ReasoningEffortConfig::High, ReasoningEffortConfig::Ultra] {
            ...
            assert_eq!(
                settings
                    ...
                    .expect("active permission profile")
                    .id,
                rexux_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY
            );
            assert_eq!(
...
async fn selecting_cyber_model_falls_back_to_user_when_auto_review_is_unavailable() {
    ...
    assert_eq!(
        ...
        Some(rexux_app_server_protocol::AskForApproval::OnRequest)
    );
    assert_eq!(
        params.approvals_reviewer,
        Some(rexux_app_server_protocol::ApprovalsReviewer::User)
    );
}
...
async fn selecting_cyber_model_respects_auto_review_requirements() {
    Box::pin(async {
        ...
        let mut requirements_with_sources = rexux_config::ConfigRequirementsWithSources::default();
        requirements_with_sources.merge_unset_fields(
            rexux_config::RequirementSource::Unknown,
            requirements_toml.clone(),
        );
        let requirements = rexux_config::ConfigRequirements::try_from(requirements_with_sources)
...
async fn inactive_thread_settings_notification_updates_cached_collaboration_mode() {
    ...
    let notification = ThreadSettingsUpdatedNotification {
        ...
        thread_settings: ThreadSettings {
            ...
            approval_policy: AskForApproval::OnRequest,
            approvals_reviewer: rexux_app_server_protocol::ApprovalsReviewer::AutoReview,
            sandbox_policy: rexux_app_server_protocol::SandboxPolicy::ReadOnly {
                network_access: false,
            },
            active_permission_profile: Some(
                rexux_app_server_protocol::ActivePermissionProfile::read_only(),
            ),
            model: "gpt-plan".to_string(),
...
async fn clear_only_ui_reset_allows_active_skill_warning_to_render_again() {
    ...
    let error = SkillErrorInfo {
        path: test_path_buf("/tmp/project/.rexux/skills/abc/SKILL.md"),
