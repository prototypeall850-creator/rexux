...
mod tests {
    ...
    use rexux_config::ConstraintError;
    use rexux_config::FeatureRequirementsToml;
    use rexux_config::FilesystemConstraints;
    use rexux_config::HookEventsToml;
    use rexux_config::HookHandlerConfig;
    use rexux_config::LoaderOverrides;
    use rexux_config::ManagedHooksRequirementsToml;
    use rexux_config::MatcherGroup;
    use rexux_config::McpServerIdentity;
    use rexux_config::McpServerRequirement;
    use rexux_config::NetworkConstraints;
    use rexux_config::NetworkDomainPermissionToml;
    use rexux_config::NetworkDomainPermissionsToml;
    use rexux_config::NetworkHeaderInjectionToml;
    use rexux_config::NetworkUnixSocketPermissionToml;
    use rexux_config::NetworkUnixSocketPermissionsToml;
    use rexux_config::RequirementSource;
    use rexux_config::ResidencyRequirement;
    use rexux_config::SandboxModeRequirement;
...
"#,
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\OpenAI\\Rexux\\requirements.toml")
        } else {
        ...
        let user_file = if cfg!(windows) {
            ...
            absolute_path("/home/alice/.rexux/config.toml")
        };
        let stack = ConfigLayerStack::new(
