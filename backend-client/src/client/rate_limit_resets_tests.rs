...
fn rate_limit_reset_contract_uses_expected_paths_and_payloads() {
    assert_eq!(
        test_client("https://example.test", PathStyle::RexuxApi).rate_limit_status_url(),
        "https://example.test/api/codex/usage"
    );
    assert_eq!(
        test_client("https://example.test", PathStyle::RexuxApi).rate_limit_reset_credits_url(),
        "https://example.test/api/codex/rate-limit-reset-credits"
    );
    assert_eq!(
        test_client("https://example.test", PathStyle::RexuxApi)
        ...
        RateLimitResetCreditsDetails {
            credits: vec![
                RateLimitResetCreditDetails {
                    ...
                    reset_type: "rexux_rate_limits".to_string(),
