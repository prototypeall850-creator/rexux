...
Runtime orchestration for Phase 1 and Phase 2 still lives in `rexux-core` under
`codex-rs/core/src/memories/`.

## Crates

- `codex-rs/memories/read` (`rexux-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `codex-rs/memories/write` (`rexux-memories-write`) owns the write path:
...
- In `rexux`, edit those undated template files in place.
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.rexux/memories/.git` by `rexux-git-utils`
