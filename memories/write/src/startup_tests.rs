...
use rexux_git_utils::diff_since_latest_init;
use rexux_git_utils::reset_git_repository;
...
use rexux_state::Phase2JobClaimOutcome;
...
async fn phase2_retries_when_clean_workspace_is_missing_artifacts() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        "# Memory Workspace Diff\n\nGenerated by Rexux before Phase 2 memory consolidation. Read this file first and do not edit it.\n\n## Status\n- none\n"
    );
...
-> anyhow::Result<()> {
    ...
    core_test_support::submit_thread_settings(
        ...
        rexux_protocol::protocol::ThreadSettingsOverrides {
            ...
            permission_profile: Some(rexux_protocol::models::PermissionProfile::workspace_write()),
            ..Default::default()
        },
    )
    .await?;
...
async fn run_memory_phase_one_model_request_test(
    ...
    let test = build_test_rexux_with_memories_config(server, Arc::clone(&home), memories).await?;
...
async fn run_memory_phase_two_model_request_test(
    ...
    let test = build_test_rexux_with_memories_config(server, home.clone(), memories).await?;
    let provider = Arc::new(MockMemoryModelProvider::new(
        test.config.model_provider.clone(),
        Some(test.thread_manager.auth_manager()),
    ));
    ...
    let (context, config) = memory_startup_context_with_provider(&test, provider).await;
    let root = memory_root(&config.rexux_home);
...
async fn build_test_codex(
    ...
    build_test_rexux_with_memories_config(server, home, startup_test_memories_config()).await
}

async fn build_test_rexux_with_memories_config(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
    memories: MemoriesConfig,
) -> anyhow::Result<TestRexux> {
...
async fn init_state_db(home: &Arc<TempDir>) -> anyhow::Result<Arc<rexux_state::StateRuntime>> {
    let db = rexux_state::StateRuntime::init(
        rexux_state::SqliteConfig::new_for_testing(home.path().abs()),
        "test-provider".into(),
    )
    .await?;
...
async fn trigger_memories_startup(test: &TestRexux) {
...
) -> (Arc<MemoryStartupContext>, Arc<rexux_core::config::Config>) {
    let config_snapshot = test.codex.config_snapshot().await;
    let mut config = test.config.clone();
    config
        .features
...
impl ModelProvider for MockMemoryModelProvider {
    ...
    ) -> rexux_models_manager::manager::SharedModelsManager {
        self.delegate
            .models_manager(rexux_home, config_model_catalog)
    }
}

async fn seed_stage1_output(
    db: &rexux_state::StateRuntime,
    rexux_home: &Path,
    ...
    let mut metadata_builder = rexux_state::ThreadMetadataBuilder::new(
        thread_id,
        rexux_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = rexux_home.join(format!("workspace-{rollout_slug}"));
...
async fn seed_stage1_candidate(
    ...
    let thread_id = ThreadId::new();
    let rollout_path = rexux_home.join(format!("rollout-{thread_id}.jsonl"));
...
) -> anyhow::Result<rexux_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.codex.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        rexux_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
...
async fn shutdown_test_codex(test: &TestRexux) -> anyhow::Result<()> {
