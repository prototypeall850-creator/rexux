...
use rexux_core::content_items_to_text;
use rexux_core::detached_memory_responses_metadata;
...
pub(crate) struct SpawnedConsolidationAgent {
    ...
    pub(crate) thread: Arc<RexuxThread>,
}
...
impl StageOneRequestContext {
    pub(crate) fn start_timer(&self, name: &str) -> Option<rexux_otel::Timer> {
...
fn build_session_telemetry(
    ...
    let auth_mode = auth.map(RexuxAuth::auth_mode).map(TelemetryAuthMode::from);
    let account_id = auth.and_then(RexuxAuth::get_account_id);
    let account_email = auth.and_then(RexuxAuth::get_account_email);
        ...
        while let Some(message) = stream.next().await.transpose()? {
            match message {
                ...
                ResponseEvent::OutputItemDone(item) => {
                    if result.is_empty()
                        && let rexux_protocol::models::ResponseItem::Message { content, .. } = item
