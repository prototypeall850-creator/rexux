...
use rexux_protocol::protocol::RealtimeAudioFrame as CoreRealtimeAudioFrame;
...
use rexux_protocol::realtime::BemItemPresentation as CoreBemItemPresentation;
use rexux_protocol::realtime::RealtimeItem as CoreRealtimeItem;
use rexux_protocol::realtime::RealtimeItemContent as CoreRealtimeItemContent;
use rexux_protocol::realtime::RealtimeSessionOutcome as CoreRealtimeSessionOutcome;
use rexux_protocol::realtime::RealtimeTranscriptRole as CoreRealtimeTranscriptRole;
...
pub struct ThreadRealtimeStartParams {
    ...
    /// Leaves Rexux response handoffs to the client's explicit append calls instead of forwarding
    ...
    /// Routes any transcript tail remaining at session end through Rexux. Defaults to false.
    ...
    #[ts(optional = nullable)]
    pub rexux_responses_as_items: Option<bool>,
    // TODO: Remove this experiment-only prefix with `rexux_responses_as_items`.
    /// Optional prefix added to automatic Rexux response items when `codexResponsesAsItems` is true.
    #[ts(optional = nullable)]
    pub rexux_response_item_prefix: Option<String>,
    /// Selects how automatic Rexux responses are routed in Frameless Bidi sessions. Omitted values
    /// default to `thinking`. Realtime V1 and V2 ignore this setting.
    #[ts(optional = nullable)]
    pub rexux_response_handoff_mode: Option<RexuxResponseHandoffMode>,
    ...
    /// Set to false to start without Rexux's startup context. Omitted or null includes it.
    #[ts(optional = nullable)]
    ...
    /// Developer instructions given to the backing Rexux model when this realtime session starts.
    #[ts(optional = nullable)]
    pub realtime_start_instructions: Option<String>,
    /// Developer instructions given to the backing Rexux model when this realtime session ends.
    #[ts(optional = nullable)]
