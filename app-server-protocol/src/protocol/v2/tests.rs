...
use rexux_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use rexux_protocol::approvals::GuardianAssessmentAction as CoreGuardianAssessmentAction;
use rexux_protocol::config_types::MultiAgentMode;
use rexux_protocol::items::AgentMessageContent;
use rexux_protocol::items::AgentMessageItem;
use rexux_protocol::items::CollabAgentTool as CoreCollabAgentTool;
use rexux_protocol::items::CollabAgentToolCallItem;
use rexux_protocol::items::CollabAgentToolCallStatus as CoreCollabAgentToolCallStatus;
use rexux_protocol::items::CommandExecutionItem;
...
use rexux_protocol::items::FileChangeItem;
use rexux_protocol::items::ImageViewItem;
use rexux_protocol::items::McpToolCallItem;
use rexux_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
use rexux_protocol::items::ReasoningItem;
use rexux_protocol::items::SubAgentActivityItem;
use rexux_protocol::items::TurnItem;
use rexux_protocol::items::UserMessageItem;
use rexux_protocol::items::WebSearchItem as CoreWebSearchItem;
use rexux_protocol::mcp::CallToolResult;
use rexux_protocol::mcp::McpServerInfo;
use rexux_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use rexux_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
...
use rexux_protocol::models::MessagePhase;
...
use rexux_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use rexux_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use rexux_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use rexux_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use rexux_protocol::protocol::AgentStatus as CoreAgentStatus;
use rexux_protocol::protocol::AskForApproval as CoreAskForApproval;
use rexux_protocol::protocol::RexuxErrorInfo as CoreRexuxErrorInfo;
use rexux_protocol::protocol::ConversationTextRole;
use rexux_protocol::protocol::ExecCommandSource as CoreExecCommandSource;
use rexux_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use rexux_protocol::protocol::NetworkAccess as CoreNetworkAccess;
...
use rexux_protocol::user_input::UserInput as CoreUserInput;
...
fn thread_sources_round_trip_as_scalar_labels() {
    for (source, label) in [
        ...
        let core_source: rexux_protocol::protocol::ThreadSource = source.clone().into();
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        rexux_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        rexux_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
fn config_requirements_read_accepts_foreign_path_uris() {
    let response: ConfigRequirementsReadResponse = serde_json::from_value(json!({
        "requirements": {
            "sqliteHome": "file:///C:/Users/alice/.rexux/state",
            "logDir": "file:///C:/Users/alice/.rexux/logs",
            "modelCatalogJson": "file:///C:/Users/alice/.rexux/models.json"
        }
    }))
    ...
    assert_eq!(
        ...
        Some(PathUri::parse("file:///C:/Users/alice/.rexux/state").expect("valid URI"))
    );
    assert_eq!(
        requirements.log_dir,
        Some(PathUri::parse("file:///C:/Users/alice/.rexux/logs").expect("valid URI"))
    );
    assert_eq!(
        requirements.model_catalog_json,
        Some(PathUri::parse("file:///C:/Users/alice/.rexux/models.json").expect("valid URI"))
    );
}
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        rexux_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    let user_item = TurnItem::UserMessage(UserMessageItem {
        ...
        content: vec![
            ...
            CoreUserInput::Skill {
                ...
                path: PathBuf::from("/repo/.rexux/skills/skill-creator/SKILL.md"),
            },
    ...
    let command_item = TurnItem::CommandExecution(CommandExecutionItem {
        ...
        parsed_cmd: vec![rexux_protocol::parse_command::ParsedCommand::Unknown {
    ...
    let dynamic_tool_call_item = TurnItem::DynamicToolCall(DynamicToolCallItem {
        ...
        content_items: Some(vec![
            rexux_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText {
                text: "ok".to_string(),
            },
    ...
    let sender_thread_id = rexux_protocol::ThreadId::default();
    let receiver_thread_id = rexux_protocol::ThreadId::default();
    ...
    let sub_agent_activity_item = TurnItem::SubAgentActivity(SubAgentActivityItem {
        ...
        agent_path: rexux_protocol::AgentPath::root()
            .join("worker")
            .expect("worker path"),
    });
    ...
    assert_eq!(
        ThreadItem::from(TurnItem::Extension(
            rexux_extension_items::ExtensionItem::WebSearch(expected_search_item.clone()),
        )),
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(rexux_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
...
#[test]
fn rexux_error_info_serializes_http_status_code_in_camel_case() {
    let value = RexuxErrorInfo::ResponseTooManyFailedAttempts {
...
fn core_error_info_converts_to_camel_case() {
    for (core, expected) in [
        (CoreRexuxErrorInfo::CyberPolicy, json!("cyberPolicy")),
        (
            CoreRexuxErrorInfo::RateLimitExceeded,
            json!("rateLimitExceeded"),
        ),
    ] {
        assert_eq!(
            serde_json::to_value(RexuxErrorInfo::from(core)).unwrap(),
            expected
        );
    }
}
...
#[test]
fn rexux_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
    let value = RexuxErrorInfo::ActiveTurnNotSteerable {
...
fn turn_start_cyber_access_program_uses_separate_wire_formats() {
    for (app_server_value, core_value) in [
        ...
        .expect("params should deserialize");
        let core_program: rexux_protocol::turn_input::CyberAccessProgram = params
...
fn turn_start_params_round_trip_multi_agent_mode() {
    ...
    assert_eq!(
        ...
        Some(rexux_protocol::config_types::MultiAgentMode::Proactive)
    );
    assert_eq!(
...
fn thread_settings_update_params_preserve_field_level_experimental_gates() {
    ...
    let collaboration_mode = ThreadSettingsUpdateParams {
        thread_id: "thread_123".to_string(),
        collaboration_mode: Some(rexux_protocol::config_types::CollaborationMode {
            mode: rexux_protocol::config_types::ModeKind::Plan,
            settings: rexux_protocol::config_types::Settings {
...
fn realtime_start_deserializes_client_handoff_channel_prefixes() {
    ...
    assert_eq!(
        params.rexux_response_handoff_channel_prefixes,
        Some(BTreeMap::from([
