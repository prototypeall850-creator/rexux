# rexux-app-server-daemon

> `rexux-app-server-daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`rexux-app-server-daemon` backs the machine-readable `rexux app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for Rexux instances launched over SSH, including fresh developer
...
```sh
rexux app-server daemon start
rexux app-server daemon restart
rexux app-server daemon enable-remote-control
rexux app-server daemon disable-remote-control
rexux app-server daemon stop
rexux app-server daemon version
rexux app-server daemon bootstrap --remote-control
```
...
$HOME/.rexux/packages/standalone/current/rexux app-server daemon bootstrap --remote-control
```

`bootstrap` requires the standalone managed install. It records the daemon
settings under `REXUX_HOME/app-server-daemon/`, starts app-server as a
pidfile-backed detached process, and launches a detached updater loop.

## Installation and update cases

The daemon assumes Rexux is installed through `install.sh` and always launches
the standalone managed binary under `REXUX_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `REXUX_HOME/packages/standalone/current/rexux` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `REXUX_HOME/packages/standalone/current/rexux` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Top-level `rexux remote-control` bootstraps with `--remote-control` when the
...
All mutating lifecycle commands are serialized per `REXUX_HOME`, so a concurrent
...
The daemon stores its local state under `REXUX_HOME/app-server-daemon/`:
