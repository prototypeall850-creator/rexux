...
use super::parse_rexux_version;

#[test]
fn parses_rexux_cli_version_output() {
    assert_eq!(
        parse_rexux_version("codex 1.2.3\n").expect("version"),
        "1.2.3"
    );
}

#[test]
fn rejects_malformed_rexux_cli_version_output() {
    assert!(parse_rexux_version("codex\n").is_err());
}
