...
use rexux_app_server_transport::app_server_control_socket_path;
use rexux_utils_home_dir::find_rexux_home;
use managed_install::managed_rexux_bin;
#[cfg(unix)]
use managed_install::managed_rexux_version;
use serde::Serialize;
...
pub struct LifecycleOutput {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub pid: Option<u32>,
    pub managed_rexux_path: PathBuf,
    pub managed_rexux_version: Option<String>,
...
struct Daemon {
    ...
    managed_rexux_bin: PathBuf,
}

impl Daemon {
    fn from_environment() -> Result<Self> {
        let rexux_home = find_rexux_home().context("failed to resolve REXUX_HOME")?;
        let socket_path = app_server_control_socket_path(rexux_home.as_path())?
            .as_path()
            .to_path_buf();
        let state_dir = rexux_home.as_path().join(STATE_DIR_NAME);
        Ok(Self {
            socket_path,
            ...
            managed_rexux_bin: managed_rexux_bin(rexux_home.as_path()),
        })
    }
    ...
    pub(crate) async fn try_restart_if_running(
        ...
        let outcome = if let Some(backend) = self.running_backend_instance(&settings).await? {
            ...
            let managed_version = if info.is_some() {
                Some(managed_rexux_version(managed_rexux_bin).await?)
            } else {
                None
            };
            match restart_decision(mode, info.as_ref(), managed_version.as_deref()) {
                ...
                RestartDecision::Restart => {
                    backend.stop().await?;
                    let _ = self
                        .start_managed_backend_with_bin(&settings, managed_rexux_bin)
                        .await?;
        ...
        if should_reexec_updater(updater_refresh_mode, outcome) {
            crate::update_loop::reexec_managed_updater(managed_rexux_bin)?;
        }
    ...
    async fn append_daemon_app_server_context(&self, context: &mut String) {
        let managed_rexux_version = self
            .managed_rexux_version_best_effort()
            .await
            .unwrap_or_else(|| "unknown".to_string());
        context.push_str(&format!(
            "\n\nDaemon used app-server:\n  path: {}\n  version: {managed_rexux_version}",
            self.managed_rexux_bin.display()
        ));
    }
    ...
    async fn bootstrap_locked(&self, options: BootstrapOptions) -> Result<BootstrapOutput> {
        ...
        let info = self.wait_until_ready().await?;
        let managed_rexux_version = self.managed_rexux_version_best_effort().await;
        Ok(BootstrapOutput {
            status: BootstrapStatus::Bootstrapped,
            backend: BackendKind::Pid,
            auto_update_enabled: true,
            remote_control_enabled: settings.remote_control_enabled,
            managed_rexux_path: self.managed_rexux_bin.clone(),
            managed_rexux_version,
    ...
    async fn start_managed_backend(&self, settings: &DaemonSettings) -> Result<Option<u32>> {
        self.start_managed_backend_with_bin(settings, &self.managed_rexux_bin)
            .await
    }
    ...
    async fn start_managed_backend_with_bin(
        ...
        let backend =
            backend::pid_backend(self.backend_paths_with_bin(settings, managed_rexux_bin));
        backend.start().await
    }
    ...
    fn ensure_managed_rexux_bin(&self) -> Result<()> {
        if self.managed_rexux_bin.is_file() {
            return Ok(());
        }

        let managed_rexux_path = self.managed_rexux_bin.display();
        Err(anyhow!(
            "managed standalone Rexux install not found at {managed_rexux_path}\n\n\
             This command requires the standalone install managed by the Rexux installer, because \
    ...
    #[cfg(unix)]
    async fn managed_rexux_version_best_effort(&self) -> Option<String> {
        managed_rexux_version(&self.managed_rexux_bin).await.ok()
    }
    ...
    fn backend_paths(&self, settings: &DaemonSettings) -> BackendPaths {
        self.backend_paths_with_bin(settings, &self.managed_rexux_bin)
    }
    ...
    fn backend_paths_with_bin(
        ...
        BackendPaths {
            rexux_bin: managed_rexux_bin.to_path_buf(),
...
mod tests {
    ...
    fn remote_control_start_output_serializes_inner_output_without_tag() {
        let lifecycle_output = LifecycleOutput {
            ...
            pid: None,
            managed_rexux_path: "codex".into(),
            managed_rexux_version: Some("1.2.3".to_string()),
        ...
        assert_eq!(
            ...
            serde_json::json!({
                ...
                "backend": "pid",
                "managedRexuxPath": "codex",
                "managedRexuxVersion": "1.2.3",
    ...
    async fn not_ready_context_reports_daemon_app_server_before_stderr() {
        ...
        let daemon = Daemon {
            ...
            managed_rexux_bin: temp_dir.path().join("missing-codex"),
        };
        ...
        assert_eq!(
            ...
            format!(
                ...
                daemon.managed_rexux_bin.display(),
                stderr_log.display()
            )
        );
    }
