...
const WFP_SETUP_SERVICE_NAME: &str = "rexux-windows-sandbox-setup";
...
fn build_wfp_metrics_provider(
    ...
    // The setup helper cannot call rexux-core's OTEL builder because core
...
fn emit_wfp_setup_metric(
    ...
    let Some(provider) = build_wfp_metrics_provider(rexux_home, otel)? else {
        return Ok(());
    };
    ...
    F: FnMut(&str),
{
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        emit_wfp_setup_metric(rexux_home, otel, metric)
    }));
    match result {
        Ok(Ok(())) => {}
    ...
    F: FnMut(&str),
{
    ...
    emit_wfp_setup_metric_safely(rexux_home, otel, offline_username, &metric, &mut log);
}
