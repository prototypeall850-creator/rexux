...
mod tests {
    ...
    #[test]
    fn denies_rexux_and_agents_inside_writable_root() {
        let tmp = TempDir::new().expect("tempdir");
        let command_cwd = tmp.path().join("workspace");
        let rexux_dir = command_cwd.join(".rexux");
        let agents_dir = command_cwd.join(".agents");
        let _ = fs::create_dir_all(&rexux_dir);
        ...
        let expected_deny: HashSet<PathBuf> = [
            dunce::canonicalize(&rexux_dir).unwrap(),
