...
pub fn current_log_file_path_for_rexux_home(rexux_home: &Path) -> PathBuf {
    current_log_file_path(&crate::sandbox_dir(rexux_home))
}
...
mod tests {
    ...
    #[test]
    fn current_log_file_path_for_rexux_home_uses_sandbox_dir() {
        let rexux_home = Path::new("rexux-home");

        assert_eq!(
            current_log_file_path_for_rexux_home(rexux_home),
            current_log_file_path(&rexux_home.join(".sandbox"))
        );
    }
}
