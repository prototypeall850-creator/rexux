...
use rexux_windows_sandbox::dpapi_protect;
...
pub const SANDBOX_USERS_GROUP: &str = "RexuxSandboxUsers";
const SANDBOX_USERS_GROUP_COMMENT: &str = "Rexux sandbox internal group (managed)";
...
fn write_secrets(
    ...
    let secrets_dir = sandbox_secrets_dir(rexux_home);
...
pub(super) fn prepare_setup_marker(rexux_home: &Path, real_user: &str) -> Result<()> {
    let marker_path = sandbox_dir(rexux_home).join("setup_marker.json");
