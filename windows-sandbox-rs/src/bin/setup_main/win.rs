...
use rexux_otel::StatsigMetricsSettings;
use rexux_windows_sandbox::SETUP_VERSION;
use rexux_windows_sandbox::SetupErrorCode;
use rexux_windows_sandbox::SetupErrorReport;
use rexux_windows_sandbox::SetupFailure;
use rexux_windows_sandbox::add_deny_write_ace;
use rexux_windows_sandbox::convert_string_sid_to_sid;
use rexux_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use rexux_windows_sandbox::ensure_allow_write_aces;
use rexux_windows_sandbox::extract_setup_failure;
use rexux_windows_sandbox::hide_newly_created_users;
use rexux_windows_sandbox::install_wfp_filters;
use rexux_windows_sandbox::log_note;
use rexux_windows_sandbox::log_writer;
use rexux_windows_sandbox::path_mask_allows;
use rexux_windows_sandbox::path_write_aces_need_refresh;
use rexux_windows_sandbox::sandbox_bin_dir;
use rexux_windows_sandbox::sandbox_dir;
use rexux_windows_sandbox::sandbox_secrets_dir;
use rexux_windows_sandbox::string_from_sid_bytes;
use rexux_windows_sandbox::sync_persistent_deny_read_acls;
use rexux_windows_sandbox::to_wide;
use rexux_windows_sandbox::workspace_write_cap_sid_for_root;
use rexux_windows_sandbox::workspace_write_root_overlaps_path;
use rexux_windows_sandbox::write_setup_error_report;
...
fn lock_sandbox_dir(
    ...
    // ProvisionOnly accepts another user's REXUX_HOME; keep its ACL mutation
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(rexux_home) = std::env::var("REXUX_HOME") {
            let sbx_dir = sandbox_dir(Path::new(&rexux_home));
...
fn real_main() -> Result<()> {
    ...
    let sbx_dir = sandbox_dir(&payload.rexux_home);
    ...
    if let Err(err) = &result {
        ...
        if let Err(write_err) = write_setup_error_report(&payload.rexux_home, &report) {
            let _ = log_line(
                &mut log,
...
fn run_setup(payload: &Payload, log: &mut dyn Write, sbx_dir: &Path) -> Result<()> {
    ...
    if writes_setup_marker {
        prepare_setup_marker(&payload.rexux_home, &payload.real_user)?;
    }
    match payload.mode {
...
fn lock_persistent_sandbox_dirs(payload: &Payload, sandbox_group_sid: &[u8]) -> Result<()> {
    lock_sandbox_dir(
        &sandbox_dir(&payload.rexux_home),
        &payload.real_user,
        sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_dir(&payload.rexux_home).display()
            ),
        ))
    })?;
    lock_sandbox_dir(
        &sandbox_secrets_dir(&payload.rexux_home),
        &payload.real_user,
        sandbox_group_sid,
        DENY_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_secrets_dir(&payload.rexux_home).display()
            ),
        ))
    })?;
    let legacy_users = sandbox_dir(&payload.rexux_home).join("sandbox_users.json");
...
fn lock_sandbox_bin_dir(payload: &Payload, sandbox_group_sid: &[u8]) -> Result<()> {
    lock_sandbox_dir(
        &sandbox_bin_dir(&payload.rexux_home),
        &payload.real_user,
        sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_EXECUTE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_bin_dir(&payload.rexux_home).display()
            ),
        ))
    })
}
...
fn run_setup_full(payload: &Payload, log: &mut dyn Write, sbx_dir: &Path) -> Result<()> {
    ...
    if refresh_only {
        setup_runtime_bin::ensure_rexux_app_runtime_paths_readable(
            sandbox_group_psid,
            &mut refresh_errors,
            log,
        )?;
    ...
    for root in &payload.write_roots {
        ...
        let root_cap_sid_str =
            workspace_write_cap_sid_for_root(&payload.rexux_home, &payload.command_cwd, root)?;
...
mod tests {
    ...
    use rexux_otel::StatsigMetricsSettings;
    use rexux_windows_sandbox::ensure_allow_mask_aces;
    use rexux_windows_sandbox::ensure_allow_write_aces;
    use rexux_windows_sandbox::load_or_create_cap_sids;
    use rexux_windows_sandbox::path_mask_allows;
    use rexux_windows_sandbox::path_write_aces_need_refresh;
    use rexux_windows_sandbox::workspace_write_cap_sid_for_root;
    ...
    fn payload_json() -> serde_json::Value {
        json!({
            ...
            "offline_username": "RexuxSandboxOffline",
            "online_username": "RexuxSandboxOnline",
            "rexux_home": "C:\\rexux-home",
    ...
    fn write_root_refresh_replaces_stale_delete_child_grant() {
        ...
        let rexux_home = temp.path().join("rexux-home");
        let workspace = temp.path().join("workspace");
        fs::create_dir_all(&rexux_home).expect("create codex home");
        fs::create_dir_all(&workspace).expect("create workspace");

        let sid = workspace_write_cap_sid_for_root(&rexux_home, &workspace, &workspace)
    ...
    fn write_root_refresh_checks_each_sid() {
        ...
        let workspace_sid = workspace_write_cap_sid_for_root(&rexux_home, &workspace, &workspace)
            .expect("workspace sid");
        let other_sid = workspace_write_cap_sid_for_root(&rexux_home, &workspace, &other_root)
    ...
    fn deny_path_under_active_root_uses_only_matching_root_sid() {
        ...
        let stale_sid = workspace_write_cap_sid_for_root(&rexux_home, &workspace, &stale_root)
            .expect("stale sid");
        let active_sid = workspace_write_cap_sid_for_root(&rexux_home, &workspace, &active_root)
            .expect("active sid");
        let workspace_sid = workspace_write_cap_sid_for_root(&rexux_home, &workspace, &workspace)
            .expect("workspace sid");
        let caps = load_or_create_cap_sids(&rexux_home).expect("load caps");
    ...
    fn deny_path_includes_nested_active_root_sid() {
        ...
        let rexux_home = temp.path().join("rexux-home");
        let workspace = temp.path().join("workspace");
        let protected_dir = workspace.join(".rexux");
        ...
        let workspace_sid = workspace_write_cap_sid_for_root(&rexux_home, &workspace, &workspace)
            .expect("workspace sid");
        let nested_sid = workspace_write_cap_sid_for_root(&rexux_home, &workspace, &nested_root)
            .expect("nested sid");
