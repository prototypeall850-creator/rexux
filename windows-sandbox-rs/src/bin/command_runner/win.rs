...
use anyhow::Context;
use anyhow::Result;
use rexux_utils_pty::JobObject;
use rexux_windows_sandbox::ConsoleMode;
use rexux_windows_sandbox::ErrorPayload;
use rexux_windows_sandbox::ErrorStage;
use rexux_windows_sandbox::ExitPayload;
use rexux_windows_sandbox::FramedMessage;
use rexux_windows_sandbox::IPC_PROTOCOL_VERSION;
use rexux_windows_sandbox::LaunchDesktop;
use rexux_windows_sandbox::LocalSid;
use rexux_windows_sandbox::Message;
use rexux_windows_sandbox::OutputPayload;
use rexux_windows_sandbox::OutputStream;
use rexux_windows_sandbox::PipeSpawnHandles;
use rexux_windows_sandbox::ResizePayload;
use rexux_windows_sandbox::SpawnReady;
use rexux_windows_sandbox::SpawnRequest;
use rexux_windows_sandbox::StderrMode;
use rexux_windows_sandbox::StdinMode;
use rexux_windows_sandbox::WindowsSandboxTokenMode;
use rexux_windows_sandbox::allow_null_device;
use rexux_windows_sandbox::create_readonly_token_with_caps_and_user_from;
use rexux_windows_sandbox::create_workspace_write_token_with_caps_and_user_from;
use rexux_windows_sandbox::decode_bytes;
use rexux_windows_sandbox::encode_bytes;
use rexux_windows_sandbox::get_current_token_for_restriction;
use rexux_windows_sandbox::hide_current_user_profile_dir;
use rexux_windows_sandbox::log_note;
use rexux_windows_sandbox::read_frame;
use rexux_windows_sandbox::read_handle_loop;
use rexux_windows_sandbox::spawn_process_with_pipes;
use rexux_windows_sandbox::to_wide;
use rexux_windows_sandbox::token_mode_for_permission_profile;
use rexux_windows_sandbox::write_frame;
use std::ffi::OsStr;
use std::fs::File;
...
// Kept in sync with rexux_exec_server::REXUX_FS_HELPER_ARG1 without introducing
// a dependency cycle.
const FS_HELPER_ARG: &str = "--rexux-run-as-fs-helper";
const READ_ACL_MUTEX_NAME: &str = "Local\\RexuxSandboxReadAcl";
...
struct IpcSpawnedProcess {
    ...
    conpty_owner: Option<rexux_windows_sandbox::ConptyInstance>,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.rexux_home.clone();
    hide_current_user_profile_dir(req.rexux_home.as_path());
    ...
    let (pi, job, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, mut conpty) = rexux_windows_sandbox::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,
