...
pub const REXUX_WINDOWS_SANDBOX_ARG1: &str = "--run-as-windows-sandbox";

const COMMAND_CWD_FLAG: &str = "--command-cwd";
const REXUX_HOME_FLAG: &str = "--rexux-home";
...
pub fn create_windows_sandbox_command_args_for_permission_profile(
    ...
    let mut args = vec![
        REXUX_WINDOWS_SANDBOX_ARG1.to_string(),
        REXUX_HOME_FLAG.to_string(),
        rexux_home.to_string_lossy().into_owned(),
...
async fn run_windows_sandbox_wrapper_request(request: WindowsSandboxWrapperRequest) -> Result<i32> {
    ...
    let spawned =
        crate::spawn_windows_sandbox_session_for_level(crate::WindowsSandboxSessionRequest {
            ...
            rexux_home: request.rexux_home.as_path(),
...
fn parse_windows_sandbox_wrapper_args(args: Vec<String>) -> Result<WindowsSandboxWrapperRequest> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            REXUX_HOME_FLAG => rexux_home = Some(PathBuf::from(next_flag_value(&mut args, &arg)?)),
            COMMAND_CWD_FLAG => {
    ...
    let rexux_home = rexux_home.ok_or_else(|| anyhow!("missing required {REXUX_HOME_FLAG}"))?;
    if !rexux_home.is_absolute() {
        bail!(
            "{REXUX_HOME_FLAG} must be absolute: {}",
            rexux_home.display()
        );
    }
