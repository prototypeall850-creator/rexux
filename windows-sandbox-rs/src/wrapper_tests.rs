...
use super::REXUX_HOME_FLAG;
use super::REXUX_WINDOWS_SANDBOX_ARG1;
...
fn windows_wrapper_args_round_trip() {
    ...
    let args = create_windows_sandbox_command_args_for_permission_profile(
        vec![
            "codex.exe".to_string(),
            "--rexux-run-as-fs-helper".to_string(),
        ],
        &command_cwd,
    ...
    assert_eq!(args[0], REXUX_WINDOWS_SANDBOX_ARG1);
    assert!(args.contains(&REXUX_HOME_FLAG.to_string()));
    ...
    assert_eq!(
        parsed.command,
        vec!["codex.exe", "--rexux-run-as-fs-helper"]
    );
