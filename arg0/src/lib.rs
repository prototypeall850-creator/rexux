...
use rexux_apply_patch::REXUX_CORE_APPLY_PATCH_ARG1;
use rexux_async_utils::THREAD_STACK_SIZE_BYTES;
#[cfg(unix)]
use rexux_exec_server::REXUX_ARG0_EXEC_HELPER_ARG1;
use rexux_exec_server::REXUX_FS_HELPER_ARG1;
use rexux_install_context::InstallContext;
use rexux_sandboxing::landlock::REXUX_LINUX_SANDBOX_ARG0;
use rexux_utils_home_dir::find_rexux_home;
use rexux_utils_file_lock::try_lock_exclusive;
#[cfg(target_os = "windows")]
use rexux_windows_sandbox::REXUX_WINDOWS_SANDBOX_ARG1;
#[cfg(unix)]
...
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "rexux-execve-wrapper";
const LOCK_FILENAME: &str = ".lock";

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Arg0DispatchPaths {
    /// Stable path to the current Rexux executable for child re-execs.
    ///
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime.block_on(
            rexux_shell_escalation::run_shell_escalation_execve_wrapper(file, argv),
        );
        match exit_code {
    ...
    if exe_name == REXUX_LINUX_SANDBOX_ARG0 {
        // Safety: [`run_main`] never returns.
        rexux_linux_sandbox::run_main();
    ...
    #[cfg(unix)]
    if argv1 == REXUX_ARG0_EXEC_HELPER_ARG1 {
        rexux_exec_server::run_arg0_exec_helper_main();
    }
    if argv1 == REXUX_FS_HELPER_ARG1 {
        rexux_exec_server::run_fs_helper_main();
    }
    #[cfg(target_os = "windows")]
    if argv1 == REXUX_WINDOWS_SANDBOX_ARG1 {
        rexux_windows_sandbox::run_windows_sandbox_wrapper_main();
    }
    if argv1 == REXUX_CORE_APPLY_PATCH_ARG1 {
        let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                let mut stdout = std::io::stdout();
                let mut stderr = std::io::stderr();
                let cwd = match rexux_utils_absolute_path::AbsolutePathBuf::current_dir() {
                    Ok(cwd) => cwd,
                    Err(_) => std::process::exit(1),
                };
                let runtime = match tokio::runtime::Builder::new_current_thread()
                ...
                let cwd = cwd.into();
                let update_file_mode = rexux_apply_patch::apply_patch_file_update_mode_from_env();
                match runtime.block_on(rexux_apply_patch::apply_patch_with_options(
                    &patch_arg,
                    rexux_apply_patch::ApplyPatchOptions {
                        update_file_mode,
                        ..Default::default()
                    },
                    &cwd,
                    &mut stdout,
                    &mut stderr,
                    rexux_exec_server::LOCAL_FS.as_ref(),
                    /*sandbox*/ None,
                )) {
                    Ok(_) => 0,
                    Err(_) => 1,
            ...
            None => {
                eprintln!("Error: {REXUX_CORE_APPLY_PATCH_ARG1} requires a UTF-8 PATCH argument.");
                1
            }
        };
    ...
    let (path_entry_guard, updated_path_env_var) = prepare_path_env_var_with_aliases(
        ...
        std::env::var_os("PATH"),
        prepare_path_entry_for_rexux_aliases,
    );
...
fn prepare_path_env_var_with_aliases(
    ...
    match prepare_aliases(path_for_aliases) {
        ...
        Err(err) => {
            // It is possible that Rexux will proceed successfully even if
...
/// While we want to deploy the Rexux CLI as a single executable for simplicity,
...
/// `rexux-linux-sandbox` we *directly* execute
/// [`rexux_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
/// 1.  Load `.env` values from `~/.rexux/.env` before creating any threads.
...
///     `rexux-linux-sandbox` helper path (falling back to the current
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    let paths = Arg0DispatchPaths {
        rexux_self_exe: current_exe.clone(),
        rexux_linux_sandbox_exe: if cfg!(target_os = "linux") {
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `rexux-linux-sandbox` alias when available so callers can
    // re-exec through a path whose basename still triggers arg0 dispatch on
    // bubblewrap builds that do not support `--argv0`.
    path_entry_guard
        .and_then(|path_entry| path_entry.paths().rexux_linux_sandbox_exe.clone())
        .or(current_exe)
}
...
const ILLEGAL_ENV_VAR_PREFIX: &str = "REXUX_";
const LEGACY_ILLEGAL_ENV_VAR_PREFIX: &str = "CODEX_";

/// Load env vars from ~/.rexux/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `REXUX_` (or legacy `CODEX_`).
fn load_dotenv() {
    if let Ok(rexux_home) = find_rexux_home()
        && let Ok(iter) = dotenvy::from_path_iter(rexux_home.join(".env"))
    {
        set_filtered(iter);
    }
}

/// Helper to set vars from a dotenvy iterator while filtering out `REXUX_` (and legacy `CODEX_`) keys.
    ...
    I: IntoIterator<Item = Result<(String, String), dotenvy::Error>>,
{
    for (key, value) in iter.into_iter().flatten() {
        ...
        if !upper.starts_with(ILLEGAL_ENV_VAR_PREFIX)
            && !upper.starts_with(LEGACY_ILLEGAL_ENV_VAR_PREFIX)
        {
...
///   with the hidden `--rexux-run-as-apply-patch` flag.
///
...
/// user to install a separate executable, simplifying the deployment of Rexux
/// CLI.
...
fn prepare_path_entry_for_rexux_aliases(
    existing_path: Option<OsString>,
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        if rexux_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (rexux_home: {rexux_home:?})"
                ),
            ));
        }
    }
    ...
    // Use a REXUX_HOME-scoped temp root to avoid cluttering the top-level directory.
    let temp_root = rexux_home.join("tmp").join("arg0");
    ...
    for filename in &[
        ...
        #[cfg(windows)]
        {
            ...
            std::fs::write(
                ...
                format!(
                    r#"@echo off
"{exe}" {REXUX_CORE_APPLY_PATCH_ARG1} %*
"#,
                ),
            )?;
        }
    ...
    let paths = Arg0DispatchPaths {
        rexux_self_exe: std::env::current_exe().ok(),
        rexux_linux_sandbox_exe: {
            #[cfg(target_os = "linux")]
            {
                Some(path.join(REXUX_LINUX_SANDBOX_ARG0))
            }
...
mod tests {
    ...
    fn package_path_test_fixture() -> anyhow::Result<PackagePathTestFixture> {
        ...
        let path_dir = package_dir.join("rexux-path");
        ...
        let install_context = InstallContext {
            ...
            package_layout: Some(RexuxPackageLayout {
    ...
    fn windows_batch_alias_preserves_unicode_executable_paths() -> anyhow::Result<()> {
        ...
        let alias_directory = profile.join(".rexux").join("tmp").join("arg0");
    ...
    #[test]
    fn linux_sandbox_exe_path_prefers_rexux_linux_sandbox_alias() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let lock_file = create_lock(temp_dir.path())?;
        let alias_path = temp_dir.path().join("rexux-linux-sandbox");
        let path_entry = Arg0PathEntryGuard::new(
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
                rexux_self_exe: Some(PathBuf::from("/usr/bin/codex")),
                rexux_linux_sandbox_exe: Some(alias_path.clone()),
    ...
    fn run_main_with_arg0_guard_keeps_aliases_alive_until_main_returns() -> anyhow::Result<()> {
        let temp_dir = TempDir::new()?;
        let alias_path = temp_dir.path().join("rexux-helper-alias");
