...
//!
//! This crate wraps [`rexux_app_server::in_process`] behind a single async API
...
//! [`InProcessClientHandle`](rexux_app_server::in_process::InProcessClientHandle),
...
pub use rexux_app_server::app_server_control_socket_path;
pub use rexux_app_server::in_process::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
pub use rexux_app_server::in_process::InProcessServerEvent;
use rexux_app_server::in_process::InProcessStartArgs;
use rexux_app_server::in_process::LogDbLayer;
pub use rexux_app_server::in_process::StateDbHandle;
...
use rexux_app_server_protocol::Result as JsonRpcResult;
...
use rexux_config::NoopThreadConfigLoader;
use rexux_core::config::Config;
pub use rexux_core::otel_init::build_provider as build_otel_provider;
...
/// module exists so clients can remove a direct `rexux-core` dependency
/// while legacy startup/config paths are migrated to RPCs.
pub mod legacy_core {
    pub mod config {
        pub use rexux_core::config::*;

        pub mod edit {
            pub use rexux_core::config::edit::*;
        }
    }
}
...
pub struct InProcessClientStartArgs {
    ...
    /// Whether auth loading should honor the `REXUX_API_KEY` environment variable.
    pub enable_rexux_api_key_env: bool,
...
impl InProcessClientStartArgs {
    ...
    fn into_runtime_start_args(self) -> InProcessStartArgs {
        ...
        InProcessStartArgs {
            ...
            enable_rexux_api_key_env: self.enable_rexux_api_key_env,
            initialize,
...
/// - [`rexux_app_server::in_process::InProcessClientHandle`], which speaks to
...
impl InProcessAppServerClient {
    ...
    pub async fn start(args: InProcessClientStartArgs) -> IoResult<Self> {
        ...
        let mut handle =
            rexux_app_server::in_process::start(args.into_runtime_start_args()).await?;
...
impl AppServerClient {
    pub fn rexux_home(&self, local_rexux_home: &AbsolutePathBuf) -> Option<AppServerPath> {
        match self {
            Self::InProcess(_) => Some(AppServerPath::from_app_server(
                local_rexux_home.display().to_string(),
            )),
            Self::Remote(client) => client.rexux_home().map(AppServerPath::from_app_server),
        }
    }
...
mod tests {
    ...
    use rexux_app_server_protocol::ServerNotification;
    use rexux_app_server_protocol::SessionSource as ApiSessionSource;
    ...
    use rexux_core::init_state_db;
    use rexux_protocol::config_types::Personality;
    use rexux_uds::UnixListener;
    use rexux_utils_absolute_path::AbsolutePathBuf;
    use futures::SinkExt;
    ...
    async fn build_test_config_for_rexux_home(rexux_home: &Path) -> Config {
        ...
        {
            Ok(config) => config,
            Err(_) => Config::load_default_with_cli_overrides_for_rexux_home(
    ...
    async fn start_test_client_with_capacity(
        ...
        let rexux_home = TempDir::new().expect("temp dir");
        let config = Arc::new(build_test_config_for_rexux_home(rexux_home.path()).await);
        ...
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            client_name: "rexux-app-server-client-test".to_string(),
    ...
    fn command_execution_output_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::CommandExecutionOutputDelta(
            rexux_app_server_protocol::CommandExecutionOutputDeltaNotification {
    ...
    fn agent_message_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::AgentMessageDelta(
            rexux_app_server_protocol::AgentMessageDeltaNotification {
                thread_id: "thread".to_string(),
                turn_id: "turn".to_string(),
                item_id: "item".to_string(),
                delta: delta.to_string(),
    ...
    fn item_completed_notification(text: &str) -> ServerNotification {
        ServerNotification::ItemCompleted(rexux_app_server_protocol::ItemCompletedNotification {
            ...
            turn_id: "turn".to_string(),
            completed_at_ms: 0,
            item: rexux_app_server_protocol::ThreadItem::AgentMessage {
                id: "item".to_string(),
                text: text.to_string(),
                phase: None,
    ...
    fn turn_completed_notification() -> ServerNotification {
        ServerNotification::TurnCompleted(rexux_app_server_protocol::TurnCompletedNotification {
            thread_id: "thread".to_string(),
            turn: rexux_app_server_protocol::Turn {
                id: "turn".to_string(),
                items_view: rexux_app_server_protocol::TurnItemsView::Full,
                items: Vec::new(),
                status: rexux_app_server_protocol::TurnStatus::Completed,
                error: None,
                started_at: None,
                completed_at: Some(0),
                duration_ms: Some(1),
    ...
    async fn typed_request_reports_json_rpc_errors() {
        ...
        let err = client
            .request_typed::<ConfigRequirementsReadResponse>(ClientRequest::ThreadRead {
                ...
                params: rexux_app_server_protocol::ThreadReadParams {
    ...
    async fn threads_started_via_app_server_are_visible_through_typed_requests() {
        ...
        let response: ThreadStartResponse = client
            ...
            .expect("thread/start should succeed");
        let read = client
            .request_typed::<rexux_app_server_protocol::ThreadReadResponse>(
    ...
    async fn remote_typed_request_roundtrip_works() {
        ...
        assert_eq!(client.rexux_home(), Some("/server/.codex"));
        let response: GetAccountResponse = client
            .request_typed(ClientRequest::GetAccount {
                request_id: RequestId::Integer(1),
                params: rexux_app_server_protocol::GetAccountParams {
                    refresh_token: false,
                },
            })
            .await
            ...
            match event {
                ...
                AppServerEvent::ServerNotification(notification) => match notification.as_ref() {
                    ...
                    ServerNotification::ItemCompleted(notification)
                        if matches!(
                            &notification.item,
                            rexux_app_server_protocol::ThreadItem::AgentMessage { text, .. }
                                if text == "hello"
                        ) =>
                    {
                    ...
                    ServerNotification::TurnCompleted(notification)
                        if notification.turn.status
                            == rexux_app_server_protocol::TurnStatus::Completed =>
                    {
