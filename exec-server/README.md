...
`rexux-exec-server` is the library backing `rexux exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`rexux-utils-pty`.

It provides:

- a CLI entrypoint: `rexux exec-server`
- a Rust client: `ExecServerClient`
- a small protocol module with shared request/response types

This crate owns the transport, protocol, and filesystem/process handlers. The
top-level `rexux` binary owns hidden helper dispatch for sandboxed
filesystem operations and `rexux-linux-sandbox`.

## Transport

The server speaks the exec-specific `rexux-exec-server-protocol` message
...
It uses the standard Rexux ChatGPT sign-in state; run `rexux login` first when
remote registration needs authentication. Containerized callers that receive an
Agent Identity JWT in `REXUX_ACCESS_TOKEN` can opt into that auth path with
`--use-agent-identity-auth`; Rexux then registers an Agent task and sends the
derived AgentAssertion headers on the registry request.

Alternatively, API users can instead use `REXUX_API_KEY`;
Rexux sends it as a bearer token on the registration request. For example:

```sh
REXUX_API_KEY="$OPENAI_API_KEY" \
rexux exec-server \
  --remote ... \
...
`rexux.exec_server.relay.v1.RelayMessageFrame`; the checked-in schema is in
`src/proto/rexux.exec_server.relay.v1.proto`. The relay frame carries stream
...
- `arg0`: optional argv0 override forwarded to `rexux-utils-pty`.
...
hidden helper process launched from the top-level `rexux` executable and
...
`HttpClientFactory` to `run_main()`. The top-level `rexux exec-server` command
builds these paths from the `rexux` arg0 dispatch state and resolves its HTTP
client factory from the effective Rexux configuration.
...
provider from Rexux auth state before starting remote mode.
