...
use rexux_websocket_client::WebSocketConnection;
use rexux_websocket_client::WebSocketConnector;
use rexux_websocket_client::WebSocketTlsMode;
    ...
    async fn register_environment_inner(
        ...
        .unwrap_or_else(|_| {
            Err(ExecServerError::EnvironmentRegistryRequest(
                rexux_http_client::RouteAwareRequestError::Timeout,
            ))
        })?;
...
/// Noise connection configuration for a Rexux harness.
///
...
impl RemoteEnvironmentConfig {
    pub fn new(
        ...
        Ok(Self {
            base_url,
            environment_id,
            name: "rexux-exec-server".to_string(),
...
mod tests {
    ...
    impl AuthProvider for StaticRegistryAuthProvider {
        ...
        fn resolve_auth_headers(&self) -> rexux_api::AuthHeadersFuture<'_> {
            Box::pin(async {
