...
use rexux_exec_server_protocol::JSONRPCErrorError;
...
use rexux_sandboxing::SandboxDirectSpawnTransformRequest;
use rexux_sandboxing::SandboxExecRequest;
...
use rexux_utils_absolute_path::canonicalize_preserving_symlinks;
...
use crate::fs_helper::REXUX_FS_HELPER_ARG1;
...
impl FileSystemSandboxRunner {
    ...
    fn sandbox_exec_request(
        ...
        let helper = &self.runtime_paths.rexux_self_exe;
        ...
        let command = SandboxCommand {
            ...
            args: vec![REXUX_FS_HELPER_ARG1.to_string()],
            cwd: cwd.uri.clone(),
        ...
        sandbox_manager
            .transform_for_direct_spawn(SandboxDirectSpawnTransformRequest {
                ...
                transform: SandboxTransformRequest {
                    ...
                    rexux_linux_sandbox_exe: self.runtime_paths.rexux_linux_sandbox_exe.as_deref(),
...
fn helper_read_roots(runtime_paths: &ExecServerRuntimePaths) -> Vec<AbsolutePathBuf> {
    let mut roots = vec![runtime_paths.rexux_self_exe.clone()];
    if let Some(path) = &runtime_paths.rexux_linux_sandbox_exe
        && !roots.contains(path)
    {
...
pub(crate) fn spawn_command(
    ...
    env.retain(|name, _| !rexux_protocol::shell_environment::is_non_inheritable_env_var(name));
    command.env_clear();
    command.envs(env);
    command.stdin(stdin);
    ...
    unsafe {
        command.pre_exec(|| {
            rexux_utils_pty::pty::close_inherited_fds_except(&[]);
            Ok(())
        });
    }
...
mod tests {
    ...
    fn helper_permissions_preserve_existing_writes() {
        let rexux_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(rexux_self_exe, /*rexux_linux_sandbox_exe*/ None)
        ...
        let readable = runtime_paths.rexux_self_exe.clone();
    ...
    fn sandbox_exec_request_carries_helper_env() {
        ...
        let rexux_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(rexux_self_exe.clone(), Some(rexux_self_exe))
                .expect("runtime paths");
        ...
        let sandbox_context = {
            ...
            crate::FileSystemSandboxContext {
                windows_sandbox_level:
                    rexux_protocol::config_types::WindowsSandboxLevel::RestrictedToken,
                ..sandbox_context
            }
        };
    ...
    fn sandbox_cwd_rejects_cwd_dependent_profile_without_context_cwd() {
        ...
        let sandbox_context = rexux_file_system::FileSystemSandboxContext::from_permission_profile(
    ...
    fn helper_permissions_include_only_the_helper_executable() {
        ...
        assert!(
            policy.can_read_path_with_cwd(runtime_paths.rexux_self_exe.as_path(), cwd.as_path())
        );
    ...
    fn helper_permissions_include_only_linux_sandbox_alias_executable() {
        ...
        let rexux_self_exe = root.path().join("bin").join("codex");
        let rexux_linux_sandbox_exe = root.path().join("aliases").join("rexux-linux-sandbox");
        let runtime_paths =
            ExecServerRuntimePaths::new(rexux_self_exe, Some(rexux_linux_sandbox_exe))
                .expect("runtime paths");
        let cwd = AbsolutePathBuf::from_absolute_path(std::env::temp_dir().as_path())
            .expect("absolute cwd");
        let mut policy = restricted_policy(Vec::new());
        let rexux_parent = runtime_paths.rexux_self_exe.parent().expect("codex parent");
        ...
        assert!(!policy.can_read_path_with_cwd(rexux_parent.as_path(), cwd.as_path()));
    ...
    fn sandbox_context_with_cwd(
        ...
        rexux_file_system::FileSystemSandboxContext::from_permission_profile_with_cwd(
            PermissionProfile::from_runtime_permissions(policy, NetworkSandboxPolicy::Restricted),
            cwd,
        )
    }
