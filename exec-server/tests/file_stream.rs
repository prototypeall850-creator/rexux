...
use rexux_exec_server::FsCloseParams;
use rexux_exec_server::FsOpenParams;
use rexux_exec_server::FsReadBlockParams;
use rexux_exec_server::FsReadBlockResponse;
...
async fn file_reads_reject_named_pipes() -> Result<()> {
    ...
    let read_path = format!(r"\\.\pipe\rexux-fs-read-{}", Uuid::new_v4());
    ...
    let stream_path = format!(r"\\.\pipe\rexux-fs-stream-{}", Uuid::new_v4());
...
#[cfg(unix)]
fn read_only_sandbox(path: std::path::PathBuf) -> rexux_exec_server::FileSystemSandboxContext {
