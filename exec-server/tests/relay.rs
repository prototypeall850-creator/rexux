...
use rexux_exec_server_protocol::ProcessSandboxType;
use rexux_http_client::HttpClientFactory;
use rexux_http_client::OutboundProxyPolicy;
use rexux_http_client::cache_system_proxy_route_for_test;
...
async fn pending_noise_environment_connects_and_reconnects_after_ready_report() -> Result<()> {
    ...
    let (rexux_exe, rexux_linux_sandbox_exe) = common::current_test_binary_helper_paths()?;
    let runtime_paths = ExecServerRuntimePaths::new(rexux_exe, rexux_linux_sandbox_exe)?;
