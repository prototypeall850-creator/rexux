...
use rexux_exec_server_protocol::JSONRPCNotification;
use rexux_exec_server_protocol::JSONRPCRequest;
...
pub(crate) struct ExecServerHarness {
    rexux_home: TempDir,
    _helper_paths: TestRexuxHelperPaths,
    child: Child,
...
pub(crate) struct TestRexuxHelperPaths {
    pub(crate) rexux_exe: PathBuf,
    pub(crate) rexux_linux_sandbox_exe: Option<PathBuf>,
}
...
pub(crate) fn test_rexux_helper_paths() -> anyhow::Result<TestRexuxHelperPaths> {
    let (helper_binary, rexux_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestRexuxHelperPaths {
        rexux_exe: helper_binary,
...
{
    let helper_paths = test_rexux_helper_paths()?;
    let rexux_home = TempDir::new()?;
    let mut child = Command::new(&helper_paths.rexux_exe);
    ...
    child.env("REXUX_HOME", rexux_home.path());
    child.envs(env);
...
impl ExecServerHarness {
    pub(crate) fn rexux_home(&self) -> &std::path::Path {
        self.rexux_home.path()
    }
