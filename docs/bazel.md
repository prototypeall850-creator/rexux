...
- `rules_rs` imports third-party crates from `codex-rs/Cargo.toml` and
  ...
  `@crates`.
- `../defs.bzl` provides `rexux_rust_crate`, which wraps `rust_library`,
...
- Each crate in `codex-rs/*/BUILD.bazel` typically uses `rexux_rust_crate` and
...
Rexux uses BuildBuddy for a shared Bazel cache and remoted builds and tests. To use it
...
| --- | --- | --- |
| Push to `main` in `openai/rexux` | Yes | Yes |
| `workflow_dispatch` in `openai/rexux` | Yes | Yes |
| Same-repository pull request in `openai/rexux` | Yes | Yes |
| Fork pull request into `openai/rexux` | No | No; local |
...
```bash
BUILDBUDDY_API_KEY=... GITHUB_REPOSITORY=my-fork/rexux \
  ./.github/scripts/run_bazel_with_buildbuddy.py \
  build --config=ci-linux //codex-rs/cli:rexux
```

The wrapper selects the OpenAI host only inside GitHub Actions for a trusted
run in `openai/rexux`. A missing or malformed pull request event
...
2. Create a `BUILD.bazel` that calls `rexux_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `rexux_rust_crate` to configure it.
