...
This crate implements the business logic for Rexux. It is designed to be used by the various Rexux UIs written in Rust.
...
Note that `rexux-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.rexux` read-only.
...
Expects the binary containing `rexux-core` to run the equivalent of `rexux sandbox` when `arg0` is `rexux-linux-sandbox`. See the `rexux-arg0` crate for details.
...
`bwrap` is missing, it falls back to the bundled `rexux-resources/bwrap`
binary shipped with Rexux and Rexux surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
Rexux also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so Rexux rejects sandboxed shell commands that would enter the
...
Expects the binary containing `rexux-core` to simulate the virtual
`apply_patch` CLI when `arg1` is `--rexux-run-as-apply-patch`. See the
`rexux-arg0` crate for details.
