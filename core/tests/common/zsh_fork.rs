...
use crate::test_rexux::TestRexux;
...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) = rexux_utils_cargo_bin::cargo_bin("rexux-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `rexux-execve-wrapper` binary");
        return Ok(None);
    };
...
pub fn zsh_fork_test_builder(
    ...
    test_rexux().with_config(move |config| {
