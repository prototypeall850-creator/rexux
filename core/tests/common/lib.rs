...
use rexux_utils_cargo_bin::CargoBinError;
use ctor::ctor;
...
pub use rexux_core::test_support::TestRexuxResponsesRequestKind;
pub use rexux_core::test_support::responses_metadata;
use rexux_utils_absolute_path::AbsolutePathBuf;
pub use rexux_utils_absolute_path::test_support::PathBufExt;
pub use rexux_utils_absolute_path::test_support::PathExt;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    rexux_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    rexux_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(rexux_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = rexux_utils_cargo_bin::repo_root()
        .ok()
...
/// avoids clobbering a developer’s real `~/.rexux`.
pub async fn load_default_config_for_test(rexux_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        rexux_linux_sandbox_exe: Some(
            find_rexux_linux_sandbox_exe().expect("should find binary for rexux-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_rexux_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    rexux_utils_cargo_bin::cargo_bin("rexux-linux-sandbox")
}

pub async fn wait_for_event<F>(
    codex: &RexuxThread,
    predicate: F,
) -> rexux_protocol::protocol::EventMsg
where
    F: FnMut(&rexux_protocol::protocol::EventMsg) -> bool,
{
...
pub async fn wait_for_mcp_server(codex: &RexuxThread, server_name: &str) -> anyhow::Result<()> {
...
pub async fn submit_thread_settings(
    codex: &RexuxThread,
    thread_settings: rexux_protocol::protocol::ThreadSettingsOverrides,
) -> anyhow::Result<()> {
    use rexux_protocol::protocol::EventMsg;
...
pub async fn wait_for_event_match<T, F>(codex: &RexuxThread, matcher: F) -> T
where
    F: Fn(&rexux_protocol::protocol::EventMsg) -> Option<T>,
{
...
pub fn sandbox_env_var() -> &'static str {
    rexux_core::spawn::REXUX_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    rexux_core::spawn::REXUX_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

pub fn format_with_current_shell(command: &str) -> Vec<String> {
    rexux_core::shell::default_user_shell().derive_exec_args(command, /*use_login_shell*/ true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    rexux_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    rexux_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
#[macro_export]
macro_rules! rexux_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_rexux_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("rexux-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
