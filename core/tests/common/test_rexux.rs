...
use rexux_core::TimeProvider;
pub use rexux_core::TurnInputRequest;
use rexux_core::config::Config;
use rexux_core::resolve_installation_id;
use rexux_core::shell::Shell;
use rexux_core::shell::get_shell_by_model_provided_path;
...
use rexux_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
...
use rexux_utils_path_uri::PathConvention;
...
const REMOTE_EXEC_SERVER_URL_ENV_VAR: &str = "REXUX_TEST_REMOTE_EXEC_SERVER_URL";
...
#[derive(Debug)]
pub struct TestEnv {
    environment: rexux_exec_server::Environment,
...
impl TestEnv {
    ...
    pub async fn local_with_exec_server_url(exec_server_url: Option<String>) -> Result<Self> {
        ...
        let selection = match exec_server_url {
            Some(_) => TurnEnvironmentSelection {
                environment_id: rexux_exec_server::REMOTE_ENVIRONMENT_ID.to_string(),
                cwd: PathUri::from_abs_path(&cwd),
                workspace_roots: vec![PathUri::from_abs_path(&cwd)],
                config: EnvironmentConfigState::FromThread,
            },
        ...
        let environment =
            rexux_exec_server::Environment::create_for_tests(exec_server_url.clone())?;
        Ok(Self {
            environment,
            exec_server_url,
            cwd,
    ...
    pub fn environment(&self) -> &rexux_exec_server::Environment {
        &self.environment
    }
...
pub async fn test_env() -> Result<TestEnv> {
    match test_environment() {
        remote_env @ (TestEnvironment::Docker { .. } | TestEnvironment::WineExec) => {
            ...
            let environment =
                rexux_exec_server::Environment::create_for_tests(Some(websocket_url.clone()))?;
            let cwd = remote_env
...
impl TestRexuxBuilder {
    ...
    pub fn with_auth(mut self, auth: RexuxAuth) -> Self {
        self.auth = auth;
        self
    }
    ...
    pub async fn build(&mut self, server: &wiremock::MockServer) -> anyhow::Result<TestRexux> {
    ...
    ///
    /// With no remote test configuration, or with `REXUX_TEST_ENVIRONMENT=local`, this uses a
    /// temporary local environment just like [`Self::build`]. `REXUX_TEST_ENVIRONMENT=docker` or
    /// `REXUX_TEST_ENVIRONMENT=wine-exec` selects the remote exec server configured by
    /// `REXUX_TEST_REMOTE_EXEC_SERVER_URL`; the legacy `REXUX_TEST_REMOTE_ENV` Docker-container
    ...
    async fn build_with_home_and_base_url(
        ...
        #[cfg(target_os = "linux")]
        let rexux_linux_sandbox_exe = Some(
            crate::find_rexux_linux_sandbox_exe()
                .context("should find binary for rexux-linux-sandbox")?,
        );
        #[cfg(not(target_os = "linux"))]
        let rexux_linux_sandbox_exe = None;
        let local_runtime_paths = rexux_exec_server::ExecServerRuntimePaths::new(
            std::env::current_exe()?,
            rexux_linux_sandbox_exe,
        )?;
        let environment_manager = Arc::new(if include_local_environment {
            rexux_exec_server::EnvironmentManager::create_for_tests_with_local(
                exec_server_url,
                local_runtime_paths,
            )
            .await
        } else {
            rexux_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        environment_manager: Arc<rexux_exec_server::EnvironmentManager>,
    ) -> anyhow::Result<TestRexux> {
        let auth = self.auth.clone();
        let state_db = rexux_core::init_state_db(&config).await;
        let thread_store = thread_store_from_config(&config, state_db.clone());
        let installation_id = resolve_installation_id(&config.rexux_home).await?;
        let user_instructions_provider =
            self.user_instructions_provider.clone().unwrap_or_else(|| {
                Arc::new(RexuxHomeUserInstructionsProvider::new(
                    config.rexux_home.clone(),
                ))
            });
        let auth_manager = rexux_core::test_support::auth_manager_from_auth_with_home(
            auth.clone(),
        ...
        let models_manager = self
            .models_manager
            .clone()
            .unwrap_or_else(|| rexux_core::build_models_manager(&config, auth_manager.clone()));
        let thread_manager = ThreadManager::new(
            ...
            rexux_core::local_agent_graph_store_from_state_db(state_db.as_ref()),
            installation_id,
        ...
        let new_conversation = match (resume_from, user_shell_override) {
            (Some(path), Some(user_shell_override)) => {
                ...
                Box::pin(
                    rexux_core::test_support::resume_thread_from_rollout_with_user_shell_override(
            ...
            (None, Some(user_shell_override)) => {
                Box::pin(
                    rexux_core::test_support::start_thread_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        user_shell_override,
                        self.supports_openai_form_elicitation,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = rexux_utils_cargo_bin::cargo_bin("codex") {
            config.rexux_self_exe = Some(path);
        } else if let Ok(path) = rexux_utils_cargo_bin::cargo_bin("rexux-exec") {
            // `rexux-exec` also supports `--rexux-run-as-apply-patch`, so use it
            ...
            && let Some(bin_dir) = exe.parent().and_then(|parent| parent.parent())
        {
            ...
            let rexux_exec = bin_dir.join("rexux-exec");
            if codex.is_file() {
                config.rexux_self_exe = Some(codex);
            } else if rexux_exec.is_file() {
                config.rexux_self_exe = Some(rexux_exec);
            }
        }
...
pub struct TestRexux {
    pub home: Arc<TempDir>,
    pub cwd: Arc<TempDir>,
    pub codex: Arc<RexuxThread>,
...
impl TestRexux {
    ...
    pub fn rexux_home_path(&self) -> &Path {
        self.config.rexux_home.as_path()
    }
...
pub struct TestRexuxHarness {
    server: MockServer,
    test: TestRexux,
}

impl TestRexuxHarness {
    pub async fn with_builder(mut builder: TestRexuxBuilder) -> Result<Self> {
        let server = start_mock_server().await;
        let test = builder.build(&server).await?;
        Ok(Self { server, test })
    }

    pub async fn with_auto_env_builder(mut builder: TestRexuxBuilder) -> Result<Self> {
        let server = start_mock_server().await;
...
pub fn test_rexux() -> TestRexuxBuilder {
    TestRexuxBuilder {
        ...
        auth: RexuxAuth::from_api_key("dummy"),
        pre_build_hooks: vec![],
