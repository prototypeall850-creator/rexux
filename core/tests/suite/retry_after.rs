use anyhow::Result;
use rexux_client::RetryOn;
use rexux_client::RetryPolicy;
use rexux_client::run_with_retry;
use rexux_http_client::Request;
use rexux_http_client::TransportError;
    ...
    S: Subscriber,
{
    fn on_event(&self, event: &Event<'_>, _context: Context<'_, S>) {
        if event.metadata().target() == "rexux_http_client::transport" {
            self.record_request_after_retry();
            return;
        }

        if event.metadata().target() != "rexux_otel.trace_safe" {
            return;
        }
...
async fn submit_user_input(test: &TestRexux, text: &str) -> Result<()> {
    test.codex
...
async fn wait_for_turn_completion(test: &TestRexux) {
        ...
        loop {
            match wait_for_event(&test.codex, |_| true).await {
                ...
                EventMsg::TurnComplete(event) => {
                    assert_eq!(
                        event.error.and_then(|error| error.rexux_error_info),
                        Some(RexuxErrorInfo::ServerOverloaded)
                    );
                    break;
                }
    ...
    loop {
        match wait_for_event(&test.codex, |_| true).await {
            EventMsg::Error(error) => {
                ...
                assert_eq!(
                    error.rexux_error_info,
                    Some(RexuxErrorInfo::RateLimitExceeded)
                );
    ...
    loop {
        match wait_for_event(&test.codex, |_| true).await {
            EventMsg::Error(error) => {
                ...
                assert_eq!(
                    error.rexux_error_info,
                    Some(RexuxErrorInfo::ResponseTooManyFailedAttempts {
