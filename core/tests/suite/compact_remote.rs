...
use rexux_core::compact::SUMMARY_PREFIX;
...
use rexux_login::auth::AgentIdentityAuth;
use rexux_login::auth::AgentIdentityAuthRecord;
use rexux_login::auth::BedrockApiKeyAuth;
use rexux_model_provider_info::AMAZON_BEDROCK_GPT_5_5_MODEL_ID;
...
use rexux_protocol::dynamic_tools::DynamicToolCallOutputContentItem;
use rexux_protocol::dynamic_tools::DynamicToolFunctionSpec;
use rexux_protocol::dynamic_tools::DynamicToolNamespaceSpec;
use rexux_protocol::dynamic_tools::DynamicToolNamespaceTool;
...
use core_test_support::test_rexux::TestRexuxBuilder;
use core_test_support::test_rexux::TestRexuxHarness;
use core_test_support::test_rexux::test_rexux as base_test_rexux;
...
fn test_rexux() -> TestRexuxBuilder {
    base_test_rexux().with_config(|config| {
...
fn remote_realtime_test_rexux_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestRexuxBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_rexux()
        .with_auth(RexuxAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(codex: &rexux_core::RexuxThread) -> Result<()> {
    codex
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            client_managed_handoffs: false,
            delegation_ack_filler: None,
            flush_transcript_tail_on_session_end: false,
            rexux_responses_as_items: false,
            rexux_response_item_prefix: None,
            rexux_response_handoff_mode:
                rexux_protocol::protocol::RexuxResponseHandoffMode::Thinking,
            rexux_response_handoff_channel_prefixes: None,
            model: None,
...
async fn close_realtime_conversation(codex: &rexux_core::RexuxThread) -> Result<()> {
...
async fn wait_for_turn_complete(codex: &rexux_core::RexuxThread) {
...
fn amazon_bedrock_test_rexux() -> TestRexuxBuilder {
    let auth = RexuxAuth::BedrockApiKey(BedrockApiKeyAuth {
...
async fn amazon_bedrock_manual_compaction_uses_v2_responses_endpoint() -> Result<()> {
    ...
    let harness = TestRexuxHarness::with_auto_env_builder(amazon_bedrock_test_rexux()).await?;
...
async fn remote_compact_v2_retains_only_client_developer_messages_when_enabled(
    ...
    let harness = TestRexuxHarness::with_auto_env_builder(
        test_rexux()
            .with_auth(RexuxAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(move |config| {
                config
...
async fn amazon_bedrock_automatic_compaction_uses_v2_responses_endpoint() -> Result<()> {
    ...
    let harness = TestRexuxHarness::with_auto_env_builder(amazon_bedrock_test_rexux().with_config(
        |config| {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestRexuxHarness::with_builder(
        test_rexux()
...
async fn remote_compact_uses_agent_identity_assertion() -> Result<()> {
    ...
    let harness = TestRexuxHarness::with_builder(
        test_rexux().with_auth(RexuxAuth::AgentIdentity(
    ...
    assert_eq!(
        compact_request.header(X_REXUX_ROUTING_HINT_HEADER),
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let uses_rexux_backend = auth.uses_rexux_backend();
    ...
    let harness = TestRexuxHarness::with_builder(builder).await?;
    let codex = harness.test().codex.clone();
    let image_url =
    ...
    assert_eq!(
        normal_request.header(X_REXUX_ROUTING_HINT_HEADER),
    ...
    let image_inputs = (1..=8)
        .map(|number| {
            ...
            Ok(UserInput::Image {
                ...
                detail: Some(rexux_protocol::models::ImageDetail::Original),
            })
        })
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let mut builder = test_rexux().with_auth(RexuxAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    let dynamic_tools = vec![DynamicToolSpec::Namespace(DynamicToolNamespaceSpec {
        name: "rexux_app".to_string(),
        description: "Rexux app tools.".to_string(),
        tools: vec![
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "rexux_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "rexux_app"),
        vec![visible_tool.to_string()]
    );
    ...
    let responses_mock = responses::mount_sse_sequence(
        ...
        vec![
            sse(vec![
                ...
                responses::ev_function_call_with_namespace(call_id, "rexux_app", tool_name, "{}"),
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestRexuxHarness::with_builder(
        test_rexux()
            .with_auth(RexuxAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    let override_harness = TestRexuxHarness::with_builder(
        test_rexux()
            .with_auth(RexuxAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
...
async fn remote_manual_compact_emits_context_compaction_items() -> Result<()> {
    ...
    let harness = TestRexuxHarness::with_builder(
        test_rexux().with_auth(RexuxAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let codex = harness.test().codex.clone();
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    let mut start_builder = test_rexux()
        .with_auth(RexuxAuth::create_dummy_chatgpt_auth_for_testing())
    ...
    let mut resume_builder = test_rexux()
        .with_auth(RexuxAuth::create_dummy_chatgpt_auth_for_testing())
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_rexux_builder(&realtime_server).with_config(|config| {
        config.model_auto_compact_token_limit = Some(200);
    });
    let test = builder.build(&server).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_rexux_builder(&realtime_server).with_config({
...
async fn active_realtime_does_not_diff_changed_start_instructions_after_resume() -> Result<()> {
    ...
    let mut initial_builder = remote_realtime_test_rexux_builder(&initial_realtime_server)
        .with_config({
    ...
    let mut resume_builder = remote_realtime_test_rexux_builder(&resumed_realtime_server)
        .with_config({
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_rexux_builder(&realtime_server);
    let test = builder.build(&server).await?;
