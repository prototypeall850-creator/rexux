...
//! request payload that Rexux would send to the model and assert that the
...
fn network_disabled() -> bool {
    std::env::var(REXUX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
fn seed_first_checkpoint_harness_metadata(path: &Path, retained_text: &str) -> Result<()> {
    ...
    for envelope in replacement_history {
        envelope.metadata = Some(RexuxHarnessMetadata::default());
    }
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<RexuxThread>) {
...
async fn user_turn(conversation: &Arc<RexuxThread>, text: &str) {
    conversation
...
async fn compact_conversation(conversation: &Arc<RexuxThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<RexuxThread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<RexuxThread>) {
    conversation
        .shutdown_and_wait()
        .await
...
async fn resume_conversation(
    ...
    let auth_manager = rexux_core::test_support::auth_manager_from_auth(
        rexux_login::RexuxAuth::from_api_key("dummy"),
    );
