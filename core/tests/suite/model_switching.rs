use anyhow::Result;
use rexux_config::types::Personality;
...
use rexux_protocol::openai_models::default_input_modalities;
...
fn read_only_user_turn(test: &TestRexux, items: Vec<UserInput>, model: String) -> TurnInputRequest {
...
async fn first_turn_model_change_appends_model_instructions_developer_message(
    ...
    let mut builder = test_rexux().with_model("gpt-5.2").with_config(|config| {
        config
            .features
...
async fn first_turn_after_empty_prefix_fork_preserves_inherited_base_instructions(
    ...
    let source_history =
        rexux_rollout::RolloutRecorder::get_rollout_history(&source_rollout_path).await?;
...
async fn rollback_first_turn_model_change_removes_its_instructions(
    ...
    let mut builder = test_rexux().with_model(initial_model);
    let test = builder.build_with_auto_env(&server).await?;
    ...
    let test = match followup {
        RollbackFollowup::ColdResume => {
            let mut resume_builder = test_rexux().with_model(switched_model);
...
async fn model_and_personality_change_only_appends_model_instructions() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    let next_model = "exp-rexux-personality";
...
async fn service_tier_change_is_applied_on_next_http_turn() -> Result<()> {
    ...
    let test = test_rexux().build(&server).await?;
