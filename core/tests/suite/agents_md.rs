...
use core_test_support::test_rexux::RecordingUserInstructionsProvider;
...
async fn agents_instructions(mut builder: TestRexuxBuilder) -> Result<String> {
...
async fn submit_thread_turn(thread: &Arc<rexux_core::RexuxThread>, prompt: &str) -> Result<()> {
    thread
...
async fn agents_override_is_preferred_over_agents_md() -> Result<()> {
    let instructions =
        agents_instructions(test_rexux().with_workspace_setup(|cwd, fs| async move {
...
-> Result<()> {
    ...
    .await;
    let mut builder = test_rexux().with_workspace_setup(|cwd, fs| async move {
        fs.write_file(
            &executor_path_uri(cwd.join(GLOBAL_AGENTS_FILENAME))?,
            PROJECT_INSTRUCTIONS.as_bytes().to_vec(),
            Default::default(),
...
async fn loads_user_instructions_without_a_primary_environment() -> Result<()> {
    ...
    let provider = Arc::new(RecordingUserInstructionsProvider::new(Arc::new(
        RexuxHomeUserInstructionsProvider::new(AbsolutePathBuf::try_from(
...
async fn invalid_utf8_global_instructions_are_lossy() -> Result<()> {
    ...
    let mut builder = test_rexux().with_home(home);
    let test = builder.build(&server).await?;
...
async fn cold_resume_invalidates_deleted_legacy_agents_md_once() -> Result<()> {
    ...
    let mut initial_builder = test_rexux().with_home(Arc::clone(&home));
    ...
    let mut resume_builder = test_rexux().with_home(Arc::clone(&home));
...
async fn fork_injects_changed_agents_md_once() -> Result<()> {
    ...
    let mut builder = test_rexux().with_home(Arc::clone(&home));
    ...
    fork_config.rexux_self_exe = parent.config.rexux_self_exe.clone();
    fork_config
        .features
