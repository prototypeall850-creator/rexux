...
async fn user_shell_command_without_local_environment_emits_error() -> anyhow::Result<()> {
    ...
    assert_eq!(error.rexux_error_info, None);
...
fn slow_user_shell_command() -> &'static str {
    match rexux_core::shell::default_user_shell().name() {
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    let mut builder = core_test_support::test_rexux::test_rexux().with_config(move |config| {
        config
            .features
    ...
    #[cfg(windows)]
    let command = r#"$val = $env:REXUX_SANDBOX; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command = r#"sh -c "printf '%s' \"${REXUX_SANDBOX:-not-set}\"""#.to_string();
...
async fn user_shell_command_does_not_set_network_sandbox_env_var() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let mut builder = core_test_support::test_rexux::test_rexux().with_config(|config| {
    ...
    #[cfg(windows)]
    let command = r#"$val = $env:REXUX_SANDBOX_NETWORK_DISABLED; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command =
        r#"sh -c "printf '%s' \"${REXUX_SANDBOX_NETWORK_DISABLED:-not-set}\"""#.to_string();
...
async fn user_shell_command_output_is_truncated_in_history() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let builder = core_test_support::test_rexux::test_rexux();
    let test = builder
        .with_config(|config| {
