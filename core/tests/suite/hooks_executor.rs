...
async fn executor_browser_and_computer_use_stop_hooks_use_separate_mcp_routes(
    ...
    assert!(node_params["_meta"].get("_rexux_apps").is_none());
    ...
    assert_eq!(browser_params["_meta"]["_rexux_apps"], routing);
...
async fn executor_plugin_hook_fixture(
    ...
    for (plugin_id, manifest) in plugins {
        ...
        let plugin_directory = plugin_root.join(".rexux-plugin")?;
