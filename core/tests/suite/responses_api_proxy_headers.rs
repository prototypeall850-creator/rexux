...
-> Result<()> {
    ...
    let mut builder = test_rexux().with_config(|config| {
        ...
        config
            ...
            .insert("rexux_security_surface".to_string(), "sdk".to_string());
    });
    ...
    assert_eq!(parent_turn_metadata["rexux_security_surface"], json!("sdk"));
    ...
    assert_eq!(child_turn_metadata["rexux_security_surface"], json!("sdk"));

    Ok(())
}

async fn submit_turn_with_timeout(test: &TestRexux, prompt: &str) -> Result<()> {
