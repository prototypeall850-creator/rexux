...
use rexux_protocol::config_types::ShellEnvironmentPolicyInherit;
...
use rexux_protocol::protocol::ExecCommandStatus;
use rexux_protocol::protocol::Op;
use rexux_protocol::protocol::ThreadSettingsOverrides;
use rexux_protocol::shell_environment::REXUX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR;
use rexux_protocol::user_input::UserInput;
use rexux_utils_output_truncation::approx_tokens_from_byte_count;
...
async fn exec_command_hides_and_rejects_login_when_disabled() -> Result<()> {
    ...
    let builder = test_rexux().with_model("gpt-5.4").with_config(|config| {
...
async fn exec_command_does_not_expose_configured_noise_auth_token() -> Result<()> {
    ...
    let builder = test_rexux().with_model("gpt-5.4").with_config(|config| {
        config.permissions.shell_environment_policy.r#set.insert(
            REXUX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR.to_string(),
            "configured-noise-token".to_string(),
        );
        config.permissions.shell_environment_policy.r#set.insert(
            REXUX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR.to_ascii_lowercase(),
            "case-variant-noise-token".to_string(),
        );
    });
    let harness = TestRexuxHarness::with_auto_env_builder(builder).await?;
    let command = match core_test_support::test_target_os() {
        core_test_support::TestTargetOs::Linux | core_test_support::TestTargetOs::MacOs => {
            "if [ -n \"${REXUX_EXEC_SERVER_NOISE_AUTH_TOKEN:-}\" ] || [ -n \"${rexux_exec_server_noise_auth_token:-}\" ]; then echo leaked; else echo unset; fi"
        }
        core_test_support::TestTargetOs::Windows => {
            "if ($env:REXUX_EXEC_SERVER_NOISE_AUTH_TOKEN) { Write-Output leaked } else { Write-Output unset }"
        }
    };
...
async fn unified_exec_rejects_justification_without_sandbox_permissions() -> Result<()> {
    ...
    let mut builder = test_rexux().with_model("gpt-5.2");
...
async fn unified_exec_network_denial_emits_failed_background_end_event() -> Result<()> {
    ...
    let args = json!({
        "cmd": "python3 -c \"import os, socket, time, urllib.parse; time.sleep(0.3); proxy = urllib.parse.urlparse(os.environ['HTTP_PROXY']); sock = socket.create_connection((proxy.hostname, proxy.port), timeout=2); sock.sendall(b'GET http://rexux-network-denied.invalid/ HTTP/1.1\\r\\nHost: rexux-network-denied.invalid\\r\\n\\r\\n'); sock.recv(1024); time.sleep(5)\"",
        "yield_time_ms": 50,
    });
    let response_mock =
...
async fn unified_exec_short_lived_network_denial_emits_failed_end_event() -> Result<()> {
    ...
    let args = json!({
        "cmd": "python3 -c \"import os, socket, urllib.parse; proxy = urllib.parse.urlparse(os.environ['HTTP_PROXY']); sock = socket.create_connection((proxy.hostname, proxy.port), timeout=2); sock.sendall(b'GET http://rexux-short-network-denied.invalid/ HTTP/1.1\\r\\nHost: rexux-short-network-denied.invalid\\r\\n\\r\\n'); sock.recv(1024)\"",
        "yield_time_ms": 1000,
    });
    let response_mock =
        mount_unified_exec_network_denial_responses(&server, call_id, &args).await?;
...
) -> Result<(TestRexux, PermissionProfile)> {
...
) -> (rexux_protocol::protocol::ExecCommandEndEvent, bool) {
...
async fn exec_command_clamps_model_requested_max_output_tokens_to_policy() -> Result<()> {
    ...
    let mut builder = test_rexux().with_model("gpt-5.4").with_config(|config| {
