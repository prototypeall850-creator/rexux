...
async fn run_turn(test: &TestRexux, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestRexux, prompt: &str) -> anyhow::Result<Duration> {
    let start = Instant::now();
    run_turn(test, prompt).await?;
    Ok(start.elapsed())
}

async fn build_rexux_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestRexux> {
    let mut builder = test_rexux().with_model("test-gpt-5.1-codex");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_rexux_with_test_tool(&server).await?;
