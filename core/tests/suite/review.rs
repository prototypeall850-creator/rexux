...
use rexux_protocol::openai_models::ModelTokenBudgetConfig;
...
use rexux_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
use rexux_protocol::protocol::EventMsg;
use rexux_protocol::protocol::ExitedReviewModeEvent;
use rexux_protocol::protocol::Op;
use rexux_protocol::protocol::ReviewCodeLocation;
use rexux_protocol::protocol::ReviewFinding;
use rexux_protocol::protocol::ReviewLineRange;
use rexux_protocol::protocol::ReviewOutputEvent;
...
use rexux_protocol::review_format::render_review_output_text;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under Rexux sandbox network restrictions.
    skip_if_no_network!();
    ...
    let codex = new_conversation_for_server(&server, rexux_home.clone(), |_| {}).await;
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let codex = Arc::clone(&test.codex);
    std::fs::remove_file(rexux_home.path().join("models_cache.json"))
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = rexux_home.path().join("resume.jsonl");
    {
        ...
        let user = rexux_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = rexux_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let codex =
        resume_conversation_for_server(&server, rexux_home.clone(), session_file.clone(), |_| {})
            .await;
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let codex = new_conversation_for_server(&server, rexux_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, rexux_home, resume_path)
        .await
