...
use rexux_extension_api::McpToolResultInput;
...
async fn mcp_result_processing_precedes_completion(
    ...
    let call = match mode {
        ...
        McpCallMode::CodeMode => responses::ev_custom_tool_call(
            call_id,
            "exec",
            r#"text(await tools.mcp__rexux_apps__calendar_list_events({query: "callback test"}));"#,
        ),
    };
...
async fn tool_start_receives_frozen_host_plugin_root() -> Result<()> {
    ...
    let server = responses::start_mock_server().await;
    let rexux_home = Arc::new(tempfile::tempdir()?);
    let plugin_root = super::plugins::write_sample_plugin_manifest_and_config(rexux_home.as_ref());
