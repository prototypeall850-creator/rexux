...
fn repo_root() -> std::path::PathBuf {
    rexux_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
fn personal_access_token_exec_command(server: &MockServer, home: &TempDir) -> Command {
    let bin = rexux_utils_cargo_bin::cargo_bin("codex").unwrap();
    ...
    cmd.arg("exec")
        ...
        .arg(repo_root())
        .arg("hello?");
    cmd.env("REXUX_HOME", home.path())
        .env(REXUX_ACCESS_TOKEN_ENV_VAR, PERSONAL_ACCESS_TOKEN)
        .env("REXUX_AUTHAPI_BASE_URL", server.uri())
        .env_remove(REXUX_API_KEY_ENV_VAR)
...
impl Drop for ChildProcessCleanupGuard {
    fn drop(&mut self) {
        #[cfg(unix)]
        {
            let _ = rexux_utils_pty::process_group::kill_process_group(self.0);
        }
...
async fn exec_cli_applies_model_instructions_file() {
    ...
    // Rexux to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    cmd.arg("exec")
        ...
        .arg(&prompt);
    cmd.env("REXUX_HOME", home.path())
        .env(REXUX_API_KEY_ENV_VAR, "dummy");
    ...
    let bin2 = rexux_utils_cargo_bin::cargo_bin("codex").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("REXUX_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy");

    let output2 = run_cli_command(&mut cmd2).unwrap();
    assert!(output2.status.success(), "resume rexux-cli run failed");
