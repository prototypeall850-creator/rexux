...
use rexux_core::EnvironmentNetworkPolicy;
use rexux_core::StartThreadOptions;
use rexux_core::TurnInputRequest;
use rexux_core::WaitForEnvironmentToolConfig;
use rexux_core::compact::SUMMARIZATION_PROMPT;
use rexux_core::config::Config;
use rexux_core::config::Constrained;
use rexux_core::windows_sandbox::WindowsSandboxLevelExt;
use rexux_exec_server::CopyOptions;
use rexux_exec_server::CreateDirectoryOptions;
use rexux_exec_server::EnvironmentReadyInfo;
use rexux_exec_server::ExecServerError;
use rexux_exec_server::ExecServerRuntimePaths;
use rexux_exec_server::FileSystemSandboxContext;
use rexux_exec_server::LOCAL_ENVIRONMENT_ID;
use rexux_exec_server::NoiseChannelPublicKey;
use rexux_exec_server::NoiseRendezvousConnectBundle;
use rexux_exec_server::NoiseRendezvousConnectProvider;
use rexux_exec_server::REMOTE_ENVIRONMENT_ID;
use rexux_exec_server::RemoteEnvironmentConfig;
...
use rexux_protocol::protocol::ENVIRONMENTS_INSTRUCTIONS_OPEN_TAG;
...
fn test_rexux_with_wait_for_environment() -> TestRexuxBuilder {
    let mut extensions = ExtensionRegistryBuilder::new();
    extensions.thread_lifecycle_contributor(Arc::new(WaitForEnvironmentTestExtension));
    test_rexux().with_extensions(Arc::new(extensions.build()))
}

async fn unified_exec_test(server: &wiremock::MockServer) -> Result<TestRexux> {
...
async fn submit_turn_with_approval_and_environments(
    ...
    let turn_environment_selections = rexux_protocol::protocol::TurnEnvironmentSelections::new(
...
async fn wait_for_completion_without_patch_approval(test: &TestRexux) {
    let event = wait_for_event(&test.codex, |event| {
        matches!(
            event,
            EventMsg::ApplyPatchApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn explicit_remote_shell_runs_in_remote_cwd() -> Result<()> {
    ...
    let (shell, command) = match test_target_os() {
        TestTargetOs::Linux => (
            "bash",
            r#"case "$PWD" in /tmp/rexux-core-test-cwd-*) ;; *) echo "unexpected cwd: $PWD" >&2; exit 1 ;; esac"#,
        ),
        TestTargetOs::Windows => (
            "powershell",
            r#"$cwd = (Get-Location).Path; if ($cwd -notlike 'C:\rexux-core-test-cwd-*') { Write-Error "unexpected cwd: $cwd"; exit 1 }"#,
        ),
        TestTargetOs::MacOs => unreachable!("remote test targets do not run macOS"),
    };
    ...
    for deferred_executor_enabled in [false, true] {
        for host_config_present in [false, true] {
            ...
            let builder = if host_config_present {
                test_rexux_with_wait_for_environment()
            } else {
                test_rexux()
            };
...
async fn settings_update_does_not_retarget_active_turn_environment() -> Result<()> {
    ...
    let preview = test
        .codex
        .preview_thread_settings_overrides(RexuxThreadSettingsOverrides {
...
async fn shared_executor_keeps_ready_capability_roots_scoped_to_each_attachment() -> Result<()> {
    ...
    let mut builder = test_rexux().with_extensions(Arc::new(extensions.build()));
    let test = builder.build_with_auto_env(&server).await?;
    let selection = test
        .codex
...
async fn owner_network_policy_rejects_unsupported_environment_authority() -> Result<()> {
    let server = start_mock_server().await;
    let test = test_rexux().build_with_auto_env(&server).await?;
...
async fn ready_before_selection_resolves_resumed_thread_capability_root_after_wait(
    ...
    let remote_environment = tokio::spawn(rexux_exec_server::run_remote_environment(
        remote_config,
        runtime_paths,
    ));
...
async fn deferred_executor_stays_pending_after_materialization() -> Result<()> {
    ...
    .await;
    let mut builder = test_rexux_with_wait_for_environment().with_config(|config| {
        assert!(config.features.enable(Feature::DeferredExecutor).is_ok());
    });
...
async fn deferred_executor_loads_agents_md_when_environment_becomes_ready() -> Result<()> {
    ...
    .await;
    let mut builder = test_rexux_with_wait_for_environment()
        .with_exec_server_url(format!("ws://{}", listener.local_addr()?))
        .with_config(|config| {
            assert!(config.features.enable(Feature::DeferredExecutor).is_ok());
        });
...
async fn remote_request_permissions_grant_unblocks_later_remote_exec() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/rexux-remote-request-permissions-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_freeform_routes_to_selected_remote_environment() -> Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/rexux-remote-apply-patch-freeform-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn apply_patch_approvals_are_remembered_per_environment() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/rexux-remote-apply-patch-approval-cwd-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
    ...
    let target_path = PathBuf::from(format!(
        "/tmp/rexux-apply-patch-approval-scope-{}.txt",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_intercepted_exec_command_routes_to_selected_remote_environment() -> Result<()>
{
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/rexux-remote-apply-patch-exec-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn remote_test_env_sandboxed_read_allows_readable_root() -> Result<()> {
    ...
    let allowed_dir = PathBuf::from(format!("/tmp/rexux-remote-readable-{}", std::process::id()));
...
async fn remote_test_env_sandboxed_read_rejects_symlink_parent_dotdot_escape() -> Result<()> {
    ...
    let root = PathBuf::from(format!("/tmp/rexux-remote-dotdot-{}", std::process::id()));
