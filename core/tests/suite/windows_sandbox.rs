...
fn rexux_home_for_windows_sandbox_test(name: &str) -> anyhow::Result<TestRexuxHome> {
    if let Some(test_tmpdir) = std::env::var_os("TEST_TMPDIR") {
        // The elevated backend provisions machine-local sandbox users. Bazel
        // retries run in the same Windows VM, so keep REXUX_HOME stable within
        // the test temp root and let setup reconcile its persisted ACL state.
        let rexux_home = PathBuf::from(test_tmpdir).join(name);
        std::fs::create_dir_all(&rexux_home)
            .with_context(|| format!("create stable test REXUX_HOME {}", rexux_home.display()))?;
        return Ok(TestRexuxHome::Persistent(rexux_home));
    }

    Ok(TestRexuxHome::Temporary(TempDir::new()?))
}
...
fn stage_windows_sandbox_helpers() -> anyhow::Result<()> {
    ...
    let resources_dir = test_exe_dir.join("rexux-resources");
    ...
    for helper_name in ["rexux-windows-sandbox-setup", "rexux-command-runner"] {
        let helper = rexux_utils_cargo_bin::cargo_bin(helper_name)?;
...
async fn windows_restricted_token_rejects_exact_and_glob_deny_read_policy() -> anyhow::Result<()> {
    let rexux_home =
        rexux_home_for_windows_sandbox_test("windows-restricted-token-deny-read-rexux-home")?;
...
async fn windows_elevated_does_not_create_missing_workspace_metadata() -> anyhow::Result<()> {
    let rexux_home =
        rexux_home_for_windows_sandbox_test("windows-elevated-missing-metadata-rexux-home")?;
    let _rexux_home_guard = EnvVarGuard::set("REXUX_HOME", rexux_home.path().as_os_str());
...
async fn windows_elevated_enforces_deny_read_and_protects_setup_marker() -> anyhow::Result<()> {
    let rexux_home = rexux_home_for_windows_sandbox_test("windows-elevated-deny-read-rexux-home")?;
    let _rexux_home_guard = EnvVarGuard::set("REXUX_HOME", rexux_home.path().as_os_str());
    stage_windows_sandbox_helpers()?;
    let workspace = TempDir::new()?;
    let cwd = dunce::canonicalize(workspace.path())?.abs();
    ...
    let setup_marker = rexux_home.path().join(".sandbox").join("setup_marker.json");
    ...
    assert!(
        sandbox_setup_is_complete(rexux_home.path()),
...
async fn windows_elevated_unified_exec_enforces_managed_deny_reads() -> anyhow::Result<()> {
    let rexux_home =
        rexux_home_for_windows_sandbox_test("windows-elevated-tool-runtime-deny-read-rexux-home")?;
    let _rexux_home_guard = EnvVarGuard::set("REXUX_HOME", rexux_home.path().as_os_str());
    stage_windows_sandbox_helpers()?;

    let configured_rexux_home = dunce::canonicalize(rexux_home.path())?.abs();
    let builder = test_rexux()
        .with_windows_cmd_shell()
        .with_config(move |config| {
            config.rexux_home = configured_rexux_home;
