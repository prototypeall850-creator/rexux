...
async fn resume_restores_dynamic_tools_from_rollout_with_sqlite_enabled() -> Result<()> {
    ...
    let mut resume_builder = test_rexux().with_config(|config| {
        config
            .features
            .enable(Feature::Sqlite)
            .expect("test config should allow feature update");
...
async fn backfill_scans_existing_rollouts() -> Result<()> {
    ...
    let mut builder = test_rexux()
        .with_pre_build_hook(move |rexux_home| {
            let rollout_path = rexux_home.join(&rollout_rel_path_for_hook);
    ...
    let db_path = test.config.sqlite.state_db_path();
    let rollout_path = test.config.rexux_home.join(&rollout_rel_path);
...
async fn tool_call_logs_include_thread_id() -> Result<()> {
    ...
    let log_db_layer = rexux_state::log_db::start(db.clone());
    ...
    let mut found = None;
    for _ in 0..80 {
        let query = rexux_state::LogQuery {
            descending: true,
            limit: Some(20),
            ..Default::default()
        };
