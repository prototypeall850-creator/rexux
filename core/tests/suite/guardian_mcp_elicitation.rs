...
use rexux_protocol::protocol::GuardianAssessmentAction;
        ...
        for index, meta in enumerate(approval_metas):
            ...
            pending_elicitations.append({
                ...
                "method": "elicitation/create", "params": {
                    ...
                    "_meta": {"rexux_request_type": "approval_request",
                              "rexux_approval_kind": "mcp_tool_call", "tool_name": "write_record",
...
async fn server_initiated_mcp_elicitation_can_require_synchronous_auto_review(
    ...
    if strict_auto_review {
        meta.insert("rexux_strict_auto_review".to_string(), json!(true));
    }
    if let Some(sensitive_action) = sensitive_action {
        meta.insert(
            "rexux_sensitive_action".to_string(),
...
async fn node_elicitations_attribute_independent_reviews_without_changing_actions(
    ...
    let meta = actions.map(|(tool_name, arguments)| {
        let mut meta = json!({
            "rexux_strict_auto_review": strict_auto_review,
            "rexux_sensitive_action": true,
            "tool_name": tool_name,
