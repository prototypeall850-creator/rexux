...
use rexux_core::parse_turn_item;
...
async fn fork_thread_twice_drops_to_first_message() {
    ...
    let fork1_path = rexux_fork1.rollout_path().expect("rollout path");
    expected_after_first.push(thread_settings_applied_item(
        fork1_thread_id,
        rexux_fork1.thread_settings_snapshot().await,
    ));
    ...
    let fork2_path = rexux_fork2.rollout_path().expect("rollout path");
    ...
    expected_after_second.push(thread_settings_applied_item(
        fork2_thread_id,
        rexux_fork2.thread_settings_snapshot().await,
    ));
...
async fn assert_copied_fork_persists_inherited_history(history_mode: ThreadHistoryMode) {
    ...
    let mut builder = test_rexux().with_history_mode(history_mode);
    let test = builder.build(&server).await.expect("create conversation");
    let codex = test.codex.clone();
    let thread_manager = test.thread_manager.clone();
    ...
    let source_meta = rexux_rollout::read_session_meta_line(source_path.as_path())
        .await
    ...
    if matches!(history_mode, ThreadHistoryMode::Paginated) {
        ...
        let resumed_history = rexux_rollout::RolloutRecorder::get_rollout_history(&forked_path)
            .await
        ...
        let resumed = thread_manager
            .resume_thread_with_history(
                test.config.clone(),
                resumed_history,
                rexux_core::test_support::auth_manager_from_auth(
