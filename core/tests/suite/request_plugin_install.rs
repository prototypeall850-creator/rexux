...
use rexux_core_plugins::startup_sync::curated_plugins_repo_path;
...
use rexux_protocol::approvals::ElicitationAction;
...
const CALENDAR_NAMESPACE: &str = "mcp__rexux_apps__calendar";
...
async fn start_gated_step_preparation(test: &TestRexux, server: &MockServer) -> Result<PathUri> {
...
async fn start_install_turn(test: &TestRexux, prompt: &str) -> Result<ElicitationRequestEvent> {
    let (sandbox_policy, permission_profile) =
        turn_permission_fields(PermissionProfile::Disabled, test.config.cwd.as_path());
    test.codex
        .start_or_steer_turn(
        ...
        if let Some(event) = requests
            ...
            .find_map(|request| {
                ...
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "rexux_plugin_install_requested")
                        .cloned()
                })
            })
        {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": turn_id,
                "model_slug": "gpt-5.4",
                "product_client_id": rexux_login::default_client::originator().value,
            }
        })
    );
