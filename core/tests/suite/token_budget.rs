...
use rexux_protocol::protocol::CONTEXT_WINDOW_CLOSE_TAG;
...
async fn assert_context_compaction_item_lifecycle(codex: &std::sync::Arc<rexux_core::RexuxThread>) {
...
async fn token_budget_guidance_precedes_standalone_context_window(
    ...
    let test = test_rexux()
        .with_auth(RexuxAuth::from_external_chatgpt_tokens(
...
#[test_case("OpenAI", "/backend-api/codex", None, true; "rexux_backend")]
#[test_case("Custom", "/backend-api/codex", None, false; "custom_provider")]
#[test_case("OpenAI", "/v1", None, false; "non_rexux_endpoint")]
#[test_case("OpenAI", "/backend-api/codex", Some("test-provider-token"), false; "provider_credentials")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn experimental_context_requires_rexux_backend(
