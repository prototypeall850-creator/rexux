...
use rexux_app_server_protocol::ThreadRealtimeSessionOutcome;
use rexux_app_server_protocol::ThreadRealtimeTranscriptRole;
...
use rexux_core::test_support::auth_manager_from_auth;
...
use rexux_protocol::protocol::RealtimeNoopRequested;
...
use rexux_thread_store::ListTimelineParams;
...
const REALTIME_BACKEND_PROMPT: &str = rexux_prompts::BACKEND_PROMPT;
...
const REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR: &str =
    "REXUX_REALTIME_CONVERSATION_TEST_SUBPROCESS";
...
fn run_realtime_conversation_test_in_subprocess(
    ...
    for &key in rexux_network_proxy::PROXY_ENV_KEYS {
...
async fn seed_recent_thread(
    ...
    let mut metadata_builder = rexux_state::ThreadMetadataBuilder::new(
        thread_id,
        rollout_path,
        updated_at,
        SessionSource::Cli,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn conversation_websocket_transports_send_rexux_headers_without_creating_a_call(
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn conversation_webrtc_frameless_chatgpt_sends_rexux_headers_to_backend(
    model: Option<&str>,
    expected_model: &str,
    thread_source: Option<ThreadSource>,
...
async fn assert_transport_close_tail_flush(
    ...
    let mut builder = test_rexux().with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.realtime.version = RealtimeWsVersion::V1;
...
async fn conversation_audio_before_start_emits_error() -> Result<()> {
    ...
    .await;
    assert_eq!(err.rexux_error_info, Some(RexuxErrorInfo::BadRequest));
    assert_eq!(err.message, "conversation is not running");
...
async fn conversation_uses_explicit_start_voice() -> Result<()> {
    ...
    .await;
    let test = test_rexux().build_with_websocket_server(&server).await?;
    assert!(
        server
            .wait_for_handshakes(/*expected*/ 1, Duration::from_secs(2))
            .await
...
async fn conversation_startup_context_current_thread_selects_many_turns_by_budget() -> Result<()> {
    ...
    let resumed_thread = test
        ...
        .resume_thread_with_history(
            ...
            auth_manager_from_auth(RexuxAuth::from_api_key("dummy")),
            /*parent_trace*/ None,
        ...
        assert_eq!(
            timeline.items.iter().filter_map(|entry| match entry {
                ...
                ThreadTimelineEntry::Item { item, .. }
                    if matches!(item.as_ref(), rexux_app_server_protocol::ThreadItem::UserMessage { .. }) => Some("typed input"),
                _ => None,
            }).collect::<Vec<_>>(),
...
async fn conversation_flushes_assistant_deltas_every_200ms_for_v3_handoff() -> Result<()> {
    ...
    test.codex
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            ...
            rexux_response_item_prefix: None,
            rexux_response_handoff_mode:
                rexux_protocol::protocol::RexuxResponseHandoffMode::BemTags,
            rexux_response_handoff_channel_prefixes: Some(BTreeMap::from([
                (
...
async fn inbound_handoff_request_starts_turn_and_promotes_its_artifact() -> Result<()> {
    ...
    let response_mock = responses::mount_sse_once(
        ...
        responses::sse(vec![
            responses::ev_response_created("resp-1"),
            responses::ev_assistant_message("msg-1", "::rexux-realtime-inline{}\nShared artifact"),
            responses::ev_completed("resp-1"),
        ]),
    )
    .await;
    ...
    assert_eq!(
        ...
        vec![ThreadRealtimeItemContent::BemItemPromoted {
            ...
            presentation:
                rexux_app_server_protocol::ThreadRealtimeBemItemPresentation::InlineMarkdown,
        }]
    );
...
async fn delegated_turn_user_role_echo_does_not_redelegate_and_still_forwards_audio() -> Result<()>
{
    ...
    let mut builder = test_rexux().with_model("gpt-5.4").with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.realtime.version = RealtimeWsVersion::V1;
