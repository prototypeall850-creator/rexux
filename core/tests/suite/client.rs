...
use rexux_core::ModelClient;
use rexux_core::NewThread;
use rexux_core::Prompt;
use rexux_core::ResponseEvent;
use rexux_core::StartThreadOptions;
use rexux_core::ThreadManager;
use rexux_core::TurnInputRequest;
use rexux_core::X_REXUX_ROUTING_HINT_HEADER;
use rexux_core::resolve_installation_id;
use rexux_core::thread_store_from_config;
use rexux_extension_api::empty_extension_registry;
use rexux_features::Feature;
use rexux_history::RolloutItem;
use rexux_history::RolloutLine;
...
use rexux_protocol::models::LocalShellAction;
use rexux_protocol::models::LocalShellExecAction;
use rexux_protocol::models::LocalShellStatus;
use rexux_protocol::models::MessagePhase;
use rexux_protocol::models::ReasoningItemContent;
use rexux_protocol::models::ReasoningItemReasoningSummary;
use rexux_protocol::models::ResponseItem;
use rexux_protocol::models::WebSearchAction;
...
use core_test_support::TestRexuxResponsesRequestKind;
...
fn test_turn_responses_metadata(
    _client: &ModelClient,
    thread_id: ThreadId,
) -> rexux_core::RexuxResponsesMetadata {
    ...
    test_responses_metadata(
        ...
        TestRexuxResponsesRequestKind::Turn,
    )
}
...
fn assert_rexux_client_metadata(
...
async fn openai_stateless_responses_requests_preserve_item_turn_metadata_across_turns() {
    ...
    .await;
    let test = test_rexux().build(&server).await.unwrap();
...
async fn synthetic_call_output_id_is_stable_across_resumes() -> anyhow::Result<()> {
    ...
    let mut builder = test_rexux();
    let first = builder
        .resume(&server, Arc::clone(&rexux_home), session_path.clone())
        .await?;
    ...
    let second = builder.resume(&server, rexux_home, session_path).await?;
...
/// Writes an `auth.json` into the provided `rexux_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        rexux_home.path().join("auth.json"),
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: rexux_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = rexux_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = rexux_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![rexux_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = rexux_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Rexux to resume from our file
    let rexux_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_rexux()
        .with_home(rexux_home.clone())
        .with_pre_build_hook(|home| {
            std::fs::write(home.join("AGENTS.md"), "be nice").expect("write global instructions");
        });
    let test = builder
        .resume(&server, rexux_home, session_path.clone())
        .await
...
async fn resume_replays_legacy_js_repl_image_rollout_shapes() {
    ...
    let rexux_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_rexux().with_model("gpt-5.4");
    let test = builder
        .resume(&server, rexux_home, session_path.clone())
        .await
        .expect("resume conversation");
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let mut builder = test_rexux().with_auth(RexuxAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    assert_eq!(request.header(X_REXUX_ROUTING_HINT_HEADER), None);
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.rexux_home_path().join(INSTALLATION_ID_FILENAME))
...
async fn send_request_with_provider(provider: ModelProviderInfo) {
    ...
    let mut config = load_default_config_for_test(&rexux_home).await;
    config.model_provider_id = provider.name.clone();
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort.clone();
    let summary = config.model_reasoning_summary;
    let model = rexux_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        rexux_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let client = ModelClient::new(
        Some(AuthManager::from_auth_for_testing(RexuxAuth::from_api_key(
            "unused-api-key",
        ))),
    ...
    let mut stream = client_session
        .stream(
            ...
            /*service_tier*/ None,
            &responses_metadata,
            &rexux_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut builder = test_rexux()
        .with_auth(create_dummy_rexux_auth())
        .with_config(move |config| {
    ...
    assert_eq!(
        request.header(X_REXUX_ROUTING_HINT_HEADER),
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth = RexuxAuth::from_auth_storage(
        rexux_home.path(),
    ...
    .await
    .expect("Failed to load RexuxAuth")
    .expect("No RexuxAuth found in rexux_home");
    let auth_manager = rexux_core::test_support::auth_manager_from_auth(auth);
    let installation_id = resolve_installation_id(&config.rexux_home)
        .await
        .expect("resolve installation id");
    let thread_manager = ThreadManager::new(
        &config,
        auth_manager.clone(),
        rexux_core::build_models_manager(&config, auth_manager),
        rexux_core::RexuxAppsToolsCache::default(),
        SessionSource::Exec,
        Arc::new(rexux_exec_server::EnvironmentManager::default_for_tests()),
        empty_extension_registry(),
        Arc::new(rexux_core::test_support::EmptyUserInstructionsProvider),
...
async fn omits_apps_guidance_when_orchestrator_mcp_is_disabled() {
    ...
    assert!(
        !request.body_contains_text("mcp__rexux_apps"),
        "did not expect rexux_apps MCP tools when orchestrator MCP is disabled, got {:?}",
    ...
    assert!(
        ...
        "did not expect rexux_apps resource calls: {resource_methods:?}"
    );
}
...
async fn powershell_shell_version_is_model_visible_only_when_enabled() -> anyhow::Result<()> {
    ...
    let shell_path = rexux_shell_command::powershell::try_find_powershell_executable_blocking()
    ...
    for enabled in [false, true] {
        ...
        let user_shell = rexux_shell_command::shell_detect::DetectedShell {
            shell_type: rexux_shell_command::shell_detect::ShellType::PowerShell,
...
async fn includes_configured_max_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestRexux { codex, .. } = test_rexux()
        .with_model("gpt-5.4")
        .with_config(|config| {
...
-> anyhow::Result<()> {
    ...
    .await;
    let TestRexux { codex, .. } = test_rexux().with_model("gpt-5.4").build(&server).await?;
...
async fn user_turn_collaboration_mode_overrides_model_and_effort() -> anyhow::Result<()> {
    ...
    .await;
    let TestRexux { codex, config, .. } = test_rexux().with_model("gpt-5.4").build(&server).await?;
...
async fn azure_responses_request_does_not_store_and_preserves_prefixed_item_ids() {
    ...
    let thread_id = ThreadId::new();
    let auth_manager =
        rexux_core::test_support::auth_manager_from_auth(RexuxAuth::from_api_key("Test API Key"));
    let session_telemetry = SessionTelemetry::new(
        thread_id,
        model.as_str(),
        model_info.slug.as_str(),
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let mut builder = test_rexux()
        .with_auth(RexuxAuth::from_api_key("test"))
        .with_config(move |config| {
            config.model_provider = provider;
        });
    let codex = builder
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let mut builder = test_rexux();
    let rexux_fixture = builder.build(&server).await?;
    let codex = rexux_fixture.codex.clone();
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let expected_context_window_message = RexuxErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
async fn env_var_overrides_loaded_auth() {
    ...
    assert_eq!(request.headers.get(X_REXUX_ROUTING_HINT_HEADER), None);
}

fn create_dummy_rexux_auth() -> RexuxAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Rexux sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
