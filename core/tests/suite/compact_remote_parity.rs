...
const FIXED_CWD: &str = "/tmp/rexux_remote_compaction_parity_workspace";
...
impl AuthCase {
    fn build(self) -> RexuxAuth {
        match self {
            AuthCase::ChatGpt => RexuxAuth::create_dummy_chatgpt_auth_for_testing(),
            AuthCase::ApiKey => RexuxAuth::from_api_key("dummy"),
        }
    }
}
...
async fn run_manual_hook_session(mode: Mode) -> Result<Value> {
    ...
    let home = harness.test().rexux_home_path();
...
async fn build_auto_harness(mode: Mode) -> Result<TestRexuxHarness> {
    build_harness_inner(
        mode,
        RunSettings::default(),
        /*hooks*/ false,
...
async fn build_harness(mode: Mode, settings: RunSettings, hooks: bool) -> Result<TestRexuxHarness> {
...
async fn build_harness_inner(
    ...
    TestRexuxHarness::with_builder(builder.with_config(move |config| {
        config.cwd = rexux_utils_absolute_path::AbsolutePathBuf::from_absolute_path(PathBuf::from(
            FIXED_CWD,
        ))
...
fn rollout_path(harness: &TestRexuxHarness) -> PathBuf {
    harness
        .test()
        .session_configured
        .rollout_path
...
async fn submit_user_input(codex: &rexux_core::RexuxThread, items: Vec<UserInput>) -> Result<()> {
    codex
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        if let Some(start_index) = start {
            text.replace_range(start_index..marker_index, "<REXUX_HOME>");
            search_start = start_index + "<REXUX_HOME>".len() + marker.len();
        } else {
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    ...
    assert_eq!(
        text,
        "file: <REXUX_HOME>/skills/.system/imagegen/SKILL.md and \
         <REXUX_HOME>/skills/custom/SKILL.md"
    );
}
