...
use rexux_protocol::protocol::ExecPolicyAmendment;
...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestRexux) -> (PathBuf, String) {
        match self {
    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured Rexux helper binary
...
impl Expectation {
    fn verify(&self, test: &TestRexux, result: &CommandResult) -> Result<()> {
        match self {
...
fn assert_active_workspace_permission_profile(test: &TestRexux) {
    assert_eq!(
...
async fn wait_for_completion_without_approval(test: &TestRexux) {
    let event = wait_for_event(&test.codex, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestRexux) {
...
async fn wait_for_spawned_thread(test: &TestRexux) -> Result<Arc<RexuxThread>> {
...
"#,
    ...
    let command = r#"printf '%s\n%s\n%s\n' "$GH_HOST" "$GH_ENTERPRISE_TOKEN" "$REXUX_NETWORK_PROXY_CREDENTIAL_BROKER_ACTIVE""#;
    let event = shell_event(
        call_id,
        command,
        /*timeout_ms*/ 10_000,
...
printenv REXUX_PERMISSION_PROFILE
touch {outside_path:?}
"#
        ),
    )?;
    ...
    assert_eq!(
        ...
        "the unsandboxed script should inherit REXUX_PERMISSION_PROFILE from the shell command"
    );
    assert!(
        outside_path.exists(),
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Print an ssh command that recreates the current Rexux sandbox remotely."
    )
...
def main():
    args = parse_args()
    profile_name = os.environ.get("REXUX_PERMISSION_PROFILE")
    if not profile_name:
        raise SystemExit("REXUX_PERMISSION_PROFILE must not be empty")
    if profile_name not in ALLOWED_PROFILES:
        raise SystemExit("REXUX_PERMISSION_PROFILE is not allowlisted")
...
"#
    ...
    assert_eq!(
        ...
        "remote_bash.py should use the allowlisted inherited profile and managed configuration to reconstruct the Rexux sandbox"
    );
...
async fn invalid_requested_prefix_rule_falls_back_for_compound_command() -> Result<()> {
    ...
    let command =
        "touch /tmp/rexux-fallback-rule-test.txt && echo hello > /tmp/rexux-fallback-rule-test.txt";
    let event = shell_event_with_prefix_rule(
        call_id,
        command,
        /*timeout_ms*/ 1_000,
...
"#,
    ...
    let fetch_command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://rexux-network-test.invalid', timeout=30).read().decode(errors='replace'))""#
        .to_string();
