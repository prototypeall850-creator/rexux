...
use anyhow::Context;
use anyhow::Result;
use rexux_config::HookStateToml;
...
use rexux_core::config::ThreadStoreConfig;
...
use rexux_plugin::PluginHookSource;
use rexux_plugin::PluginId;
use rexux_protocol::items::parse_hook_prompt_fragment;
...
async fn managed_hook_discovery_failure_rejects_session_startup_when_hooks_enabled() -> Result<()> {
    ...
    let mut builder = test_rexux().with_cloud_config_bundle(
...
async fn stop_hook_can_block_multiple_times_in_same_turn() -> Result<()> {
    ...
    let hook_inputs = read_stop_hook_inputs(test.rexux_home_path())?;
...
async fn session_start_hook_sees_materialized_transcript_path() -> Result<()> {
    ...
    let hook_inputs = read_session_start_hook_inputs(test.rexux_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
...
async fn session_start_runs_before_user_prompt_submit_on_first_turn() -> Result<()> {
    ...
    let hook_inputs = read_hook_order_inputs(test.rexux_home_path())?;
    assert_eq!(
        hook_inputs
            .iter()
    ...
    test.codex
        .start_or_steer_turn(
            ...
            .with_thread_settings(
                rexux_protocol::protocol::ThreadSettingsOverrides {
...
async fn resumed_thread_runs_resume_then_compact_session_start_hooks() -> Result<()> {
    ...
    let mut resume_builder = test_rexux().with_config(move |config| {
        config.model_auto_compact_token_limit = Some(limit);
        trust_discovered_hooks(config);
    });
    ...
    let hook_inputs = read_session_start_hook_inputs(resumed.rexux_home_path())?;
    assert_eq!(
        hook_inputs
            .iter()
            .filter_map(|input| input.get("source").and_then(Value::as_str))
...
async fn resumed_thread_keeps_stop_continuation_prompt_in_history() -> Result<()> {
    ...
    let mut initial_builder = test_rexux()
        .with_pre_build_hook(|home| {
    ...
    let mut resume_builder = test_rexux().with_config(trust_discovered_hooks);
    let resumed = resume_builder.restart(&server, &initial).await?;
...
async fn blocked_user_prompt_submit_persists_additional_context_for_next_turn() -> Result<()> {
    ...
    let hook_inputs = read_user_prompt_submit_hook_inputs(test.rexux_home_path())?;
...
async fn permission_request_hook_allows_network_approval_without_prompt() -> Result<()> {
    let command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://rexux-network-test.invalid', timeout=2).read().decode(errors='replace'))""#;
...
async fn permission_request_hook_denies_network_approval_with_custom_message() -> Result<()> {
    ...
    let command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); opener.open('http://rexux-network-test.invalid', timeout=2).read()""#;
    run_network_permission_hook_test(
        "deny",
        denial,
...
"#,
    ...
    let test = test_rexux()
        ...
        .with_config(move |config| {
            trust_discovered_hooks(config);
            config.approvals_reviewer = rexux_config::types::ApprovalsReviewer::AutoReview;
    ...
    assert_single_permission_request_hook_input(
        test.rexux_home_path(),
        command,
        Some("network-access http://rexux-network-test.invalid:80"),
    )?;
    let requests = responses.requests();
    assert_eq!(
        requests
...
async fn pre_tool_use_json_deny_blocks_exec_command_before_execution() -> Result<()> {
    ...
    let hook_inputs = read_pre_tool_use_hook_inputs(test.rexux_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
...
"#
    ...
    let hook_inputs = read_post_tool_use_hook_inputs(test.rexux_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
    assert_eq!(hook_inputs[0]["tool_input"]["command"], command);
    assert_eq!(
...
async fn local_bundled_cleanup_hook_runs_without_saved_trust(
    ...
    fs::create_dir_all(plugin_root.join(".rexux-plugin"))?;
    ...
    let hook_key = rexux_hooks::hook_key(
    ...
    let mut builder = test_rexux().with_home(home).with_config(move |config| {
        for feature in [Feature::Plugins, Feature::RexuxHooks] {
    ...
    assert_eq!(
        test.config.features.enabled(Feature::RexuxHooks),
        hooks_enabled
    );
    ...
    if !hooks_enabled {
        let regular_hooks = rexux_hooks::list_hooks(rexux_hooks::HooksConfig {
            feature_enabled: true,
    ...
    assert_eq!(
        rexux_hooks::hook_states_from_stack(Some(&test.config.config_layer_stack)),
...
"#,
  ...
  "hooks": {
    ...
    let plugin_hook_sources = vec![PluginHookSource {
        ...
        hooks: serde_json::from_str::<rexux_config::HooksFile>(plugin_hooks_json)
...
async fn pre_tool_use_merges_hooks_json_and_config_toml() -> Result<()> {
    ...
    let json_hook_inputs = read_pre_tool_use_hook_inputs(test.rexux_home_path())?
        .into_iter()
        .map(|hook_input| {
            serde_json::json!({
...
async fn post_tool_use_blocks_when_exec_session_completes_via_write_stdin() -> Result<()> {
    ...
    let pre_hook_inputs = read_pre_tool_use_hook_inputs(test.rexux_home_path())?;
    ...
    let post_hook_inputs = read_post_tool_use_hook_inputs(test.rexux_home_path())?;
