...
async fn rexux_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    rexux_returns_json_result("gpt-5.4".to_string()).await
}

async fn rexux_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("rexux_output_schema".into()))
    ...
    let TestRexux { codex, config, .. } = test_rexux().build(&server).await?;
