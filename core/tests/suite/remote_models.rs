...
use rexux_models_manager::model_info::BASE_INSTRUCTIONS;
...
const REMOTE_MODEL_SLUG: &str = "rexux-test";
...
async fn remote_models_get_model_info_uses_longest_matching_prefix() -> Result<()> {
    ...
    let specific = ModelInfo {
        display_name: "GPT 5.3 Rexux".to_string(),
    ...
    let manager = rexux_core::test_support::models_manager_with_provider(
        rexux_home.path().to_path_buf(),
        rexux_core::test_support::auth_manager_from_auth(auth),
        provider,
    );
    ...
    let model_info = manager
        .get_model_info("gpt-5.3-rexux-test", &config.to_models_manager_config())
        .await;

    assert_eq!(model_info.slug, "gpt-5.3-rexux-test");
    assert_eq!(
...
async fn remote_models_long_model_slug_is_sent_with_supported_reasoning(
    ...
    let server = MockServer::start().await;
    let requested_model = "gpt-5.3-rexux-test";
...
async fn remote_models_truncation_policy_without_override_preserves_remote() -> Result<()> {
    ...
    let slug = "rexux-test-truncation-policy";
...
async fn remote_models_truncation_policy_with_tool_output_override() -> Result<()> {
    ...
    let slug = "rexux-test-truncation-override";
    let remote_model = test_remote_model_with_policy(
        slug,
        ModelVisibility::List,
        /*priority*/ 1,
...
async fn remote_models_hide_picker_only_models() -> Result<()> {
    ...
    let hidden = available
        .iter()
        .find(|model| model.model == "rexux-auto-balanced")
...
fn bundled_default_model_slug() -> String {
    rexux_core::test_support::all_model_presets()
        .iter()
