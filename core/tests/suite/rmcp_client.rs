...
use rexux_config::types::McpServerAuth;
...
use rexux_core::EnvironmentConfig;
use rexux_core::EnvironmentMcpPolicy;
...
use rexux_protocol::mcp_policy::McpServerIdentity;
use rexux_protocol::mcp_policy::McpServerRequirement;
use rexux_protocol::mcp_policy::PluginMcpRequirements;
...
use rexux_protocol::openai_models::ConfigShellToolType;
use rexux_protocol::openai_models::ConfirmationPolicies;
use rexux_protocol::openai_models::InputModality;
use rexux_protocol::openai_models::ModelInfo;
use rexux_protocol::openai_models::ModelVisibility;
...
fn read_only_user_turn(fixture: &TestRexux, text: impl Into<String>) -> TurnInputRequest {
...
fn auto_approved_user_turn(fixture: &TestRexux, text: impl Into<String>) -> TurnInputRequest {
...
pub(super) fn remote_aware_environment_id() -> String {
    if is_remote_test_environment() {
        ...
        rexux_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string()
    }
}
...
fn unique_remote_path(binary_name: &str) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "/tmp/rexux-remote-env/{binary_name}-{}-{unique_suffix}",
        std::process::id()
    ))
}
...
async fn environment_mcp_policy_filters_runtime_config_and_model_tools(
    ...
    let rexux_home = Arc::new(tempdir()?);
    let test_env = test_env().await?;
    if from_plugin {
        let plugin_root =
            super::plugins::write_sample_plugin_manifest_and_config(rexux_home.as_ref());
...
async fn stdio_mcp_tool_names_respect_selected_servers() -> anyhow::Result<()> {
    ...
    let command = remote_aware_stdio_server_bin()?;
    let fixture = test_rexux()
        .with_pre_build_hook(move |rexux_home| {
            fs::write(
...
async fn apps_enabled_turn_skips_pending_optional_mcp_without_cached_tools() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            let event = fixture
                ...
                .next_event()
                .await
                .context("event stream ended before Rexux Apps became ready")?;
            if let EventMsg::McpStartupUpdate(update) = event.msg
                && update.server == REXUX_APPS_MCP_SERVER_NAME
    ...
    .await
    .context("Rexux Apps should finish starting before the first turn")??;
...
async fn stdio_mcp_tool_call_includes_sandbox_state_meta(
    ...
    let namespace = format!("mcp__{server_name}");
    let mut models = rexux_models_manager::bundled_models_response()?;
    let model = models
        .models
        .iter_mut()
    ...
    fixture
        ...
        .list_models(
            ...
            rexux_core::test_support::default_http_client_factory(),
        )
        .await;
    ...
    assert_eq!(
        ...
        SandboxState {
            ...
            rexux_linux_sandbox_exe: fixture.config.rexux_linux_sandbox_exe.clone(),
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: rexux_protocol::openai_models::ReasoningEffort::Medium,
...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that Rexux should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: Rexux can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip(mode: HeadersHelperMode) -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Rexux will call the MCP echo tool
    ...
    // Phase 3: configure Rexux with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert Rexux begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.codex, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn streamable_http_configured_auth_precedes_chatgpt_auth() -> anyhow::Result<()> {
    ...
    let configured_auth_fixture = test_rexux()
        .with_auth(RexuxAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(move |config| {
            insert_mcp_server(
                config,
...
/// This test writes to a fallback credentials file in REXUX_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire REXUX_HOME through the code.
#[test]
#[serial(rexux_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Rexux will call the OAuth-backed
    ...
    // Phase 3: seed an isolated REXUX_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _rexux_home_guard = EnvVarGuard::set("REXUX_HOME", temp_home.path().as_os_str());
    let unset_authorization_env_var = format!(
        "REXUX_TEST_UNSET_MCP_OAUTH_AUTHORIZATION_{}",
        std::process::id()
    );
    ...
    // Phase 4: configure Rexux with the OAuth-backed Streamable HTTP MCP
    ...
    assert!(rexux_rmcp_client::delete_oauth_tokens(
        discovered_credential_name.as_ref(),
        http_server.url(),
        OAuthCredentialsStoreMode::File,
        rexux_config::types::AuthKeyringBackendKind::default(),
    )?);
    ...
    // Phase 7: assert Rexux begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.codex, |ev| {
        matches!(
            ev,
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the Rexux fixture.
...
fn write_fallback_oauth_tokens(
    ...
    rexux_rmcp_client::save_oauth_tokens(
        server_name,
        &tokens,
        OAuthCredentialsStoreMode::File,
        rexux_config::types::AuthKeyringBackendKind::default(),
