...
use rexux_core::config::CurrentTimeReminderConfig;
use rexux_core::context::NodeReplReviewEvidence;
use rexux_extension_api::ExtensionData;
use rexux_extension_api::ExtensionRegistryBuilder;
use rexux_extension_api::ToolCallOutcome;
use rexux_extension_api::ToolContributor;
use rexux_extension_api::ToolFinishInput;
use rexux_extension_api::ToolLifecycleContributor;
use rexux_extension_api::ToolLifecycleFuture;
use rexux_extension_api::ToolStartInput;
...
use rexux_tools::FreeformTool;
use rexux_tools::FreeformToolFormat;
use rexux_tools::FunctionCallError;
use rexux_tools::JsonToolOutput;
use rexux_tools::ResponsesApiNamespace;
use rexux_tools::ResponsesApiNamespaceTool;
use rexux_tools::ToolCall;
use rexux_tools::ToolExecutor;
use rexux_tools::ToolExecutorFuture;
use rexux_tools::ToolName;
use rexux_tools::ToolOutput;
use rexux_tools::ToolPayload;
use rexux_tools::ToolSpec;
use rexux_web_search_extension::install as install_web_search_extension;
...
async fn run_code_mode_turn_with_model_and_config(
    ...
    let builder = test_rexux().with_model(model).with_config(move |config| {
...
async fn missing_process_host_falls_back_to_direct_tools_and_warns_once() -> Result<()> {
    ...
    let mut builder = test_rexux()
        ...
        .with_code_mode_host_program("rexux-code-mode-host-does-not-exist".into())
        .with_config(|config| {
            config
                .features
    ...
    assert!(
        first_warnings.iter().any(|warning| {
            warning.contains("Code Mode is unavailable")
                && warning.contains("rexux-code-mode-host-does-not-exist")
        }),
...
async fn missing_process_host_keeps_code_mode_only_and_fails_closed() -> Result<()> {
    ...
    assert!(
        output.contains("rexux-code-mode-host-does-not-exist"),
...
"#,
    ...
    let auth = RexuxAuth::from_api_key("dummy");
    let auth_manager = rexux_core::test_support::auth_manager_from_auth(auth.clone());
...
async fn run_code_mode_turn_with_rmcp_config(
    ...
    let mut builder = test_rexux().with_model(model).with_config(move |config| {
        ...
        assert!(
            metadata["executed_tool_calls"]
                ...
                .is_some_and(|calls| calls.iter().any(|call| call["arguments"]
                    .get("_rexux_executed_tool_call_truncated")
                    .is_some()))
        );
    } else {
        assert_eq!(
...
const tool = ALL_TOOLS.find(
  ({ name }) => name === "mcp__rexux_apps__calendar_timezone_option_99"
);
if (!tool) {
...
"#,
        visible_tool_name = "mcp__rexux_apps__calendar_timezone_option_99",
...
impl ToolLifecycleContributor for InterruptedNestedToolObserver {
    fn on_tool_start<'a>(&'a self, input: ToolStartInput<'a>) -> ToolLifecycleFuture<'a> {
        Box::pin(async move {
            let rexux_extension_api::ToolCallSource::CodeMode { cell_id, .. } = input.source else {
                return;
            };
    ...
    fn on_tool_finish<'a>(&'a self, input: ToolFinishInput<'a>) -> ToolLifecycleFuture<'a> {
        Box::pin(async move {
            if input.tool_name.name != "test_sync_tool"
                || !matches!(
                    input.source,
                    rexux_extension_api::ToolCallSource::CodeMode { .. }
                )
            {
                return;
            }
            ...
            if reviewer_constraint.is_some_and(|value| value != "large_prompt") || check_detail {
                ...
                if reviewer_token_budget {
                    ...
                    reviewer.input_modalities =
                        vec![rexux_protocol::openai_models::InputModality::Text];
                } else {
        ...
        test.codex
            .start_or_steer_turn(
                ...
                .with_thread_settings(ThreadSettingsOverrides {
                    environments: Some(rexux_protocol::protocol::TurnEnvironmentSelections::new(
                        cwd,
                        Vec::new(),
                    )),
...
async fn code_mode_can_call_hidden_dynamic_tools() -> Result<()> {
    ...
    let code = r#"
const tool = ALL_TOOLS.find(({ name }) => name === "rexux_app__hidden_dynamic_tool");
const out = await tools.rexux_app__hidden_dynamic_tool({ city: "Paris" });
text(
  JSON.stringify({
    name: tool?.name ?? null,
    description: tool?.description ?? null,
...
"#;
    ...
    .await;
    assert_eq!(request.namespace.as_deref(), Some("rexux_app"));
    ...
    assert_eq!(
        parsed.get("name"),
        Some(&Value::String("rexux_app__hidden_dynamic_tool".to_string()))
    );
    assert_eq!(
        parsed.get("out"),
    ...
    assert!(
        parsed
            ...
            .and_then(Value::as_str)
            .is_some_and(|description| {
                description.contains("Rexux app tools.")
                    && description.contains("A hidden dynamic tool.")
                    && description.contains("declare const tools:")
                    && description.contains("rexux_app__hidden_dynamic_tool(args:")
            })
    );
