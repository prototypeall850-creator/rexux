...
use core_test_support::test_rexux::ApplyPatchModelOutput;
...
use core_test_support::test_rexux::executor_path_uri;
...
pub async fn apply_patch_harness() -> Result<TestRexuxHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestRexuxBuilder) -> TestRexuxBuilder,
) -> Result<TestRexuxHarness> {
    let builder = configure(test_rexux());
    // Box harness construction so apply_patch_cli tests do not inline the
    // full test-thread startup path into each test future.
    Box::pin(TestRexuxHarness::with_auto_env_builder(builder)).await
}

async fn submit_without_wait(harness: &TestRexuxHarness, prompt: &str) -> Result<()> {
...
#[cfg(target_os = "linux")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn apply_patch_cli_uses_rexux_self_exe_with_linux_sandbox_helper_alias() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let harness = apply_patch_harness().await?;
    let rexux_linux_sandbox_exe = harness
        .test()
        .config
        .rexux_linux_sandbox_exe
        .as_ref()
        .expect("linux test config should include rexux-linux-sandbox helper");
    assert_eq!(
        rexux_linux_sandbox_exe
            .file_name()
            .and_then(|name| name.to_str()),
        Some(REXUX_LINUX_SANDBOX_ARG0),
    );
...
async fn apply_patch_cli_does_not_widen_permissions_for_workspace_directory_target() -> Result<()> {
    ...
    let harness = apply_patch_harness_with(move |builder| {
        builder.with_config(move |config| {
            config.approvals_reviewer = rexux_protocol::config_types::ApprovalsReviewer::User;
            config.workspace_roots = vec![harness_work_dir.clone()];
            config
                .permissions
                .set_workspace_roots(config.workspace_roots.clone());
...
async fn apply_patch_custom_tool_streaming_emits_updated_changes() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&rexux_protocol::protocol::FileChange::Add {
            content: String::new(),
        })
    );
    assert_eq!(
...
#[test_case("rexux_work_web", true, "work.txt"; "web work uses cwd when enabled")]
#[test_case("rexux_work_mobile", true, "work.txt"; "mobile work uses cwd when enabled")]
#[test_case("rexux_work_web", false, "subdir/work.txt"; "disabled feature keeps repository root")]
#[test_case("rexux_work_desktop", true, "work.txt"; "desktop uses cwd when enabled")]
#[test_case("rexux_work_cca", true, "work.txt"; "legacy cca uses cwd when enabled")]
