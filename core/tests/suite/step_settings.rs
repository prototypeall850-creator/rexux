...
use rexux_core::config::TokenBudgetConfig;
...
use rexux_protocol::openai_models::ToolMessage;
use rexux_protocol::openai_models::ToolMessages;
use rexux_protocol::protocol::AskForApproval;
use rexux_protocol::protocol::CONTEXT_WINDOW_GUIDANCE_CLOSE_TAG;
...
use rexux_protocol::request_user_input::RequestUserInputEvent;
...
fn step_settings_test() -> TestRexuxBuilder {
    test_rexux().with_model(MODEL_A).with_config(move |config| {
        for feature in [
...
async fn start_paused_turn(thread: &RexuxThread) -> Result<RequestUserInputEvent> {
    thread
...
async fn answer_paused_turn(thread: &RexuxThread, turn_id: &str) -> Result<()> {
    thread
...
async fn active_model_switch_resolves_token_budget_from_original_preferences(
    ...
    if reload_user_config {
        std::fs::write(
            test.rexux_home_path().join("config.toml"),
