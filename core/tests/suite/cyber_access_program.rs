use anyhow::Result;
use rexux_core::RecoverTurnRequest;
...
async fn cyber_access_program_omits_api_key_and_spoofed_custom_provider() -> Result<()> {
    ...
    for (auth, provider_id) in [
        (RexuxAuth::from_api_key("test-key"), "openai"),
        (RexuxAuth::create_dummy_chatgpt_auth_for_testing(), "custom"),
    ] {
        let server = responses::start_mock_server().await;
...
async fn submit(test: &TestRexux, program: Option<CyberAccessProgram>) -> Result<()> {
    test.codex
        .start_or_steer_turn(
            TurnInputRequest::user_input(vec![UserInput::Text {
                text: "hello".to_owned(),
