...
use rexux_core::config::RolloutBudgetConfig;
                ...
                json!({
                    ...
                    "response": {
                        ...
                        "usage": {
                            ...
                            "total_tokens": 75,
                            "rexux_rollout_budget_units": rollout_budget_units
                        }
                    }
                }),
            ]),
...
async fn invalid_provider_rollout_budget_units_fail_without_retry() -> Result<()> {
    ...
    completed["response"]["usage"]["rexux_rollout_budget_units"] = json!(-1.0);
    let responses = mount_sse_sequence(
        &server,
    ...
    assert_eq!(
        error.message,
        "Fatal error: response.completed usage.rexux_rollout_budget_units must be finite and non-negative"
    );
    assert_eq!(error.rexux_error_info, Some(RexuxErrorInfo::Other));
        ...
        wait_for_event(&test.codex, |event| {
            matches!(
                ...
                EventMsg::Error(error)
                    if error.rexux_error_info == Some(RexuxErrorInfo::SessionBudgetExceeded)
            )
        })
        .await;
        wait_for_event(&test.codex, |event| {
...
async fn compaction_budget_exhaustion_fails_without_retry(
    ...
    if provider_units {
        completed["response"]["usage"]["rexux_rollout_budget_units"] = json!(10.0);
    }
