...
use rexux_extension_items::sleep::SleepItem;
...
async fn build_codex(server: &StreamingSseServer) -> Arc<RexuxThread> {
    test_rexux()
        .with_config(|config| config.update_plan_enabled = true)
        .with_model("gpt-5.4")
        .build_with_streaming_server(server)
        .await
        .expect("build streaming Rexux test session")
        .codex
}

async fn submit_user_input(codex: &RexuxThread, text: &str) {
    codex
...
async fn submit_danger_full_access_user_turn(test: &TestRexux, text: &str) {
...
async fn steer_user_input(codex: &RexuxThread, text: &str) {
...
async fn enqueue_queue_only_agent_mail(codex: &RexuxThread, text: &str) {
    codex
...
async fn submit_queue_only_agent_mail(codex: &RexuxThread, text: &str) {
...
async fn wait_for_reasoning_item_started(codex: &RexuxThread) {
    wait_for_event(codex, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(codex: &RexuxThread, text: &str) {
...
async fn wait_for_turn_complete(codex: &RexuxThread) {
    wait_for_event(codex, |event| matches!(event, EventMsg::TurnComplete(_))).await;
}

async fn wait_for_sleep_item_started(codex: &RexuxThread, call_id: &str, duration_ms: u64) {
...
async fn wait_for_sleep_item_completed(codex: &RexuxThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(codex, |event| {
        matches!(
            event,
...
impl rexux_extension_api::ThreadLifecycleContributor<rexux_core::config::Config>
    for SleepingRootExtension
{
    fn on_thread_start<'a>(
        &'a self,
        input: rexux_extension_api::ThreadStartInput<'a, rexux_core::config::Config>,
...
async fn queue_only_agent_mail_wakes_sleeping_root_with_previous_turn_context() {
    ...
    .await;
    let mut extensions =
        rexux_extension_api::ExtensionRegistryBuilder::<rexux_core::config::Config>::new();
    ...
    let codex = test_rexux()
        ...
        .await
        .expect("build Rexux test session")
        .codex;
    ...
    assert!(history.items.iter().any(|item| {
        matches!(
            ...
            RolloutItem::ResponseItem(envelope)
                if matches!(
                    &envelope.item,
                    rexux_protocol::models::ResponseItem::AgentMessage { content, .. }
                        if content.iter().any(|content| matches!(
                            content,
                            rexux_protocol::models::AgentMessageInputContent::InputText { text }
    ...
    wait_for_event(codex, |event| {
        match event {
            EventMsg::Error(error) => {
                assert_eq!(
                    error.rexux_error_info,
                    Some(RexuxErrorInfo::UsageLimitExceeded)
                );
