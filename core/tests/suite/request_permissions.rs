...
async fn request_permissions_auto_review_applies_guardian_decision(outcome: &str) -> Result<()> {
    ...
    skip_if_wine_exec!(
        Ok(()),
        "request_permissions requires a cwd native to the Rexux host"
    );
...
async fn partial_request_permissions_grants_do_not_preapprove_new_permissions() -> Result<()> {
    ...
    let rexux_protocol::models::LegacyReadWriteRoots {
        read: approval_reads,
...
async fn denied_child_permissions_require_fresh_approval(
    ...
    let harness =
        TestRexuxHarness::with_auto_env_builder(test_rexux().with_config(move |config| {
