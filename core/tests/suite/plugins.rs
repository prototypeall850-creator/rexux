...
use rexux_core_plugins::store::PluginStore;
...
use rexux_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID;
...
use rexux_skills_extension::HostSkillsLoadInput;
use rexux_skills_extension::SkillsExtensionConfig;
use rexux_skills_extension::install;
...
use core_test_support::test_rexux::local_selections;
use core_test_support::test_rexux::test_rexux;
use core_test_support::test_rexux::turn_permission_fields;
...
const SAMPLE_PLUGIN_APP_NAMESPACE: &str = "mcp__rexux_apps__google_calendar";
...
async fn build_analytics_plugin_test_codex(
    server: &MockServer,
    rexux_home: Arc<TempDir>,
) -> Result<TestRexux> {
    let chatgpt_base_url = server.uri();
    let mut builder = test_rexux()
        .with_home(rexux_home)
...
async fn persisted_remote_plugin_command_attribution_flows_through_turn_context(
    ...
    skip_if_remote!(
        Ok(()),
        "remote plugin attribution fixture uses a local Rexux home cache"
    );

    let server = start_mock_server().await;
    let rexux_home = Arc::new(TempDir::new()?);
    let script_path = write_remote_plugin_script_and_config(rexux_home.as_ref());
    std::fs::write(
        &script_path,
        r#"printf '%s' '{"version":1,"measurements":[{"name":"files_scanned","value":7}]}' > "$REXUX_PLUGIN_METRICS_OUTPUT"
"#,
    )?;
    ...
    let mut builder = builder
        .with_home(Arc::clone(&rexux_home))
        .with_auth(RexuxAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model("gpt-5.2")
        .with_config(move |config| config.chatgpt_base_url = chatgpt_base_url);
    let test_rexux = builder.build_with_auto_env(&server).await?;
    let codex = Arc::clone(&test_rexux.codex);
    let cwd = test_rexux.config.cwd.clone();
    let session_model = test_rexux.session_configured.model.clone();
    let (sandbox_policy, permission_profile) =
        turn_permission_fields(PermissionProfile::read_only(), cwd.as_path());
    codex
        .start_or_steer_turn(
            TurnInputRequest::user_input(vec![rexux_protocol::user_input::UserInput::Text {
    ...
    let measurement = wait_for_analytics_event(&server, "rexux_plugin_measurement_event").await;
    assert_eq!(
        serde_json::json!({
...
async fn agent_plugin_skills_use_shared_catalog_and_direct_child_discovery() -> Result<()> {
    ...
    .await;
    let rexux_home = Arc::new(TempDir::new()?);
    let skill_path = dunce::canonicalize(write_agent_plugin_skill_plugin(rexux_home.as_ref()))?;
    let mut builder = test_rexux()
        .with_home(Arc::clone(&rexux_home))
    ...
    test_rexux
        ...
        .await?;
    let warning = wait_for_event(&test_rexux.codex, |ev| {
        matches!(
            ev,
    ...
    .await;
    wait_for_event(&test_rexux.codex, |ev| {
...
async fn plugin_skill_product_policy_and_migrated_command_precedence_reach_agent_turns(
    ...
    .await;
    let rexux_home = Arc::new(TempDir::new()?);
    let plugin_root = write_sample_plugin_manifest_and_config(rexux_home.as_ref());
    ...
    if include_migrated_command {
        let migrated_skill_dir =
            plugin_root.join(".rexux-plugin/migrated-command-skills/source-command-review");
...
async fn legacy_plugin_skill_prompt_remains_complete() -> Result<()> {
    ...
    .await;
    let rexux_home = Arc::new(TempDir::new()?);
    let skill_path = write_plugin_skill_plugin(rexux_home.as_ref());
...
"#,
    ...
    for (marketplace, skill) in [
        ...
        std::fs::create_dir_all(root.join(".rexux-plugin"))?;
        std::fs::write(
            root.join(".rexux-plugin/plugin.json"),
            r#"{"name":"sites"}"#,
        )?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn agent_plugin_root_mcp_stdio_tool_round_trip_expands_reserved_paths_and_rexux_env_overlay()
-> Result<()> {
    skip_if_no_network!(Ok(()));
    let server = start_mock_server().await;
    ...
    .await;
    let rexux_home = Arc::new(TempDir::new()?);
    write_agent_plugin_skill_plugin(rexux_home.as_ref());
    ...
    let mut builder = test_rexux().with_home(Arc::clone(&rexux_home));
    let test_rexux = builder.build_with_remote_and_local_env(&server).await?;
    wait_for_mcp_server(&test_rexux.codex, "agent").await?;
    let data_root = dunce::canonicalize(
        std::fs::read_dir(rexux_home.path().join("plugins/data/agent-plugins"))?
            .next()
    ...
    test_rexux
        ...
        .await?;
    let end = wait_for_event(&test_rexux.codex, |event| {
        matches!(event, EventMsg::McpToolCallEnd(_))
    })
    .await;
    let overlay_end = wait_for_event(&test_rexux.codex, |event| {
        matches!(event, EventMsg::McpToolCallEnd(_))
    })
    .await;
    wait_for_event(&test_rexux.codex, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn curated_plugin_skills_follow_auth_switch() -> Result<()> {
    ...
    const FIXTURES: &[Fixture] = &[
        ...
        Fixture {
            ...
            target_auth: TargetAuth::NoRexuxAuth,
            target_model_provider_id: AMAZON_BEDROCK_PROVIDER_ID,
            expected_target_loaded_plugin_skills: &[API_CURATED_PLUGIN_SKILL],
            expected_target_skill_description: "api description before",
        },
    ...
    async fn loaded_plugin_skills_for_config(test_rexux: &TestRexux, config: &Config) -> String {
        let plugins_input = config.plugins_config_input();
        let plugins_manager = test_rexux.thread_manager.plugins_manager();
        ...
        let skills_snapshot = test_rexux
            .thread_manager
            .skills_service()
...
"#,
        ...
        let mut builder = test_rexux()
            ...
            .with_auth(RexuxAuth::create_dummy_chatgpt_auth_for_testing());
        let test_rexux = builder.build_with_auto_env(&server).await?;
        let initial_skills = loaded_plugin_skills_for_config(&test_rexux, &test_rexux.config).await;
        ...
        let expected_auth_mode = match fixture.target_auth {
            ...
            TargetAuth::ApiKey => {
                rexux_login::login_with_api_key(
                    rexux_home.path(),
                    "test-api-key",
                    rexux_login::AuthCredentialsStoreMode::File,
                    rexux_login::AuthKeyringBackendKind::default(),
                )?;
                test_rexux.thread_manager.auth_manager().reload().await;
                Some(AuthMode::ApiKey)
            }
            TargetAuth::BedrockApiKey => {
                rexux_login::login_with_bedrock_api_key(
                    rexux_home.path(),
            ...
            TargetAuth::NoRexuxAuth => {
                test_rexux.thread_manager.auth_manager().logout().await?;
                None
            }
        };
        assert_eq!(
            test_rexux.thread_manager.auth_manager().get_api_auth_mode(),
            expected_auth_mode
        );
        test_rexux.thread_manager.skills_service().clear_cache();
        let mut target_config = test_rexux.config.clone();
        target_config.model_provider_id = fixture.target_model_provider_id.to_string();
        let target_skills = loaded_plugin_skills_for_config(&test_rexux, &target_config).await;
        assert_loaded_plugin_skills(
            fixture.name,
            "target config",
            &target_skills,
...
async fn explicit_plugin_mentions_use_apps_for_chatgpt_dual_surface_plugins(
    ...
    write_plugin_skill_plugin(rexux_home.as_ref());
    write_plugin_mcp_plugin(rexux_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(rexux_home.as_ref());
    ...
    let test_rexux =
        build_apps_enabled_plugin_test_codex(&server, rexux_home, apps_server.chatgpt_base_url)
            .await?;
    let codex = Arc::clone(&test_rexux.codex);
    wait_for_mcp_server(&codex, REXUX_APPS_MCP_SERVER_NAME).await?;

    codex
        .start_or_steer_turn(TurnInputRequest::user_input(vec![
            rexux_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
...
async fn explicit_plugin_mentions_keep_non_conflicting_mcp_for_chatgpt_auth() -> Result<()> {
    ...
    write_plugin_skill_plugin(rexux_home.as_ref());
    write_plugin_mcp_plugin(rexux_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin_with_name(rexux_home.as_ref(), "sample_app");
...
async fn system_marketplace_plugin_honors_layered_activation_and_mcp_policy(
    ...
    write_plugin_mcp_plugin(rexux_home.as_ref(), &stdio_server_bin()?);
    let user_config_path = rexux_home.path().join("config.toml");
    ...
    let system_config_path = rexux_home.path().join("system.toml");
    ...
    let requirements_path = rexux_home.path().join("requirements.toml");
    std::fs::write(
        &requirements_path,
        format!(
    ...
    std::fs::create_dir_all(project.path().join(".rexux"))?;
    std::fs::write(
    ...
    set_project_trust_level(rexux_home.path(), project.path(), trust_level)?;
...
async fn explicitly_requested_mcp_waits_for_startup(request: ExplicitMcpRequest) -> Result<()> {
    ...
    let skill_path = dunce::canonicalize(write_plugin_skill_plugin(rexux_home.as_ref()))?;
    write_plugin_mcp_plugin(rexux_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(rexux_home.as_ref());
    let initialize_barrier = block_plugin_mcp_startup(rexux_home.as_ref(), &rmcp_test_server_bin);

    let mut builder = test_rexux()
        .with_home(rexux_home)
        .with_auth(RexuxAuth::from_api_key("Test API Key"))
        .with_config(move |config| {
            config
                .features
                .enable(Feature::Apps)
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let rexux_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(rexux_home.as_ref());
    let test_rexux = build_analytics_plugin_test_codex(&server, rexux_home).await?;
    let codex = Arc::clone(&test_rexux.codex);
    ...
    let event = wait_for_analytics_event(&server, "rexux_plugin_used").await;
    ...
    assert_eq!(
        ...
        serde_json::json!(rexux_login::default_client::originator().value)
    );
...
async fn explicit_plugin_skill_invocation_tracks_remote_plugin_id() -> Result<()> {
    ...
    let rexux_home = Arc::new(TempDir::new()?);
    let skill_path = dunce::canonicalize(write_remote_plugin_skill_plugin(rexux_home.as_ref()))?;
    persist_sample_remote_plugin_id(rexux_home.as_ref());
    let test_rexux = build_analytics_plugin_test_codex(&server, rexux_home).await?;
    let codex = Arc::clone(&test_rexux.codex);
...
async fn implicit_plugin_skill_invocation_tracks_remote_plugin_id(
    ...
    let server = start_mock_server().await;
    let rexux_home = Arc::new(TempDir::new()?);
    let skill_path = write_remote_plugin_skill_plugin(rexux_home.as_ref());
    persist_sample_remote_plugin_id(rexux_home.as_ref());
