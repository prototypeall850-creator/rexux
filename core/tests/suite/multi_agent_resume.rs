...
fn configure_multi_agent_v2_with_role(
    ...
    let role_path = config.rexux_home.join("durable-worker-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn cold_root_resume_restores_agent_identity_and_role_on_followup() -> Result<()> {
    ...
    let mut initial_builder = test_rexux().with_config(move |config| {
        ...
        if let Some(thread_id) = initial_child_request
            ...
            .find_map(|request| {
                ...
                body["client_metadata"]["thread_id"]
                    .as_str()
                    .and_then(|thread_id| rexux_protocol::ThreadId::from_string(thread_id).ok())
            })
        {
            break thread_id;
        }
    ...
    std::fs::write(
        resumed.config.rexux_home.join("durable-worker-role.toml"),
        format!(
            r#"model = "{ROLE_MODEL}"
model_reasoning_effort = "high"
developer_instructions = "{ROLE_DEVELOPER_INSTRUCTIONS}"
...
"#
    ...
    let body_for = |text: &str, thread: rexux_protocol::ThreadId| {
        requests
            .iter()
            .find_map(|request| {
