...
use chrono::Utc;
use rexux_core::RolloutRecorder;
use rexux_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(rexux_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = rexux_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(rexux_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(rexux_home, "sessions", id)
}
...
async fn upsert_thread_metadata(
    ...
    builder.cwd = rexux_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_rexux_home_directory() {
    let repo = TempDir::new().unwrap();
    let rexux_home = repo.path().join(".rexux");
    std::fs::create_dir_all(&rexux_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".codex/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&rexux_home, id);

    let found =
        find_thread_path_by_id_str(&rexux_home, &id.to_string(), /*state_db_ctx*/ None)
            .await
            .unwrap();
