...
const SNAPSHOT_MARKER_VAR: &str = "REXUX_SNAPSHOT_POLICY_MARKER";
...
async fn wait_for_snapshot(rexux_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = rexux_home.join("shell_snapshots");
...
async fn run_snapshot_command_with_options(
    ...
    let builder = test_rexux().with_config(move |config| {
        config
            .features
    ...
    let rexux_home = test.home.path().to_path_buf();
    ...
    .await;
    let snapshot_path = wait_for_snapshot(&rexux_home).await?;
...
#[cfg(unix)]
fn shell_snapshot_v2_prewarm_builder(profile_home: &Path) -> TestRexuxBuilder {
...
#[cfg(unix)]
async fn run_no_shell_turn(harness: &TestRexuxHarness) -> Result<()> {
...
async fn shell_snapshot_v2_prewarm_preserves_the_sandbox() -> Result<()> {
    ...
    let builder =
        shell_snapshot_v2_prewarm_builder(profile_home.path()).with_config(move |config| {
            ...
            let rules = config.rexux_home.join("rules");
...
async fn shell_snapshot_v2_prewarm_stops_on_shutdown() -> Result<()> {
    ...
    let harness = TestRexuxHarness::with_auto_env_builder(shell_snapshot_v2_prewarm_builder(
        profile_home.path(),
    ))
    .await?;
    run_no_shell_turn(&harness).await?;
...
async fn shell_snapshot_v2_prewarm_skips_ineligible_sessions(
    configure: fn(&mut rexux_core::config::Config),
) -> Result<()> {
    skip_if_remote!(Ok(()), "profile fixture uses a host-local HOME directory");
    let profile_home = tempfile::tempdir()?;
    fs::write(
...
async fn shell_snapshot_v2_guardian_uses_its_resolved_permissions_and_tools(
    ...
    fs::write(
        profile_home.path().join(".bashrc"),
        "printf capture > \"$HOME/$REXUX_THREAD_ID\"\n",
    )
    .await?;
    let builder =
        shell_snapshot_v2_prewarm_builder(profile_home.path()).with_config(move |config| {
...
async fn shell_snapshot_v2_preserves_legacy_snapshots_for_user_shell() -> Result<()> {
    ...
    let builder = test_rexux().with_config(|config| {
        config
            .features
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.rexux_home));
...
async fn unified_exec_snapshot_preserves_shell_environment_policy_set() -> Result<()> {
    ...
    let harness = TestRexuxHarness::with_builder(builder).await?;
    let rexux_home = harness.test().home.path().to_path_buf();
    ...
    assert_eq!(end.exit_code, 0);
    assert!(snapshot_path.starts_with(rexux_home));
...
async fn shell_snapshot_deleted_after_shutdown_with_skills() -> Result<()> {
    ...
    let rexux_home = home.path().to_path_buf();
