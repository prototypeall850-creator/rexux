...
use rexux_core::config::NetworkProxySpec;
use rexux_core::shell::ShellType;
use rexux_core::shell::get_shell;
...
use rexux_protocol::approvals::NetworkApprovalContext;
...
const NETWORK_TEST_HOST: &str = "rexux-network-test.invalid";
const NETWORK_TEST_TARGET: &str = "http://rexux-network-test.invalid:80";
...
#[test_case("GET", "http://rexux-network-test.invalid/"; "plain_http")]
#[test_case("CONNECT", "rexux-network-test.invalid:443"; "connect")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg_attr(
    not(target_os = "linux"),
    ignore = "requires the trusted Linux proxy bridge"
...
async fn user_network_approval_once_session_and_denial_semantics() -> Result<()> {
    ...
    assert!(
        approval
            .call_id
            .starts_with("network#local#http#rexux-network-test.invalid#80#")
    );
    ...
    assert_eq!(
        ...
        Some("rexux-network-test.invalid is not in the allowed_domains")
    );
    test.codex
...
async fn allowing_network_policy_amendment_persists_context_and_bypasses_prompt() -> Result<()> {
    ...
    assert!(policy.contains(
        r#"network_rule(host="rexux-network-test.invalid", protocol="http", decision="allow""#
    ));
    assert!(first_responses.requests().iter().any(|request| {
        request.body_contains_text(
            "Allowed network rule saved in execpolicy (allowlist): rexux-network-test.invalid",
        )
    }));
    mount_exec_network_turn(
        &server,
...
async fn denying_network_policy_amendment_persists_and_blocks_request() -> Result<()> {
    ...
    let policy = fs::read_to_string(test.home.path().join("rules/default.rules"))?;
    assert!(policy.contains(
        r#"network_rule(host="rexux-network-test.invalid", protocol="http", decision="deny""#
    ));
    let output = responses
        .requests()
        .iter()
...
async fn managed_network_unified_exec_test(server: &wiremock::MockServer) -> Result<TestRexux> {
...
async fn wait_for_completion_without_network_prompt(test: &TestRexux) {
    let event = wait_for_event(&test.codex, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_turn_complete(test: &TestRexux) {
    wait_for_event(&test.codex, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
