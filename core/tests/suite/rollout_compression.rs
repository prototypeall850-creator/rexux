...
async fn compressed_shared_fork_resume_preserves_checkpoint_and_frozen_history() -> Result<()> {
    ...
    assert_eq!(
        rexux_rollout::read_session_meta_line(&child_path)
            .await?
            .meta
            .history_base
    ...
    let store =
        rexux_core::thread_store_from_config(&config, rexux_core::init_state_db(&config).await);
    ...
    let resumed = test
        ...
        .resume_thread_with_history(
            ...
            rexux_core::test_support::auth_manager_from_auth(rexux_login::RexuxAuth::from_api_key(
                "dummy",
            )),
            /*parent_trace*/ None,
            ClientMcpExtensions::default(),
