...
//! `just test -p rexux-core --test all --run-ignored only live_cli` provided they set a valid
//! `OPENAI_API_KEY`.
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let rexux_home = home.path().join(".rexux");
    ...
    let mut cmd = Command::new(rexux_utils_cargo_bin::cargo_bin("codex-rs").unwrap());
