...
use rexux_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use rexux_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use rexux_core::RexuxResponsesMetadata;
use rexux_core::ModelClient;
use rexux_core::ModelClientSession;
...
use rexux_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
use rexux_core::test_support::with_parent_turn;
...
use rexux_rollout_trace::ConversationPart;
...
fn turn_metadata(harness: &WebsocketTestHarness, turn_id: Option<&str>) -> RexuxResponsesMetadata {
    responses_metadata(harness, turn_id, TestRexuxResponsesRequestKind::Turn)
}
...
fn prewarm_metadata(
    ...
    responses_metadata(harness, turn_id, TestRexuxResponsesRequestKind::Prewarm)
}

fn websocket_connection_metadata(harness: &WebsocketTestHarness) -> RexuxResponsesMetadata {
    responses_metadata(
        harness,
        /*turn_id*/ None,
...
async fn responses_websocket_preserves_credit_usage_metadata() {
    ...
    let harness = websocket_harness_for_rexux_backend(&server).await;
    ...
    assert_eq!(
        usage_metadata,
        Some(rexux_protocol::ResponseUsageMetadata {
...
async fn responses_websocket_streams_request() {
    ...
    assert_eq!(
        handshake.header(X_REXUX_ROUTING_HINT_HEADER),
    ...
    assert_eq!(
        ...
        Some(rexux_login::default_client::get_rexux_user_agent())
    );
    assert_eq!(
...
async fn responses_websocket_omits_routing_hint_for_provider_with_own_credentials() {
    ...
    assert_eq!(handshake.header(X_REXUX_ROUTING_HINT_HEADER), None);
...
async fn responses_websocket_request_prewarm_traces_logical_request() {
    ...
    writer
        .append(RawTraceEventPayload::RexuxTurnStarted {
...
async fn responses_websocket_preconnect_runs_when_only_v2_feature_enabled() {
    ...
    assert_eq!(
        server
            .single_handshake()
            .header(X_REXUX_ROUTING_HINT_HEADER),
        None
    );
...
async fn websocket_harness_for_rexux_backend(server: &WebSocketTestServer) -> WebsocketTestHarness {
...
async fn websocket_harness_with_provider_options_and_auth(
    ...
    let model_info = rexux_core::test_support::construct_model_info_offline(MODEL, &config);
    let thread_id = ThreadId::new();
    let session_id = SessionId::new();
    let client_auth_manager = auth.map(rexux_core::test_support::auth_manager_from_auth);
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "rexux-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )
