...
use rexux_config::ConfigLayerEntry;
use rexux_config::ConfigLayerSource;
use rexux_config::ConfigLayerStack;
use rexux_config::McpServerCommandMatcher;
use rexux_config::McpServerIdentity;
use rexux_config::McpServerRequirement;
use rexux_config::McpServerValueMatcher;
use rexux_config::ProfileV2Name;
use rexux_config::RequirementSource;
use rexux_config::Sourced;
use rexux_config::config_toml::AgentRoleToml;
use rexux_config::config_toml::AgentsToml;
use rexux_config::config_toml::AutoReviewToml;
use rexux_config::config_toml::ConfigToml;
use rexux_config::config_toml::ExperimentalRequestUserInput;
use rexux_config::config_toml::ProjectConfig;
use rexux_config::config_toml::RealtimeConfig;
use rexux_config::config_toml::RealtimeToml;
use rexux_config::config_toml::RealtimeTransport;
use rexux_config::config_toml::RealtimeWsMode;
use rexux_config::config_toml::RealtimeWsVersion;
use rexux_config::config_toml::ToolsToml;
...
use rexux_config::permissions_toml::NetworkDomainPermissionToml;
use rexux_config::permissions_toml::NetworkDomainPermissionsToml;
use rexux_config::permissions_toml::NetworkMitmActionToml;
use rexux_config::permissions_toml::NetworkMitmHookToml;
use rexux_config::permissions_toml::NetworkMitmToml;
...
use rexux_config::permissions_toml::WorkspaceRootsToml;
use rexux_config::types::AppToolApproval;
use rexux_config::types::ApprovalsReviewer;
use rexux_config::types::BundledSkillsConfig;
use rexux_config::types::FeedbackConfigToml;
use rexux_config::types::HistoryPersistence;
use rexux_config::types::McpServerEnvVar;
use rexux_config::types::McpServerOAuthConfig;
use rexux_config::types::McpServerToolConfig;
use rexux_config::types::McpServerTransportConfig;
use rexux_config::types::MemoriesConfig;
use rexux_config::types::MemoriesToml;
use rexux_config::types::ModelAvailabilityNuxConfig;
use rexux_config::types::Notice;
use rexux_config::types::NotificationCondition;
use rexux_config::types::NotificationMethod;
use rexux_config::types::Notifications;
use rexux_config::types::OtelConfigToml;
use rexux_config::types::OtelExporterKind;
use rexux_config::types::ResumeCwdMode;
use rexux_config::types::SandboxWorkspaceWrite;
use rexux_config::types::SessionPickerViewMode;
use rexux_config::types::SkillsConfig;
use rexux_config::types::ToolSuggestDisabledTool;
use rexux_config::types::ToolSuggestDiscoverableType;
use rexux_config::types::Tui;
use rexux_config::types::TuiKeymap;
use rexux_config::types::TuiNotificationSettings;
use rexux_config::types::TuiPetAnchor;
use rexux_config::types::WindowsSandboxModeToml;
use rexux_config::types::WindowsToml;
use rexux_exec_server::LOCAL_FS;
use rexux_features::Feature;
use rexux_features::FeaturesToml;
use rexux_login::default_client::RESIDENCY_HEADER_NAME;
use rexux_login::test_support::auth_manager_from_optional_auth;
use rexux_model_provider::ProviderCapabilities;
use rexux_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use rexux_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use rexux_model_provider_info::WireApi;
use rexux_models_manager::bundled_models_response;
use rexux_network_proxy::NetworkMode;
use rexux_protocol::config_types::ModelProviderAuthInfo;
use rexux_protocol::config_types::SERVICE_TIER_DEFAULT_REQUEST_VALUE;
use rexux_protocol::config_types::ServiceTier;
use rexux_protocol::models::ActivePermissionProfile;
use rexux_protocol::models::BUILT_IN_PERMISSION_PROFILE_DANGER_FULL_ACCESS;
use rexux_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY;
use rexux_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE;
use rexux_protocol::models::ManagedFileSystemPermissions;
use rexux_protocol::models::PermissionProfile;
use rexux_protocol::models::SandboxEnforcement;
use rexux_protocol::openai_models::MultiAgentRoleMessages;
...
use rexux_protocol::protocol::NetworkAccess;
use rexux_protocol::protocol::RealtimeVoice;
use rexux_protocol::protocol::SandboxPolicy;
use rexux_utils_path_uri::LegacyAppPathString;
...
use rexux_config::test_support::CloudConfigBundleFixture;
...
fn stdio_mcp_with_args(command: &str, args: &[&str]) -> McpServerConfig {
    McpServerConfig {
        ...
        environment_id: rexux_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string(),
        enabled: true,
        required: false,
...
excluded_tool_namespaces = ["mcp__rexux_apps", "multi_agent_v1"]
...
"#,
    ...
    assert_eq!(
        ...
        vec!["mcp__rexux_apps".to_string(), "multi_agent_v1".to_string()]
    );
    assert_eq!(
...
name = "OpenAI"
http_headers = { "X-OpenAI-Internal-Rexux-Residency" = "request-override", "x-provider-header" = "preserved" }
env_http_headers = { "x-openai-internal-codex-residency" = "REXUX_TEST_UNSET_RESIDENCY_HEADER", "x-provider-env-header" = "REXUX_TEST_UNSET_PROVIDER_HEADER" }
"#,
        )
        ...
        assert_eq!(
            static_headers.get("X-OpenAI-Internal-Rexux-Residency"),
        ...
        assert_eq!(
            environment_headers
                ...
                .map(String::as_str),
            Some("REXUX_TEST_UNSET_RESIDENCY_HEADER")
        );
        assert_eq!(
        ...
        assert_eq!(
            environment_headers
                ...
                .map(String::as_str),
            Some("REXUX_TEST_UNSET_PROVIDER_HEADER")
        );
...
async fn tui_auto_recap_defaults_and_cli_overrides() -> anyhow::Result<()> {
    for (toml, override_value, expected) in [
        ...
        std::fs::write(rexux_home.path().join(CONFIG_TOML_FILE), toml)?;
...
async fn network_proxy_cli_overrides_merge_toggle_with_proxy_config() -> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    std::fs::write(
        rexux_home.path().join(CONFIG_TOML_FILE),
        r#"
...
"#,
    ...
    assert_eq!(
        ...
        rexux_http_client::OutboundProxyPolicy::RespectSystemProxy
    );
    assert_eq!(
        config
            .auth_route_config()
    ...
    assert_eq!(
        ...
        rexux_core_plugins::remote::RemotePluginServiceConfig::new(
            config.chatgpt_base_url,
            rexux_http_client::HttpClientFactory::new(
                rexux_http_client::OutboundProxyPolicy::RespectSystemProxy,
            ),
        )
    );
...
"#,
    ...
    assert_eq!(
        resolve_bootstrap_auth_route_config(&configured, Some(&disabled))?
            .http_client_factory()
            .outbound_proxy_policy(),
        rexux_http_client::OutboundProxyPolicy::ReqwestDefault
    );
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        rexux_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
-> std::io::Result<()> {
    ...
    let memories_root = rexux_home.path().join("memories").abs();
    assert!(
        !config
            .permissions
            .file_system_sandbox_policy()
...
-> std::io::Result<()> {
    ...
    let rexux_home = temp_dir.path().join("rexux-home");
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".rexux"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".rexux".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&root.join(".rexux"), cwd.as_path()),
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".rexux"), cwd.path()),
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".rexux"] {
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of Rexux and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of Rexux and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `dev` does not define any recognized filesystem entries for this version of Rexux."
        )),
        "{:?}",
        config.startup_warnings
    );
                        ...
                        file_system_policy
                            ...
                            .contains(&FileSystemSandboxEntry {
                                ...
                                missing_path_behavior: Some(
                                    rexux_protocol::permissions::FileSystemSandboxEntryMissingPathBehavior::Skip,
                                ),
                            }),
...
fn filter_plugin_mcp_servers_without_allowlists_does_not_filter_any_plugin() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            rexux_config::PluginRequirementsToml { mcp_servers: None },
        )]),
        RequirementSource::LegacyManagedConfigTomlFromMdm,
    );
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    let rexux_home = TempDir::new()?;
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, rexux_home.path());
    let project_dot_codex =
        AbsolutePathBuf::resolve_path_against_base("project/.codex", rexux_home.path());
    ...
    let requirements_toml = rexux_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = rexux_config::ConfigRequirements {
        ...
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_rexux_folder: project_dot_codex.clone(),
                },
                toml::toml! {
    ...
    let refreshed_config = Config::load_config_with_layer_stack(
        ...
        refreshed_toml,
        ConfigOverrides {
            cwd: Some(rexux_home.path().to_path_buf()),
            ..Default::default()
        },
        rexux_home.abs(),
    ...
    let thread_layer_stack = ConfigLayerStack::new(
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_rexux_folder: project_dot_codex,
                },
                toml::toml! {
                    [mcp_servers.fresh_project]
...
-> anyhow::Result<()> {
    let rexux_home = TempDir::new()?;
    let plugin_root = rexux_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    std::fs::create_dir_all(plugin_root.join(".rexux-plugin"))?;
    std::fs::write(
        plugin_root.join(".rexux-plugin/plugin.json"),
        r#"{"name":"sample"}"#,
    )?;
    std::fs::write(
...
"#,
    ...
    assert_eq!(
        ...
        Some((
            rexux_mcp::McpServerSource::SelectedPlugin(McpPluginAttribution::new(
                "selected-root".to_string(),
                "Selected Plugin".to_string(),
            )),
            selected,
...
async fn default_zsh_path_sets_runtime_zsh_path() -> std::io::Result<()> {
    let rexux_home = TempDir::new()?;
    let default_zsh_path = rexux_home.path().join("packaged-zsh");
...
#[tokio::test]
async fn sqlite_home_defaults_to_rexux_home_for_workspace_write() -> std::io::Result<()> {
    let rexux_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite.home(), rexux_home.path());
...
-> std::io::Result<()> {
    let rexux_home = TempDir::new()?;
    let memories_root = rexux_home.path().join("memories");
    let writable_root = rexux_home.path().join("writable").abs();
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
async fn config_applies_managed_auth_store_and_chatgpt_base_url() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        requirements,
        rexux_config::ConfigRequirementsToml::default(),
    )?;
...
"#,
    ...
    let project_config_dir = workspace.path().join(".rexux");
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let rexux_home = TempDir::new()?;
    let managed_path = rexux_home.path().join("managed_config.toml");
    let config_path = rexux_home.path().join(CONFIG_TOML_FILE);
    ...
    let config_layer_stack = load_config_layers_state(
        ...
        &Vec::new(),
        overrides,
        &rexux_config::NoopThreadConfigLoader,
    )
    .await?;
    let cfg =
        deserialize_config_toml_with_base(config_layer_stack.effective_config(), rexux_home.path())
            .map_err(|e| {
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(rexux_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    let rexux_home = TempDir::new()?;
    let expected_cwd = LegacyAppPathString::from_path(rexux_home.path());
    ...
    let loaded = load_global_mcp_servers(rexux_home.path()).await?;
...
async fn to_mcp_config_flows_mcp_2026_feature_from_config() -> std::io::Result<()> {
    ...
    let mcp_config = config.to_mcp_config(&plugins_manager).await;
    assert_eq!(mcp_config.protocol_mode, rexux_mcp::McpProtocolMode::Legacy);

    let _ = config.features.enable(Feature::Mcp20260728);
    let mcp_config = config.to_mcp_config(&plugins_manager).await;
    assert_eq!(
        mcp_config.protocol_mode,
        rexux_mcp::McpProtocolMode::V20260728
    );
...
"#,
    ...
    let err = load_global_mcp_servers(rexux_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/rexux-mcp");
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/rexux-mcp""#),
...
async fn replace_mcp_servers_streamable_http_serializes_oauth_resource() -> anyhow::Result<()> {
    ...
    let servers = BTreeMap::from([(
        ...
        McpServerConfig {
            ...
            oauth: Some(McpServerOAuthConfig {
                client_id: Some("eci-prd-pub-rexux-123".to_string()),
                callback_url: None,
                callback_port: None,
            }),
    ...
    assert!(serialized.contains(r#"client_id = "eci-prd-pub-rexux-123""#));
    ...
    assert_eq!(docs.oauth_client_id(), Some("eci-prd-pub-rexux-123"));
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(rexux_home.path())
        .set_model(Some("gpt-5.4"), Some(ReasoningEffort::High))
        .apply()
        .await?;

    let serialized = tokio::fs::read_to_string(rexux_home.path().join(CONFIG_TOML_FILE)).await?;
...
async fn for_config_writes_selected_user_config_file() -> anyhow::Result<()> {
    let rexux_home = TempDir::new()?;
    let base_config = rexux_home.path().join(CONFIG_TOML_FILE);
    let selected_config = rexux_home.path().join("work.config.toml");
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(rexux_home.path(), &profile_name),
        rexux_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
impl PrecedenceTestFixture {
    ...
    fn rexux_home(&self) -> AbsolutePathBuf {
        self.rexux_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let rexux_home = TempDir::new()?;
    let workspace = rexux_home.path().join("workspace");
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, rexux_home.abs()).await?;
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        rexux_config::ConfigRequirementsToml {
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let rexux_home = TempDir::new()?;
    let missing_path = rexux_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let rexux_home = TempDir::new()?;
    let role_config_path = rexux_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config_layer_stack = rexux_config::ConfigLayerStack::new(
        vec![rexux_config::ConfigLayerEntry::new(
            ConfigLayerSource::User {
                file: rexux_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".rexux").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let rexux_home = TempDir::new()?;
    let researcher_path = rexux_home.path().join("agents").join("researcher.toml");
    let reviewer_path = rexux_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = rexux_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let rexux_home = TempDir::new()?;
    let catalog_path = rexux_home.path().join("catalog.json");
...
"#;
    ...
    let rexux_home_temp_dir = TempDir::new().unwrap();

    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        rexux_home: rexux_home_temp_dir,
        cfg,
    })
}
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        ...
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: rexux_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn config_toml_priority_service_tier_uses_priority_request_value() -> std::io::Result<()> {
    ...
    let rexux_home = fixture.rexux_home();
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = rexux_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![rexux_config::WebSearchModeRequirement::Cached]),
    ...
    let requirement_source = rexux_config::RequirementSource::Unknown;
    ...
    let requirements = rexux_config::ConfigRequirements {
        web_search_mode: rexux_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        rexux_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    let temp_dir = TempDir::new()?;
    let rexux_home = temp_dir.path();
    let config_path = rexux_home.join(CONFIG_TOML_FILE);

    // Test setting valid provider on empty config
    set_default_oss_provider(rexux_home, OLLAMA_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    assert!(content.contains("oss_provider = \"ollama\""));

    // Test updating existing config
    std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
    set_default_oss_provider(rexux_home, LMSTUDIO_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    ...
    let result = set_default_oss_provider(rexux_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(rexux_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
...
-> std::io::Result<()> {
    ...
    let instructions = "x".repeat(rexux_utils_string::approx_bytes_for_tokens(
        /*tokens*/ 10_000,
    ));
...
"#,
    ...
    assert_eq!(
        ...
        Some(rexux_config::types::WindowsSandboxModeToml::Elevated)
    );
    assert!(
...
"#,
    ...
    assert_eq!(
        err.to_string(),
        "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; Rexux would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode."
    );
    Ok(())
}
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let rexux_home = TempDir::new()?;
    let cwd = rexux_home.path().join("workspace");
    let outside_root = rexux_home.path().join("outside-write");
...
-> std::io::Result<()> {
    let rexux_home = TempDir::new()?;
    let selected_config = rexux_home.path().join("default.config.toml");
    std::fs::write(
        &selected_config,
        r#"approvals_reviewer = "user"
"#,
...
async fn exact_requirements_apply_to_runtime_config() -> std::io::Result<()> {
    let rexux_home = TempDir::new()?;
    let catalog_path = rexux_home.path().join("required-models.json");
    let mut catalog = bundled_models_response()
        .unwrap_or_else(|err| panic!("bundled models.json should parse: {err}"));
    catalog.models = catalog.models.into_iter().take(1).collect();
    std::fs::write(
...
"#,
    ...
    let required_sqlite_home = rexux_home.path().join("required-state");
    let required_log_dir = rexux_home.path().join("required-logs");
...
"#,
    ...
    let config = load_with_enterprise_requirement(&rexux_home, requirements).await?;
...
async fn absent_allow_login_shell_does_not_report_an_override() -> std::io::Result<()> {
    let rexux_home = TempDir::new()?;
    let config = load_with_enterprise_requirement(&rexux_home, "allow_login_shell = false").await?;
...
fn sqlite_home_env_conflict_reports_an_override() -> std::io::Result<()> {
    let rexux_home = TempDir::new()?;
    let required = AbsolutePathBuf::try_from(rexux_home.path().join("required-state"))?;
    let environment = rexux_home.path().join("environment-state");
    ...
    assert_eq!(
        warnings,
        vec![format!(
            "Environment value for `$REXUX_SQLITE_HOME` is overridden by the required `sqlite_home` value {required:?} from {}.",
            RequirementSource::Unknown
        )]
    );
