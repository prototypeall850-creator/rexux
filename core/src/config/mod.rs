...
use rexux_agent_roles::load_agent_roles;
use rexux_config::CloudConfigBundleLoader;
use rexux_config::ConfigLayerSource;
use rexux_config::ConfigLayerStack;
use rexux_config::ConfigRequirements;
use rexux_config::ConfigRequirementsToml;
use rexux_config::ConstrainedWithSource;
use rexux_config::FeatureRequirementsToml;
use rexux_config::ManagedAuthPolicy;
use rexux_config::McpServerRequirement;
use rexux_config::PluginRequirementsToml;
use rexux_config::ProfileV2Name;
use rexux_config::ResidencyRequirement;
use rexux_config::SandboxModeRequirement;
use rexux_config::Sourced;
use rexux_config::ThreadConfigLoader;
use rexux_config::config_toml::ConfigToml;
use rexux_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
use rexux_config::config_toml::ProjectConfig;
use rexux_config::config_toml::RealtimeAudioConfig;
use rexux_config::config_toml::RealtimeConfig;
use rexux_config::config_toml::ThreadStoreToml;
use rexux_config::config_toml::validate_model_providers;
use rexux_config::loader::load_config_layers_state;
...
use rexux_config::sandbox_mode_requirement_for_permission_profile;
use rexux_config::types::ApprovalsReviewer;
use rexux_config::types::AuthCredentialsStoreMode;
use rexux_config::types::AuthKeyringBackendKind;
use rexux_config::types::History;
use rexux_config::types::McpServerConfig;
use rexux_config::types::McpServerDisabledReason;
use rexux_config::types::MemoriesConfig;
use rexux_config::types::ModelAvailabilityNuxConfig;
use rexux_config::types::Notice;
use rexux_config::types::OAuthCredentialsStoreMode;
use rexux_config::types::ResumeCwdMode;
use rexux_config::types::SessionPickerViewMode;
use rexux_config::types::ToolSuggestConfig;
use rexux_config::types::ToolSuggestDisabledTool;
use rexux_config::types::ToolSuggestDiscoverable;
use rexux_config::types::TuiKeymap;
use rexux_config::types::TuiNotificationSettings;
use rexux_config::types::TuiPetAnchor;
use rexux_config::types::UriBasedFileOpener;
use rexux_config::types::WindowsSandboxModeToml;
use rexux_core_plugins::PluginLoadOutcome;
use rexux_core_plugins::PluginsConfigInput;
use rexux_exec_server::ExecutorFileSystem;
use rexux_exec_server::LOCAL_FS;
use rexux_exec_server::ReadFileOptions;
use rexux_features::CodeModeConfigToml;
use rexux_features::CurrentTimeReminderConfigToml;
use rexux_features::CurrentTimeReminderDeliveryMode;
use rexux_features::CurrentTimeSource;
use rexux_features::Feature;
use rexux_features::FeatureConfigSource;
use rexux_features::FeatureOverrides;
use rexux_features::FeatureToml;
use rexux_features::Features;
use rexux_features::FeaturesToml;
use rexux_features::MultiAgentV2ConfigToml;
use rexux_features::NetworkProxyConfigToml;
use rexux_features::SleepToolMode;
use rexux_features::TokenBudgetConfigToml;
...
use rexux_install_context::InstallContext;
use rexux_login::AuthManagerConfig;
use rexux_login::AuthRouteConfig;
use rexux_mcp::DEFAULT_OPTIONAL_MCP_STARTUP_GRACE;
use rexux_mcp::McpConfig;
use rexux_mcp::McpPluginAttribution;
use rexux_mcp::McpProtocolMode;
use rexux_mcp::McpServerRegistration;
use rexux_mcp::ResolvedMcpCatalog;
use rexux_memories_read::memory_root;
use rexux_model_provider::ProviderCapabilities;
use rexux_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use rexux_model_provider_info::ModelProviderInfo;
use rexux_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use rexux_model_provider_info::built_in_model_providers;
use rexux_model_provider_info::merge_configured_model_providers;
use rexux_models_manager::ModelsManagerConfig;
use rexux_protocol::config_types::AltScreenMode;
use rexux_protocol::config_types::AutoCompactTokenLimitScope;
...
use rexux_protocol::config_types::SandboxMode;
use rexux_protocol::config_types::ServiceTier;
use rexux_protocol::config_types::ShellEnvironmentPolicy;
use rexux_protocol::config_types::TrustLevel;
use rexux_protocol::config_types::Verbosity;
use rexux_protocol::config_types::WebSearchConfig;
use rexux_protocol::config_types::WebSearchMode;
use rexux_protocol::config_types::WindowsSandboxLevel;
use rexux_protocol::models::ActivePermissionProfile;
use rexux_protocol::models::BaseInstructionsProvenance;
...
use rexux_protocol::openai_models::ModelMessages;
...
use rexux_protocol::permissions::ReadDenyMatcher;
...
use rexux_rmcp_client::McpOAuthRefreshMode;
pub use rexux_thread_store::ExtraConfig;
use rexux_utils_absolute_path::AbsolutePathBuf;
use rexux_utils_absolute_path::AbsolutePathBufGuard;
...
use crate::config::permissions::get_readable_roots_required_for_rexux_runtime;
...
pub use rexux_agent_roles::AgentRoleConfig;
pub use rexux_config::ConfigLoadOptions;
pub use rexux_config::Constrained;
pub use rexux_config::ConstraintError;
pub use rexux_config::ConstraintResult;
pub use rexux_config::LoaderOverrides;
pub use rexux_network_proxy::NetworkProxyAuditMetadata;
use rexux_sandboxing::compatibility_sandbox_policy_for_permission_profile;
pub use rexux_sandboxing::system_bwrap_warning;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<AbsolutePathBuf> {
    let raw = std::env::var(rexux_state::SQLITE_HOME_ENV).ok()?;
...
pub(crate) async fn test_config() -> Config {
    ...
    Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(rexux_home.path()).expect("temp dir should resolve"),
    )
    .await
...
impl Permissions {
    ...
    /// selection. Internal Rexux-only writable roots are intentionally excluded.
...
// when Rexux is still responsible for the network policy. `Disabled` means no
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<rexux_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, Rexux will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Rexux
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.rexux/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Rexux"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Rexux '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Rexux home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Rexux can reach out to for tool calls.
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Rexux unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/codex/blob/main/codex-rs/rmcp-client/src/oauth.rs#L2
    /// file: REXUX_HOME/.credentials.json
    ///       This file will be readable to Rexux and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, Rexux will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all Rexux state (defaults to `~/.rexux` but can be
    /// overridden by the `REXUX_HOME` environment variable).
    pub rexux_home: AbsolutePathBuf,

    /// Resolved configuration shared by all Rexux SQLite databases.
    pub sqlite: rexux_state::SqliteConfig,

    /// Directory where Rexux writes log files (defaults to `$REXUX_HOME/log`).
    pub log_dir: PathBuf,

    /// Settings that govern if and what will be written to `~/.rexux/history.jsonl`.
    pub history: History,
    ...
    /// Path to the current Rexux executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub rexux_self_exe: Option<PathBuf>,

    /// Path to the `rexux-linux-sandbox` executable. This must be set if
    /// [`rexux_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `rexux-linux-sandbox`.
    pub rexux_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `rexux-execve-wrapper` executable used for shell
    ...
    /// Whether Rexux-owned clients should respect host system proxy settings.
    ...
    /// When `true`, checks for Rexux updates on startup and surfaces update prompts.
    /// Set to `false` only if your Rexux updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across Rexux product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across Rexux product surfaces.
    /// Defaults to `true`.
    ...
    pub otel: rexux_config::types::OtelConfig,
}
...
impl AuthManagerConfig for Config {
    fn rexux_home(&self) -> PathBuf {
        self.rexux_home.to_path_buf()
    }
...
impl ConfigBuilder {
    pub fn rexux_home(mut self, rexux_home: PathBuf) -> Self {
        self.rexux_home = Some(rexux_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let rexux_home = match rexux_home {
            Some(rexux_home) => AbsolutePathBuf::from_absolute_path(rexux_home)?,
            None => find_rexux_home()?,
        };
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&rexux_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) = rexux_config::first_layer_config_error::<ConfigToml>(
                    &config_layer_stack,
                    rexux_config::CONFIG_TOML_FILE,
                )
                .await
                {
                    return Err(rexux_config::io_error_from_config_error(
...
impl Config {
    pub fn sqlite_config(&self) -> &rexux_state::SqliteConfig {
        &self.sqlite
    }

    /// Whether Guardian may use the unmetered Rexux inference endpoints.
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &rexux_core_plugins::PluginsManager,
    ) -> McpConfig {
    ...
    pub(crate) fn to_mcp_config_with_loaded_plugins(
        ...
        McpConfig {
            ...
            rexux_home: self.rexux_home.to_path_buf(),
            ...
            rexux_linux_sandbox_exe: self.rexux_linux_sandbox_exe.clone(),
            ...
            connector_snapshot:
                rexux_connectors::ConnectorSnapshot::from_plugin_capability_summaries(
    ...
    pub async fn rebuild_preserving_session_layers(
        ...
        Self::load_config_with_layer_stack(
            ...
            refreshed_config.rexux_home.clone(),
            config_layer_stack,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        Self::load_default_with_cli_overrides_for_rexux_home(
            rexux_home.to_path_buf(),
            cli_overrides,
        )
        .await
    }

    /// Load a default configuration for a specific Rexux home without reading
    /// user, project, or system config layers.
    pub async fn load_default_with_cli_overrides_for_rexux_home(
        rexux_home: PathBuf,
        cli_overrides: Vec<(String, TomlValue)>,
    ) -> std::io::Result<Self> {
        ...
        let cli_layer = rexux_config::build_cli_overrides_layer(&cli_overrides);
        rexux_config::merge_toml_values(&mut merged, &cli_layer);
        let rexux_home = AbsolutePathBuf::from_absolute_path_checked(rexux_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &rexux_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
            ConfigOverrides::default(),
    ...
    /// in [ConfigToml], such as `cwd`, `rexux_self_exe`, `rexux_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_and_loader_overrides(
    ...
    load_config_as_toml_with_cli_and_load_options(rexux_home, cwd, cli_overrides, loader_overrides)
        .await
}
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    load_config_toml_with_layer_stack(rexux_home, cwd, cli_overrides, options)
        .await
...
pub async fn load_config_toml_with_layer_stack(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, rexux_home).map_err(|e| {
...
/// Patch `REXUX_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(rexux_home: &Path, provider: &str) -> std::io::Result<()> {
    rexux_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &rexux_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            rexux_protocol::permissions::FileSystemSandboxEntry {
                path: rexux_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: rexux_protocol::permissions::FileSystemAccessMode::Deny,
...
fn resolve_orchestrator_feature_enabled(
    feature: Option<&rexux_config::config_toml::OrchestratorFeatureToml>,
) -> bool {
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let memories_root = memory_root(&rexux_home);
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(rexux_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| rexux_home.join("log").to_path_buf());
        ...
        let sqlite_home = cfg
            ...
            .cloned()
            .or(sqlite_home_env)
            .unwrap_or_else(|| rexux_home.clone());
        ...
        let mut helper_readable_roots = get_readable_roots_required_for_rexux_runtime(
            &rexux_home,
            zsh_path.as_ref(),
        ...
        let config = Self {
            ...
            rexux_home,
            sqlite: rexux_state::SqliteConfig::from_sqlite_home(sqlite_home),
            log_dir,
            config_layer_stack,
            history,
    ...
    pub fn bundled_skills_enabled(&self) -> bool {
        rexux_config::bundled_skills_enabled_from_stack(&self.config_layer_stack)
    }
...
/// Returns the path to the Rexux configuration directory, which can be
/// specified by the `REXUX_HOME` environment variable. If not set, defaults to
/// `~/.rexux`.
///
/// - If `REXUX_HOME` is set, the value must exist and be a directory. The
///   value will be canonicalized and this function will Err otherwise.
/// - If `REXUX_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_rexux_home() -> std::io::Result<AbsolutePathBuf> {
    rexux_utils_home_dir::find_rexux_home()
}

/// Returns the path to the folder where Rexux logs are stored. Does not verify
