...
use rexux_config::ConfigError;
use rexux_config::ConfigLayerEntry;
use rexux_config::ConfigLayerSource;
use rexux_config::ConfigLoadError;
use rexux_config::ConfigLoadOptions;
use rexux_config::ConfigRequirements;
use rexux_config::ConfigRequirementsToml;
use rexux_config::ConfigRequirementsWithSources;
use rexux_config::FilesystemDenyReadPattern;
use rexux_config::LoaderOverrides;
use rexux_config::RequirementSource;
use rexux_config::RequirementsLayerEntry;
use rexux_config::SessionThreadConfig;
use rexux_config::StaticThreadConfigLoader;
use rexux_config::ThreadConfigSource;
use rexux_config::compose_requirements;
use rexux_config::config_error_from_ignored_toml_fields;
use rexux_config::config_error_from_toml;
use rexux_config::config_error_from_typed_toml;
...
use rexux_config::loader::load_requirements_toml;
...
use rexux_protocol::config_types::EnvironmentVariablePattern;
...
async fn write_project_hook_config(
    dot_rexux_folder: &Path,
    foo: Option<&str>,
    command: &str,
) -> std::io::Result<()> {
    tokio::fs::create_dir_all(dot_rexux_folder).await?;
    let foo = foo
        .map(|value| format!("foo = \"{value}\"\n\n"))
        .unwrap_or_default();
    tokio::fs::write(
        dot_rexux_folder.join(CONFIG_TOML_FILE),
        format!(
            r#"{foo}[hooks]
...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when REXUX_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn ignore_project_config_skips_project_discovery() -> std::io::Result<()> {
    let tmp = tempdir().expect("tempdir");
    let rexux_home = tmp.path().join("home");
    let workspace = tmp.path().join("workspace");
    tokio::fs::create_dir_all(rexux_home.as_path()).await?;
    ...
    .await?;
    let project_config_dir = workspace.join(".rexux");
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .rexux_home(tmp.path().to_path_buf())
    ...
    let config_error = config_error_from_io(&err);
    let _guard = rexux_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        rexux_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
collaboration_modes = "true""#;
    ...
    let _guard = rexux_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = rexux_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn returns_packaged_defaults_when_other_layers_are_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when REXUX_HOME/config.toml does not exist");
...
#[cfg(target_os = "macos")]
#[tokio::test]
async fn managed_preferences_requirements_resolve_paths_against_rexux_home() -> anyhow::Result<()> {
    use base64::Engine;

    let tmp = tempdir()?;
    let rexux_home = tmp.path().join("rexux-home");
...
"#
    ...
    .await?;
    let expected_log_dir = AbsolutePathBuf::resolve_path_against_base("~/.rexux/logs", &rexux_home);
    let requirements = layers.requirements_toml();

    assert_eq!(
        requirements.sqlite_home.as_deref(),
        Some(rexux_home.join("state").as_path())
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(rexux_home.join("models.json").as_path())
    );
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![rexux_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(rexux_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(rexux_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
...
async fn system_requirements_control_in_app_updates() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let rexux_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&rexux_home).await?;
...
"#,
    ...
    let mut config = ConfigBuilder::default()
        ...
        .build()
        .await?;
    let config_toml = AbsolutePathBuf::from_absolute_path(rexux_home.join(CONFIG_TOML_FILE))?;
...
"#,
    ...
    assert_eq!(
        ...
        vec![
            ...
            ConfigLayerSource::User {
                file: AbsolutePathBuf::from_absolute_path(rexux_home.join(CONFIG_TOML_FILE))?,
                profile: None,
            },
        ]
    );
...
#[tokio::test]
async fn load_config_layers_resolves_relative_bundle_requirements_paths_against_rexux_home()
-> anyhow::Result<()> {
    let tmp = tempdir()?;
    let rexux_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&rexux_home).await?;
...
"#;
    ...
    assert_eq!(
        ...
        Some(vec![
            FilesystemDenyReadPattern::from_input(&format!("{}/secrets/**", rexux_home.display()))
                .expect("bundle requirements path should resolve against rexux_home")
        ])
    );
...
async fn strict_config_rejects_unknown_cloud_config_key() {
    let tmp = tempdir().expect("tempdir");
    let rexux_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&rexux_home)
        .await
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            rexux_config::SandboxModeRequirement::ReadOnly,
            rexux_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
        ...
        ConfigLoadOptions {
            cloud_config_bundle: CloudConfigBundleLoader::new(async {
                Err(CloudConfigBundleLoadError::new(
                    rexux_config::CloudConfigBundleLoadErrorCode::RequestFailed,
                    /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".rexux")).await?;
    tokio::fs::create_dir_all(project_root.join(".rexux")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".rexux").join(CONFIG_TOML_FILE),
        r#"foo = "root"
"#,
    )
    .await?;
    tokio::fs::write(
        nested.join(".rexux").join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            ConfigLayerSource::Project { dot_rexux_folder } => Some(dot_rexux_folder),
            _ => None,
        })
        .collect();
    assert_eq!(project_layers.len(), 2);
    assert_eq!(project_layers[0].as_path(), nested.join(".rexux").as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".rexux").as_path()
    );
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".rexux")).await?;
    tokio::fs::create_dir_all(worktree_child.join(".rexux")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    write_project_hook_config(
        &repo_root.join(".rexux"),
        Some("repo-root"),
        "echo repo root hook",
    )
    .await?;
    write_project_hook_config(
        &repo_child.join(".rexux"),
        Some("repo-child"),
        "echo repo child hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_root.join(".rexux"),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(".rexux"),
        Some("worktree-child"),
...
async fn forged_linked_worktree_does_not_inherit_repo_trust() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(attacker_root.join(".rexux")).await?;
    tokio::fs::write(
    ...
    .await?;
    tokio::fs::write(
        attacker_root.join(".rexux").join(CONFIG_TOML_FILE),
        r#"foo = "attacker"
"#,
    )
    .await?;
...
async fn malformed_untrusted_linked_worktree_does_not_read_root_hooks() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".rexux")).await?;
    tokio::fs::create_dir_all(repo_root.join(".rexux")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    tokio::fs::write(worktree_root.join(".rexux").join(CONFIG_TOML_FILE), "foo =").await?;
    tokio::fs::write(repo_root.join(".rexux").join(CONFIG_TOML_FILE), [0xff]).await?;
...
async fn nested_project_root_markers_do_not_redirect_regular_repo_hooks() -> std::io::Result<()> {
    ...
    assert_eq!(
        project_layers[0].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(nested.join(".rexux"))?)
    );
    assert_eq!(
        project_layers[1].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_rexux_and_override_in_order() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    tokio::fs::write(project_root.join(".rexux").join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(".rexux").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".rexux").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".rexux").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let rexux_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&rexux_home).await?;
    tokio::fs::write(rexux_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_rexux_exists_without_config_toml() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_rexux_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".rexux"))?,
        },
        TomlValue::Table(toml::map::Map::new()),
    );
...
#[tokio::test]
async fn rexux_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let rexux_home = home_dir.join(".rexux");
    tokio::fs::create_dir_all(&rexux_home).await?;
    tokio::fs::write(
        rexux_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
#[tokio::test]
async fn rexux_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_codex = project_root.join(".rexux");
...
"#,
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_rexux_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_codex)?,
        },
        child_config,
    );
    assert_eq!(vec![&expected_project_layer], project_layers);
...
"#,
    ...
    let rexux_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&rexux_home_untrusted).await?;
    make_config_for_test(
        &rexux_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = rexux_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let rexux_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&rexux_home_unknown).await?;
    tokio::fs::write(
        rexux_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
responses_api_metadata = { rexux_security_surface = "attacker" }
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".rexux")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".rexux").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let rexux_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&rexux_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let rexux_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&rexux_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &rexux_home,
...
mod requirements_exec_policy_tests {
    ...
    use rexux_config::ConfigRequirementsWithSources;
    use rexux_config::RequirementSource;
    use rexux_config::RequirementsExecPolicyDecisionToml;
    use rexux_config::RequirementsExecPolicyParseError;
    use rexux_config::RequirementsExecPolicyPatternTokenToml;
    use rexux_config::RequirementsExecPolicyPrefixRuleToml;
    use rexux_config::RequirementsExecPolicyToml;
    use rexux_execpolicy::Decision;
    use rexux_execpolicy::Evaluation;
    use rexux_execpolicy::RuleMatch;
    use rexux_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
    ...
    fn config_stack_for_dot_rexux_folder_with_requirements(
        dot_rexux_folder: &Path,
        requirements: ConfigRequirements,
    ) -> ConfigLayerStack {
        let dot_rexux_folder = AbsolutePathBuf::from_absolute_path(dot_rexux_folder)
            .expect("absolute dot_rexux_folder");
        let layer = ConfigLayerEntry::new(
            ConfigLayerSource::Project { dot_rexux_folder },
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_rexux_folder_with_requirements(temp_dir.path(), requirements);
