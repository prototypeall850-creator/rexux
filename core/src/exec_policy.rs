...
use rexux_execpolicy::AmendError;
use rexux_execpolicy::Decision;
use rexux_execpolicy::Error as ExecPolicyRuleError;
...
use rexux_execpolicy::blocking_append_allow_prefix_rule;
use rexux_execpolicy::blocking_append_network_rule;
...
use rexux_shell_command::is_dangerous_command::DangerousCommandMatch;
use rexux_shell_command::is_dangerous_command::DangerousCommandPlatform;
use rexux_shell_command::is_dangerous_command::dangerous_command_match_for_platform;
use thiserror::Error;
use tokio::fs;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            .config_layer_stack
            .layers_low_to_high()
            .filter_map(rexux_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(rexux_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &rexux_execpolicy::Error) -> String {
...
fn dangerous_command_match_for_origin(
    ...
    match command_origin {
        ...
        ExecPolicyCommandOrigin::PowerShell => {
            rexux_shell_command::is_dangerous_command::dangerous_powershell_words_match(
                command,
                command_platform,
            )
        }
...
pub(crate) fn default_policy_path(rexux_home: &Path) -> PathBuf {
    rexux_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
fn commands_for_exec_policy_for_platform(
    ...
    if command_platform == DangerousCommandPlatform::Windows
        && let Some(commands) =
            rexux_shell_command::powershell::parse_powershell_command_into_plain_commands(command)
        && !commands.is_empty()
    {
        return ExecPolicyCommands {
            commands,
