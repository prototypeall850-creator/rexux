...
use crate::client::X_REXUX_INSTALLATION_ID_HEADER;
use crate::client::X_REXUX_PARENT_THREAD_ID_HEADER;
use crate::client::X_REXUX_TURN_METADATA_HEADER;
use crate::client::X_REXUX_WINDOW_ID_HEADER;
...
pub(crate) enum RexuxResponsesRequestKind {
    Turn,
    Prewarm,
...
impl RexuxResponsesRequestKind {
    fn metadata(self) -> (&'static str, Option<CompactionTurnMetadata>) {
        match self {
            RexuxResponsesRequestKind::Turn => ("turn", None),
            RexuxResponsesRequestKind::Prewarm => ("prewarm", None),
            RexuxResponsesRequestKind::Compaction(metadata) => ("compaction", Some(metadata)),
            RexuxResponsesRequestKind::Memory => ("memory", None),
        }
    }

    fn has_turn_identity(self) -> bool {
        !matches!(self, RexuxResponsesRequestKind::Memory)
    }
}
...
/// Caller-owned snapshot of Rexux metadata sent to ResponsesAPI.
///
/// The full Rexux turn metadata blob is transported canonically as
...
/// truth.
#[derive(Clone, Debug)]
pub struct RexuxResponsesMetadata {
    ...
    pub(crate) request_kind: Option<RexuxResponsesRequestKind>,
...
impl RexuxResponsesMetadata {
    ...
    pub(crate) fn client_metadata(&self) -> HashMap<String, String> {
        let mut client_metadata = HashMap::from([
            (
                X_REXUX_INSTALLATION_ID_HEADER.to_string(),
                self.installation_id.clone(),
            ),
            (SESSION_ID_KEY.to_string(), self.session_id.clone()),
            (THREAD_ID_KEY.to_string(), self.thread_id.clone()),
            (X_REXUX_WINDOW_ID_HEADER.to_string(), self.window_id.clone()),
        ]);
        ...
        if let Some(parent_thread_id) = self.parent_thread_id {
            client_metadata.insert(
                X_REXUX_PARENT_THREAD_ID_HEADER.to_string(),
        ...
        {
            client_metadata.insert(X_REXUX_TURN_METADATA_HEADER.to_string(), turn_metadata_json);
        }
        client_metadata
    }
    ...
    pub(crate) fn compatibility_headers(&self) -> ApiHeaderMap {
        ...
        insert_header(&mut headers, X_REXUX_WINDOW_ID_HEADER, &self.window_id);
        ...
        if self.has_turn_metadata()
            && let Ok(turn_metadata_json) = to_ascii_json_string(&RexuxTurnMetadataPayload {
                tool_namespaces_info: None,
    ...
    fn turn_metadata_payload(&self) -> RexuxTurnMetadataPayload<'_> {
        ...
        let has_turn_identity =
            request_kind.is_none_or(RexuxResponsesRequestKind::has_turn_identity);
        let has_request_identity =
            request_kind.is_some_and(RexuxResponsesRequestKind::has_turn_identity);
        RexuxTurnMetadataPayload {
            ...
            compaction,
            // Extra metadata enriches the Rexux turn metadata blob, not literal top-level
            // Responses client_metadata. Product metadata is validated while loading config;
            // app-server metadata has reserved Rexux-owned keys filtered when it enters turn state.
            extra: &self.extra,
        }
    }
}
...
#[derive(Serialize)]
struct RexuxTurnMetadataPayload<'a> {
    #[serde(default, skip_serializing_if = "Option::is_none")]
