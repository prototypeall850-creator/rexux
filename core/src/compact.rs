...
use rexux_analytics::RexuxCompactionEvent;
use rexux_analytics::CompactionImplementation;
use rexux_analytics::CompactionPhase;
use rexux_analytics::CompactionReason;
use rexux_analytics::CompactionStatus;
use rexux_analytics::CompactionStrategy;
use rexux_analytics::CompactionTrigger;
use rexux_analytics::now_unix_seconds;
use rexux_context_fragments::AnnotatedContent;
use rexux_context_fragments::set_annotated_content;
...
use rexux_protocol::items::ContextCompactionItem;
...
use rexux_utils_output_truncation::approx_token_count;
use rexux_utils_output_truncation::truncate_text;
use futures::prelude::*;
use tracing::error;

pub use rexux_prompts::SUMMARIZATION_PROMPT;
pub use rexux_prompts::SUMMARY_PREFIX;
...
async fn run_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            let error = RexuxErr::TurnAborted;
            attempt
                .track(
                    sess.as_ref(),
    ...
    let rexux_error = result.as_ref().err();
    if result.is_ok() {
...
async fn run_compact_task_inner_impl(
    ...
    let responses_metadata = sess
        .responses_metadata(
            turn_context.as_ref(),
            RexuxResponsesRequestKind::Compaction(compaction_metadata),
        )
        .await;
    ...
    let compaction_response_id = loop {
        ...
        match attempt_result {
            ...
            Err(err)
                if matches!(
                    err.details(),
                    RexuxErrorDetails::Interrupted | RexuxErrorDetails::TurnAborted
                ) =>
            {
                return Err(err);
            }
            Err(e) if matches!(e.details(), RexuxErrorDetails::SessionBudgetExceeded) => {
                sess.track_turn_rexux_error(turn_context.as_ref(), &e);
                let event = EventMsg::Error(e.to_error_event(/*message_prefix*/ None));
                sess.send_event(&turn_context, event).await;
                return Err(e);
            }
            Err(e) if matches!(e.details(), RexuxErrorDetails::ContextWindowExceeded) => {
                if turn_input_len > 1 {
...
impl CompactionAnalyticsAttempt {
    ...
    pub(crate) async fn track(
        ...
        sess.services
            ...
            .track_compaction(RexuxCompactionEvent {
                ...
                status,
                rexux_error_kind: rexux_error.map(Into::into),
                rexux_error_http_status_code: rexux_error
                    .and_then(RexuxErr::http_status_code_value),
                active_context_tokens_before,
...
pub(crate) fn compaction_status_from_result<T>(result: &RexuxResult<T>) -> CompactionStatus {
    match result {
...
pub(crate) struct CompactedUserMessage {
    ...
    harness_metadata: Option<RexuxHarnessMetadata>,
}
