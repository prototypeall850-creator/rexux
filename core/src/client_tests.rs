...
use super::X_REXUX_INSTALLATION_ID_HEADER;
use super::X_REXUX_PARENT_THREAD_ID_HEADER;
use super::X_REXUX_TURN_METADATA_HEADER;
use super::X_REXUX_WINDOW_ID_HEADER;
use super::X_OPENAI_SUBAGENT_HEADER;
use crate::AttestationContext;
use crate::AttestationProvider;
use crate::GenerateAttestationFuture;
use crate::responses_metadata::RexuxResponsesMetadata;
use crate::test_support::TestRexuxResponsesRequestKind;
use crate::test_support::responses_metadata as test_responses_metadata;
use rexux_api::AgentIdentityTelemetry;
use rexux_api::ApiError;
use rexux_api::ResponseEvent;
...
use rexux_login::AuthCredentialsStoreMode;
...
use rexux_model_provider::BearerAuthProvider;
use rexux_model_provider::ModelProvider;
use rexux_model_provider::ModelProviderFuture;
use rexux_model_provider::ProviderAccountResult;
use rexux_model_provider::ProviderAuthRecoveryMessages;
...
use rexux_model_provider_info::CHATGPT_REXUX_BASE_URL;
use rexux_model_provider_info::ModelProviderInfo;
use rexux_model_provider_info::WireApi;
use rexux_model_provider_info::create_oss_provider_with_base_url;
...
use rexux_rollout_trace::RawTraceEventPayload;
use rexux_rollout_trace::RolloutTrace;
use rexux_rollout_trace::TraceWriter;
...
async fn compact_uses_bearer_after_agent_identity_session_fallback() -> anyhow::Result<()> {
    ...
    let auth_manager = chatgpt_auth_manager(&rexux_home, server.uri()).await;
    ...
    let output = client
        .compact_conversation_history(
            ...
            CompactConversationRequestSettings {
                effort: None,
                summary: rexux_protocol::config_types::ReasoningSummary::None,
                service_tier: None,
            },
...
fn test_responses_metadata_for_client(
    ...
    request_kind: TestRexuxResponsesRequestKind,
) -> RexuxResponsesMetadata {
...
fn responses_lite_prefix_ids_track_thread_and_payload() -> anyhow::Result<()> {
    ...
    let build = |client: &ModelClient, prompt: &Prompt| {
        client.build_responses_request(
            ...
            &model,
            /*effort*/ None,
            rexux_protocol::config_types::ReasoningSummary::None,
            /*service_tier*/ None,
    ...
    prompt.tools = vec![rexux_tools::ToolSpec::Freeform(rexux_tools::FreeformTool {
        name: "exec".to_string(),
        description: "Execute JavaScript.".to_string(),
        defer_loading: None,
        format: rexux_tools::FreeformToolFormat {
...
fn write_chatgpt_auth_json(rexux_home: &std::path::Path) {
    let auth_json = json!({
        "tokens": {
...
async fn chatgpt_auth_manager(
    ...
    write_chatgpt_auth_json(rexux_home.path());
...
fn started_inference_attempt(temp: &TempDir) -> anyhow::Result<InferenceTraceAttempt> {
    ...
    writer.append(RawTraceEventPayload::RexuxTurnStarted {
        rexux_turn_id: "turn-1".to_string(),
        thread_id: "thread-root".to_string(),
    })?;
...
fn output_message(id: &str, text: &str) -> ResponseItem {
    ResponseItem::Message {
        id: Some(rexux_protocol::ResponseItemId::with_suffix("msg", id)),
...
fn build_ws_client_metadata_includes_window_lineage_and_turn_metadata() {
    ...
    for (client_key, metadata_key, expected) in [
        ...
        (
            X_REXUX_WINDOW_ID_HEADER,
            "window_id",
            expected_window_id.as_str(),
        ),
        (
            X_REXUX_PARENT_THREAD_ID_HEADER,
            "parent_thread_id",
...
async fn bedrock_unauthorized_error_uses_provider_mapping() {
    ...
    assert_eq!(
        error.to_string(),
        format!(
            "Amazon Bedrock rejected the request because its AWS signature has expired. Refresh your AWS credentials and retry. If `AWS_BEARER_TOKEN_BEDROCK` is set, update or unset it, then restart Rexux, url: {url}"
        )
    );
}
...
impl ModelProvider for TestRecoveryProvider {
    ...
    fn auth(&self) -> ModelProviderFuture<'_, Option<RexuxAuth>> {
        self.inner.auth()
    }
    ...
    fn recover_from_unauthorized(
        &self,
    ) -> ModelProviderFuture<'_, rexux_protocol::error::Result<ProviderUnauthorizedRecovery>> {
        self.attempts.fetch_add(1, Ordering::Relaxed);
        Box::pin(async move {
            if self.should_fail {
                Err(RexuxErr::Io(std::io::Error::other(
    ...
    fn models_manager(
        ...
        self.inner.models_manager(rexux_home, config_model_catalog)
    }
}
...
async fn provider_owned_auth_recovery_is_bounded_and_preserves_unauthorized_failures() {
    for should_fail in [false, true] {
        ...
        match error.details() {
            RexuxErrorDetails::UnexpectedStatus(response) => {
...
fn model_client_with_counting_attestation(
    ...
    let (auth_manager, provider) = if include_attestation {
        (
            ...
            ModelProviderInfo::create_openai_provider(Some(CHATGPT_REXUX_BASE_URL.to_string())),
        )
    } else {
        (
            None,
...
#[test]
fn guardian_reviewer_uses_dedicated_endpoint_only_with_rexux_backend_auth() {
    ...
    assert_eq!(
        model_client.responses_endpoint(
            Some(&RexuxAuth::from_api_key("test-api-key")),
            "rexux-auto-review",
        ),
        ResponsesEndpoint::Responses
    );
...
#[tokio::test]
async fn websocket_handshake_includes_attestation_for_chatgpt_rexux_responses() {
    ...
    let responses_metadata = test_responses_metadata_for_client(
        ...
        /*parent_thread_id*/ None,
        TestRexuxResponsesRequestKind::WebsocketConnection,
    );
...
#[tokio::test]
async fn non_chatgpt_rexux_endpoints_omit_attestation_generation() {
    let (model_client, attestation_calls) =
