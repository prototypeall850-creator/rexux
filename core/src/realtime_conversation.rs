...
use crate::client::X_REXUX_TURN_METADATA_HEADER;
...
use rexux_api::RealtimeAudioFrame;
use rexux_api::RealtimeContextAppendChannel;
use rexux_api::RealtimeEvent;
use rexux_api::RealtimeEventParser;
use rexux_api::RealtimeSessionConfig;
use rexux_api::RealtimeSessionMode;
use rexux_api::RealtimeWebsocketClient;
use rexux_api::RealtimeWebsocketConnection;
use rexux_api::RealtimeWebsocketEvents;
use rexux_api::RealtimeWebsocketWriter;
use rexux_api::build_session_headers;
use rexux_api::map_api_error;
...
use rexux_login::default_client::default_headers;
...
use rexux_protocol::protocol::RexuxResponseHandoffMode;
...
use rexux_protocol::protocol::RealtimeConversationClosedEvent;
use rexux_protocol::protocol::RealtimeConversationRealtimeEvent;
use rexux_protocol::protocol::RealtimeConversationSdpEvent;
use rexux_protocol::protocol::RealtimeConversationStartedEvent;
use rexux_protocol::protocol::RealtimeHandoffRequested;
use rexux_protocol::protocol::RealtimeOutputModality;
use rexux_protocol::protocol::RealtimeTranscriptEntry;
use rexux_protocol::protocol::RealtimeVoice;
use rexux_protocol::protocol::RealtimeVoicesList;
use rexux_utils_output_truncation::approx_bytes_for_tokens;
use rexux_utils_string::approx_token_count;
use rexux_utils_string::take_bytes_at_char_boundary;
use rexux_utils_string::to_ascii_json_string;
use http::HeaderMap;
...
struct RealtimeHandoffState {
    ...
    rexux_responses_as_items: bool,
    rexux_response_item_prefix: Option<String>,
    rexux_response_handoff_mode: RexuxResponseHandoffMode,
    rexux_response_handoff_channel_prefixes: Arc<BTreeMap<String, Vec<String>>>,
...
impl RealtimeHandoffState {
    fn streams_handoff_append(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            ...
            && !self.rexux_responses_as_items
    }

    fn routes_handoff_by_bem(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            && self.rexux_response_handoff_mode == RexuxResponseHandoffMode::BemTags
    }
}
...
struct RealtimeStart {
    ...
    rexux_response_item_prefix: Option<String>,
    rexux_response_handoff_mode: RexuxResponseHandoffMode,
    rexux_response_handoff_channel_prefixes: Option<BTreeMap<String, Vec<String>>>,
...
impl RealtimeConversationManager {
    ...
    ) -> RexuxResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> RexuxResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
            ...
            rexux_responses_as_items,
            rexux_response_item_prefix,
            rexux_response_handoff_mode,
            rexux_response_handoff_channel_prefixes,
        ...
        let handoff = RealtimeHandoffState {
            ...
            rexux_response_item_prefix,
            rexux_response_handoff_mode,
            rexux_response_handoff_channel_prefixes: Arc::new(
                rexux_response_handoff_channel_prefixes.unwrap_or_default(),
            ),
            session_kind,
            event_parser,
        };
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> RexuxResult<()> {
        let sender = {
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(RexuxErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, mut params: ConversationTextParams) -> RexuxResult<()> {
        let sender = {
            let guard = self.state.lock().await;
            guard
                .as_ref()
        ...
        sender
            .send(params)
            .await
            .map_err(|_| RexuxErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }
    ...
    pub(crate) async fn handoff_out(
        ...
        let phase = if handoff.routes_handoff_by_bem() {
            match bem_message_phase(
                &output_text,
                &handoff.rexux_response_handoff_channel_prefixes,
            ) {
        ...
        let output = match active_handoff {
            Some(handoff_id) => {
                ...
                *handoff.last_output.lock().await = Some(RealtimeHandoffOutput {
                ...
                if handoff.rexux_responses_as_items {
                    RealtimeOutbound::ConversationItem {
                        text: realtime_backend_item(
                            output_text,
                            handoff.rexux_response_item_prefix.as_deref(),
                        ),
                        phase,
                    }
    ...
    pub(crate) async fn append_speech(&self, text: String) -> RexuxResult<()> {
    ...
    pub(crate) async fn handoff_complete(&self) -> RexuxResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
            guard.as_ref().map(|state| state.handoff.clone())
        };
        ...
        let output = if handoff.rexux_responses_as_items {
    ...
    pub(crate) async fn shutdown(&self) -> RexuxResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> RexuxResult<PreparedRealtimeConversationStart> {
    ...
    {
        extra_headers.insert(X_REXUX_TURN_METADATA_HEADER, metadata);
    }
    Ok(PreparedRealtimeConversationStart {
        ...
        rexux_responses_as_items: params.rexux_responses_as_items,
        rexux_response_item_prefix: params.rexux_response_item_prefix,
        rexux_response_handoff_mode: params.rexux_response_handoff_mode,
        rexux_response_handoff_channel_prefixes: params.rexux_response_handoff_channel_prefixes,
...
) -> RexuxResult<RealtimeSessionConfig> {
...
fn validate_realtime_voice(version: RealtimeWsVersion, voice: RealtimeVoice) -> RexuxResult<()> {
    let voices = RealtimeVoicesList::builtin();
...
pub(crate) async fn handle_audio(
    ...
    if let Err(err) = sess.conversation.audio_in(params.frame).await {
        ...
        if sess.conversation.running_state().await.is_some() {
            ...
            send_conversation_error(sess, sub_id, err.to_string(), RexuxErrorInfo::BadRequest)
                .await;
        }
    }
}
...
fn realtime_api_key(auth: Option<&RexuxAuth>, provider: &ModelProviderInfo) -> RexuxResult<String> {
    ...
    if let Some(api_key) = auth.and_then(RexuxAuth::api_key) {
...
fn realtime_request_headers(
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            RexuxErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;
...
fn v3_output_writer(
    writer: &RealtimeWebsocketWriter,
    phase: Option<&MessagePhase>,
    handoff_mode: RexuxResponseHandoffMode,
) -> RealtimeWebsocketWriter {
    let channel = match handoff_mode {
        RexuxResponseHandoffMode::Thinking => None,
        RexuxResponseHandoffMode::Commentary => Some(RealtimeContextAppendChannel::Commentary),
        RexuxResponseHandoffMode::BemTags => match phase {
        ...
        RealtimeEventParser::FramelessBidi => match handoff_output {
            RealtimeOutbound::StandaloneHandoff { text, phase } => {
                v3_output_writer(
                    writer,
                    phase.as_ref(),
                    handoff_state.rexux_response_handoff_mode,
                )
                .send_standalone_handoff(STANDALONE_HANDOFF_ID.to_string(), text)
                .await
            }
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    rexux_error_info: RexuxErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,
