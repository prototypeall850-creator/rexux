...
use rexux_core_plugins::ResolvedPluginMetricsOperation;
use rexux_core_plugins::TrustedPluginRoots;
...
use rexux_protocol::permissions::RawFileSystemSandboxPolicy;
...
use rexux_sandboxing::policy_transforms::effective_permission_profile;
...
pub struct TurnContext {
    ...
    pub(crate) extension_data: Arc<rexux_extension_api::ExtensionData>,
...
impl TurnContext {
    ...
    pub(crate) fn apps_enabled(&self) -> bool {
        let uses_rexux_backend = self
            .auth_manager
            .as_deref()
            .is_some_and(AuthManager::current_auth_uses_rexux_backend);
        self.config
            .features
            .apps_enabled_for_auth(uses_rexux_backend)
...
impl Session {
    ...
    pub(crate) fn make_turn_context(
        ...
        let extension_data = Arc::new(rexux_extension_api::ExtensionData::new(sub_id.clone()));
    ...
    ) -> RexuxResult<(Arc<TurnContext>, ThreadSettingsSnapshot)> {
    ...
    pub(super) async fn new_turn_with_sub_id_if(
    ...
    ) -> RexuxResult<Option<(Arc<TurnContext>, ThreadSettingsSnapshot)>> {
        ...
        let commit = match self.update_settings_if(updates, should_start).await {
            ...
            Err(error) => {
                let message = RexuxErr::InvalidRequest(error.to_string()).to_string();
                ...
                .await;
                return Err(RexuxErr::InvalidRequest(message));
            }
        };
    ...
    async fn new_turn_context_from_configuration(
        ...
        let trusted_plugin_roots = TrustedPluginRoots::from_plugin_load_outcome(
            &plugin_outcome,
            per_turn_config.rexux_home.as_path(),
        );
        ...
        if turn_context.config.model_provider_id == rexux_model_provider_info::OPENAI_PROVIDER_ID {
    ...
    pub(crate) async fn maybe_emit_model_warnings_for_turn(&self, tc: &TurnContext) {
        ...
        if !tc.code_mode_available
            && matches!(
                ...
                rexux_protocol::openai_models::ToolMode::CodeMode
                    | rexux_protocol::openai_models::ToolMode::CodeModeOnly
            )
