...
pub(super) async fn suspend_turn_and_shutdown(
...
) -> RexuxResult<SuspendTurnOutcome> {
    {
    ...
    let live_thread = session
        ...
        .map_err(|error| RexuxErr::Fatal(error.to_string()))?;
    ...
    live_thread.flush().await.map_err(|error| {
        RexuxErr::Fatal(format!("flush before root turn suspension failed: {error}"))
    })?;
    ...
    let mut turn = {
        ...
        active.take().ok_or_else(|| {
            RexuxErr::Fatal("accepted root turn suspension had no running turn".to_string())
        })?
    };

    let task = turn.task.take().ok_or_else(|| {
        RexuxErr::Fatal("accepted root turn suspension had no running task".to_string())
    })?;
    ...
    live_thread.flush().await.map_err(|error| {
        RexuxErr::Fatal(format!("flush after root turn suspension failed: {error}"))
    })?;
    live_thread.shutdown().await.map_err(|error| {
        RexuxErr::Fatal(format!("close suspended root turn writer failed: {error}"))
    })?;
