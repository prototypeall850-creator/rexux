...
use rexux_network_proxy::NetworkDecision;
use rexux_network_proxy::NetworkPolicyRequest;
use rexux_network_proxy::NetworkProtocol;
use rexux_protocol::config_types::ApprovalsReviewer;
use rexux_protocol::error::SandboxErr;
use rexux_protocol::models::AdditionalPermissionProfile as PermissionProfile;
...
use core_test_support::rexux_linux_sandbox_exe_or_skip;
    ...
    let (current_turn, _) = session
        .new_turn_with_sub_id(
            ...
            SessionSettingsUpdate {
                ...
                permission_profile: Some(rexux_protocol::models::PermissionProfile::Disabled),
                ..Default::default()
            },
...
) -> rexux_protocol::protocol::ExecApprovalRequestEvent {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                rexux_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_exec_command_additional_permissions_requests_past_policy_validation() {
    ...
    config
        .permissions
        .set_permission_profile(rexux_protocol::models::PermissionProfile::Disabled)
    ...
    config.rexux_linux_sandbox_exe = rexux_linux_sandbox_exe_or_skip!();
    config
        .features
        .enable(Feature::GuardianApproval)
        .expect("test setup should allow enabling guardian approvals");
...
async fn strict_auto_review_turn_grant_forces_guardian_for_exec_command_policy_skip() {
    ...
    *session.active_turn.lock().await = Some(active_turn);
    ...
    config
        .permissions
        .set_permission_profile(rexux_protocol::models::PermissionProfile::External {
...
async fn sandbox_denied_retry_uses_the_action_policy_and_reviewer() {
    ...
    impl ToolRuntime<TurnEnvironment, String> for DeniedOnceRuntime {
        ...
        async fn run(
            ...
            self.attempts += 1;
            if self.attempts == 1 {
                return Err(ToolError::Rexux(RexuxErr::Sandbox(SandboxErr::Denied {
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    let rexux_home = tempdir().expect("create codex home");
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_rexux_folder: project_dir.path().abs(),
            },
    ...
    let thread_store = Arc::new(rexux_thread_store::LocalThreadStore::new(
        rexux_thread_store::LocalThreadStoreConfig::from_config(&config),
        /*state_db*/ None,
    ));
    ...
    let (session, io) = Session::spawn(SessionSpawnArgs {
        ...
        plugins_manager,
        mcp_manager,
        code_mode_session_provider: Arc::new(rexux_code_mode::DisabledCodeModeSessionProvider),
        extensions: rexux_extension_api::empty_extension_registry(),
        ...
        parent_rollout_thread_trace: rexux_rollout_trace::ThreadTraceContext::disabled(),
        user_shell_override: None,
        parent_trace: None,
        environment_selections: Vec::new(),
        thread_extension_init: rexux_extension_api::ExtensionDataInit::default(),
