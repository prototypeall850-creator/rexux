...
use rexux_analytics::ImagePreparationFact;
use rexux_analytics::ImagePreparationMetadata;
use rexux_analytics::SubAgentThreadStartedInput;
use rexux_analytics::TurnRexuxErrorFact;
use rexux_async_utils::OrCancelExt;
use rexux_connectors::connector_runtime_context_key;
use rexux_context_fragments::RenderedFragment;
use rexux_exec_server::Environment;
use rexux_exec_server::EnvironmentManager;
use rexux_execpolicy::prefix_rule_migration;
use rexux_extension_api::ConversationHistorySnapshot;
use rexux_extension_api::ExtensionDataInit;
use rexux_extension_api::LoadedUserInstructions;
use rexux_extension_api::PromptSlot;
use rexux_extension_api::TurnContextContributionInput;
use rexux_features::FEATURES;
use rexux_features::Feature;
use rexux_features::unstable_features_warning_event;
use rexux_history::RolloutItem;
use rexux_hooks::Hooks;
use rexux_hooks::HooksConfig;
use rexux_login::AuthManager;
use rexux_login::RexuxAuth;
use rexux_login::auth_env_telemetry::collect_auth_env_telemetry;
use rexux_mcp::McpResourceClient;
...
use rexux_models_manager::manager::SharedModelsManager;
use rexux_network_proxy::NetworkProxy;
use rexux_network_proxy::NetworkProxyAuditMetadata;
use rexux_network_proxy::normalize_host;
use rexux_otel::current_span_trace_id;
use rexux_otel::current_span_w3c_trace_context;
use rexux_otel::set_parent_from_w3c_trace_context;
use rexux_protocol::ResponseUsageMetadata;
use rexux_protocol::SessionId;
use rexux_protocol::ThreadId;
use rexux_protocol::approvals::ElicitationRequest;
use rexux_protocol::approvals::ElicitationRequestEvent;
use rexux_protocol::approvals::ExecPolicyAmendment;
use rexux_protocol::approvals::NetworkPolicyAmendment;
use rexux_protocol::approvals::NetworkPolicyRuleAction;
...
use rexux_protocol::dynamic_tools::DynamicToolResponse;
use rexux_protocol::dynamic_tools::DynamicToolSpec;
use rexux_protocol::items::EnteredReviewModeItem;
use rexux_protocol::items::SubAgentActivityItem;
use rexux_protocol::items::TurnItem;
use rexux_protocol::items::UserMessageItem;
use rexux_protocol::models::ActivePermissionProfile;
use rexux_protocol::models::AdditionalPermissionProfile;
...
use rexux_protocol::openai_models::ModelInfo;
use rexux_protocol::openai_models::ModelPreset;
...
use rexux_protocol::protocol::FileChange;
use rexux_protocol::protocol::HasLegacyEvent;
...
use rexux_protocol::protocol::MULTI_AGENT_MODE_OPEN_TAG;
use rexux_protocol::protocol::MultiAgentVersion;
use rexux_protocol::protocol::RawResponseItemEvent;
...
use rexux_protocol::protocol::ThreadHistoryMode;
use rexux_protocol::protocol::ThreadSettingsOverrides;
use rexux_protocol::protocol::ThreadSettingsSnapshot;
use rexux_protocol::protocol::ThreadSource;
use rexux_protocol::protocol::TurnAbortReason;
use rexux_protocol::protocol::TurnContextItem;
use rexux_protocol::protocol::TurnContextNetworkItem;
use rexux_protocol::protocol::TurnEnvironmentSelection;
use rexux_protocol::protocol::W3cTraceContext;
use rexux_protocol::protocol::WorldStateItem;
...
use rexux_protocol::request_permissions::RequestPermissionsArgs;
use rexux_protocol::request_permissions::RequestPermissionsEvent;
use rexux_protocol::request_permissions::RequestPermissionsResponse;
use rexux_protocol::request_user_input::RequestUserInputArgs;
use rexux_protocol::request_user_input::RequestUserInputResponse;
use rexux_rmcp_client::ElicitationResponse;
use rexux_rollout::state_db;
use rexux_rollout_trace::AgentResultTracePayload;
use rexux_rollout_trace::ThreadStartedTraceMetadata;
use rexux_rollout_trace::ThreadTraceContext;
use rexux_sandboxing::policy_transforms::intersect_permission_profiles_with_context;
use rexux_shell_command::parse_command::parse_command;
use rexux_terminal_detection::user_agent;
use rexux_thread_store::CreateThreadParams;
use rexux_thread_store::LiveThread;
use rexux_thread_store::LiveThreadInitGuard;
use rexux_thread_store::LocalThreadStore;
use rexux_thread_store::PersistContext;
use rexux_thread_store::ReadThreadParams;
use rexux_thread_store::ResumeThreadParams;
use rexux_thread_store::ThreadPersistenceMetadata;
use rexux_thread_store::ThreadStore;
use rexux_utils_audio::prepare_response_items as prepare_audio_response_items;
use rexux_utils_git_discovery::GitRootDiscovery;
...
use crate::rexux_thread::RexuxThreadSettingsOverrides;
use crate::rexux_thread::ThreadConfigSnapshot;
#[cfg(test)]
...
#[cfg(test)]
use rexux_protocol::exec_output::StreamOutput;
...
use rexux_core_plugins::PluginCommandAttribution;
use rexux_core_plugins::PluginsManager;
use rexux_core_plugins::RecommendedPluginCandidatesInput;
use rexux_git_utils::get_git_repo_root;
...
use rexux_mcp::effective_mcp_servers;
use rexux_otel::SessionTelemetry;
use rexux_otel::THREAD_STARTED_METRIC;
use rexux_otel::TelemetryAuthMode;
use rexux_protocol::ResponseItemId;
use rexux_protocol::approvals::ExecApprovalKind;
...
use rexux_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
...
use rexux_protocol::models::LocalImagePreparation;
use rexux_protocol::models::ResponseInputItem;
use rexux_protocol::models::ResponseItem;
use rexux_protocol::openai_models::ReasoningEffort as ReasoningEffortConfig;
use rexux_protocol::protocol::ApplyPatchApprovalRequestEvent;
use rexux_protocol::protocol::AskForApproval;
use rexux_protocol::protocol::RexuxErrorInfo;
use rexux_protocol::protocol::DeprecationNoticeEvent;
...
use rexux_protocol::protocol::ModelRerouteEvent;
use rexux_protocol::protocol::ModelRerouteReason;
use rexux_protocol::protocol::ModelVerification;
use rexux_protocol::protocol::ModelVerificationEvent;
use rexux_protocol::protocol::NetworkApprovalContext;
use rexux_protocol::protocol::Op;
use rexux_protocol::protocol::RateLimitSnapshot;
use rexux_protocol::protocol::RawResponseCompletedEvent;
use rexux_protocol::protocol::RequestUserInputEvent;
...
use rexux_protocol::protocol::SessionNetworkProxyRuntime;
use rexux_protocol::protocol::StreamErrorEvent;
use rexux_protocol::protocol::Submission;
use rexux_protocol::protocol::ThreadMemoryMode;
...
use rexux_protocol::protocol::TurnModerationMetadataEvent;
use rexux_protocol::protocol::WarningEvent;
...
use rexux_protocol::turn_input::TurnStartOptions;
use rexux_protocol::user_input::UserInput;
use rexux_skills_extension::HostSkillsService;
use rexux_tools::ToolName;
use rexux_tools::UnifiedExecShellMode;
use rexux_utils_absolute_path::AbsolutePathBuf;
#[cfg(test)]
use rexux_utils_stream_parser::ProposedPlanSegment;
...
pub(crate) struct SessionSpawnArgs {
    ...
    pub(crate) code_mode_session_provider: Arc<dyn rexux_code_mode::CodeModeSessionProvider>,
    pub(crate) extensions: Arc<rexux_extension_api::ExtensionRegistry<crate::config::Config>>,
    ...
    pub(crate) windows_sandbox_proxy_settings_mode:
        rexux_sandboxing::WindowsSandboxProxySettingsMode,
}
...
impl Session {
    ...
    ) -> BoxFuture<'static, RexuxResult<(Arc<Self>, SessionIo)>> {
        Box::pin(async move {
            let parent_trace = match args.parent_trace {
                Some(trace) => {
                    if rexux_otel::context_from_w3c_trace_context(&trace).is_some() {
                        Some(trace)
                    } else {
    ...
    async fn spawn_internal(args: SessionSpawnArgs) -> RexuxResult<(Arc<Self>, SessionIo)> {
        ...
        let exec_policy = if crate::guardian::is_basic_session_source(&session_source) {
            let managed_policy = config
                ...
                .exec_policy
                .as_deref()
                .map_or_else(rexux_execpolicy::Policy::empty, |policy| {
                    policy.as_ref().clone()
                });
            ...
            {
                let rexux_home = config.rexux_home.clone();
                let policy_path = default_policy_path(rexux_home.as_path());
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| RexuxErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };

        let mut config = Arc::new(config);
        let refresh_strategy = if session_source.is_non_root_agent() {
            rexux_models_manager::manager::RefreshStrategy::Offline
        } else {
            rexux_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
            ...
            && !trusted_guardian_reviewer
        {
            ...
            if matches!(
                ...
                config
                    ...
                    .map_err(|err| RexuxErr::InvalidRequest(err.to_string()))?;
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.rexux_home)
        })?;
...
impl SessionIo {
    ...
    pub(crate) async fn submit(&self, op: Op) -> RexuxResult<String> {
    ...
    pub(crate) async fn submit_with_id(&self, mut sub: Submission) -> RexuxResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| RexuxErr::InternalAgentDied)?;
        Ok(())
    }
    ...
    pub(crate) async fn submit_turn_input(
        ...
        mode: TurnInputMode,
    ) -> RexuxResult<TurnInputSubmission> {
        let id = new_submission_id();
        ...
        .await?;
        reply_rx.await.unwrap_or(Err(RexuxErr::InternalAgentDied))
    }
    ...
    pub(crate) async fn shutdown_and_wait(&self) -> RexuxResult<()> {
        let session_loop_termination = self.session_loop_termination.clone();
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(err) if matches!(err.details(), RexuxErrorDetails::InternalAgentDied) => {}
    ...
    pub(crate) async fn next_event(&self) -> RexuxResult<Event> {
        let event = self
            .rx_event
            .recv()
            .await
...
fn session_permission_profile_state_from_config(
    config: &Config,
) -> RexuxResult<PermissionProfileState> {
...
impl Session {
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &rexux_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn rexux_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn rexux_network_proxy::BlockedRequestObserver>>,
    ...
    #[cfg(test)]
    pub(crate) async fn rexux_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.rexux_home().clone()
    }
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect Rexux performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn restorable_thread_settings(&self) -> RexuxThreadSettingsOverrides {
        let state = self.state.lock().await;
        state
            .session_configuration
    ...
    pub(crate) async fn user_instructions(&self) -> Option<rexux_extension_api::Instructions> {
    ...
    /// Record a terminal RexuxErr before the app-server completion notification is reduced.
    pub(crate) fn track_turn_rexux_error(&self, turn_context: &TurnContext, error: &RexuxErr) {
        self.services
            .analytics_events_client
            .track_turn_rexux_error(TurnRexuxErrorFact::from_rexux_err(
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        if let EventMsg::Error(error) = &legacy_source
            && error
                .rexux_error_info
                .as_ref()
                .is_some_and(RexuxErrorInfo::affects_turn_status)
        {
            turn_context
                .terminal_error
                .lock()
        ...
        self.services
            ...
            .record_rexux_turn_event(&turn_context.sub_id, &legacy_source);
        self.services
            .rollout_thread_trace
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &rexux_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| rexux_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn send_event_raw_without_materializing_rollout(&self, event: Event) {
        let persist = match self.current_rollout_path().await {
            Ok(Some(path)) => rexux_rollout::existing_rollout_path(&path).await.is_some(),
            Ok(None) => true,
            Err(err) => {
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&rexux_home, amendment)
            .await?;
    ...
    pub(crate) async fn request_permissions_for_environment(
        ...
        let Ok(native_cwd) = cwd.to_abs_path() else {
            warn!(
                cwd = %cwd,
                "request_permissions interactive approval requires a cwd native to the Rexux host"
            );
            return Some(RequestPermissionsResponse {
                permissions: RequestPermissionProfile::default(),
                scope: PermissionGrantScope::Turn,
    ...
    async fn capture_step_context_inner(
        ...
        let executor_capability_discovery = self
            ...
            .await?;
        let extension_data = rexux_extension_api::ExtensionData::new(turn_context.sub_id.clone());
        ...
        let mut selected_plugins = self
            ...
            .get::<rexux_extension_api::SelectedPluginSnapshot>()
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        {
            let mut state = self.state.lock().await;
            state.queue_pending_session_start_source(rexux_hooks::SessionStartSource::Compact);
        }
    }
            ...
            && turn_context.model_context_window().is_some()
        {
            ...
            separate_developer_sections.push(
                crate::context::TokenBudgetContext::new(
                    session_source
                        .get_agent_path()
                        .unwrap_or_else(rexux_protocol::AgentPath::root),
    ...
    pub(crate) async fn notify_stream_error(
        ...
        rexux_error: RexuxErr,
    ) {
        let additional_details = rexux_error.to_string();
        let rexux_error_info = RexuxErrorInfo::ResponseStreamDisconnected {
            http_status_code: rexux_error.http_status_code_value(),
        };
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<rexux_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
...
async fn build_hooks_config(
    ...
    HooksConfig {
        ...
        feature_enabled: config.features.enabled(Feature::RexuxHooks),
