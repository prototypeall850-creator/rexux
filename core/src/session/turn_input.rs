...
use rexux_protocol::protocol::AdditionalContextEntry;
...
use rexux_protocol::protocol::NonSteerableTurnKind;
use rexux_protocol::protocol::ThreadSettingsOverrides;
use rexux_protocol::turn_input::NotSubmittedReason;
...
impl PreparedTurnInputSettings {
    ...
    async fn prepare(
        ...
        let thread_settings_update = if thread_settings == ThreadSettingsOverrides::default() {
            ...
            session
                ...
                .await
                .map_err(|error| RexuxErr::InvalidRequest(error.to_string()))?;
            Some(updates)
        };
        Ok(Self {
    ...
    async fn apply_started(
        ...
        kind: TurnStartKind,
    ) -> RexuxResult<Option<Arc<TurnContext>>> {
    ...
    async fn apply_steered(self, session: &Session, submission_id: String) -> RexuxResult<()> {
