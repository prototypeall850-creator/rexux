...
use crate::responses_metadata::RexuxResponsesRequestKind;
...
use rexux_analytics::TurnResolvedConfigFact;
...
use rexux_extension_api::TurnInputContext;
use rexux_extension_api::TurnInputEnvironment;
use rexux_features::Feature;
use rexux_file_system::FindUpErrorPolicy;
use rexux_file_system::find_nearest_ancestor_with_markers;
use rexux_login::RexuxAuth;
use rexux_model_provider::RemoteCompactionSupport;
...
use rexux_protocol::items::PlanItem;
...
use rexux_protocol::protocol::AgentMessageContentDeltaEvent;
use rexux_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use rexux_protocol::protocol::PlanDeltaEvent;
use rexux_protocol::protocol::ReasoningContentDeltaEvent;
use rexux_protocol::protocol::ReasoningRawContentDeltaEvent;
use rexux_protocol::protocol::SafetyBufferingEvent;
...
use rexux_skills::ToolMentionKind;
use rexux_skills::app_id_from_path;
use rexux_skills::build_skill_name_counts;
use rexux_skills::collect_explicit_skill_mentions;
use rexux_skills::tool_kind_for_path;
use rexux_skills_extension::HostSkillPrompts;
use rexux_skills_extension::InjectedHostSkillPrompts;
use rexux_thread_store::PersistContext;
use rexux_tools::DiscoverableTool;
use rexux_tools::ToolName;
use rexux_tools::filter_request_plugin_install_discoverable_tools_for_client;
use rexux_utils_path_uri::PathUri;
use rexux_utils_stream_parser::AssistantTextChunk;
use rexux_utils_stream_parser::AssistantTextStreamParser;
use rexux_utils_stream_parser::ProposedPlanSegment;
use rexux_utils_stream_parser::extract_proposed_plan_text;
use rexux_utils_stream_parser::strip_citations;
use futures::prelude::*;
...
const POST_SAMPLING_TOKEN_ESTIMATE_TARGET: &str = "rexux_core::post_sampling_token_estimate";
...
pub(crate) async fn run_turn(
    ...
    .await
    {
        if matches!(err.details(), RexuxErrorDetails::TurnAborted) {
            run_hooks_and_record_inputs(&sess, &turn_context, &input, PersistContext::Standard)
                .await;
            return Err(err);
        }
        if matches!(err.details(), RexuxErrorDetails::ToolCollision(_)) {
            return Err(err);
        }
        let error = err.to_rexux_protocol_error();
        ...
        .await
    {
        ...
        Err(err) if matches!(err.details(), RexuxErrorDetails::TurnAborted) => {
            run_hooks_and_record_inputs(&sess, &turn_context, &input, PersistContext::Standard)
                .await;
            return Err(err);
        }
    ...
    loop {
        ...
        let sampling_request_result: RexuxResult<_> = async {
            ...
            let responses_metadata = sess
                .responses_metadata(turn_context.as_ref(), RexuxResponsesRequestKind::Turn)
                .await;
        ...
        match sampling_request_result {
            ...
            {
                sess.track_turn_rexux_error(turn_context.as_ref(), &rexux_error);
                let error = RexuxErrorInfo::BadRequest;
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
            ...
            Err(e) => {
                info!("Turn error: {e:#}");
                let error = e.to_rexux_protocol_error();
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                sess.track_turn_rexux_error(turn_context.as_ref(), &e);
...
async fn required_mcp_servers_for_input(
    ...
    let connector_slug_counts = if turn_context.apps_enabled() && !mentions.plain_names.is_empty() {
        ...
        build_connector_slug_counts(
            &rexux_connectors::merge::merge_plugin_connectors_with_accessible(
                connector_ids,
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = rexux_connectors::merge::merge_plugin_connectors_with_accessible(
            connector_snapshot
                .connector_ids()
                .iter()
                .map(|connector_id| connector_id.0.clone()),
    ...
    sess.services
        ...
        .track_turn_resolved_config(TurnResolvedConfigFact {
            ...
            guardian_v2_enabled: sess
                ...
                .get::<rexux_extension_api::GuardianV2Enabled>()
                .is_some_and(|state| {
...
///
/// Returns `None` when the active authentication does not use the Rexux backend, the provider is
...
async fn capture_current_model_fallback_step_context(
    ...
    previous_model: &str,
    cancellation_token: &CancellationToken,
) -> RexuxResult<Option<Arc<StepContext>>> {
    let uses_rexux_backend = turn_context
        .auth_manager
        .as_deref()
        .is_some_and(rexux_login::AuthManager::current_auth_uses_rexux_backend);
    if !uses_rexux_backend
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = rexux_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    step_context: Arc<StepContext>,
    turn_store: Arc<rexux_extension_api::ExtensionData>,
    turn_diff_tracker: SharedTurnDiffTracker,
    client_session: &mut ModelClientSession,
    responses_metadata: &RexuxResponsesMetadata,
    input: Vec<ResponseItem>,
    cancellation_token: CancellationToken,
) -> RexuxResult<(SamplingRequestResult, Vec<ResponseItem>)> {
    ...
    loop {
        ...
        {
            rexux_protocol::models::bound_executed_tool_calls_for_prompt(&mut prompt_input);
        }
        ...
        .await
        {
            ...
            Err(err) => match err.details() {
                ...
                RexuxErrorDetails::UsageLimitReached(e) => {
...
pub(crate) async fn built_tools(
    sess: &Session,
    turn_context: &TurnContext,
    model_info: &rexux_protocol::openai_models::ModelInfo,
    model_messages: Option<&ModelMessages>,
    environments: &TurnEnvironmentSnapshot,
    mcp: &Arc<rexux_mcp::McpBinding>,
    step_store: &ExtensionData,
    prepared_recommendations: PreparedToolRecommendations,
) -> RexuxResult<Arc<ToolRouter>> {
...
pub(super) fn agent_message_text(item: &rexux_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            rexux_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: rexux_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(rexux_protocol::items::AgentMessageItem {
...
async fn handle_assistant_item_done_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    turn_store: &rexux_extension_api::ExtensionData,
    item: &ResponseItem,
    state: &mut PlanModeStreamState,
    previously_active_item: Option<&TurnItem>,
...
async fn try_run_sampling_request(
    ...
    prompt: &Prompt,
    cancellation_token: CancellationToken,
) -> RexuxResult<SamplingRequestResult> {
    let turn_context = Arc::clone(&step_context.turn);
    feedback_tags!(
    ...
    let outcome: RexuxResult<SamplingRequestResult> = loop {
        ...
        {
            Ok(event) => event,
            Err(rexux_async_utils::CancelErr::Cancelled) => {
                break Err(RexuxErr::TurnAborted);
            }
        };
                ...
                if analytics_tool_call_ids.len() < MAX_ANALYTICS_TOOL_CALL_IDS_PER_RESPONSE {
                    let call_id = match &item {
                        ...
                        | ResponseItem::ImageGenerationCall { id, .. } => {
                            id.as_ref().map(rexux_protocol::ResponseItemId::as_str)
                        }
                        _ => None,
                    };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![rexux_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
            ...
            ResponseEvent::Completed {
                ...
                sess.services
                    ...
                    .track_code_mode_tool_call(
                        rexux_analytics::CodeModeToolCallFact::SamplingResponseCompleted {
                            thread_id: sess.thread_id.to_string(),
                            turn_id: turn_context.sub_id.clone(),
    ...
    if cancellation_token.is_cancelled() {
        return Err(RexuxErr::TurnAborted);
    }
