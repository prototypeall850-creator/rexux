...
use rexux_analytics::CompactionPhase;
use rexux_analytics::CompactionReason;
use rexux_config::ConfigLayerStack;
use rexux_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID;
use rexux_config::LoaderOverrides;
use rexux_config::NetworkConstraints;
use rexux_config::NetworkDomainPermissionToml;
use rexux_config::NetworkDomainPermissionsToml;
use rexux_config::RequirementSource;
use rexux_config::Sourced;
use rexux_config::loader::project_trust_key;
use rexux_config::types::McpServerConfig;
use rexux_config::types::McpServerTransportConfig;
use rexux_config::types::ToolSuggestDisabledTool;
use rexux_config::types::WindowsSandboxModeToml;
use core_test_support::test_rexux::TurnInputRequest as ExternalTurnInputRequest;

use rexux_features::Feature;
use rexux_file_system::FileSystemSandboxContext;
...
use rexux_login::auth::AgentIdentityAuthPolicy;
use rexux_model_provider::create_model_provider;
use rexux_model_provider_info::ModelProviderInfo;
use rexux_model_provider_info::built_in_model_providers;
use rexux_models_manager::bundled_models_response;
use rexux_models_manager::model_info;
use rexux_models_manager::test_support::construct_model_info_offline_for_tests;
use rexux_models_manager::test_support::get_model_offline_for_tests;
use rexux_protocol::AgentPath;
use rexux_protocol::ResponseItemId;
use rexux_protocol::SessionId;
use rexux_protocol::ThreadId;
use rexux_protocol::config_types::SERVICE_TIER_DEFAULT_REQUEST_VALUE;
use rexux_protocol::config_types::ServiceTier;
use rexux_protocol::config_types::TrustLevel;
use rexux_protocol::exec_output::ExecToolCallOutput;
use rexux_protocol::models::AgentMessageInputContent;
use rexux_protocol::models::FileSystemPermissions;
use rexux_protocol::models::FunctionCallOutputBody;
use rexux_protocol::models::FunctionCallOutputContentItem;
use rexux_protocol::models::FunctionCallOutputPayload;
use rexux_protocol::models::ImageDetail;
use rexux_protocol::models::PermissionProfile;
use rexux_protocol::models::SandboxEnforcement;
use rexux_protocol::openai_models::ModelServiceTier;
use rexux_protocol::openai_models::ToolMode;
...
use rexux_protocol::permissions::FileSystemSandboxPolicyContext;
use rexux_protocol::permissions::FileSystemSpecialPath;
use rexux_protocol::protocol::EnvironmentConfigState;
use rexux_protocol::protocol::ErrorEvent;
use rexux_protocol::protocol::SandboxPolicy;
use rexux_protocol::protocol::TurnEnvironmentSelections;
use rexux_protocol::request_permissions::PermissionGrantScope;
use rexux_protocol::request_permissions::RequestPermissionProfile;
use rexux_protocol::turn_input::TurnInput as SubmittedTurnInput;
use rexux_protocol::turn_input::TurnInputMode;
use rexux_protocol::turn_input::TurnInputRequest;
use rexux_protocol::turn_input::TurnInputSubmission;
use rexux_tools::ToolSpec;
...
use rexux_config::config_toml::ProjectConfig;
use rexux_config::permissions_toml::FilesystemPermissionToml;
use rexux_config::permissions_toml::FilesystemPermissionsToml;
use rexux_config::permissions_toml::NetworkToml;
use rexux_config::permissions_toml::PermissionProfileToml;
use rexux_config::permissions_toml::PermissionsToml;
use rexux_execpolicy::Decision;
use rexux_execpolicy::NetworkRuleProtocol;
use rexux_execpolicy::Policy;
use rexux_history::RexuxHarnessMetadata;
use rexux_history::CompactedItem;
use rexux_history::InitialHistory;
use rexux_history::ResponseItemEnvelope;
use rexux_history::ResumedHistory;
use rexux_history::RolloutItem;
use rexux_network_proxy::NetworkProxyConfig;
use rexux_otel::MetricsClient;
use rexux_otel::MetricsConfig;
use rexux_otel::TelemetryAuthMode;
use rexux_protocol::config_types::CollaborationMode;
use rexux_protocol::config_types::ModeKind;
use rexux_protocol::config_types::Settings;
use rexux_protocol::items::HookPromptFragment;
use rexux_protocol::items::build_hook_prompt_message;
use rexux_protocol::models::BaseInstructions;
use rexux_protocol::models::ContentItem;
use rexux_protocol::models::ContentItemKind;
use rexux_protocol::models::InternalChatMessageMetadataPassthrough;
use rexux_protocol::models::ResponseItem;
use rexux_protocol::protocol::AskForApproval;
use rexux_protocol::protocol::RexuxErrorInfo;
use rexux_protocol::protocol::ConversationAudioParams;
use rexux_protocol::protocol::CreditsSnapshot;
use rexux_protocol::protocol::GranularApprovalConfig;
use rexux_protocol::protocol::HistoryPosition;
use rexux_protocol::protocol::InterAgentCommunication;
use rexux_protocol::protocol::MultiAgentVersion;
use rexux_protocol::protocol::NetworkApprovalProtocol;
use rexux_protocol::protocol::RateLimitSnapshot;
use rexux_protocol::protocol::RateLimitWindow;
use rexux_protocol::protocol::RealtimeAudioFrame;
use rexux_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
use rexux_protocol::protocol::RealtimeVoice;
use rexux_protocol::protocol::RealtimeVoicesList;
use rexux_protocol::protocol::SessionMeta;
use rexux_protocol::protocol::SessionMetaLine;
use rexux_protocol::protocol::Submission;
use rexux_protocol::protocol::ThreadRolledBackEvent;
use rexux_protocol::protocol::ThreadSettingsOverrides;
use rexux_protocol::protocol::TokenCountEvent;
use rexux_protocol::protocol::TokenUsage;
use rexux_protocol::protocol::TokenUsageInfo;
use rexux_protocol::protocol::TokenUsageRecord;
use rexux_protocol::protocol::TurnAbortedEvent;
use rexux_protocol::protocol::TurnCompleteEvent;
use rexux_protocol::protocol::TurnStartedEvent;
use rexux_protocol::protocol::UserMessageEvent;
use rexux_protocol::protocol::W3cTraceContext;
use rexux_rmcp_client::ElicitationAction;
...
use core_test_support::test_rexux::local;
...
use rexux_protocol::mcp::CallToolResult as McpCallToolResult;
...
pub(crate) fn mcp_config_for_test(config: &crate::config::Config) -> Arc<rexux_mcp::McpConfig> {
    Arc::new(config.to_mcp_config_with_loaded_plugins(
        &rexux_core_plugins::PluginLoadOutcome::default(),
        std::iter::empty(),
    ))
}
...
impl StepContext {
    pub(crate) fn for_test(turn: Arc<TurnContext>) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            mcp: Arc::new(rexux_mcp::McpBinding::empty(mcp_config_for_test(
                &turn.config,
            ))),
...
async fn world_state_extension_metrics_follow_turn_model_switch() {
    ...
    impl rexux_extension_api::ContextContributor for WorldStateMetricsRecorder {
        fn contribute_world_state<'a>(
            &'a self,
            input: rexux_extension_api::WorldStateContributionInput<'a>,
        ) -> rexux_extension_api::ExtensionFuture<
            'a,
            Vec<rexux_extension_api::WorldStateSectionContribution>,
        > {
            Box::pin(async move {
                input
                    .extension_metrics
    ...
    let mut builder = rexux_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
...
async fn regular_turn_emits_turn_started_with_trace_id_without_waiting_for_startup_prewarm() {
    let _trace_test_context = install_test_tracing("rexux-core-tests");
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        rexux_protocol::protocol::SessionSource::Exec,
        "test_originator".to_string(),
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        rexux_home.join(rexux_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<rexux_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&rexux_hooks::SessionStartRequest {
            session_id: thread_id,
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            target: rexux_hooks::StartHookTarget::SessionStart {
                source: rexux_hooks::SessionStartSource::Startup,
            },
        }),
    )
}
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn rexux_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { rexux_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Approvable<TurnEnvironment> for ProbeToolRuntime {
        fn approval_action(
            ...
            Ok(crate::tools::sandboxing::ApprovalAction::ExecCommand {
                ...
                environment_id: rexux_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
                command: Vec::new(),
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> rexux_sandboxing::SandboxablePreference {
            rexux_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(rexux_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: rexux_tools::ToolName::plain("probe"),
    };
...
async fn get_base_instructions_no_user_content() {
    let prompt_path = rexux_utils_cargo_bin::find_resource!(
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let rexux_home = session.rexux_home().await;
    std::fs::create_dir_all(&rexux_home).expect("create codex home");
    let config_toml_path = rexux_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = rexux_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let rexux_home = session.rexux_home().await;
    std::fs::create_dir_all(&rexux_home).expect("create codex home");
    let base_config_path = rexux_home.join(CONFIG_TOML_FILE);
    let profile_config_path = rexux_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    assert_eq!(
        config
            ...
            .map(rexux_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    let session = make_session_with_config(|config| {
        config
            ...
            .expect("enable Rexux hooks");
    })
    .await?;
    let rexux_home = session.rexux_home().await;
    std::fs::create_dir_all(&rexux_home)?;
    let config_toml_path = rexux_home.join(CONFIG_TOML_FILE);
    let user_config: rexux_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let request = rexux_hooks::SessionStartRequest {
    ...
    let config = session.get_config().await;
    let hook_list = rexux_hooks::list_hooks(rexux_hooks::HooksConfig {
        feature_enabled: true,
    ...
    assert_eq!(
        ...
        rexux_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: rexux_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: rexux_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: rexux_config::MatcherGroup {
                matcher: None,
                hooks: vec![rexux_config::HookHandlerConfig::Command {
        ...
        let identity = rexux_config::TomlValue::try_from(identity)?;
        rexux_config::version_for_toml(&identity)
    };
...
async fn refresh_runtime_config_updates_runtime_refreshable_fields_and_keeps_session_static_settings()
 {
    ...
    std::fs::create_dir_all(&rexux_home).expect("create codex home");
    std::fs::write(
        rexux_home.join(CONFIG_TOML_FILE),
        r#"[apps.calendar]
enabled = false
destructive_enabled = false
...
async fn refresh_mcp_config_replaces_managed_server_and_plugin_requirements() {
    ...
    next_config.mcp_servers = rexux_config::Constrained::normalized(
...
async fn reconstruct_history_uses_replacement_history_verbatim() {
    ...
    let replacement_history = vec![
        ResponseItemEnvelope {
            ...
            metadata: Some(RexuxHarnessMetadata::default()),
        },
...
async fn subagent_activity_emits_matching_start_and_completion() {
    let (session, turn_context, rx) = make_session_and_context_with_rx().await;
    let item = rexux_protocol::items::SubAgentActivityItem {
        id: "activity-1".to_string(),
        kind: rexux_protocol::protocol::SubAgentActivityKind::Started,
...
async fn record_inter_agent_communication_preserves_item_id_in_rollout_and_resume() {
    let (mut session, turn_context, _rx) = make_session_and_context_with_auth_and_config_and_rx(
        RexuxAuth::from_api_key("Test API Key"),
        Vec::new(),
        |_| {},
    )
    .await;
...
async fn prepares_resumed_history_before_installing_it() {
    ...
    assert_eq!(
        ...
        Some(RexuxHarnessMetadata::default())
    );
}
...
async fn record_initial_history_seeds_token_info_from_rollout() {
    ...
    let info1 = TokenUsageInfo {
        total_token_usage: TokenUsage {
            ...
            total_tokens: 30,
            rexux_rollout_budget_units: None,
        },
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl rexux_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage<'a>(
            &'a self,
            session_store: &'a rexux_extension_api::ExtensionData,
            thread_store: &'a rexux_extension_api::ExtensionData,
            turn_store: &'a rexux_extension_api::ExtensionData,
            token_usage: &'a TokenUsageInfo,
        ) -> rexux_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_start_lifecycle_exposes_turn_metadata_and_token_baseline() {
    ...
    impl rexux_extension_api::TurnLifecycleContributor for TurnStartRecorder {
        fn on_turn_start<'a>(
            &'a self,
            input: rexux_extension_api::TurnStartInput<'a>,
        ) -> rexux_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    impl rexux_extension_api::TurnLifecycleContributor for TurnErrorRecorder {
        fn on_turn_error<'a>(
            &'a self,
            input: rexux_extension_api::TurnErrorInput<'a>,
        ) -> rexux_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
    ...
    let expected = RecordedTurnError {
        ...
        turn_level_id: turn_context.sub_id.clone(),
        turn_id: turn_context.sub_id.clone(),
        error: RexuxErrorInfo::UsageLimitExceeded,
        saw_session_store: true,
        saw_thread_store: true,
    };

    session
        .emit_turn_error_lifecycle(&turn_context, RexuxErrorInfo::UsageLimitExceeded)
        .await;
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl rexux_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
        fn on_config_changed(
            &self,
            session_store: &rexux_extension_api::ExtensionData,
            thread_store: &rexux_extension_api::ExtensionData,
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: rexux_protocol::protocol::NetworkAccess::Restricted,
    };
    let permission_profile = PermissionProfile::External {
        network: NetworkSandboxPolicy::Restricted,
    };
    let expected_permission_profile = permission_profile.clone();
    let mut builder = test_rexux().with_config(move |config| {
        config
            .permissions
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_rexux().with_config(|config| {
        config.update_plan_enabled = true;
        config.permissions.approval_policy =
            rexux_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    fork_config.permissions.approval_policy =
        rexux_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "rexux_core__rexux_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: rexux_protocol::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            rexux_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            rexux_protocol::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            rexux_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_thread_history() {
    ...
    assert_eq!(
        error_event.rexux_error_info,
        Some(RexuxErrorInfo::ThreadRollbackFailed)
    );
    assert_eq!(
...
async fn set_rate_limits_retains_previous_credits() {
    let rexux_home = tempfile::tempdir().expect("create temp dir");
    let config = build_test_config(rexux_home.path()).await;
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(rexux_protocol::account::PlanType::Plus),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(initial.clone());

    let update = RateLimitSnapshot {
        limit_id: Some("rexux_other".to_string()),
        limit_name: Some("rexux_other".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 40.0,
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        individual_limit: None,
        spend_control_reached: None,
        plan_type: Some(rexux_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.rexux_error_info == Some(RexuxErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
            _ => continue,
...
async fn build_test_config(rexux_home: &Path) -> Config {
...
async fn emit_subagent_session_started_includes_fork_lineage_and_originator() {
    ...
    let auth_manager =
        AuthManager::from_auth_for_testing(RexuxAuth::create_dummy_chatgpt_auth_for_testing());
            ...
            if let Some(requests) = server.received_requests().await {
                for request in requests {
                    ...
                    if let Some(event) = payload["events"].as_array().and_then(|events| {
                        events
                            .iter()
                            .find(|event| event["event_type"] == "rexux_thread_initialized")
                    }) {
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = rexux_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        rexux_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn active_profile_update_rebuilds_network_proxy_config() -> std::io::Result<()> {
    let rexux_home = tempfile::tempdir().expect("create codex home");
    let cwd = tempfile::tempdir().expect("create cwd");
    ...
    std::fs::write(
        rexux_home.path().join(rexux_config::CONFIG_TOML_FILE),
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let rexux_home = parent_config.rexux_home.clone();
    let skill_dir = rexux_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&rexux_exec_server::LOCAL_FS));
    ...
    let role_path = rexux_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    let rexux_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(rexux_home.path()).await;
    config
        .features
    ...
    let auth_manager = AuthManager::from_auth_for_testing(RexuxAuth::from_api_key("Test API Key"));
    let models_manager = models_manager_with_provider(
        config.rexux_home.to_path_buf(),
        auth_manager.clone(),
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(rexux_code_mode::DisabledCodeModeSessionProvider),
        Arc::new(rexux_extension_api::ExtensionRegistryBuilder::new().build()),
        rexux_extension_api::ExtensionDataInit::default(),
        ...
        Arc::new(rexux_thread_store::LocalThreadStore::new(
            rexux_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        rexux_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
        /*external_time_provider*/ None,
        Some(config.multi_agent_version_from_features()),
        GitEnrichmentPolicy::Fresh,
        rexux_sandboxing::WindowsSandboxProxySettingsMode::Reconcile,
    )
    .await;
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let mcp_runtime = Arc::new(rexux_mcp::McpRuntime::empty(config.prefix_mcp_tool_names()));
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: rexux_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(rexux_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: rexux_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: rexux_extension_api::ExtensionData::new(thread_id.to_string()),
        selected_capability_roots: Vec::new(),
        mcp_thread_init: rexux_extension_api::ExtensionDataInit::default(),
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(rexux_thread_store::LocalThreadStore::new(
            rexux_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
...
async fn load_latest_config_for_session(session: &Session) -> Config {
    let config = session.get_config().await;
    ConfigBuilder::default()
        .rexux_home(config.rexux_home.to_path_buf())
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            rexux_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(rexux_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
    ...
    assert_eq!(
        session
            .granted_turn_permissions(rexux_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
        None
    );
}
    ...
    *session.active_turn.lock().await = Some(current_active_turn);
    ...
    session
        .record_granted_request_permissions_for_turn(
            ...
            rexux_exec_server::LOCAL_ENVIRONMENT_ID,
    ...
    assert_eq!(
        originating_turn_state
            .lock()
            .await
            .granted_permissions(rexux_exec_server::LOCAL_ENVIRONMENT_ID),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = rexux_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(rexux_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            ...
            session
                .request_permissions_for_environment(
                    ...
                    call_id,
                    rexux_protocol::request_permissions::RequestPermissionsArgs {
                        environment_id: None,
    ...
    assert_eq!(
        ...
        Some(rexux_exec_server::LOCAL_ENVIRONMENT_ID)
    );
    #[allow(deprecated)]
    let turn_cwd = turn_context.cwd.clone();
        ...
        async move {
            handler
                .handle(ToolInvocation {
                    ...
                    tracker,
                    call_id,
                    tool_name: rexux_tools::ToolName::plain("request_permissions"),
...
async fn request_permissions_tool_rejects_invalid_requests(
    ...
    Arc::make_mut(&mut turn_context.config)
        .permissions
        .approval_policy = rexux_config::Constrained::allow_any(AskForApproval::Never);
...
async fn request_permissions_response_materializes_session_cwd_grants_before_recording() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_session_permissions(rexux_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
...
async fn primary_environment_uses_first_turn_environment() {
    ...
    let second_cwd_uri = rexux_utils_path_uri::PathUri::from_abs_path(&second_cwd);
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        rexux_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context = rexux_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(rexux_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn rexux_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    result_sender
        .try_send(
            serde_json::from_value::<rexux_protocol::protocol::HookCompletedEvent>(json!({
                "turn_id": "turn-1",
                "run": {
    ...
    assert_eq!(
        rexux_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_runs_full_thread_teardown() {
    ...
    impl rexux_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadStopRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: rexux_extension_api::ThreadStopInput<'a>,
        ) -> rexux_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl rexux_extension_api::ThreadLifecycleContributor<crate::config::Config> for LifecycleRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: rexux_extension_api::ThreadStopInput<'a>,
        ) -> rexux_extension_api::ExtensionFuture<'a, ()> {
    ...
    impl rexux_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort<'a>(
            &'a self,
            input: rexux_extension_api::TurnAbortInput<'a>,
        ) -> rexux_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
{
    let (tx_event, rx_event) = async_channel::unbounded();
    let mut config = build_test_config(rexux_home).await;
    configure_config(&mut config);
    let state_db = None;
    let config = Arc::new(config);
    let thread_id = ThreadId::default();
    let auth_manager = AuthManager::from_auth_for_testing_with_home(auth, rexux_home.to_path_buf());
    let models_manager = models_manager_with_provider(
        config.rexux_home.to_path_buf(),
        auth_manager.clone(),
        config.model_provider.clone(),
...
async fn refresh_mcp_servers_uses_latest_state_for_existing_turns() {
    ...
    .expect("parse refreshed MCP servers");
    {
        ...
        config.mcp_oauth_credentials_store_mode =
            rexux_config::types::OAuthCredentialsStoreMode::Auto;
        config
            .features
    ...
    let configured_servers = rexux_mcp::configured_mcp_servers(new_step.mcp.config());
    assert_eq!(
        configured_servers.get("refreshed"),
        refreshed_mcp_servers.get("refreshed")
    );
    assert!(
        !rexux_mcp::configured_mcp_servers(old_step.mcp.config()).contains_key("refreshed"),
        "an already-bound step must keep its captured config"
    );
    assert!(
        rexux_mcp::configured_mcp_servers(rematerialized_old.config()).contains_key("refreshed"),
    ...
    assert!(
        rexux_mcp::configured_mcp_servers(current.config()).contains_key("refreshed"),
...
async fn mcp_elicitation_reviewer_uses_active_reviewer_and_latest_runtime_policy(
    ...
    if live_update {
        assert_eq!(
            session
                .apply_turn_settings(
                    &old_turn.sub_id,
                    rexux_protocol::protocol::TurnSettingsUpdate {
                        approvals_reviewer: Some(ApprovalsReviewer::AutoReview),
                        ..Default::default()
                    },
                )
                .await,
            rexux_protocol::protocol::TurnSettingsUpdateOutcome::Applied
        );
    } else {
        session
            .update_settings(SessionSettingsUpdate {
    ...
    let request = rexux_mcp::ElicitationReviewRequest {
        server_name: "browser-use".to_string(),
        request_id: rmcp::model::NumberOrString::Number(7),
        elicitation: rexux_rmcp_client::Elicitation::Mcp(
            rmcp::model::ElicitRequestParams::FormElicitationParams {
                meta: Some(rmcp::model::RequestMetaObject::from(
                    serde_json::Map::from_iter([
                        ("rexux_approval_kind".to_string(), json!("mcp_tool_call")),
                        ("rexux_request_type".to_string(), json!("approval_request")),
...
async fn mcp_refresh_detects_shared_auth_manager_changes() {
    ...
    assert_eq!(
        ...
        Some(rexux_protocol::auth::AuthMode::ApiKey)
    );
    session.refresh_mcp_if_dirty().await;
    assert!(
        session
...
async fn conflicting_ready_environment_root_ids_keep_first_location() {
    let (session, turn_context) = make_session_and_context().await;
    let selected_root =
        |environment_id: &str, path: &str| rexux_protocol::capabilities::SelectedCapabilityRoot {
            id: "shared-root".to_string(),
            location: rexux_protocol::capabilities::CapabilityRootLocation::Environment {
    ...
    for selected_root in &selected_roots {
        let rexux_protocol::capabilities::CapabilityRootLocation::Environment {
            environment_id,
            ..
        ...
        turn_environments.push(TurnEnvironment::new(
            ...
            Arc::new(
                rexux_exec_server::Environment::create_for_tests(/*exec_server_url*/ None)
...
async fn step_context_keeps_its_mcp_runtime_for_tools() -> anyhow::Result<()> {
    ...
    let next_step = session
        ...
        .await
        .expect("a fresh cancellation token cannot be cancelled");
    assert!(rexux_mcp::configured_mcp_servers(next_step.mcp.config()).contains_key("newer"));
    ...
    let current = session
        ...
        .await
        .expect("refreshed runtime should be available");
    assert!(rexux_mcp::configured_mcp_servers(current.config()).contains_key("newer"));
...
async fn build_initial_context_reuses_in_flight_recommendation_prewarm() {
    ...
    let (session, turn_context, _rx_event) = make_session_and_context_with_auth_and_config_and_rx(
        RexuxAuth::create_dummy_chatgpt_auth_for_testing(),
        /*dynamic_tools*/ Vec::new(),
        |config| {
...
impl rexux_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a rexux_extension_api::ExtensionData,
        thread_store: &'a rexux_extension_api::ExtensionData,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<rexux_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
            thread_store
                ...
                .is_some()
                .then(|| {
                    rexux_extension_api::PromptFragment::developer_policy(
                        "prompt extension enabled",
                        rexux_extension_api::ContentItemKind("test.prompt_extension".to_string()),
                    )
                })
                .into_iter()
                .collect()
...
-> Arc<rexux_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = rexux_extension_api::ExtensionRegistryBuilder::new();
    builder.prompt_contributor(Arc::new(PromptExtensionTestContributor));
    Arc::new(builder.build())
}

impl rexux_extension_api::ContextContributor for TurnContextExtensionTestContributor {
    fn contribute_turn_context<'a>(
        &'a self,
        input: rexux_extension_api::TurnContextContributionInput<'a>,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<rexux_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
            ...
            .then(|| {
                rexux_extension_api::PromptFragment::developer_policy(
                    ...
                    rexux_extension_api::ContentItemKind("test.turn_context".to_string()),
                )
            })
            .into_iter()
            .collect()
...
async fn attach_in_memory_thread_store(
    session: &mut Session,
) -> Arc<rexux_thread_store::InMemoryThreadStore> {
    let store = Arc::new(rexux_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn rexux_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
...
async fn wait_for_flush_count(
    store: &rexux_thread_store::InMemoryThreadStore,
...
async fn guardian_auto_review_emits_thread_idle_after_interrupt() {
    ...
    impl rexux_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            _input: rexux_extension_api::ThreadIdleInput<'a>,
        ) -> rexux_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = rexux_protocol::user_input::TextElement::new(
        rexux_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn task_finish_emits_thread_idle_lifecycle_after_active_turn_clears() {
    ...
    impl rexux_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            input: rexux_extension_api::ThreadIdleInput<'a>,
        ) -> rexux_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn thread_idle_lifecycle_waits_for_trigger_turn_mailbox_work() {
    ...
    session
        .emit_thread_idle_lifecycle_if_idle(rexux_extension_api::ThreadIdleCause::Completed)
        .await;
...
async fn active_turn_keeps_first_root_when_mail_coalesces() {
    ...
    for (communication, parent_turn_id, root_turn_id) in [
        ...
        sess.input_queue
            .enqueue_mailbox_communication(
                communication,
                rexux_protocol::turn_input::TurnStartOptions {
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(rexux_extension_api::ExtensionData::new(tc.sub_id.clone())),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: rexux_tools::ToolName::plain("exec_command"),
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let rexux_home = temp.path().join("home");
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let root_dot_codex = project_root.join(".rexux");
    let nested_dot_codex = nested.join(".rexux");
    ...
    write_project_trust_config(&rexux_home, &[(&nested, TrustLevel::Trusted)]).await?;

    let config = ConfigBuilder::default()
        .rexux_home(rexux_home)
    ...
    let expected_source_path = rexux_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_codex = project_root.join(".rexux");
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let rexux_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&rexux_home)?;
        write_project_trust_config(&rexux_home, &trust_entries).await?;
