...
async fn make_config(root: &TempDir, limit: usize, instructions: Option<&str>) -> TestConfig {
    ...
    let user_instructions = instructions.map(|text| Instructions {
        text: text.to_owned(),
        source: config.rexux_home.join(DEFAULT_AGENTS_MD_FILENAME),
    });
    TestConfig {
        config,
        user_instructions,
...
async fn project_root_markers_are_honored_for_agents_discovery() {
    ...
    fs::write(root.path().join(".rexux-root"), "").unwrap();
...
async fn project_layers_do_not_override_project_root_markers() {
    ...
    config.cwd = nested.abs();
    let project_layer = |dot_rexux_folder: AbsolutePathBuf, marker: &str| {
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![
            project_layer(root.path().join(".rexux").abs(), ".ignored-root-marker"),
            project_layer(config.cwd.join(".rexux"), ".ignored-nested-marker"),
        ],
...
async fn instruction_sources_include_global_before_agents_md_docs() {
    ...
    let global_agents = cfg.rexux_home.join(DEFAULT_AGENTS_MD_FILENAME);
    fs::create_dir_all(&cfg.rexux_home).unwrap();
...
fn create_skill(rexux_home: PathBuf, name: &str, description: &str) {
    let skill_dir = rexux_home.join(format!("skills/{name}"));
