...
use rexux_diagnostics::Gauge;
use rexux_diagnostics::GaugeGuard;
use rexux_exec_server::SelectedCapabilityRootsStatus;
...
use rexux_protocol::turn_input::RecoverTurnRequest;
use rexux_protocol::turn_input::StartIfIdleSubmission;
use rexux_protocol::turn_input::SteerSubmission;
use rexux_protocol::turn_input::SuspendTurnOutcome;
...
use rexux_thread_store::ThreadStoreResult;
...
impl ThreadConfigSnapshot {
    ...
    pub fn sandbox_policy(&self) -> SandboxPolicy {
        rexux_sandboxing::compatibility_sandbox_policy_for_permission_profile(
...
pub struct RexuxThreadSettingsOverrides {
...
pub use rexux_guardian_context::GuardianRootMessage;
...
/// (formerly called a conversation) in Rexux.
impl RexuxThread {
    pub(crate) fn new(
        session: Arc<Session>,
        io: SessionIo,
    ...
    pub async fn submit(&self, op: Op) -> RexuxResult<String> {
    ...
    pub fn thread_extension_data(&self) -> &rexux_extension_api::ExtensionData {
        &self.session.services.thread_extension_data
    }

    pub async fn shutdown_and_wait(&self) -> RexuxResult<()> {
    ...
    pub(crate) async fn emit_thread_ready_lifecycle(&self) {
        ...
        {
            contributor
                .on_thread_ready(rexux_extension_api::ThreadReadyInput {
    ...
    pub(crate) async fn emit_thread_resume_lifecycle(&self) {
        ...
        {
            contributor
                .on_thread_resume(rexux_extension_api::ThreadResumeInput {
                    session_store: &self.session.services.session_extension_data,
                    thread_store: &self.session.services.thread_extension_data,
                })
                .await;
    ...
    pub async fn start_turn_if_idle(
        &self,
        request: TurnInputRequest,
    ) -> RexuxResult<StartIfIdleSubmission> {
        match self
    ...
    pub async fn suspend_turn_and_shutdown(&self) -> RexuxResult<SuspendTurnOutcome> {
        if self.session_source.is_non_root_agent() {
            return Err(RexuxErr::UnsupportedOperation(
        ...
        self.io
            ...
            .await
            .map_err(|_| RexuxErr::Fatal("thread session has stopped".to_string()))?;
        let outcome = result
            .await
            .map_err(|_| RexuxErr::Fatal("thread suspension reply was lost".to_string()))??;
    ...
    /// callers that only hold a `RexuxThread`.
    ...
    pub async fn preview_thread_settings_overrides(
        &self,
        overrides: RexuxThreadSettingsOverrides,
    ...
    fn thread_settings_update(overrides: RexuxThreadSettingsOverrides) -> SessionSettingsUpdate {
        let RexuxThreadSettingsOverrides {
            environments,
    ...
    pub async fn next_event(&self) -> RexuxResult<Event> {
    ...
    pub async fn inject_response_items(&self, items: Vec<ResponseItem>) -> RexuxResult<()> {
    ...
    pub async fn restorable_thread_settings(&self) -> RexuxThreadSettingsOverrides {
    ...
    ) -> std::collections::HashMap<String, rexux_protocol::mcp::McpServerConnectionStatus> {
        self.session
            .services
            .mcp_runtime
    ...
    ) -> (rexux_mcp::McpConfig, rexux_mcp::McpRuntimeContext) {
    ...
    pub async fn current_mcp_config_and_runtime_context(
        &self,
    ) -> (Arc<rexux_mcp::McpConfig>, rexux_mcp::McpRuntimeContext) {
    ...
    pub async fn start_mcp_event_stream(
    ...
    ) -> anyhow::Result<rexux_mcp::McpEventStream> {
        ...
        self.session.refresh_mcp_if_dirty().await;
        rexux_mcp::McpResourceClient::new(Arc::clone(&self.session.services.mcp_runtime))
    ...
    pub async fn increment_out_of_band_elicitation_count(&self) -> RexuxResult<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        let incremented = elicitations.count.checked_add(1).ok_or_else(|| {
            RexuxErr::Fatal("out-of-band elicitation count overflowed".to_string())
        })?;
    ...
    pub async fn decrement_out_of_band_elicitation_count(&self) -> RexuxResult<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        if elicitations.count == 0 {
            return Err(RexuxErr::InvalidRequest(
