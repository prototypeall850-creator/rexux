...
use crate::rexux_thread::BackgroundTerminalInfo;
...
use rexux_core_plugins::PluginMetricsSidecar;
use rexux_core_plugins::strip_output_env;
use rexux_network_proxy::NetworkPolicyDecider;
...
use rexux_protocol::protocol::TerminalInteractionEvent;
use rexux_protocol::shell_environment::is_non_inheritable_env_var;
use rexux_sandboxing::SandboxCommand;
use rexux_shell_command::is_dangerous_command::DangerousCommandPlatform;
...
const NETWORK_ACCESS_DENIED_MESSAGE: &str =
    "Network access was denied by the Rexux sandbox network proxy.";
...
pub(super) fn exec_env_policy_from_shell_policy(
...
) -> rexux_exec_server::ExecEnvPolicy {
    ...
    exclude.extend([
        ...
        rexux_apply_patch::REXUX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR.to_string(),
    ...
    r#set.retain(|key, _| {
        ![
            ...
            rexux_apply_patch::REXUX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR,
...
fn exec_server_env_for_request(
    ...
    Option<rexux_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
fn network_approval_error_message(err: ToolError) -> String {
    match err {
        ...
        ToolError::Rexux(err) => err.to_string(),
    }
}
...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        network_proxy_launch: Option<rexux_network_proxy::RemoteNetworkProxyLaunchConfig>,
        environment_id: Option<&str>,
        exec_server_env_config: Option<ExecServerEnvConfig>,
        shell_snapshot: Option<rexux_exec_server::ShellSnapshotRequest>,
        windows_sandbox_proxy_settings_mode: rexux_sandboxing::WindowsSandboxProxySettingsMode,
        tty: bool,
        ...
        .map_err(|err| match err {
            UnifiedExecError::SandboxDenied { output, .. } => {
                ToolError::Rexux(RexuxErr::Sandbox(SandboxErr::Denied {
    ...
    pub(crate) async fn open_session_with_prepared_exec_env(
        ...
        let network_proxy_restricting_sid = {
            ...
            {
                if request.sandbox == rexux_sandboxing::SandboxType::WindowsRestrictedToken {
                    request
                        .network
                        .as_ref()
                        .map(|network| {
        ...
        let windows_sandbox =
            if request.sandbox == rexux_sandboxing::SandboxType::WindowsRestrictedToken {
                Some(rexux_sandboxing::WindowsSandboxSpawnRequest {
        ...
        let spawn_result = rexux_sandboxing::spawn_process(rexux_sandboxing::SpawnRequest {
    ...
    pub(super) async fn open_session_with_sandbox(
        ...
        env.insert(
            REXUX_THREAD_ID_ENV_VAR.to_string(),
        ...
        orchestrator
            ...
            .map_err(|err| match err {
                ToolError::Rexux(err) => match err.details() {
