pub use rexux_apply_patch::REXUX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR;
...
pub use rexux_protocol::shell_environment::REXUX_SESSION_ID_ENV_VAR;
pub use rexux_protocol::shell_environment::REXUX_THREAD_ID_ENV_VAR;
...
pub const REXUX_PERMISSION_PROFILE_ENV_VAR: &str = "REXUX_PERMISSION_PROFILE";
...
///
/// `REXUX_THREAD_ID` is injected when a thread id is provided, even when
/// `include_only` is set.
pub fn create_env(
...
pub(crate) fn inject_session_id_env(env: &mut HashMap<String, String>, session_id: SessionId) {
    env.insert(REXUX_SESSION_ID_ENV_VAR.to_string(), session_id.to_string());
}
...
pub(crate) fn inject_permission_profile_env(
    ...
    if cfg!(windows) {
        env.retain(|key, _| !key.eq_ignore_ascii_case(REXUX_PERMISSION_PROFILE_ENV_VAR));
    } else {
        env.remove(REXUX_PERMISSION_PROFILE_ENV_VAR);
    }
...
pub fn inject_apply_patch_env(env: &mut HashMap<String, String>, features: &Features) {
    env.retain(|key, _| !key.eq_ignore_ascii_case(REXUX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR));
