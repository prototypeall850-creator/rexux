...
use rexux_otel::STARTUP_PREWARM_AGE_AT_FIRST_TURN_METRIC;
use rexux_otel::STARTUP_PREWARM_DURATION_METRIC;
...
pub(crate) struct SessionStartupPrewarmHandle {
    task: AbortOnDropHandle<RexuxResult<ModelClientSession>>,
    started_at: Instant,
    timeout: Duration,
}
...
impl SessionStartupPrewarmHandle {
    pub(crate) fn new(
        task: JoinHandle<RexuxResult<ModelClientSession>>,
        started_at: Instant,
        timeout: Duration,
    ) -> Self {
        Self {
    ...
    fn resolution_from_join_result(
        result: std::result::Result<RexuxResult<ModelClientSession>, tokio::task::JoinError>,
        started_at: Instant,
    ) -> SessionStartupPrewarmResolution {
        match result {
...
async fn schedule_startup_prewarm_inner(
...
) -> RexuxResult<ModelClientSession> {
    ...
    let responses_metadata = session
        .responses_metadata(&startup_turn_context, RexuxResponsesRequestKind::Prewarm)
        .await;
