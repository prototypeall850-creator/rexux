...
use rexux_thread_store::ArchiveThreadParams;
...
impl AgentControlHarness {
    ...
    async fn new_with_config(home: TempDir, config: Config) -> Self {
        ...
        let manager = ThreadManager::with_models_provider_home_and_state_for_tests(
            ...
            std::sync::Arc::new(rexux_exec_server::EnvironmentManager::default_for_tests()),
            state_db.clone(),
        );
    ...
    async fn start_thread(&self) -> (ThreadId, Arc<RexuxThread>) {
        let new_thread = self
            .manager
    ...
    async fn start_paginated_thread(&self) -> (ThreadId, Arc<RexuxThread>) {
        let new_thread = self
            .manager
...
async fn persisted_originator(thread: &RexuxThread) -> String {
...
async fn wait_for_recorded_user_message(thread: &RexuxThread, needle: &str) {
...
async fn wait_for_subagent_notification(parent_thread: &Arc<RexuxThread>) -> bool {
    let wait = async {
        loop {
...
async fn persist_thread_for_tree_resume(thread: &Arc<RexuxThread>, message: &str) {
...
async fn assert_thread_not_loaded(manager: &ThreadManager, thread_id: ThreadId) {
    match manager.get_thread(thread_id).await {
        Err(err) => match err.details() {
            RexuxErrorDetails::ThreadNotFound(id) => assert_eq!(*id, thread_id),
...
async fn send_input_errors_when_thread_missing() {
    ...
    assert_matches!(
        err.details(),
        RexuxErrorDetails::ThreadNotFound(id) if *id == thread_id
    );
}
...
async fn check_v2_agent_reload(route: V2ReloadRoute) {
    ...
    match harness.manager.get_thread(spawned_agent.thread_id).await {
        Err(err) => match err.details() {
            RexuxErrorDetails::ThreadNotFound(id) => assert_eq!(*id, spawned_agent.thread_id),
            _ => panic!("expected ThreadNotFound, got {err:?}"),
        },
...
async fn resume_agent_from_rollout_does_not_reopen_v2_descendants() {
    ...
    let resumed_manager = ThreadManager::with_models_provider_home_and_state_for_tests(
        RexuxAuth::from_api_key("dummy"),
        harness.config.model_provider.clone(),
        harness.config.rexux_home.to_path_buf(),
        std::sync::Arc::new(rexux_exec_server::EnvironmentManager::default_for_tests()),
...
async fn spawn_agent_without_fork_from_paginated_parent_stays_fresh_and_paginated() {
    ...
    child_thread
        ...
        .await
        .expect("child rollout should flush");
    let meta = rexux_rollout::read_session_meta_line(
        &child_thread
            .rollout_path()
            .expect("child rollout should exist"),
    )
    ...
    parent_thread
        ...
        .persist_rollout_items(&[
            RolloutItem::Compacted(CompactedItem {
                ...
                guardian_history: Some(rexux_history::GuardianHistoryCheckpoint(vec![
...
async fn spawn_agent_full_fork_legacy_compaction_rebuilds_child_instructions_once() {
    ...
    for (case, parent_developer_instructions) in [
        ...
        parent_config.config_layer_stack = rexux_config::ConfigLayerStack::new(
            parent_config
                .config_layer_stack
...
async fn spawn_agent_respects_legacy_max_threads_alias() {
    ...
    let RexuxErrorDetails::AgentLimitReached {
...
async fn spawn_agent_limit_shared_across_clones() {
    ...
    let RexuxErrorDetails::AgentLimitReached { max_threads } = err.details() else {
        panic!("expected AgentLimitReached");
    };
...
async fn spawn_thread_subagents_persist_parent_originator_across_new_and_truncated_fork() {
    ...
    assert_eq!(parent_originator, "rexux_work_desktop");
