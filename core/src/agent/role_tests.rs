...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.rexux_linux_sandbox_exe = Some(PathBuf::from("/tmp/rexux-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/rexux-execve-wrapper"));
    let role_path = write_role_config(
        &home,
    ...
    assert_eq!(
        config.rexux_linux_sandbox_exe,
        Some(PathBuf::from("/tmp/rexux-linux-sandbox"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/rexux-execve-wrapper"))
    );
...
"#,
    ...
    let snapshot = skills_service
        .snapshot_for_config(
            &skills_input,
            Some(Arc::clone(&rexux_exec_server::LOCAL_FS)),
        )
        .await;
