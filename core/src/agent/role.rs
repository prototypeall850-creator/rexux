...
use anyhow::anyhow;
use rexux_agent_roles::parse_agent_role_file_contents;
use rexux_config::ConfigLayerEntry;
use rexux_config::ConfigLayerSource;
use rexux_config::ConfigLayerStack;
use rexux_config::SkillsConfig;
use rexux_config::loader::resolve_relative_paths_in_config_toml;
use rexux_exec_server::read_sensitive_file_to_string;
...
async fn apply_role_to_config_inner(
    ...
    let role_config = deserialize_config_toml_with_base(role_layer_toml, &config.rexux_home)?;
...
async fn load_role_layer_toml(
    ...
    let (role_config_toml, role_config_base) = if is_built_in {
        ...
        (role_config_toml, config.rexux_home.as_path())
    } else {
