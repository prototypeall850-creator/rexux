...
use rexux_protocol::capabilities::CapabilityRootLocation;
use rexux_protocol::capabilities::SelectedCapabilityRoot;
use rexux_protocol::config_types::WindowsSandboxLevel;
use rexux_protocol::mcp::ClientMcpExtensions;
use rexux_protocol::mcp::MCP_APP_UI_EXTENSION_ID;
use rexux_protocol::mcp::OPENAI_FORM_EXTENSION_ID;
...
use rexux_protocol::models::ReasoningItemReasoningSummary;
...
async fn reserved_thread_id_is_used_without_changing_normal_id_generation() {
    ...
    config.rexux_home = temp_dir.path().join("rexux-home").abs();
    config.cwd = config.rexux_home.abs();
    std::fs::create_dir_all(&config.rexux_home).expect("create codex home");
    ...
    assert!(matches!(
        ...
        rexux_protocol::error::RexuxErrorDetails::InvalidRequest(message)
...
impl rexux_agent_graph_store::AgentGraphStore for FakeAgentGraphStore {
    fn upsert_thread_spawn_edge(
        &self,
        _parent_thread_id: ThreadId,
        _child_thread_id: ThreadId,
        _status: rexux_agent_graph_store::ThreadSpawnEdgeStatus,
    ) -> rexux_agent_graph_store::AgentGraphStoreFuture<'_, ()> {
    ...
    fn list_thread_spawn_children(
        &self,
        _parent_thread_id: ThreadId,
        _status_filter: Option<rexux_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> rexux_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
    ...
    fn list_thread_spawn_descendants(
        &self,
        root_thread_id: ThreadId,
        status_filter: Option<rexux_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> rexux_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
...
async fn mcp_invalidation_refreshes_threads_that_are_still_starting() {
    ...
    impl rexux_extension_api::ThreadLifecycleContributor<Config> for BlockingThreadStartup {
        fn on_thread_start<'a>(
            &'a self,
            _input: rexux_extension_api::ThreadStartInput<'a, Config>,
    ...
    impl rexux_extension_api::McpServerContributor<Config> for BlockingThreadStartup {
        ...
        fn contribute<'a>(
            &'a self,
            _context: rexux_extension_api::McpServerContributionContext<'a, Config>,
        ) -> rexux_extension_api::ExtensionFuture<'a, Vec<rexux_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
    ...
    let auth_manager = AuthManager::from_auth_for_testing(RexuxAuth::from_api_key("dummy"));
    let manager = Arc::new(ThreadManager::new(
        &config,
        Arc::clone(&auth_manager),
        build_models_manager(&config, auth_manager),
        crate::RexuxAppsToolsCache::default(),
        SessionSource::Exec,
        Arc::new(rexux_exec_server::EnvironmentManager::default_for_tests()),
...
async fn spawn_internal_session_preserves_parent_lineage_without_forking_history() {
    ...
    impl rexux_extension_api::ThreadLifecycleContributor<Config> for ParentLifecycleContributor {}

    impl rexux_extension_api::McpServerContributor<Config> for ParentLifecycleContributor {
        fn id(&self) -> &'static str {
        ...
        fn contribute<'a>(
            &'a self,
            context: rexux_extension_api::McpServerContributionContext<'a, Config>,
        ) -> rexux_extension_api::ExtensionFuture<'a, Vec<rexux_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
    ...
    struct ParentInstructionsProvider(rexux_extension_api::Instructions);

    impl rexux_extension_api::UserInstructionsProvider for ParentInstructionsProvider {
        fn load_user_instructions(&self) -> rexux_extension_api::LoadUserInstructionsFuture<'_> {
            Box::pin(async move {
                rexux_extension_api::LoadedUserInstructions {
    ...
    let mut managed_exec_policy = rexux_execpolicy::Policy::empty();
    managed_exec_policy
        .add_prefix_rule(&["rm".to_string()], rexux_execpolicy::Decision::Forbidden)
        .expect("add managed execution restriction");
    let mut requirements = config.config_layer_stack.requirements().clone();
    requirements.exec_policy = Some(rexux_config::Sourced::new(
        rexux_execpolicy::RequirementsExecPolicy::new(managed_exec_policy),
        rexux_config::RequirementSource::Unknown,
    ));
    requirements.additional_developer_instructions = Some(rexux_config::Sourced::new(
    ...
    config.config_layer_stack = rexux_config::ConfigLayerStack::new(
        config
            .config_layer_stack
    ...
    let parent_instructions = rexux_extension_api::Instructions {
        text: "parent user instructions must not be inherited".to_string(),
        source: config.rexux_home.join("AGENTS.md"),
    };
    ...
    manager_state.mcp_manager = Arc::new(McpManager::new_with_extensions(
        ...
        manager_state.mcp_manager.rexux_apps_tools_cache(),
    ));
    manager_state.user_instructions_provider =
        Arc::new(ParentInstructionsProvider(parent_instructions.clone()));
    let parent = manager
        .start_thread(StartThreadOptions {
            metrics_service_name: Some("rexux_work_desktop".to_string()),
    ...
    reviewer_environment.config =
        EnvironmentConfigState::Ready(rexux_protocol::protocol::EnvironmentConfig {
            ...
            exec_policy: Some(rexux_execpolicy::RequirementsExecPolicy::new(
                rexux_execpolicy::Policy::empty(),
            )),
            mcp_policy: None,
            network_policy: None,
    ...
    assert_eq!(reviewer_config.originator, "rexux_work_desktop");
    assert_eq!(
...
async fn start_thread_seeds_extension_data_for_mcp_and_lifecycle_contributors() {
    ...
    impl rexux_extension_api::ThreadLifecycleContributor<Config> for InitialDataRecorder {
        fn on_thread_start<'a>(
            &'a self,
            input: rexux_extension_api::ThreadStartInput<'a, Config>,
        ) -> rexux_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
    ...
    impl rexux_extension_api::McpServerContributor<Config> for InitialDataRecorder {
        fn id(&self) -> &'static str {
            "selected_root_test"
        }
        ...
        ) -> rexux_extension_api::ExtensionFuture<'a, Vec<rexux_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
                ...
                let mut server = rexux_mcp::rexux_apps_mcp_server_config(
                ...
                vec![rexux_extension_api::McpServerContribution::SelectedPlugin {
    ...
    let selected_root_init = |id: &str, environment_id: &str| {
        let mut init = rexux_extension_api::ExtensionDataInit::new();
    ...
    let selected_servers = |config: &rexux_mcp::McpConfig| {
        rexux_mcp::configured_mcp_servers(config)
            .into_iter()
    ...
    let rexux_apps_server = rexux_mcp::configured_mcp_servers(&first_resolved.config)
        .remove(rexux_mcp::REXUX_APPS_MCP_SERVER_NAME)
        .expect("Rexux Apps server should be configured");
    let rexux_apps_headers = match rexux_apps_server.transport {
        rexux_config::McpServerTransportConfig::StreamableHttp { http_headers, .. } => http_headers,
        rexux_config::McpServerTransportConfig::Stdio { .. } => {
            panic!("Rexux Apps server should use streamable HTTP")
        }
    };
    assert_eq!(
        rexux_apps_headers
            .expect("Rexux Apps headers should be configured")
            .get("originator"),
...
#[tokio::test]
async fn explicit_installation_id_skips_rexux_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.rexux_home = temp_dir.path().join("rexux-home").abs();
    config.cwd = config.rexux_home.abs();
    ...
    assert!(!config.rexux_home.join(INSTALLATION_ID_FILENAME).exists());
...
async fn injected_models_manager_controls_refresh_policy() {
    ...
    assert!(!config.rexux_home.join("models_cache.json").exists());
}
