...
use rexux_network_proxy::BlockedRequest;
...
use rexux_network_proxy::NetworkRequestDisconnect;
...
use rexux_sandboxing::record_network_sandbox_violation;
        ...
        let approval_decision = match session.request_approval(action, approval_context).await {
            ...
            Err(ToolError::Rexux(err)) => {
                let telemetry_decision = if matches!(
                    err.details(),
                    rexux_protocol::error::RexuxErrorDetails::TurnAborted
                ) {
...
pub(crate) async fn begin_network_approval(
    ...
    let execution_proxy = network
        ...
        .map_err(|err| {
            ToolError::Rexux(rexux_protocol::error::RexuxErr::Io(io::Error::other(
