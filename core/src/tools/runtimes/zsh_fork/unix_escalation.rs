...
use rexux_execpolicy::MatchOptions;
...
use rexux_protocol::protocol::NetworkPolicyRuleAction;
...
use rexux_shell_command::bash::parse_shell_lc_plain_commands;
use rexux_shell_escalation::EscalateServer;
use rexux_shell_escalation::EscalationDecision;
use rexux_shell_escalation::EscalationExecution;
use rexux_shell_escalation::EscalationPermissions;
use rexux_shell_escalation::EscalationPolicy;
use rexux_shell_escalation::EscalationPolicyFuture;
use rexux_shell_escalation::EscalationSession;
use rexux_shell_escalation::ExecResult;
use rexux_shell_escalation::PreparedExec;
use rexux_shell_escalation::ResolvedPermissionProfile;
use rexux_shell_escalation::ShellCommandExecutor;
use rexux_shell_escalation::ShellCommandExecutorFuture;
use rexux_shell_escalation::Stopwatch;
...
pub(crate) async fn prepare_unified_exec_zsh_fork(
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        rexux_linux_sandbox_exe: ctx.step_context.turn.config.rexux_linux_sandbox_exe.clone(),
...
impl CoreShellActionProvider {
    ...
    async fn prompt(
            ...
            .await
        {
            ...
            Err(ToolError::Rexux(err)) => Err(err.into()),
        }
    }
...
struct CoreShellCommandExecutor {
    ...
    network: Option<rexux_network_proxy::NetworkProxy>,
...
impl CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["REXUX_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            rexux_linux_sandbox_exe: self.rexux_linux_sandbox_exe.as_deref(),
