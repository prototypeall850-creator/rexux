...
*/
use crate::exec_env::REXUX_PERMISSION_PROFILE_ENV_VAR;
use crate::exec_env::REXUX_SESSION_ID_ENV_VAR;
use crate::exec_env::REXUX_THREAD_ID_ENV_VAR;
use crate::sandboxing::SandboxPermissions;
use crate::shell::Shell;
use crate::shell::ShellType;
use rexux_apply_patch::REXUX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR;
use rexux_core_plugins::PLUGIN_METRICS_OUTPUT_ENV_VAR;
#[cfg(unix)]
...
pub(crate) use rexux_network_proxy::is_managed_proxy_env_var;
pub(crate) use rexux_network_proxy::strip_managed_proxy_env;
use rexux_protocol::config_types::WindowsSandboxLevel;
use rexux_protocol::shell_environment::is_non_inheritable_env_var;
...
pub(crate) fn exec_env_for_sandbox_permissions(
    ...
    // defensive cleanup for a caller that passes an environment already prepared by Rexux.
...
/// PATH entries owned by Rexux runtime setup.
///
...
pub(crate) fn prepare_powershell_command_for_elevated_windows_sandbox(
    ...
    prepare_powershell_command_for_elevated_windows_sandbox_with_fallback(
        ...
        |path| {
            rexux_shell_command::shell_detect::fallback_powershell_shell_for_elevated_windows_sandbox(
                path,
            )
        },
    )
...
fn prepare_powershell_command_for_elevated_windows_sandbox_with_fallback(
    ...
    windows_sandbox_level: WindowsSandboxLevel,
    environment_is_remote: bool,
    find_fallback: impl FnOnce(&Path) -> Option<rexux_shell_command::shell_detect::DetectedShell>,
) -> Vec<String> {
...
/// runtime-only vars like `REXUX_THREAD_ID` without pretending they came from
/// the explicit override policy.
///
/// `runtime_path_prepends` contains Rexux-owned PATH entries already applied to
...
pub(crate) fn maybe_wrap_shell_lc_with_snapshot(
    ...
    for key in [
        REXUX_SESSION_ID_ENV_VAR,
        REXUX_THREAD_ID_ENV_VAR,
        REXUX_PERMISSION_PROFILE_ENV_VAR,
        REXUX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR,
...
fn build_override_exports(
    ...
    build_override_exports_for_keys("__REXUX_SNAPSHOT_OVERRIDE", &keys)
}
...
fn build_proxy_env_exports(env: &HashMap<String, String>) -> (String, String) {
    let mut keys = PROXY_ENV_KEYS
        .iter()
        .copied()
        .chain(rexux_network_proxy::brokered_credential_env_keys(env))
    ...
    let (captures, restores) =
        build_override_exports_for_keys("__REXUX_SNAPSHOT_PROXY_OVERRIDE", &keys);
    let key = PROXY_ACTIVE_ENV_KEY;
    let proxy_blocks = (
        format!("{captures}\n__REXUX_SNAPSHOT_PROXY_ENV_SET=\"${{{key}+x}}\""),
        format!(
            "if [ -n \"$__REXUX_SNAPSHOT_PROXY_ENV_SET\" ] || [ -n \"${{{key}+x}}\" ]; then\n{restores}\nfi"
        ),
    );
    let git_blocks = build_rexux_proxy_git_ssh_command_exports();
    (
...
#[cfg(target_os = "macos")]
fn build_rexux_proxy_git_ssh_command_exports() -> (String, String) {
    let key = PROXY_GIT_SSH_COMMAND_ENV_KEY;
    let marker_pattern = format!("{}\\ *", REXUX_PROXY_GIT_SSH_COMMAND_MARKER.trim_end());
    (
        format!(
            "__REXUX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET=\"${{{key}+x}}\"\n__REXUX_SNAPSHOT_PROXY_GIT_SSH_COMMAND=\"${{{key}-}}\"\ncase \"$__REXUX_SNAPSHOT_PROXY_GIT_SSH_COMMAND\" in\n  {marker_pattern}) __REXUX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED=1 ;;\n  *) __REXUX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED= ;;\nesac"
        ),
        format!(
            "case \"${{{key}-}}\" in\n  {marker_pattern}) __REXUX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED=1 ;;\n  *) __REXUX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED= ;;\nesac\nif [ -n \"$__REXUX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED\" ]; then\n  if [ -z \"${{{key}+x}}\" ] || [ -n \"$__REXUX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n    export {key}=\"$__REXUX_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  fi\nelif [ -n \"$__REXUX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n  if [ -n \"$__REXUX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET\" ]; then\n    export {key}=\"$__REXUX_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  else\n    unset {key}\n  fi\nfi"
        ),
    )
}
...
mod prepare_powershell_command_tests {
    ...
    fn local_elevated_powershell_uses_discovered_fallback() {
        ...
        let rewritten = prepare_powershell_command_for_elevated_windows_sandbox_with_fallback(
            ...
            /*environment_is_remote*/ false,
            |_| {
                Some(rexux_shell_command::shell_detect::DetectedShell {
