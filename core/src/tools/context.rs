...
use rexux_protocol::models::function_call_output_content_items_to_text;
use rexux_tools::LoadableToolSpec;
...
use rexux_utils_output_truncation::formatted_truncate_text;
...
pub enum ToolCallSource {
    ...
    CodeMode {
        ...
        /// debugging the JS/runtime bridge, but it is not the Rexux tool call id
    ...
    pub(crate) async fn originating_item_id(&self) -> Option<ResponseItemId> {
        if let ToolCallSource::CodeMode { cell_id, .. } = &self.source {
            return self
                ...
                .services
                .code_mode_service
                .cell_originating_item_id(&rexux_code_mode::CellId::new(cell_id.clone()));
        }
