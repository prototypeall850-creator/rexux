...
use rexux_protocol::DEFAULT_FUNCTION_NAMESPACE;
...
use rexux_tools::TOOL_SEARCH_TOOL_NAME;
use rexux_tools::ToolCall as ExtensionToolCall;
use rexux_tools::ToolEnvironmentMode;
use rexux_tools::ToolExecutor;
use rexux_tools::ToolExposures;
use rexux_tools::ToolName;
use rexux_tools::ToolSearchInfo;
use rexux_tools::ToolSpec;
use rexux_tools::UnifiedExecShellMode;
use rexux_tools::can_request_original_image_detail;
use rexux_tools::collect_code_mode_exec_prompt_tool_definitions;
use rexux_tools::collect_request_plugin_install_entries;
use rexux_tools::default_namespace_description;
use rexux_tools::request_user_input_available_modes;
...
pub(crate) fn finalize_tool_router(
    ...
    if code_mode_enabled {
        for tool_name in [
            ToolName::plain(rexux_code_mode::PUBLIC_TOOL_NAME),
            ToolName::plain(rexux_code_mode::WAIT_TOOL_NAME),
        ] {
    ...
    if turn_context.config.tool_registry.error_on_tool_collisions {
        if let Some(tool_name) = registry.first_collision() {
            ...
            return Err(RexuxErrorDetails::ToolCollision(name).into());
        }
        ...
        for tool in registry.entries() {
            ...
            if include_tool_namespaces_info && tool.exposure != ToolExposure::Hidden {
                ...
                match namespace_owners.get(namespace_name) {
                    Some(existing_owner) if existing_owner != &owner => {
                        return Err(
                            RexuxErrorDetails::ToolCollision(namespace_name.to_string()).into()
                        );
                    }
                    Some(_) => {}
                    None => {
            ...
            match namespace_descriptions.entry(namespace.name.clone()) {
                ...
                Entry::Occupied(entry) if entry.get() != &namespace.description => {
                    return Err(RexuxErrorDetails::ToolCollision(entry.key().clone()).into());
                }
                Entry::Occupied(_) => {}
            }
        }
...
fn spec_for_model_request(
    ...
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly)
        ...
        && rexux_code_mode::is_code_mode_nested_tool(spec.name())
        && code_mode_tool_names
            .get(&rexux_code_mode::normalize_code_mode_identifier(
                &rexux_tools::code_mode_name_for_tool_name(tool_name),
            ))
            .is_some_and(|winner| winner == tool_name)
    {
        rexux_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
    ...
    provider.uses_openai_actor_authorization()
        || (provider.requires_openai_auth
            && turn_context
                .auth_manager
                .as_deref()
                .is_some_and(AuthManager::current_auth_uses_rexux_backend))
}
...
fn is_hidden_by_code_mode_only(
    ...
    tool_mode == ToolMode::CodeModeOnly
        && exposure.is_available_in_code_mode()
        && rexux_code_mode::is_code_mode_nested_tool(&rexux_tools::code_mode_name_for_tool_name(
            tool_name,
        ))
}
...
fn register_code_mode_executors(
    ...
    for tool in registry.entries() {
        ...
        if !rexux_code_mode::is_code_mode_nested_tool(&code_mode_name) {
            continue;
        }
        match code_mode_tool_names.entry(rexux_code_mode::normalize_code_mode_identifier(
            &code_mode_name,
        )) {
            Entry::Vacant(entry) => {
    ...
    let execute_handler = CodeModeExecuteHandler::new(
        create_code_mode_tool(
            ...
            if unified_image_budget_enabled(&turn_context.config.features, model_info) {
                rexux_code_mode::ImageDetailVisibility::Hidden
            } else {
                rexux_code_mode::ImageDetailVisibility::Visible
            },
        ),
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, rexux_code_mode::ToolNamespaceDescription> {
    let mut namespace_descriptions = BTreeMap::new();
    for spec in specs {
        let ToolSpec::Namespace(namespace) = spec else {
            continue;
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| rexux_code_mode::ToolNamespaceDescription {
...
impl ToolExecutor<ToolInvocation> for MultiAgentV2NamespaceOverride {
    ...
    fn handle<'a>(&'a self, invocation: ToolInvocation) -> rexux_tools::ToolExecutorFuture<'a>
    where
        ToolInvocation: 'a,
    {
...
fn compare_code_mode_tools(
    left: &rexux_code_mode::ToolDefinition,
    right: &rexux_code_mode::ToolDefinition,
    namespace_descriptions: &BTreeMap<String, rexux_code_mode::ToolNamespaceDescription>,
) -> std::cmp::Ordering {
...
fn code_mode_namespace_name<'a>(
    tool: &rexux_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, rexux_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
