...
impl ToolOrchestrator {
        ...
        T: ToolRuntime<Rq, Out>,
    {
        ...
        let attempt_with_network_approval = SandboxAttempt {
            ...
            rexux_linux_sandbox_exe: attempt.rexux_linux_sandbox_exe,
        ...
        T: ToolRuntime<Rq, Out>,
    {
        ...
        let initial_attempt = SandboxAttempt {
            ...
            workspace_roots,
            rexux_linux_sandbox_exe: turn_ctx.config.rexux_linux_sandbox_exe.as_ref(),
        ...
        match first_result {
            ...
            Err(ToolError::Rexux(err)) => {
                let RexuxErrorDetails::Sandbox(SandboxErr::Denied {
                    output,
                    network_policy_decision,
                }) = err.details()
                else {
                    let err = ToolError::Rexux(err);
                ...
                if network_policy_decision.is_some() && network_approval_context.is_none() {
                    ...
                    return Err(ToolError::Rexux(err));
                }
                ...
                let retry_rexux_linux_sandbox_exe = if unsandboxed_allowed {
                    None
                } else {
                    turn_ctx.config.rexux_linux_sandbox_exe.as_ref()
                };
                let retry_attempt = SandboxAttempt {
                    ...
                    sandbox_cwd: &sandbox_policy_cwd,
                    workspace_roots,
                    rexux_linux_sandbox_exe: retry_rexux_linux_sandbox_exe,
                    use_legacy_landlock: sandbox_config.use_legacy_landlock,
                    windows_sandbox_level: sandbox_config.windows_sandbox_level,
                    windows_sandbox_private_desktop: sandbox_config.windows_sandbox_private_desktop,
                    network_denial_cancellation_token: None,
...
fn sandbox_outcome_from_tool_error(err: &ToolError) -> Option<&'static str> {
    match err {
        ToolError::Rexux(err) => match err.details() {
            RexuxErrorDetails::Sandbox(SandboxErr::Denied { .. }) => Some("denied"),
            RexuxErrorDetails::Sandbox(SandboxErr::Timeout { .. }) => Some("timed_out"),
            RexuxErrorDetails::Sandbox(SandboxErr::Signal(_)) => Some("signal"),
            _ => None,
        },
