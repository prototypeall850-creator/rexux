...
use rexux_tools::ConversationHistory;
use rexux_tools::ExtensionTurnItem;
use rexux_tools::ResponsesApiNamespaceTool;
use rexux_tools::ToolCall as ExtensionToolCall;
use rexux_tools::ToolEnvironment;
use rexux_tools::ToolName;
use rexux_tools::ToolSearchInfo;
use rexux_tools::ToolSpec;
use rexux_tools::TurnItemEmissionFuture;
use rexux_tools::TurnItemEmitter;
...
pub(crate) struct ExtensionToolAdapter(
    Arc<dyn for<'call> rexux_tools::ToolExecutor<ExtensionToolCall<'call>>>,
);

impl ExtensionToolAdapter {
    pub(crate) fn new(
        executor: Arc<dyn for<'call> rexux_tools::ToolExecutor<ExtensionToolCall<'call>>>,
    ) -> Self {
        Self(executor)
    }
}
...
async fn to_extension_call(invocation: &ToolInvocation) -> ExtensionToolCall<'_> {
    ...
    let rexux_turn_metadata = invocation
        .turn
        .turn_metadata_state
...
mod tests {
    ...
    use rexux_extension_items::web_search::WebSearchItem;
    use rexux_protocol::items::TurnItem;
    ...
    use rexux_tools::ExtensionTurnItem;
    use rexux_tools::ToolCallSource as ExtensionToolCallSource;
    ...
    impl<'call> rexux_extension_api::ToolExecutor<rexux_tools::ToolCall<'call>>
        for StubExtensionExecutor
    {
        fn tool_name(&self) -> rexux_tools::ToolName {
            rexux_tools::ToolName::plain("extension_echo")
        }
        ...
        fn spec(&self) -> rexux_tools::ToolSpec {
            rexux_tools::ToolSpec::Function(rexux_tools::ResponsesApiTool {
                ...
                strict: true,
                parameters: rexux_tools::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
        ...
        fn handle<'a>(
            &'a self,
            _call: rexux_tools::ToolCall<'call>,
        ) -> rexux_tools::ToolExecutorFuture<'a>
        where
            'call: 'a,
        {
            Box::pin(async {
                Ok(
                    Box::new(rexux_tools::JsonToolOutput::new(json!({ "ok": true })))
                        as Box<dyn rexux_tools::ToolOutput>,
                )
            })
        }
    }
    ...
    struct CapturingExtensionExecutor {
        captured_call: Arc<Mutex<Option<rexux_tools::ToolCall<'static>>>>,
        ...
        for CapturingExtensionExecutor
    {
        ...
        fn handle<'a>(
            &'a self,
            call: rexux_tools::ToolCall<'call>,
        ) -> rexux_tools::ToolExecutorFuture<'a>
        where
            'call: 'a,
        {
    ...
    impl CapturingExtensionExecutor {
        async fn handle_call(
            &self,
            call: rexux_tools::ToolCall<'_>,
        ) -> Result<Box<dyn rexux_tools::ToolOutput>, rexux_tools::FunctionCallError> {
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: rexux_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = rexux_tools::JsonToolOutput::new(json!({ "ok": true }));
    ...
    async fn image_generation_publication_preserves_extension_saved_path() {
        ...
        rexux_tools::TurnItemEmitter::emit_started(
            &emitter,
            ExtensionTurnItem {
        ...
        .await;
        rexux_tools::TurnItemEmitter::emit_completed(
            &emitter,
            ExtensionTurnItem {
