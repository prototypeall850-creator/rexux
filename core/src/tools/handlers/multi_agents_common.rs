...
pub(crate) fn collab_spawn_error(err: RexuxErr) -> FunctionCallError {
    match err.details() {
        RexuxErrorDetails::UnsupportedOperation(message) if message == "thread manager dropped" => {
...
pub(crate) fn collab_agent_error(agent_id: ThreadId, err: RexuxErr) -> FunctionCallError {
    match err.details() {
        RexuxErrorDetails::ThreadNotFound(id) => {
