...
use rexux_analytics::PluginInstallRequestSource;
use rexux_analytics::PluginInstallRequested;
use rexux_analytics::PluginInstallRequestedPlugin;
...
use rexux_tools::DiscoverableToolAction;
use rexux_tools::DiscoverableToolType;
use rexux_tools::LIST_AVAILABLE_PLUGINS_TO_INSTALL_TOOL_NAME;
use rexux_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use rexux_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use rexux_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use rexux_tools::RequestPluginInstallArgs;
use rexux_tools::RequestPluginInstallResult;
use rexux_tools::ToolName;
use rexux_tools::ToolSpec;
use rexux_tools::all_requested_connectors_picked_up;
use rexux_tools::build_request_plugin_install_elicitation_request;
use rexux_tools::filter_request_plugin_install_discoverable_tools_for_client;
use rexux_tools::verified_connector_install_completed;
...
impl RequestPluginInstallHandler {
    async fn handle_call(
        ...
        let tool = if self.presentation == ToolSuggestPresentation::RecommendationContext {
            ...
            match rexux_core_plugins::hydrate_selected_recommended_plugin_install_metadata(
                &plugins_config,
                auth.as_ref(),
                tool,
            )
...
async fn maybe_persist_disabled_install_request(
    ...
    if let Err(err) = persist_disabled_install_request(&turn.config.rexux_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn persist_disabled_install_request(
    rexux_home: &rexux_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn verify_request_plugin_install_completed(
    ...
    tool: &DiscoverableTool,
    auth: Option<&rexux_login::RexuxAuth>,
) -> bool {
    match tool {
...
async fn refresh_missing_requested_connectors(
    ...
    match session.hard_refresh_latest_rexux_apps_tools().await {
        Ok(mcp_tools) => {
