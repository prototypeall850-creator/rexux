...
use rexux_code_mode::CellId;
use rexux_code_mode::CodeModeNestedToolCall;
use rexux_code_mode::CodeModeSession;
use rexux_code_mode::CodeModeSessionProvider;
use rexux_code_mode::CodeModeToolKind;
use rexux_code_mode::RuntimeResponse;
...
use rexux_utils_output_truncation::formatted_truncate_text_content_items_with_policy;
use rexux_utils_output_truncation::truncate_function_output_items_with_policy;
...
pub(crate) const PUBLIC_TOOL_NAME: &str = rexux_code_mode::PUBLIC_TOOL_NAME;
pub(crate) const WAIT_TOOL_NAME: &str = rexux_code_mode::WAIT_TOOL_NAME;
pub(crate) const DEFAULT_WAIT_YIELD_TIME_MS: u64 = rexux_code_mode::DEFAULT_WAIT_YIELD_TIME_MS;
...
impl CodeModeService {
    ...
    pub(crate) fn take_unavailable_warning(&self, tool_mode: ToolMode) -> Option<String> {
        ...
        .then(|| {
            format!(
                "Code Mode is unavailable because {error}. {behavior}; enable `features.code_mode_host` and install `rexux-code-mode-host`."
            )
        })
    }
    ...
    pub(crate) async fn execute(
        &self,
        mut request: rexux_code_mode::ExecuteRequest,
    ) -> Result<rexux_code_mode::StartedCell, String> {
        request
            .yield_time_ms
    ...
    ) -> Result<rexux_code_mode::WaitOutcome, String> {
    ...
    pub(crate) fn mark_cell_ready_for_dispatch(
        ...
        originating_item_id: Option<rexux_protocol::ResponseItemId>,
    ) {
        self.dispatch_broker
    ...
    pub(crate) fn cell_originating_item_id(
        &self,
        cell_id: &rexux_code_mode::CellId,
    ) -> Option<rexux_protocol::ResponseItemId> {
...
> {
    ...
    exec.session
        ...
        .track_code_mode_tool_call(rexux_analytics::CodeModeToolCallFact::ChildStarted {
