...
use rexux_mcp::McpBinding;
use rexux_mcp::ToolInfo as McpToolInfo;
...
impl McpHandlerCache {
    pub(crate) fn append_mcp_tools(
        ...
        config: &Config,
        apps_enabled: bool,
        mcp_server_catalog: &rexux_mcp::ResolvedMcpCatalog,
...
fn append_mcp_tools(
    ...
    let non_app_tools = filter_non_rexux_apps_mcp_tools_only(all_mcp_tools);
    let app_tools = apps_enabled
        .then(|| filter_rexux_apps_mcp_tools(all_mcp_tools, config))
        .into_iter()
        .flatten();
...
fn filter_non_rexux_apps_mcp_tools_only(
    mcp_tools: &[McpToolInfo],
) -> impl Iterator<Item = &McpToolInfo> + '_ {
    mcp_tools.iter().filter(|tool| {
        tool.server_name != REXUX_APPS_MCP_SERVER_NAME && tool_is_model_visible(tool)
    })
}

fn filter_rexux_apps_mcp_tools<'a>(
