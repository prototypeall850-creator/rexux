...
use rexux_shell_command::shell_snapshot::snapshot_script;
...
impl ShellSnapshot {
    ...
    async fn try_create(
        ...
        let temp_path = rexux_home
            .join(SNAPSHOT_DIR)
            .join(format!("{session_id}.tmp-{nonce}"));

        // Clean the (unlikely) leaked snapshot files.
        let rexux_home = rexux_home.clone();
        let cleanup_session_id = session_id;
        tokio::spawn(async move {
            if let Err(err) =
                cleanup_stale_snapshots(&rexux_home, cleanup_session_id, state_db).await
            {
...
async fn run_script_with_timeout(
    ...
    rexux_protocol::shell_environment::scrub_non_inheritable_env_vars(handler.as_std_mut());
    handler.args(&args[1..]);
...
pub async fn cleanup_stale_snapshots(
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path =
            find_thread_path_by_id_str(rexux_home, session_id, state_db.as_deref()).await?;
