...
use rexux_config::types::OtelExporterKind as Kind;
use rexux_config::types::OtelHttpProtocol as Protocol;
use rexux_features::Feature;
use rexux_login::default_client::originator;
use rexux_otel::OtelExporter;
use rexux_otel::OtelHttpProtocol;
use rexux_otel::OtelProvider;
use rexux_otel::OtelSettings;
use rexux_otel::OtelTlsConfig as OtelTlsSettings;
use std::error::Error;
...
pub fn record_process_start(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let _ = rexux_otel::record_process_start_once(metrics, originator);
}
...
pub fn install_sqlite_telemetry(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let telemetry = rexux_rollout::sqlite_telemetry_recorder(metrics.clone(), originator);
