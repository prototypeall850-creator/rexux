...
use rexux_analytics::TurnProfileFact;
use rexux_analytics::TurnTokenUsageFact;
use rexux_context_fragments::RenderedFragment;
use rexux_otel::SessionTelemetry;
use rexux_otel::TURN_E2E_DURATION_METRIC;
use rexux_otel::TURN_MEMORY_METRIC;
use rexux_otel::TURN_NETWORK_PROXY_METRIC;
use rexux_otel::TURN_TOKEN_USAGE_METRIC;
use rexux_otel::TURN_TOOL_CALL_METRIC;
use rexux_otel::TURN_UNIFIED_EXEC_RUNNING_PROCESSES_METRIC;
...
pub(crate) type SessionTaskResult = RexuxResult<Option<String>>;
...
///
/// Implementations encapsulate a specific Rexux workflow (regular chat,
...
pub(crate) trait SessionTask: Send + Sync + 'static {
    ...
    /// [`RexuxErr::TurnAborted`] completes the task through the aborted-turn
    /// lifecycle instead.
    fn run(
        self: Arc<Self>,
        session: Arc<Session>,
...
impl Session {
                ...
                if let Err(err) = sess.flush_rollout().await {
                    ...
                    sess.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                            message: format!(
                                "Failed to save the conversation transcript; Rexux will continue retrying. Error: {err}"
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) fn has_outstanding_durable_sleep(&self) -> bool {
        self.services
            ...
            .get::<rexux_extension_items::sleep::SleepItem>()
            .is_some()
    }
    ...
    pub async fn on_task_finished(
        ...
        let (last_agent_message, abort_reason) = match task_result {
            ...
            Err(err) => {
                ...
                .await;
                self.track_turn_rexux_error(turn_context.as_ref(), &err);
                self.send_event(
                    turn_context.as_ref(),
