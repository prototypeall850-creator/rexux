use crate::RexuxAppsToolsCache;
use crate::agent::AgentControl;
use crate::attestation::AttestationProvider;
use crate::rexux_thread::RexuxThread;
...
use rexux_agent_graph_store::AgentGraphStore;
use rexux_agent_graph_store::LocalAgentGraphStore;
...
use rexux_code_mode::CodeModeSessionProvider;
use rexux_code_mode::DisabledCodeModeSessionProvider;
use rexux_code_mode::ProcessOwnedCodeModeSessionProvider;
use rexux_core_plugins::PluginsManager;
use rexux_exec_server::EnvironmentManager;
use rexux_extension_api::ExtensionDataInit;
use rexux_extension_api::ExtensionRegistry;
use rexux_extension_api::LoadedUserInstructions;
use rexux_extension_api::UserInstructionsProvider;
use rexux_extension_api::empty_extension_registry;
...
use rexux_login::default_client::REXUX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR;
...
use rexux_model_provider_info::OPENAI_PROVIDER_ID;
...
use rexux_protocol::mcp::OPENAI_STANDARD_FORM_INPUT_EXTENSION_ID;
...
use rexux_thread_store::InMemoryThreadStore;
use rexux_thread_store::LoadThreadHistoryParams;
use rexux_thread_store::LocalThreadStore;
use rexux_thread_store::LocalThreadStoreConfig;
use rexux_thread_store::MoveThreadToSectionParams;
use rexux_thread_store::PreparedFork;
use rexux_thread_store::ReadThreadByRolloutPathParams;
use rexux_thread_store::ReadThreadParams;
use rexux_thread_store::StoredModelContext;
use rexux_thread_store::StoredThread;
use rexux_thread_store::ThreadMetadataPatch;
use rexux_thread_store::ThreadStore;
use rexux_thread_store::ThreadStoreError;
use rexux_thread_store::UpdateThreadMetadataParams;
...
struct TempRexuxHomeGuard {
    path: PathBuf,
}

impl Drop for TempRexuxHomeGuard {
    fn drop(&mut self) {
        let _ = std::fs::remove_dir_all(&self.path);
    }
}

/// Represents a newly created Rexux thread (formerly called a conversation), including the first event
...
pub struct ThreadManager {
    ...
    _test_rexux_home_guard: Option<TempRexuxHomeGuard>,
}
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<rexux_protocol::dynamic_tools::DynamicToolSpec>,
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<RexuxThread>>>>,
    ...
    match &config.experimental_thread_store {
        ThreadStoreConfig::Local => {
            ...
            if has_state_db && background_migration_enabled {
                ...
                let rexux_home = config.rexux_home.to_path_buf();
                tokio::spawn(async move {
                    ...
                    if compression_enabled {
                        rexux_rollout::spawn_rollout_compression_worker(rexux_home);
                    }
                });
            } else if compression_enabled {
                rexux_rollout::spawn_rollout_compression_worker(config.rexux_home.to_path_buf());
            }
            store
        }
...
impl ThreadManager {
    ...
    pub fn new(
        ...
        auth_manager: Arc<AuthManager>,
        models_manager: SharedModelsManager,
        rexux_apps_tools_cache: RexuxAppsToolsCache,
        session_source: SessionSource,
        environment_manager: Arc<EnvironmentManager>,
        extensions: Arc<ExtensionRegistry<Config>>,
        user_instructions_provider: Arc<dyn UserInstructionsProvider>,
    ...
    /// Construct with a dummy AuthManager containing the provided RexuxAuth.
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let rexux_home = std::env::temp_dir().join(format!(
            "rexux-thread-manager-test-{}",
            uuid::Uuid::new_v4()
        ));
        ...
        manager._test_rexux_home_guard = Some(TempRexuxHomeGuard { path: rexux_home });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided RexuxAuth and codex home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        let absolute_rexux_home = match AbsolutePathBuf::from_absolute_path_checked(&rexux_home) {
            Ok(rexux_home) => rexux_home,
            Err(err) => panic!("test rexux_home should be absolute: {err}"),
        };
        let (thread_created_tx, _) = broadcast::channel(THREAD_CREATED_CHANNEL_CAPACITY);
        let restriction_product = SessionSource::Exec.restriction_product();
        let skills_service = Arc::new(HostSkillsService::new_with_restriction_product(
            absolute_rexux_home.clone(),
        ...
        let thread_store: Arc<dyn ThreadStore> = Arc::new(LocalThreadStore::new(
            LocalThreadStoreConfig {
                ...
                sqlite: rexux_state::SqliteConfig::new_for_testing(absolute_rexux_home),
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                models_manager: create_model_provider(provider, Some(auth_manager.clone()))
                    .models_manager(rexux_home, /*config_model_catalog*/ None),
                git_root_discovery: Arc::default(),
                environment_manager,
                starting_mcp_runtimes: std::sync::Mutex::new(Vec::new()),
                skills_service,
    ...
    pub fn validate_environment_selections(
        ...
        for environment in environments {
            ...
            self.state
                ...
                .ok_or_else(|| {
                    RexuxErr::InvalidRequest(format!(
    ...
    pub async fn list_models(
        ...
        http_client_factory: rexux_http_client::HttpClientFactory,
    ) -> Vec<ModelPreset> {
        self.state
            .models_manager
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> RexuxResult<Arc<RexuxThread>> {
        self.state.get_thread(thread_id).await
    }

    /// Updates metadata for loaded and cold threads through one entrypoint.
    ///
    /// Loaded threads route through `RexuxThread`/`LiveThread`, so metadata changes stay ordered
        ...
        let updated = self
            ...
            .map_err(|err| match err {
                ThreadStoreError::ThreadNotFound { thread_id } => {
                    RexuxErr::ThreadNotFound(thread_id)
                }
    ...
    pub async fn list_agent_subtree_thread_ids(
        ...
        if let Some(agent_graph_store) = self.state.agent_graph_store() {
            for descendant_id in agent_graph_store
                ...
                .await
                .map_err(|err| {
                    RexuxErr::Fatal(format!("failed to load thread-spawn descendants: {err}"))
                })?
            {
    ...
    pub async fn start_thread(&self, options: StartThreadOptions) -> RexuxResult<NewThread> {
    ...
    /// as `Arc<RexuxThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<RexuxThread>> {
    ...
    pub async fn remove_thread_if_matches(
        &self,
        thread_id: &ThreadId,
        expected: &Arc<RexuxThread>,
    ) -> Option<Arc<RexuxThread>> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(&self, thread_id: ThreadId) -> RexuxResult<Arc<RexuxThread>> {
        let threads = self.threads.read().await;
        match threads.get(&thread_id) {
            Some(thread) if !thread.session_source.is_internal() => Ok(thread.clone()),
            Some(_) | None => Err(RexuxErr::ThreadNotFound(thread_id)),
        }
    }
    ...
    pub(crate) async fn read_stored_thread(
        ...
        self.thread_store
            ...
            .map_err(|err| match err {
                ...
                err => RexuxErr::Fatal(format!("failed to read stored thread {thread_id}: {err}")),
            })
    }
    ...
    ) -> RexuxResult<StoredModelContext> {
        let thread_id = params.thread_id;
        self.thread_store
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<RexuxThread>> {
    ...
    async fn effective_originator(
        ...
        let env_originator = std::env::var(REXUX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
            .is_ok()
    ...
    async fn spawn_thread(&self, request: ThreadSpawnRequest) -> RexuxResult<NewThread> {
        ...
        let windows_sandbox_proxy_settings_mode = if matches!(
            ...
            rexux_sandboxing::WindowsSandboxProxySettingsMode::Preserve
        } else {
    ...
    async fn finalize_thread_spawn(
        ...
        let session_configured = match event {
            ...
            _ => {
                return Err(RexuxErr::SessionConfiguredNotFirstEvent);
            }
        };

        {
            ...
            if let std::collections::hash_map::Entry::Vacant(e) = threads.entry(thread_id) {
                let thread = Arc::new(RexuxThread::new(
                    session,
                    io,
        ...
        Err(RexuxErr::InvalidRequest(format!(
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> rexux_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return rexux_rollout_trace::ThreadTraceContext::disabled();
        };
        ...
        self.get_thread(*parent_thread_id)
            ...
            .unwrap_or_else(rexux_rollout_trace::ThreadTraceContext::disabled)
    }
}
...
fn thread_store_rollout_read_error(err: ThreadStoreError) -> RexuxErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => RexuxErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => RexuxErr::InvalidRequest(message),
        err => RexuxErr::Fatal(format!("failed to read thread by rollout path: {err}")),
    }
}

fn thread_store_metadata_update_error(thread_id: ThreadId, err: ThreadStoreError) -> RexuxErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => RexuxErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => RexuxErr::InvalidRequest(message),
        ThreadStoreError::Unsupported { operation } => RexuxErr::UnsupportedOperation(format!(
