...
use rexux_analytics::CompactionTrigger;
use rexux_analytics::HookRunFact;
...
use rexux_core_plugins::executor_plugin_hook_sources;
use rexux_hooks::InterruptRequest;
use rexux_hooks::PermissionRequestDecision;
use rexux_hooks::PermissionRequestOutcome;
use rexux_hooks::PermissionRequestRequest;
use rexux_hooks::PostToolUseOutcome;
use rexux_hooks::PostToolUseRequest;
use rexux_hooks::PreToolUseOutcome;
use rexux_hooks::PreToolUseRequest;
use rexux_hooks::SessionStartOutcome;
use rexux_hooks::StartHookTarget;
use rexux_hooks::StopHookTarget;
use rexux_hooks::StopOutcome;
use rexux_hooks::SubagentHookContext;
use rexux_hooks::UserPromptSubmitOutcome;
use rexux_hooks::UserPromptSubmitRequest;
use rexux_hooks::hook_execution_mode_label;
use rexux_hooks::hook_handler_type_label;
use rexux_mcp::REXUX_APPS_MCP_SERVER_NAME;
use rexux_otel::HOOK_RUN_DURATION_METRIC;
use rexux_otel::HOOK_RUN_METRIC;
use rexux_plugin::ExecutorPluginHookSource;
use rexux_protocol::items::FunctionCallOutputItem;
...
use rexux_protocol::protocol::HookCompletedEvent;
...
use rexux_protocol::protocol::HookOutputEntryKind;
use rexux_protocol::protocol::HookRunStatus;
use rexux_protocol::protocol::HookRunSummary;
use rexux_protocol::protocol::HookSource;
use rexux_protocol::protocol::HookStartedEvent;
        ...
        let target = match &turn_context.session_source {
            SessionSource::SubAgent(SubAgentSource::ThreadSpawn { agent_role, .. })
                if matches!(
                    ...
                    rexux_hooks::SessionStartSource::Startup
                ) =>
            {
...
pub(crate) async fn run_turn_stop_hooks(
    ...
    let request = rexux_hooks::StopRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_session_end_hooks(sess: &Arc<Session>) {
    ...
    let request = rexux_hooks::SessionEndRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = rexux_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = rexux_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(rexux_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: rexux_hooks::HookEvent::AfterAgent {
                event: rexux_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            rexux_hooks::HookResult::Success => continue,
            rexux_hooks::HookResult::FailedContinue(error) => (error, false),
            rexux_hooks::HookResult::FailedAbort(error) => (error, true),
        };
    ...
    let event = EventMsg::Error(rexux_protocol::protocol::ErrorEvent {
        misalignment: None,
        message,
...
fn hook_run_analytics_payload(
    ...
    turn_context: &TurnContext,
    completed: &HookCompletedEvent,
) -> (rexux_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use rexux_protocol::protocol::HookHandlerType;
    use rexux_protocol::protocol::HookRunStatus;
    use rexux_protocol::protocol::HookScope;
    use rexux_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    rexux_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
    ...
    async fn hook_lifecycle_notifications_hide_builtin_and_async_runs_but_preserve_metrics() {
        ...
        assert!(matches!(
            started.msg,
            rexux_protocol::protocol::EventMsg::HookStarted(event)
        ...
        assert!(matches!(
            completed.msg,
            rexux_protocol::protocol::EventMsg::HookCompleted(event)
                if event.run.id == synchronous_run.id
        ));
        assert!(events.try_recv().is_err());
