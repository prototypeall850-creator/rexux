...
use rexux_analytics::GuardianReviewDecision;
use rexux_analytics::GuardianReviewFailureReason;
use rexux_analytics::GuardianReviewTerminalStatus;
use rexux_analytics::GuardianReviewTrackContext;
use rexux_analytics::GuardianReviewedAction;
use rexux_analytics::GuardianV2Event;
use rexux_analytics::GuardianV2EventKind;
use rexux_async_utils::THREAD_STACK_SIZE_BYTES;
use rexux_core_plugins::PluginCommandAttribution;
use rexux_extension_api::GuardianV2Enabled;
...
use rexux_protocol::protocol::GuardianAssessmentDecisionSource;
...
impl GuardianReviewError {
    ...
    fn session_with_error_info(err: anyhow::Error, error_info: RexuxErrorInfo) -> Self {
        Self::Session {
            message: err.to_string(),
                ...
                .is_some()
            {
                session
                    ...
                    .track_guardian_v2_event(GuardianV2Event {
                        ...
                        occurred_at_ms: rexux_analytics::now_unix_millis(),
...
pub(crate) fn spawn_approval_request_review(
    ...
    let spawn_result = std::thread::Builder::new()
        .name("rexux-approval-review".to_string())
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = review_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset
                ...
                .iter()
                .any(|effort| effort.effort == rexux_protocol::openai_models::ReasoningEffort::Low),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info()
                ...
                .iter()
                .any(|preset| preset.effort == rexux_protocol::openai_models::ReasoningEffort::Low),
...
fn should_retry_guardian_review(outcome: &GuardianReviewOutcome) -> bool {
    matches!(
        ...
        GuardianReviewOutcome::Error(
            GuardianReviewError::Session {
                error_info: Some(
                    RexuxErrorInfo::ServerOverloaded
                        | RexuxErrorInfo::HttpConnectionFailed { .. }
                        | RexuxErrorInfo::ResponseStreamConnectionFailed { .. }
                        | RexuxErrorInfo::InternalServerError
                        | RexuxErrorInfo::ResponseStreamDisconnected { .. }
                ),
                ..
            } | GuardianReviewError::Parse { .. }
        )
