...
use anyhow::anyhow;
use rexux_analytics::GuardianReviewAnalyticsResult;
use rexux_analytics::GuardianReviewSessionAnalyticsParams;
use rexux_analytics::GuardianReviewSessionKind;
...
use crate::rexux_delegate::run_rexux_thread_interactive;
...
use rexux_tools::normalize_output_image_detail;
...
pub(crate) enum GuardianReviewSessionOutcome {
    ...
    SessionFailed {
        error: anyhow::Error,
        error_info: Option<RexuxErrorInfo>,
    },
    TimedOut,
    Aborted,
}
...
impl GuardianReviewSessionReuseKey {
    fn from_spawn_config(
        ...
        Self {
            ...
            rexux_linux_sandbox_exe: spawn_config.rexux_linux_sandbox_exe.clone(),
...
async fn spawn_guardian_review_session(
    ...
    let (session, io) = Box::pin(run_rexux_thread_interactive(
        spawn_config,
...
async fn run_review_on_session(
    ...
    let submission = review_session.io.submit_turn_input(
        TurnInputRequest::user_input(prompt_items.items)
            .with_thread_settings(rexux_protocol::protocol::ThreadSettingsOverrides {
                                ...
                                && let Some(error) = last_error
                            {
                                return (
                                    GuardianReviewSessionOutcome::SessionFailed {
                                        ...
                                        error_info: error.rexux_error_info,
                                    },
                                    true,
                                    true,
                                );
...
fn read_only_guardian_permission_profile(
    ...
    permission_profile
        ...
        .unwrap_or(PermissionProfile::External {
            network: rexux_protocol::permissions::NetworkSandboxPolicy::Restricted,
        })
}

pub(crate) fn build_guardian_review_session_config(
    parent_config: &Config,
    live_network_config: Option<rexux_network_proxy::NetworkProxyConfig>,
    active_model: &str,
