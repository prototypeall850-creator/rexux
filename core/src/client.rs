...
//!
//! `ModelClient` is intended to live for the lifetime of a Rexux session and holds the stable
...
use rexux_api::AgentIdentityTelemetry;
use rexux_api::ApiError;
use rexux_api::AuthProvider;
use rexux_api::CompactClient as ApiCompactClient;
use rexux_api::CompactionInput as ApiCompactionInput;
use rexux_api::Compression;
use rexux_api::MemoriesClient as ApiMemoriesClient;
use rexux_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use rexux_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use rexux_api::Provider as ApiProvider;
use rexux_api::RawMemory as ApiRawMemory;
use rexux_api::RealtimeCallClient as ApiRealtimeCallClient;
use rexux_api::RealtimeSessionConfig as ApiRealtimeSessionConfig;
use rexux_api::Reasoning;
use rexux_api::ReasoningContext;
use rexux_api::RequestTelemetry;
use rexux_api::ReqwestTransport;
use rexux_api::ResponseCreateWsRequest;
use rexux_api::ResponsesApiRequest;
use rexux_api::ResponsesClient as ApiResponsesClient;
use rexux_api::ResponsesEndpoint;
use rexux_api::ResponsesOptions as ApiResponsesOptions;
use rexux_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use rexux_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use rexux_api::ResponsesWsRequest;
use rexux_api::SharedAuthProvider;
use rexux_api::SseTelemetry;
use rexux_api::StreamOptions;
use rexux_api::TransportError;
use rexux_api::WebsocketTelemetry;
use rexux_api::auth_header_telemetry;
use rexux_api::build_session_headers;
use rexux_api::create_text_param_for_request;
use rexux_api::response_create_client_metadata;
...
use rexux_login::RefreshTokenError;
use rexux_login::UnauthorizedRecovery;
use rexux_login::default_client::add_originator_header;
use rexux_login::default_client::create_client_for_route;
...
use rexux_protocol::config_types::Verbosity as VerbosityConfig;
...
use rexux_protocol::protocol::Event as ProtocolEvent;
...
use rexux_rollout_trace::CompactionTraceContext;
use rexux_rollout_trace::InferenceTraceAttempt;
use rexux_rollout_trace::InferenceTraceContext;
use rexux_tools::create_tools_json_for_responses_api;
use rexux_tools::create_tools_json_for_responses_lite;
use rexux_tools::create_tools_raw_json_for_responses_api;
...
use crate::responses_metadata::RexuxResponsesMetadata;
use crate::responses_metadata::subagent_header_value;
use crate::util::emit_feedback_auth_recovery_tags;
use rexux_feedback::FeedbackRequestTags;
use rexux_feedback::emit_feedback_request_tags_with_auth_env;
use rexux_login::auth::AgentIdentityAuthPolicy;
use rexux_login::auth_env_telemetry::AuthEnvTelemetry;
use rexux_login::auth_env_telemetry::collect_auth_env_telemetry;
use rexux_model_provider::AgentIdentitySessionFallback;
use rexux_model_provider::ProviderAuthScope;
use rexux_model_provider::ProviderUnauthorizedRecovery;
use rexux_model_provider::SharedModelProvider;
use rexux_model_provider::create_model_provider;
#[cfg(test)]
use rexux_model_provider_info::DEFAULT_WEBSOCKET_CONNECT_TIMEOUT_MS;
use rexux_model_provider_info::ModelProviderInfo;
use rexux_model_provider_info::WireApi;
use rexux_protocol::error::RexuxErr;
use rexux_protocol::error::Result;
use rexux_response_debug_context::extract_response_debug_context;
use rexux_response_debug_context::extract_response_debug_context_from_api_error;
use rexux_response_debug_context::telemetry_api_error_message;
use rexux_response_debug_context::telemetry_transport_error_message;

pub const OPENAI_BETA_HEADER: &str = "OpenAI-Beta";
pub const X_REXUX_INSTALLATION_ID_HEADER: &str = "x-codex-installation-id";
pub const X_REXUX_ROUTING_HINT_HEADER: &str = "x-codex-routing-hint";
pub const X_REXUX_TURN_STATE_HEADER: &str = "x-codex-turn-state";
pub const X_REXUX_TURN_METADATA_HEADER: &str = "x-codex-turn-metadata";
pub const X_REXUX_PARENT_THREAD_ID_HEADER: &str = "x-codex-parent-thread-id";
pub const X_REXUX_WINDOW_ID_HEADER: &str = "x-codex-window-id";
...
///
/// This holds configuration and state that should be shared across turns within a Rexux session
...
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each Rexux turn. Reusing it across turns would replay
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a Rexux session. Per-turn values
    ...
    pub fn new(
        ...
        let rexux_api_key_env_enabled = model_provider
            .auth_manager()
            .as_ref()
            .is_some_and(|manager| manager.rexux_api_key_env_enabled());
        let auth_env_telemetry =
            collect_auth_env_telemetry(model_provider.info(), rexux_api_key_env_enabled);
    ...
    fn prompt_cache_key(&self, responses_metadata: &RexuxResponsesMetadata) -> String {
    ...
    pub(crate) async fn compact_conversation_history(
        ...
        responses_metadata: &RexuxResponsesMetadata,
        ...
        let request_telemetry = Self::build_request_telemetry(
            ...
            AuthRequestTelemetryContext::new(
                client_setup.auth.as_ref().map(RexuxAuth::auth_mode),
        ...
        if let Ok(header_value) = HeaderValue::from_str(&responses_metadata.installation_id) {
            extra_headers.insert(X_REXUX_INSTALLATION_ID_HEADER, header_value);
        }
        ...
        if let Some(header_value) = self.build_routing_hint_header(
            ...
            extra_headers.insert(X_REXUX_ROUTING_HINT_HEADER, header_value);
        }
    ...
    fn build_responses_request(
        ...
        .then_some(StreamOptions {
            reasoning_summary_delivery: rexux_api::ReasoningSummaryDelivery::SequentialCutoff,
        });
    ...
    fn responses_endpoint(&self, auth: Option<&RexuxAuth>, model: &str) -> ResponsesEndpoint {
        if self.free_guardian_enabled
            && crate::guardian::is_basic_session_source(&self.state.session_source)
            && self.uses_rexux_backend(auth)
            && self.state.provider.info().supports_rexux_backend_routes()
    ...
    fn uses_rexux_backend(&self, auth: Option<&RexuxAuth>) -> bool {
        let provider = self.state.provider.info();
        auth.is_some_and(RexuxAuth::uses_rexux_backend)
            && provider.is_openai()
    ...
    fn build_routing_hint_header(
        ...
        if !self.uses_rexux_backend(auth) {
            return None;
        }
    ...
    async fn connect_websocket(
        &self,
        session_telemetry: &SessionTelemetry,
        api_provider: rexux_api::Provider,
        api_auth: SharedAuthProvider,
        responses_metadata: &RexuxResponsesMetadata,
        auth_context: AuthRequestTelemetryContext,
        request_route_telemetry: RequestRouteTelemetry,
        ...
        let result = match tokio::time::timeout(
            ...
            ApiWebSocketResponsesClient::new(api_provider, api_auth)
                ...
                .connect(
                    &self.http_client_factory,
                    headers,
                    rexux_login::default_client::default_headers(),
                    /*turn_state*/ None,
    ...
    async fn build_websocket_headers(
        ...
        if let Some(routing_hint) = &responses_metadata.routing_hint {
            headers.insert(X_REXUX_ROUTING_HINT_HEADER, routing_hint.clone());
        }
        if let Some(header_value) = self.generate_attestation_header_for().await {
...
impl ModelClientSession {
    ...
    fn responses_request_compression(&self, auth: Option<&RexuxAuth>) -> Compression {
    ...
    async fn stream_responses_api(
        ...
        loop {
                ...
                )
            {
                options
                    .extra_headers
                    .insert(X_REXUX_ROUTING_HINT_HEADER, header_value);
            }
    ...
    async fn stream_responses_websocket(
        ...
        loop {
            ...
            if let Some(turn_state) = self.turn_state.get() {
                client_metadata.insert(X_REXUX_TURN_STATE_HEADER.to_string(), turn_state.clone());
            }
            match self
    ...
    /// Permanently disables WebSockets for this Rexux session and resets WebSocket state.
    ///
...
///
/// These headers implement Rexux-specific conventions:
///
...
fn build_responses_headers(
    ...
    {
        headers.insert(X_REXUX_TURN_STATE_HEADER, header_value);
    }
    headers
}
...
fn map_response_stream(
    api_stream: rexux_api::ResponseStream,
    session_telemetry: SessionTelemetry,
    inference_trace_attempt: InferenceTraceAttempt,
    provider: SharedModelProvider,
) -> (ResponseStream, oneshot::Receiver<LastResponse>) {
    let rexux_api::ResponseStream {
        rx_event,
        upstream_request_id,
    } = api_stream;
    let api_stream = rexux_api::ResponseStream {
        rx_event,
...
/// the mapped `RexuxErr` is returned to the caller.
...
struct WebsocketConnectParams<'a> {
    ...
    api_provider: rexux_api::Provider,
    api_auth: SharedAuthProvider,
    responses_metadata: &'a RexuxResponsesMetadata,
    auth_context: AuthRequestTelemetryContext,
    request_route_telemetry: RequestRouteTelemetry,
    endpoint: ResponsesEndpoint,
}
        ...
        && recovery.has_next()
    {
        ...
        return match recovery.next().await {
            ...
            Err(RefreshTokenError::Permanent(failed)) => {
                ...
                Err(RexuxErr::RefreshTokenFailed(failed))
            }
