...
use rexux_protocol::error::SandboxErr;
...
use rexux_protocol::protocol::ExecCommandOutputDeltaEvent;
use rexux_protocol::protocol::ExecOutputStream;
use rexux_sandboxing::SandboxCommand;
use rexux_sandboxing::SandboxManager;
use rexux_sandboxing::SandboxTransformRequest;
use rexux_sandboxing::SandboxType;
use rexux_sandboxing::SandboxablePreference;
use rexux_sandboxing::WindowsSandboxFilesystemOverrides;
pub(crate) use rexux_sandboxing::is_likely_sandbox_denied;
#[cfg(test)]
use rexux_sandboxing::permission_profile_supports_windows_restricted_token_sandbox;
use rexux_sandboxing::record_filesystem_sandbox_violation;
#[cfg(test)]
use rexux_sandboxing::resolve_windows_elevated_filesystem_overrides;
#[cfg(test)]
use rexux_sandboxing::resolve_windows_restricted_token_filesystem_overrides;
#[cfg(test)]
use rexux_sandboxing::unsupported_windows_restricted_token_sandbox_reason;
#[cfg(any(test, target_os = "windows"))]
use rexux_sandboxing::windows_sandbox_uses_elevated_backend;
...
use rexux_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: rexux_protocol::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: rexux_protocol::config_types::WindowsSandboxLevel,
...
fn network_proxy_environment_error(
    ...
    RexuxErr::Io(io::Error::other(format!(
...
pub fn build_exec_request(
    ...
    let request = manager
        .transform(SandboxTransformRequest {
            ...
            rexux_linux_sandbox_exe: rexux_linux_sandbox_exe.as_deref(),
            use_legacy_landlock,
            windows_sandbox_level,
            windows_sandbox_private_desktop,
        })
        .map_err(RexuxErr::from)?;
...
pub(crate) async fn execute_exec_request(
    ...
    let cwd = cwd
        .to_abs_path()
        .map_err(|err| RexuxErr::InvalidRequest(format!("invalid exec cwd: {err}")))?;
    ...
    let windows_sandbox_policy_cwd = windows_sandbox_policy_cwd
        .to_abs_path()
        .map_err(|err| RexuxErr::InvalidRequest(format!("invalid sandbox cwd: {err}")))?;
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        rexux_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    };
    if let Some(metrics) = rexux_otel::global() {
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_rexux_home;
    use rexux_windows_sandbox::run_windows_sandbox_capture_for_permission_profile_elevated;
    use rexux_windows_sandbox::run_windows_sandbox_capture_with_filesystem_overrides;
    ...
    let (cancellation, timeout_ms) = if capture_policy.uses_expiration() {
        let cancellation = expiration.cancellation_token().map(|token| {
            rexux_windows_sandbox::WindowsSandboxCancellationToken::new(move || {
                token.is_cancelled()
            })
        });
    ...
    let rexux_home = find_rexux_home().map_err(|err| {
        RexuxErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve rexux_home: {err}"
        )))
    })?;
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_for_permission_profile_elevated(
                rexux_windows_sandbox::ElevatedSandboxProfileCaptureRequest {
                    permission_profile: &permission_profile,
                    workspace_roots: workspace_roots.as_slice(),
                    rexux_home: rexux_home.as_ref(),
                    command,
                    cwd: &cwd,
                    env_map: env,
                    timeout_ms,
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(RexuxErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, RexuxErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(RexuxErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(RexuxErr::Sandbox(SandboxErr::Timeout {
            ...
            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                ...
                return Err(RexuxErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
...
/// REXUX_SANDBOX_NETWORK_DISABLED=1 is added to the environment of the spawned
/// process.
///
            ...
            match outcome {
                ...
                Some(ExecExpirationOutcome::Cancelled) => {
                    ...
                    let should_escalate = if let Some(process_group_id) = process_group_id {
                        rexux_utils_pty::process_group::terminate_process_group(process_group_id)?
                    } else {
                        false
                    };
                    ...
                    .await
                    {
                        Ok(status) => {
                            ...
                            {
                                rexux_utils_pty::process_group::kill_process_group(
                                    process_group_id,
                                )?;
                            }
                        }
