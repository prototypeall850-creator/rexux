...
Set a stable path to the skill CLI (default `REXUX_HOME` is `~/.rexux`):

```
export REXUX_HOME="${REXUX_HOME:-$HOME/.rexux}"
export IMAGE_GEN="$REXUX_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Network/sandbox notes for fallback CLI mode: `references/rexux-network.md`
