...
impl OtelProvider {
    ...
    fn prepare_shutdown_worker(&mut self) -> io::Result<()> {
        self.prepare_shutdown_worker_with_spawner(|startup| {
            std::thread::Builder::new()
                .name("rexux-otel-shutdown".to_string())
                .spawn(move || {
    ...
    pub fn rexux_export_filter(meta: &tracing::Metadata<'_>) -> bool {
...
mod tests {
    ...
    fn log_export_target_excludes_trace_safe_events() {
        assert!(is_log_export_target("rexux_otel.log_only"));
        assert!(is_log_export_target("rexux_otel.network_proxy"));
        assert!(!is_log_export_target("rexux_otel.trace_safe"));
        assert!(!is_log_export_target("rexux_otel.trace_safe.debug"));
    }

    #[test]
    fn trace_export_target_only_includes_trace_safe_prefix() {
        assert!(is_trace_safe_target("rexux_otel.trace_safe"));
        assert!(is_trace_safe_target("rexux_otel.trace_safe.summary"));
        assert!(!is_trace_safe_target("rexux_otel.log_only"));
        assert!(!is_trace_safe_target("rexux_otel.network_proxy"));
    }
    ...
    fn test_otel_settings() -> OtelSettings {
        OtelSettings {
            ...
            service_name: "rexux-test".to_string(),
            service_version: "0.0.0".to_string(),
            rexux_home: PathBuf::from("."),
