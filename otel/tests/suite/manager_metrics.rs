...
use rexux_otel::PLUGIN_INSTALL_ELICITATION_SENT_METRIC;
use rexux_otel::PLUGIN_INSTALL_SUGGESTION_METRIC;
...
fn manager_attaches_metadata_tags_to_metrics() -> Result<()> {
    let (metrics, exporter) = build_metrics_with_defaults(&[("service", "rexux-cli")])?;
