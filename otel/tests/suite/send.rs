...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "rexux-cli"), ("env", "prod")])?;

    metrics.counter_with_description(
        "codex.turns",
        "Total number of Rexux turns.",
        /*inc*/ 1,
    ...
    metrics.gauge_with_description(
        "codex.active",
        "Number of active Rexux operations.",
        /*value*/ 2,
        &[("component", "test")],
    )?;
    metrics.shutdown()?;
    ...
    assert_eq!(counter.description(), "Total number of Rexux turns.");
    ...
    let expected_counter_attributes = BTreeMap::from([
        ("service".to_string(), "rexux-cli".to_string()),
    ...
    assert_eq!(gauge.description(), "Number of active Rexux operations.");
