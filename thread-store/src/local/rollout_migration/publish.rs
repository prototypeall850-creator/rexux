...
pub(super) fn migration_journal_path(rexux_home: &Path, thread_id: ThreadId) -> PathBuf {
    rexux_home
...
pub(super) async fn pending_migration_thread_ids(
    ...
    let mut entries = match tokio::fs::read_dir(rexux_home.join(MIGRATION_JOURNAL_DIRECTORY)).await
    {
        Ok(entries) => entries,
