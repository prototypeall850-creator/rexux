...
use rexux_rollout::ReverseJsonlScanner;
use rexux_rollout::RolloutItem;
use rexux_rollout::RolloutLine;
use rexux_rollout::ScanOutcome;
...
pub(super) async fn load_for_fork(
    ...
    let session_meta = rexux_rollout::read_session_meta_line(source_path)
        .await
        .map_err(|err| ThreadStoreError::Internal {
            message: format!(
...
fn scan_model_context_from_lineage_blocking(
    ...
    'segments: for segment in lineage.segments().iter().rev() {
        let file = rexux_rollout::open_rollout_seekable_reader(segment.rollout_path.as_path())?;
