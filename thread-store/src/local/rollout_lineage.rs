    ...
    async fn resolve_rollout_lineage_with_representation(
        ...
        loop {
            ...
            let rollout_path = match representation {
                ...
                LineageRepresentation::PlainForReference => {
                    let outside_rexux_home = || ThreadStoreError::InvalidRequest {
                        message: format!(
                            "rollout path `{}` must be in Rexux home directory",
                            rollout_path.display()
                        ),
                    };
                    let canonical_rollout_path = std::fs::canonicalize(rollout_path.as_path())
                        .map_err(|_| outside_rexux_home())?;
                    // Resume can retain either the logical Rexux home path or its canonical
                    // target. Keep references inside canonical managed roots so nested symlinks
                    // cannot escape them.
                    let is_managed_rollout = [
                        self.config.rexux_home.join(rexux_rollout::SESSIONS_SUBDIR),
                        self.config
                            .rexux_home
                            .join(rexux_rollout::ARCHIVED_SESSIONS_SUBDIR),
                    ]
                    .into_iter()
                    .filter_map(|root| std::fs::canonicalize(root).ok())
                    .any(|root| canonical_rollout_path.starts_with(root));
                    if !is_managed_rollout {
                        return Err(outside_rexux_home());
                    }
                    canonical_rollout_path
                }
            };
            let meta = rexux_rollout::read_session_meta_line(rollout_path.as_path())
                .await
            ...
            let rollout_path = match representation {
                    ...
                    if next_rollout_id.is_none() && meta.meta.history_base.is_none() =>
                {
                    ...
                    rexux_rollout::materialize_rollout_for_reference(rollout_path.as_path())
                        .await
                        .map_err(|err| ThreadStoreError::Internal {
                            message: format!(
...
async fn resolve_rollout_path_by_id(
    ...
    rexux_rollout::find_rollout_path_by_rollout_id(store.config.rexux_home.as_path(), rollout_id)
        .await
        .map_err(|err| ThreadStoreError::Internal {
...
async fn validate_cutoff_bounds(
    ...
    let contains_prefix = tokio::task::spawn_blocking(move || {
        rexux_rollout::rollout_contains_prefix(&path, end_byte_offset)
    })
    .await
    .map_err(|err| ThreadStoreError::Internal {
