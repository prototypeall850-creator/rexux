...
use rexux_protocol::realtime::BemItemPresentation;
use rexux_protocol::realtime::RealtimeItem;
use rexux_protocol::realtime::RealtimeItemContent;
use rexux_protocol::realtime::RealtimeSessionOutcome;
use rexux_protocol::realtime::RealtimeTranscriptRole;
use rexux_rollout::RolloutConfig;
use rexux_rollout::RolloutItem;
use rexux_rollout::RolloutLine;
use rexux_rollout::RolloutRecorder;
use rexux_rollout::RolloutRecorderParams;
...
/// Separate Rexux and SQLite homes must work together across startup backfill,
...
async fn split_homes_support_backfill_listing_and_paginated_history() {
    ...
    let rexux_home = root.path().join("codex");
    ...
    let runtime = rexux_rollout::state_db::try_init(&rollout_config)
        .await
        .expect("backfill state from Rexux home");
    assert!(
        runtime
    ...
    for sqlite_path in [&state_db_path, &thread_history_db_path] {
        ...
        assert!(
            !rexux_home.join(filename).exists(),
            "SQLite database should not be created under Rexux home"
        );
    }
}
...
async fn paginated_live_append_materializes_turn_items_and_state() {
    ...
    let pool = rexux_state::open_thread_history_db(&rexux_state::SqliteConfig::new_for_testing(
        home.path().abs(),
    ))
    .await
...
async fn paginated_fork_reads_compressed_shared_lineage_without_materializing() {
    ...
    assert!(matches!(
        error,
        crate::ThreadStoreError::InvalidRequest { message } if message.contains("must be in Rexux home")
    ));
    store
        .shutdown_thread(source_thread_id)
        .await
...
async fn summary_items_use_final_answers_and_ignore_commentary() {
    ...
    let config = test_config(home.path());
    let thread_id = ThreadId::default();
    let runtime = rexux_state::StateRuntime::init(
        config.sqlite.clone(),
...
async fn catch_up_rejects_invalid_complete_suffixes_without_advancing_state() {
    ...
    for (name, suffix) in cases {
        ...
        let pool = rexux_state::open_thread_history_db(
...
async fn projection_store(rexux_home: &Path) -> LocalThreadStore {
    let config = test_config(rexux_home);
