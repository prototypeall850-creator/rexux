...
use rexux_rollout::ThreadItem;
use rexux_rollout::find_thread_names_by_ids;
...
pub(super) fn rollout_path_is_archived(rexux_home: &Path, path: &Path) -> bool {
    path.starts_with(rexux_home.join(ARCHIVED_SESSIONS_SUBDIR))
        || path
            .components()
...
pub(super) fn validated_rollout_file_name(
    ...
    if rexux_rollout::rollout_id_from_path(rollout_path).is_some() {
        Ok(file_name)
    } else {
        Err(ThreadStoreError::InvalidRequest {
            message: format!(
...
pub(super) fn stored_thread_from_rollout_item(
    ...
    let preview = item
        ...
        .unwrap_or_default();
    let rollout_path = rexux_rollout::plain_rollout_path(item.path.as_path());
...
pub(super) async fn resolve_thread_names(
    ...
    if let Ok(legacy_names) =
        find_thread_names_by_ids(store.config.rexux_home.as_path(), &legacy_thread_ids).await
    {
