...
fn state(store: &LocalThreadStore) -> ThreadStoreResult<&rexux_rollout::StateDbHandle> {
    store
        .state_db
        .as_ref()
...
fn state_root(root: StoredProjectRoot) -> rexux_state::ProjectRoot {
    rexux_state::ProjectRoot { path: root.path }
}

fn stored_project(project: rexux_state::Project) -> StoredProject {
    StoredProject {
        id: project.id,
        name: project.name,
        roots: project
