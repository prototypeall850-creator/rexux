...
use rexux_rollout::persisted_rollout_items;
...
fn stored_thread_from_state(
    ...
    Ok(StoredThread {
        ...
        section: state
            ...
            .get(&thread_id)
            .cloned()
            .map(|id| rexux_state::ThreadSection {
                name: if id == rexux_state::PINNED_THREAD_SECTION_ID {
                    rexux_state::PINNED_THREAD_SECTION_NAME.to_string()
                } else {
                    id.clone()
                },
                id,
...
fn git_info_from_patch(patch: &ThreadMetadataPatch) -> Option<rexux_protocol::protocol::GitInfo> {
    ...
    Some(rexux_protocol::protocol::GitInfo {
