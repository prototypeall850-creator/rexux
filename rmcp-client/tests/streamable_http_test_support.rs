...
use rexux_exec_server::RemoteExecServerConnectArgs;
...
fn streamable_http_server_bin() -> Result<PathBuf, CargoBinError> {
    rexux_utils_cargo_bin::cargo_bin("test_streamable_http_server")
}
...
fn init_params() -> InitializeRequestParams {
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("rexux-test", "0.0.0-test").with_title("Rexux rmcp recovery test"),
    )
...
fn control_client() -> anyhow::Result<rexux_http_client::HttpClient> {
    Ok(rexux_http_client::HttpClientBuilder::new().build_direct()?)
}
