...
impl TestToolServer {
    fn new() -> Self {
        ...
        let entitlement_tools = std::env::var("MCP_TEST_DAYBREAK_READ_ONLY")
            ...
            .into_iter()
            .flat_map(|read_only| {
                ["get_rexux_security_daybreak_access", "get_daybreak_access"].map(|name| {
                    ...
                    if name == "get_rexux_security_daybreak_access" {
    ...
    /// Tool intended for manual testing of Rexux TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Rexux TUI):
    /// - Build this binary: `cargo build -p rexux-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `codex mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Rexux TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        ResourceTemplate::new("memo://codex/{slug}", "rexux-memo")
            .with_title("Rexux Memo")
...
/// Scenarios for `image_scenario`, intended to exercise Rexux TUI handling of MCP image outputs.
///
...
impl ServerHandler for TestToolServer {
    ...
    fn get_info(&self) -> ServerInfo {
        ...
        let server_info = match dynamic_server_process_label() {
            Some(process_label) => server_info
                .with_server_info(
                    Implementation::new("rexux-rmcp-test-server", env!("CARGO_PKG_VERSION"))
    ...
    async fn call_tool(
        ...
        match request.name.as_ref() {
            ...
            "sandbox_meta" | "get_rexux_security_daybreak_access" | "get_daybreak_access" => Ok(
