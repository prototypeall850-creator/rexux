...
//!
//! If the keyring is not available or fails, we fall back to REXUX_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "Rexux MCP Credentials";
...
fn load_oauth_tokens_from_secrets_keyring_with_lock_held<K: KeyringStore + Clone + 'static>(
    ...
    let rexux_home = find_rexux_home().map_err(anyhow::Error::from)?;
...
fn compute_store_key(server_name: &str, server_url: &str) -> Result<String> {
    ...
    let payload = if enterprise_owned {
        ...
        // isolated by Rexux profile as well as authenticated user and workspace.
        let rexux_home = find_rexux_home()?;
        fs::create_dir_all(&rexux_home)?;
        payload.insert(
            "rexux_home".to_string(),
            serde_json::to_value(rexux_home.as_path().canonicalize()?)?,
        );
...
fn fallback_file_path() -> Result<PathBuf> {
    Ok(find_rexux_home()?.join(FALLBACK_FILENAME).to_path_buf())
}
...
mod tests {
    ...
    fn fallback_file_is_private_at_creation() -> Result<()> {
        use std::os::unix::fs::PermissionsExt;
        const CHILD: &str = "REXUX_TEST_OAUTH_PERMISSIVE_UMASK";
