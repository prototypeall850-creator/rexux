...
use rexux_keyring_store::CredentialStoreError;
    ...
    fn load(&self, service: &str, account: &str) -> Result<Option<String>, CredentialStoreError> {
        ...
        match outcome {
            ...
            ReadOutcome::BackendError => Err(CredentialStoreError::new(
                rexux_keyring_store::KeyringError::PlatformFailure(
                    "credential backend unavailable".to_string(),
                ),
            )),
...
async fn refresh_subject_reread_is_cancellable_and_releases_guard_on_failure() -> Result<()> {
    ...
    for outcome in [
        ...
        // Drain the detached read before TempRexuxHome changes the process environment.
