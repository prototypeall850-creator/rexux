...
const LOCK_CONTENTION_EVENT_TARGET: &str = "rexux_rmcp_client::oauth::store_lock::contention";
...
impl OAuthStoreLock {
    ...
    fn acquire_with_timeout(
        ...
        // authority. Those stores are REXUX_HOME-backed today: if REXUX_HOME is unset they use
        // the default home (`~/.rexux`), and if an embedder has no local home/filesystem authority
        ...
        let rexux_home = find_rexux_home()
            .map_err(|source| OAuthStoreLockFailure::RexuxHome { store, source })?;
        Self::acquire_in_with_mode(&rexux_home, store, acquire_timeout, mode)
    }
    ...
    fn acquire_in_with_mode(
        ...
        let path = oauth_store_lock_path(rexux_home, store);
...
pub(super) enum OAuthStoreLockFailure {
    #[error("failed to resolve REXUX_HOME for MCP OAuth {store} aggregate-store lock")]
    RexuxHome {
        store: OAuthStore,
        #[source]
        source: io::Error,
...
fn oauth_store_lock_path(rexux_home: &Path, store: OAuthStore) -> PathBuf {
    rexux_home.join(OAUTH_LOCK_DIR).join(store.lock_filename())
}
