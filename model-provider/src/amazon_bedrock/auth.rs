...
use rexux_aws_auth::AwsAccessKeys;
use rexux_aws_auth::AwsAuthContext;
use rexux_aws_auth::AwsAuthError;
use rexux_aws_auth::AwsRequestToSign;
use rexux_http_client::Request;
use rexux_http_client::RequestBody;
use rexux_http_client::RequestCompression;
...
pub(super) fn auth_source(
        ...
        .is_some_and(|aws| aws.profile.is_some())
    {
        ...
        Some(RexuxAuth::BedrockApiKey(_))
    ) {
        BedrockAuthSource::ManagedBearerToken
    } else if matches!(
        auth_manager.and_then(AuthManager::auth_cached),
        Some(RexuxAuth::BedrockAccessKeys(_))
    ) {
...
pub(super) async fn resolve_auth_method(
    ...
    managed_auth: Option<&RexuxAuth>,
    aws: &ModelProviderAwsAuthInfo,
    endpoint: BedrockEndpoint,
) -> Result<BedrockAuthMethod> {
    match source {
        BedrockAuthSource::CommandBearerToken => Err(RexuxErr::Fatal(
            "Amazon Bedrock command authentication must be resolved by the model provider"
                .to_string(),
        )),
        BedrockAuthSource::ManagedBearerToken => {
            let Some(RexuxAuth::BedrockApiKey(auth)) = managed_auth else {
                return Err(RexuxErr::Fatal(
                    "selected Rexux-managed Amazon Bedrock API key is no longer available"
                        .to_string(),
                ));
            };
        ...
        BedrockAuthSource::ConfiguredAwsProfile => {
            ...
            let context = AwsAuthContext::load_profile(config)
                .await
                .map_err(aws_auth_error_to_rexux_error)?;
            Ok(BedrockAuthMethod::AwsSdkAuth { context })
        }
        BedrockAuthSource::ManagedAccessKeys => {
            let Some(RexuxAuth::BedrockAccessKeys(auth)) = managed_auth else {
                return Err(RexuxErr::Fatal(
                    "selected Rexux-managed Amazon Bedrock access keys are no longer available"
                        .to_string(),
                ));
            };
...
fn aws_auth_error_to_rexux_error(error: AwsAuthError) -> RexuxErr {
    RexuxErr::Fatal(format!("failed to resolve Amazon Bedrock auth: {error}"))
}
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    ...
    // richer Rexux agent requests fail even though raw Responses requests work.
...
impl AuthProvider for BedrockSigV4AuthProvider {
    ...
    fn apply_auth(&self, request: Request) -> rexux_api::AuthProviderFuture<'_> {
...
mod tests {
    ...
    fn bedrock_auth_source_distinguishes_static_environment_credentials() {
        ...
        let managed_auth =
            AuthManager::from_auth_for_testing(RexuxAuth::BedrockApiKey(BedrockApiKeyAuth {
                api_key: "managed-bedrock-api-key".to_string(),
                region: "us-east-1".to_string(),
            }));
        let managed_access_keys = AuthManager::from_auth_for_testing(RexuxAuth::BedrockAccessKeys(
