...
pub(super) const BEDROCK_EXPIRED_SIGNATURE_MESSAGE: &str = concat!(
    ...
    "update or unset it, then restart Rexux",
);

pub(super) fn map_api_error(error: ApiError) -> RexuxErr {
    let error = rexux_api::map_api_error(error);
    if let RexuxErrorDetails::UnexpectedStatus(response) = error.details()
        ...
        && response.body.contains("Signature expired:")
    {
        ...
        let mapped_error = RexuxErr::new(RexuxErrorDetails::UnexpectedStatus(response));
