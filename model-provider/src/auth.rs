...
use rexux_agent_identity::AgentIdentityKey;
use rexux_agent_identity::authorization_header_for_agent_task;
use rexux_api::AgentIdentityTelemetry;
use rexux_api::AuthError;
use rexux_api::AuthHeadersFuture;
...
use rexux_login::auth::AgentIdentityAuthError;
...
struct AuthManagerAuthProvider {
    ...
    expected_auth: RexuxAuth,
}

impl AuthManagerAuthProvider {
    fn is_expected_auth(&self, auth: &RexuxAuth) -> bool {
        auth.uses_rexux_backend()
    ...
    fn current_auth(&self) -> Option<RexuxAuth> {
        self.auth_manager
            .auth_cached()
...
pub(crate) fn resolve_provider_auth(
    ...
    provider: &ModelProviderInfo,
) -> rexux_protocol::error::Result<SharedAuthProvider> {
    ...
    if matches!(
        auth,
        Some(RexuxAuth::BedrockApiKey(_) | RexuxAuth::BedrockAccessKeys(_))
    ) {
...
) -> rexux_protocol::error::Result<ResolvedProviderAuth> {
    let ProviderAuthScope {
        agent_identity_policy,
        session_source,
        agent_identity_session_fallback,
    } = scope;
    if let Some(RexuxAuth::AgentIdentity(agent_identity_auth)) = auth {
...
fn should_bootstrap_chatgpt_agent_identity(
    ...
    agent_identity_policy == AgentIdentityAuthPolicy::ChatGptAuth
        && matches!(auth, Some(RexuxAuth::Chatgpt(_)))
}

fn bearer_auth_for_provider(
    provider: &ModelProviderInfo,
) -> rexux_protocol::error::Result<Option<BearerAuthProvider>> {
...
/// Builds request-header auth for a first-party Rexux auth snapshot.
pub fn auth_provider_from_auth(auth: &RexuxAuth) -> SharedAuthProvider {
    match auth {
        RexuxAuth::AgentIdentity(auth) => {
            Arc::new(AgentIdentityAuthProvider { auth: auth.clone() })
        }
        RexuxAuth::Headers(auth) => Arc::new(HeaderAuthProvider { auth: auth.clone() }),
        ...
        | RexuxAuth::PersonalAccessToken(_) => Arc::new(BearerAuthProvider {
...
#[cfg(test)]
mod tests {
    use rexux_agent_identity::generate_agent_key_material;
    ...
    use rexux_login::auth::login_with_chatgpt_auth_tokens;
    ...
    fn test_rexux_home() -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "rexux-model-provider-agent-identity-{pid}-{id}",
            pid = std::process::id()
        ));
    ...
    fn write_chatgpt_auth_json(rexux_home: &Path) {
        let auth_json = json!({
            "tokens": {
    ...
    ) -> (PathBuf, Arc<AuthManager>, RexuxAuth) {
        let rexux_home = test_rexux_home();
        write_chatgpt_auth_json(&rexux_home);
    ...
    fn custom_provider_does_not_inherit_ambient_auth_headers() {
        ...
        let ambient_auth = RexuxAuth::Headers(AuthHeaders::new(ambient_headers));
    ...
    fn custom_provider_does_not_inherit_ambient_bedrock_auth() {
        let provider =
            create_oss_provider_with_base_url("http://localhost:11434/v1", WireApi::Responses);
        let ambient_auth = RexuxAuth::BedrockApiKey(BedrockApiKeyAuth {
    ...
    fn custom_provider_uses_command_resolved_auth() {
        ...
        let command_auth = RexuxAuth::from_api_key("command-token");
    ...
    fn openai_provider_preserves_ambient_auth_headers() {
        ...
        let ambient_auth = RexuxAuth::Headers(AuthHeaders::new(expected.clone()));
    ...
    fn header_auth_adds_predefined_headers() {
        ...
        let auth = RexuxAuth::Headers(AuthHeaders::new(expected.clone()));
    ...
    async fn first_party_run_scope_uses_agent_assertion_and_exposes_telemetry() {
        let auth = RexuxAuth::AgentIdentity(
    ...
    async fn agent_identity_auth_provider_preserves_account_routing_headers() {
        ...
        let provider = auth_provider_from_auth(&RexuxAuth::AgentIdentity(auth));
    ...
    async fn chatgpt_bootstrap_unavailable_uses_session_bearer_fallback() {
        ...
        .await;
        let (_rexux_home, auth_manager, auth) = chatgpt_auth_manager(server.uri()).await;
        let provider = ModelProviderInfo::create_openai_provider(/*base_url*/ None);
