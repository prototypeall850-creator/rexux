...
use rexux_login::default_client::ResidencyRequirement;
use rexux_login::default_client::read_default_client_residency_requirement;
use rexux_model_provider_info::ModelProviderInfo;
use rexux_models_manager::cache::ModelsCache;
use rexux_models_manager::manager::OpenAiModelsManager;
use rexux_models_manager::manager::SharedModelsManager;
use rexux_models_manager::manager::StaticModelsManager;
use rexux_protocol::account::ProviderAccount;
...
/// Optional provider-backed features that Rexux may expose at runtime.
///
...
pub const DEFAULT_APPROVAL_REVIEW_PREFERRED_MODEL: &str = "rexux-auto-review";
...
pub trait ModelProvider: fmt::Debug + Send + Sync {
    ...
    /// to think through whether Rexux should have a unified provider-specific auth
    ...
    fn map_api_error(&self, error: ApiError) -> RexuxErr {
        rexux_api::map_api_error(error)
    }

    /// Returns provider configuration adapted for the API client.
    fn api_provider(&self) -> ModelProviderFuture<'_, rexux_protocol::error::Result<Provider>> {
        Box::pin(async move {
            let auth = self.auth().await;
            let mut provider = self
                .info()
                .to_api_provider(auth.as_ref().map(RexuxAuth::auth_mode))?;
    ...
    fn runtime_base_url(
        &self,
    ) -> ModelProviderFuture<'_, rexux_protocol::error::Result<Option<String>>> {
    ...
    fn api_auth(
        &self,
    ) -> ModelProviderFuture<'_, rexux_protocol::error::Result<SharedAuthProvider>> {
        Box::pin(async move {
            let auth = self.auth().await;
    ...
    /// Returns request credentials, optionally scoped to a Rexux session task.
    fn api_auth_for_scope(
        &self,
        scope: ProviderAuthScope,
    ) -> ModelProviderFuture<'_, rexux_protocol::error::Result<ResolvedProviderAuth>> {
        Box::pin(async move {
    ...
    fn models_manager_without_cache(
        ...
        let model_catalog = config_model_catalog
            .or_else(|| rexux_models_manager::bundled_models_response().ok())
            .unwrap_or_default();
        ...
        let account = if self.info.requires_openai_auth {
            self.auth_manager
                ...
                .and_then(|auth_manager| {
                    ...
                    if matches!(auth, RexuxAuth::Headers(_)) {
                        return None;
                    }
                    Some(auth)
                })
                .map(|auth| match &auth {
                    RexuxAuth::ApiKey(_) => Ok(ProviderAccount::ApiKey),
                    RexuxAuth::BedrockApiKey(_) | RexuxAuth::BedrockAccessKeys(_) => {
...
mod tests {
    ...
    use rexux_model_provider_info::AwsAuthRefreshConfig;
    use rexux_model_provider_info::ModelProviderAwsAuthInfo;
    ...
    use rexux_utils_redacted_string::RedactedString;
    ...
    fn test_rexux_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("rexux-model-provider-test-{}", std::process::id()))
    }
    ...
    fn bedrock_api_key_auth() -> RexuxAuth {
        RexuxAuth::BedrockApiKey(BedrockApiKeyAuth {
    ...
    fn create_model_provider_does_not_use_openai_auth_manager_for_amazon_bedrock_provider() {
        let provider = create_model_provider(
            ModelProviderInfo::create_amazon_bedrock_provider(Some(ModelProviderAwsAuthInfo {
                profile: Some("rexux-bedrock".to_string()),
                region: None,
                auth_refresh: None,
            })),
            Some(AuthManager::from_auth_for_testing(RexuxAuth::from_api_key(
    ...
    async fn shared_bedrock_auth_refresh_is_reused_only_for_matching_configuration() {
        ...
        const HELPER_ARG: &str = "REXUX_BEDROCK_SHARED_AUTH_REFRESH_COMMAND";
        const SUBPROCESS_ARG: &str = "REXUX_BEDROCK_SHARED_AUTH_REFRESH_SUBPROCESS";
        ...
        assert_eq!(
            ...
            Ok(ProviderAccountState {
                account: Some(ProviderAccount::AmazonBedrock {
                    uses_rexux_managed_credentials: false,
                }),
                requires_openai_auth: false,
            })
        );
    ...
    async fn amazon_bedrock_provider_creates_static_models_manager() {
        ...
        let manager =
            provider.models_manager(test_rexux_home(), /*config_model_catalog*/ None);
    ...
    async fn configured_bedrock_catalog_only_allows_default_service_tier() {
        let configured_model = rexux_models_manager::bundled_models_response()
