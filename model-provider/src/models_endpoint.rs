...
use rexux_login::collect_auth_env_telemetry;
use rexux_login::default_client::create_client_for_route_async;
use rexux_model_provider_info::ModelProviderInfo;
use rexux_models_manager::manager::ModelsEndpointClient;
use rexux_models_manager::manager::ModelsEndpointFuture;
use rexux_otel::TelemetryAuthMode;
use rexux_protocol::error::RexuxErr;
use rexux_protocol::error::Result as CoreResult;
...
impl OpenAiModelsEndpoint {
    ...
    async fn auth(&self) -> Option<RexuxAuth> {
    ...
    async fn uses_rexux_backend(&self) -> bool {
        self.auth()
            .await
            .as_ref()
            .is_some_and(RexuxAuth::uses_rexux_backend)
    }
    ...
    async fn list_models(
        ...
        let _timer =
            rexux_otel::start_global_timer("codex.remote_models.fetch_update.duration_ms", &[]);
        let auth = self.auth().await;
        let auth_mode = auth.as_ref().map(RexuxAuth::auth_mode);
        ...
        let agent_identity_telemetry = if let Some(RexuxAuth::AgentIdentity(auth)) = auth.as_ref() {
        ...
        .await
        .map_err(|_| RexuxErr::Timeout)?
    }

    fn auth_env(&self) -> AuthEnvTelemetry {
        let rexux_api_key_env_enabled = self
            .auth_manager
            .as_ref()
            .is_some_and(|auth_manager| auth_manager.rexux_api_key_env_enabled());
        collect_auth_env_telemetry(&self.provider_info, rexux_api_key_env_enabled)
    }
}
...
impl ModelsEndpointClient for OpenAiModelsEndpoint {
    ...
    fn uses_rexux_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(OpenAiModelsEndpoint::uses_rexux_backend(self))
    }
...
impl RequestTelemetry for ModelsRequestTelemetry {
    fn on_request(
        ...
        tracing::event!(
            ...
            auth.env_rexux_api_key_present = self.auth_env.rexux_api_key_env_present,
            auth.env_rexux_api_key_enabled = self.auth_env.rexux_api_key_env_enabled,
...
mod tests {
    ...
    use rexux_login::default_client::create_client;
