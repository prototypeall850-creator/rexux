...
- a `rexux-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Rexux CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `rexux-exec` CLI can check if its arg0 is `rexux-linux-sandbox` and, if so, execute as if it were `rexux-linux-sandbox`
  - this should also be true of the `rexux` multitool CLI

On Linux, Rexux prefers the first `bwrap` found on `PATH`
...
the helper falls back to the bundled `rexux-resources/bwrap` binary shipped
with Rexux.
Rexux also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. Rexux surfaces the same startup warning
...
it cannot create the required user namespaces, so Rexux rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the bundled
  `rexux-resources/bwrap` path.
- If `bwrap` is missing, Rexux also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, Rexux surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; Rexux rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.rexux`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface is `rexux sandbox`; the host OS selects the sandbox backend.
