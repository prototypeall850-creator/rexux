...
use rexux_core::exec::process_exec_tool_call;
...
fn rexux_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_rexux-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> rexux_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<rexux_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let rexux_linux_sandbox_exe = Some(rexux_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(output: &rexux_protocol::exec_output::ExecToolCallOutput) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(err) => match err.details() {
            ...
            RexuxErrorDetails::Sandbox(SandboxErr::Timeout { .. }) => true,
...
fn expect_denied(
    result: Result<rexux_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> rexux_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let rexux_linux_sandbox_exe: Option<PathBuf> = Some(rexux_linux_sandbox_exe());
...
#[tokio::test]
async fn sandbox_blocks_git_and_rexux_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_codex = tmpdir.path().join(".rexux");
    ...
    let rexux_target = dot_codex.join("config.toml");
    ...
    let rexux_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", rexux_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
    ...
    assert_ne!(rexux_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_rexux_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
...
#[tokio::test]
async fn sandbox_reports_rexux_symlink_build_failure_without_panicking() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
...
async fn sandbox_rejects_symlinked_synthetic_mount_registry() {
    ...
    let registry = temp.path().join(format!(
        "rexux-bwrap-synthetic-mount-targets-{effective_uid}"
    ));
...
registry="${{TMPDIR:-/tmp}}/rexux-bwrap-synthetic-mount-targets-$(id -u)"
if touch "$registry/forged-marker" 2>/dev/null; then
  exit 22
fi
redirected_registry='{redirected_tmp}'/rexux-bwrap-synthetic-mount-targets-$(id -u)
...
"#,
    ...
    let mkdir_rexux_output = expect_denied(
        run_cmd_result_with_cwd_and_writable_roots(
            &["mkdir", ".rexux"],
            &subdir,
            std::slice::from_ref(&subdir),
    ...
    assert_ne!(mkdir_rexux_output.exit_code, 0);
    assert!(!subdir.join(".rexux").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = rexux_linux_sandbox_exe()
        .parent()
...
async fn sandbox_starts_with_denied_tmp_without_exposing_registry() {
    ...
    let sandbox_helper = rexux_linux_sandbox_exe();
...
registry="$TMPDIR/rexux-bwrap-synthetic-mount-targets-$(id -u)"
test ! -e "$registry"
"#,
                ],
                cwd.clone(),
