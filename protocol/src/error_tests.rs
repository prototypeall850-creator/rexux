...
use chrono::TimeZone;
use chrono::Utc;
use rexux_http_client::HttpResponse;
...
#[test]
fn rexux_err_debug_preserves_legacy_shape() {
    let actual = [
        RexuxErr::Timeout,
        RexuxErr::Stream("disconnected".to_string()),
        RexuxErr::Stream("retry later".to_string()).with_retry_delay(Duration::from_secs(2)),
        RexuxErr::InternalServerError.with_retry_delay(Duration::from_secs(3)),
    ]
    .map(|err| format!("{err:?}"));
...
fn retryability_preserves_error_details_distinctions() {
    let errors = [
        (RexuxErr::ServerOverloaded, false),
        (
            RexuxErr::new(RexuxErrorDetails::RateLimitExceeded("retry later".into())),
            true,
        ),
        (
        ...
        (
            RexuxErrorDetails::ToolCollision("functions.update_plan".to_string()).into(),
            false,
        ),
        (RexuxErr::InternalServerError, true),
    ];
...
fn server_overloaded_maps_to_protocol() {
    let err = RexuxErr::ServerOverloaded;
    assert_eq!(
        err.to_rexux_protocol_error(),
...
fn sandbox_denied_uses_aggregated_output_when_stderr_empty() {
    ...
    let err = RexuxErr::Sandbox(SandboxErr::Denied {
...
fn to_error_event_handles_response_stream_failed() {
    ...
    let err = RexuxErr::ResponseStreamFailed(ResponseStreamFailed {
        source,
    ...
    assert_eq!(
        event.rexux_error_info,
        Some(RexuxErrorInfo::ResponseStreamConnectionFailed {
...
fn usage_limit_reached_error_formats_free_plan() {
    ...
    assert_eq!(
        err.to_string(),
        "You've hit your usage limit. Upgrade to Plus to continue using Rexux (https://chatgpt.com/explore/plus), or try again later."
    );
}
...
#[test]
fn usage_limit_reached_error_hides_upsell_for_non_rexux_limit_name() {
    let base = Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap();
    let resets_at = base + ChronoDuration::hours(1);
    with_now_override(base, move || {
        let expected_time = format_retry_timestamp(&resets_at);
        ...
        let expected = format!(
            "You've hit your usage limit for rexux_other. Switch to another model now, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
fn usage_limit_reached_with_promo_message() {
    ...
    with_now_override(base, move || {
        ...
        let err = UsageLimitReachedError {
            ...
            rate_limits: Some(Box::new(rate_limit_snapshot())),
            promo_message: Some(
                "To continue using Rexux, start a free trial of <PLAN> today".to_string(),
            ),
            rate_limit_reached_type: None,
        };
        let expected = format!(
            "You've hit your usage limit. To continue using Rexux, start a free trial of <PLAN> today, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
