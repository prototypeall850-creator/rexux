...
//! `schemars`) so they can be embedded in Rexux's own protocol structures.
...
/// MCP extensions supplied by the client that created a Rexux session.
...
// === Adapter helpers ===
//
// These types and conversions intentionally live in `rexux-protocol` so other crates can convert
