//! Defines the protocol for a Rexux session between a client and an agent.
//!
...
use crate::error::Result as RexuxResult;
...
use rexux_extension_items::image_generation::ImageGenerationFailure;
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Rexux:";
...
pub struct ConversationStartParams {
    /// Whether Rexux response handoffs are managed through explicit client append calls.
    ...
    /// Whether to route any remaining transcript tail through Rexux when the session ends.
    ...
    /// Sends automatic Rexux responses as realtime conversation items instead of handoff appends.
    pub rexux_responses_as_items: bool,
    /// Optional prefix added to automatic Rexux response items when `rexux_responses_as_items` is set.
    pub rexux_response_item_prefix: Option<String>,
    /// Selects how automatic Rexux handoffs are routed in Frameless Bidi sessions.
    /// Realtime V1 and V2 ignore this setting.
    pub rexux_response_handoff_mode: RexuxResponseHandoffMode,
    /// Optional client-selected BEM prefixes keyed by `analysis`, `commentary`, and `final`.
    pub rexux_response_handoff_channel_prefixes: Option<BTreeMap<String, Vec<String>>>,
    /// Overrides the configured realtime model for this session only.
    pub model: Option<String>,
    /// Selects whether the realtime session should produce text or audio output.
    pub output_modality: RealtimeOutputModality,
    /// Whether to append Rexux's startup context to the realtime backend prompt.
    ...
    /// Developer instructions given to Rexux when this realtime session starts.
    pub realtime_start_instructions: Option<String>,
    /// Developer instructions given to Rexux when this realtime session ends.
...
pub enum Op {
    ...
    TurnInput {
        ...
        mode: TurnInputMode,
        reply: oneshot::Sender<RexuxResult<TurnInputSubmission>>,
    },
    ...
    SuspendTurnAndShutdown {
        reply: oneshot::Sender<RexuxResult<SuspendTurnOutcome>>,
    },
    ...
    /// Request Rexux to drop the last N user turns from in-memory context.
    ///
...
/// running the command proposed by Rexux.
#[derive(
    Debug,
    Clone,
    Copy,
...
#[serde(rename_all = "camelCase")]
#[ts(rename_all = "camelCase")]
pub enum RexuxResponseHandoffMode {
    #[default]
    Thinking,
    Commentary,
    BemTags,
...
/// Rexux errors that we expose to clients.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "snake_case")]
#[ts(rename_all = "snake_case")]
pub enum RexuxErrorInfo {
...
pub struct ErrorEvent {
    pub message: String,
    #[serde(default)]
    pub rexux_error_info: Option<RexuxErrorInfo>,
...
impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.rexux_error_info
            .as_ref()
            .is_none_or(RexuxErrorInfo::affects_turn_status)
    }
}
...
pub struct TokenUsage {
    ...
    #[schemars(skip)]
    #[ts(skip)]
    pub rexux_rollout_budget_units: Option<serde_json::Number>,
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::Rexux),
...
pub struct TurnContextItem {
    ...
    // Compatibility-only field written with a default value so older Rexux
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(rexux_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                rexux_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
mod tests {
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::Rexux])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::Rexux]));
        assert!(
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd = rexux_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
        ...
        let expected_blocked = AbsolutePathBuf::from_absolute_path(
            rexux_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd")
                .join("blocked"),
        )
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".rexux")).expect("create .codex");
        let canonical_cwd = rexux_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
            .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_codex = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".rexux"))
    ...
    fn restricted_file_system_policy_treats_read_entries_as_read_only_subpaths() {
        ...
        let expected_dot_codex = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".rexux"))
            .expect("canonical .codex");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
    ...
    fn generic_error_affects_turn_status() {
        let event = ErrorEvent {
            ...
            rexux_error_info: Some(RexuxErrorInfo::Other),
        };
        assert!(event.affects_turn_status());
    }
    ...
    fn misalignment_explanation_and_steer_are_never_serialized_into_error_events() {
        let event = ErrorEvent {
            ...
            rexux_error_info: Some(RexuxErrorInfo::MisalignmentPolicyViolation),
        ...
        assert_eq!(
            ...
            json!({
                ...
                "rexux_error_info": "misalignment_policy_violation"
            })
        );
