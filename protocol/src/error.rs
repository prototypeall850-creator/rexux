...
use crate::protocol::RexuxErrorInfo;
...
use rexux_http_client::HttpError;
use rexux_utils_string::truncate_middle_chars;
use rexux_utils_string::truncate_middle_with_token_budget;
use http::StatusCode;
use serde_json;
use std::fmt;
use std::io;
...
pub type Result<T> = std::result::Result<T, RexuxErr>;
...
pub struct RexuxErr {
    details: RexuxErrorDetails,
    retry_delay: Option<Duration>,
}

/// The semantic category and diagnostic payload for a [`RexuxErr`].
#[derive(Error, Debug, EnumDiscriminants)]
#[strum_discriminants(name(RexuxErrKind))]
...
pub enum RexuxErrorDetails {
    ...
    #[error(
        "Rexux ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `RexuxError::Spawn` variant.
    ...
    QuotaExceeded,
    #[error(
        "To use Rexux with your ChatGPT plan, upgrade to Plus: https://chatgpt.com/explore/plus."
    )]
    UsageNotIncluded,
    ...
    #[error("rexux-linux-sandbox was required but not provided")]
...
impl From<&RexuxErr> for RexuxErrKind {
    fn from(error: &RexuxErr) -> Self {
        error.details().into()
    }
}

impl fmt::Debug for RexuxErr {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match &self.details {
            RexuxErrorDetails::Stream(message) => formatter
                .debug_tuple("Stream")
                .field(message)
...
impl fmt::Display for RexuxErr {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::Display::fmt(&self.details, formatter)
    }
}

impl std::error::Error for RexuxErr {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        self.details.source()
    }
}

impl From<RexuxErrorDetails> for RexuxErr {
    fn from(details: RexuxErrorDetails) -> Self {
        Self {
            details,
            retry_delay: None,
        }
...
impl From<CancelErr> for RexuxErr {
    fn from(error: CancelErr) -> Self {
        RexuxErrorDetails::from(error).into()
    }
}

impl From<SandboxErr> for RexuxErr {
    fn from(error: SandboxErr) -> Self {
        RexuxErrorDetails::from(error).into()
    }
}

impl From<io::Error> for RexuxErr {
    fn from(error: io::Error) -> Self {
        RexuxErrorDetails::from(error).into()
    }
}

impl From<serde_json::Error> for RexuxErr {
    fn from(error: serde_json::Error) -> Self {
        RexuxErrorDetails::from(error).into()
    }
}

impl From<JoinError> for RexuxErr {
...
#[cfg(target_os = "linux")]
impl From<landlock::RulesetError> for RexuxErr {
...
#[cfg(target_os = "linux")]
impl From<landlock::PathFdError> for RexuxErr {
    fn from(error: landlock::PathFdError) -> Self {
        RexuxErrorDetails::from(error).into()
    }
}

impl From<CancelErr> for RexuxErrorDetails {
    fn from(_: CancelErr) -> Self {
        RexuxErrorDetails::TurnAborted
    }
}

// TODO(anp): Remove this compatibility macro once callers construct
// `RexuxErrorDetails` directly.
macro_rules! rexux_err_unit_constructors {
    ($($variant:ident),* $(,)?) => {
        $(
            #[doc(hidden)]
            #[allow(non_upper_case_globals)]
            pub const $variant: Self = Self {
                details: RexuxErrorDetails::$variant,
                retry_delay: None,
            };
        )*
    };
...
// TODO(anp): Remove this compatibility macro once callers construct
// `RexuxErrorDetails` directly.
macro_rules! rexux_err_tuple_constructors {
    ($($(#[$attr:meta])* $variant:ident($value:ident: $value_type:ty)),* $(,)?) => {
        $(
            ...
            pub fn $variant($value: $value_type) -> Self {
                RexuxErrorDetails::$variant($value).into()
            }
        )*
    };
}
...
impl RexuxErr {
    ...
    pub fn InvalidImageRequest() -> Self {
        RexuxErrorDetails::InvalidImageRequest().into()
    }

    /// Creates an error with no server-provided retry delay.
    pub fn new(details: RexuxErrorDetails) -> Self {
        details.into()
    }

    /// Returns the semantic failure and its diagnostic payload.
    pub fn details(&self) -> &RexuxErrorDetails {
        &self.details
    }
    ...
    pub fn is_retryable(&self) -> bool {
        match self.details() {
            RexuxErrorDetails::TurnAborted
            | RexuxErrorDetails::SessionBudgetExceeded
            | RexuxErrorDetails::Interrupted
            | RexuxErrorDetails::EnvVar(_)
            | RexuxErrorDetails::Fatal(_)
            | RexuxErrorDetails::UsageNotIncluded
            | RexuxErrorDetails::QuotaExceeded
            | RexuxErrorDetails::InvalidImageRequest()
            | RexuxErrorDetails::InvalidRequest(_)
            | RexuxErrorDetails::ToolCollision(_)
            | RexuxErrorDetails::RefreshTokenFailed(_)
            | RexuxErrorDetails::UnsupportedOperation(_)
            | RexuxErrorDetails::Sandbox(_)
            | RexuxErrorDetails::LandlockSandboxExecutableNotProvided
            | RexuxErrorDetails::RetryLimit(_)
            | RexuxErrorDetails::ContextWindowExceeded
            | RexuxErrorDetails::ThreadNotFound(_)
            | RexuxErrorDetails::AgentLimitReached { .. }
            | RexuxErrorDetails::Spawn
            | RexuxErrorDetails::SessionConfiguredNotFirstEvent
            | RexuxErrorDetails::UsageLimitReached(_)
            | RexuxErrorDetails::ServerOverloaded
            | RexuxErrorDetails::CyberPolicy { .. }
            | RexuxErrorDetails::MisalignmentPolicyViolation { .. } => false,
            RexuxErrorDetails::Stream(..)
            | RexuxErrorDetails::RateLimitExceeded(_)
            | RexuxErrorDetails::Timeout
            | RexuxErrorDetails::RequestTimeout
            | RexuxErrorDetails::UnexpectedStatus(_)
            | RexuxErrorDetails::ResponseStreamFailed(_)
            | RexuxErrorDetails::ConnectionFailed(_)
            | RexuxErrorDetails::InternalServerError
            | RexuxErrorDetails::InternalAgentDied
            | RexuxErrorDetails::Io(_)
            | RexuxErrorDetails::Json(_)
            | RexuxErrorDetails::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            RexuxErrorDetails::LandlockRuleset(_) | RexuxErrorDetails::LandlockPathFd(_) => false,
        }
    }
    ...
    /// Minimal shim so that existing `e.downcast_ref::<RexuxErr>()` checks continue to compile
    ...
    pub fn to_rexux_protocol_error(&self) -> RexuxErrorInfo {
        match &self.details {
            RexuxErrorDetails::ContextWindowExceeded => RexuxErrorInfo::ContextWindowExceeded,
            RexuxErrorDetails::SessionBudgetExceeded => RexuxErrorInfo::SessionBudgetExceeded,
            RexuxErrorDetails::RateLimitExceeded(_) => RexuxErrorInfo::RateLimitExceeded,
            RexuxErrorDetails::UsageLimitReached(_)
            | RexuxErrorDetails::QuotaExceeded
            | RexuxErrorDetails::UsageNotIncluded => RexuxErrorInfo::UsageLimitExceeded,
            RexuxErrorDetails::ServerOverloaded => RexuxErrorInfo::ServerOverloaded,
            RexuxErrorDetails::CyberPolicy { .. } => RexuxErrorInfo::CyberPolicy,
            RexuxErrorDetails::MisalignmentPolicyViolation { .. } => {
                RexuxErrorInfo::MisalignmentPolicyViolation
            }
            RexuxErrorDetails::RetryLimit(_) => RexuxErrorInfo::ResponseTooManyFailedAttempts {
                http_status_code: self.http_status_code_value(),
            },
            RexuxErrorDetails::ConnectionFailed(_) => RexuxErrorInfo::HttpConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            RexuxErrorDetails::ResponseStreamFailed(_) => {
                RexuxErrorInfo::ResponseStreamConnectionFailed {
                    http_status_code: self.http_status_code_value(),
                }
            }
            RexuxErrorDetails::RefreshTokenFailed(_) => RexuxErrorInfo::Unauthorized,
            RexuxErrorDetails::SessionConfiguredNotFirstEvent
            | RexuxErrorDetails::InternalServerError
            | RexuxErrorDetails::InternalAgentDied => RexuxErrorInfo::InternalServerError,
            RexuxErrorDetails::UnsupportedOperation(_)
            | RexuxErrorDetails::ThreadNotFound(_)
            | RexuxErrorDetails::AgentLimitReached { .. } => RexuxErrorInfo::BadRequest,
            RexuxErrorDetails::Sandbox(_) => RexuxErrorInfo::SandboxError,
            _ => RexuxErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            rexux_error_info: Some(self.to_rexux_protocol_error()),
            misalignment: match &self.details {
                RexuxErrorDetails::MisalignmentPolicyViolation { misalignment, .. } => {
                    misalignment.clone()
                }
                _ => None,
            },
    ...
    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match &self.details {
            RexuxErrorDetails::RetryLimit(err) => Some(err.status),
            RexuxErrorDetails::UnexpectedStatus(err) => Some(err.status),
            RexuxErrorDetails::ConnectionFailed(err) => err.source.status(),
            RexuxErrorDetails::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let message = match self.plan_type.as_ref() {
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using Rexux (https://chatgpt.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
...
pub fn get_error_message_ui(e: &RexuxErr) -> String {
    let message = match e.details() {
        RexuxErrorDetails::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        RexuxErrorDetails::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
