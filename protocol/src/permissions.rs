...
const PROTECTED_METADATA_REXUX_PATH_NAME: &str = ".rexux";
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_REXUX_PATH_NAME,
];
...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer Rexux degrades to warn-and-ignore
    /// instead of failing to load. Rexux 0.112.0 rejected unknown values here,
...
const PROJECT_ROOTS_GLOB_PATTERN_PREFIX: &str = "rexux-project-roots://";
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        append_default_read_only_project_root_subpath_if_no_explicit_rule(&mut entries, ".rexux");
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    let top_level_codex = writable_root.join(PROTECTED_METADATA_REXUX_PATH_NAME);
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "REXUX_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    fn uri_deny_and_metadata_matcher_use_executor_paths() {
        ...
        assert!(!policy.can_write_path(&path("file:///C:/workspace/.rexux/config"), &context));
        assert_eq!(
            policy.metadata_write_denial(&path("file:///C:/workspace/.rexux/config"), &context),
            Some(".rexux"),
        );
    }
    ...
    fn writable_roots_proactively_protect_missing_dot_codex() {
        ...
        let expected_dot_codex = expected_root.join(".rexux");
    ...
    fn legacy_workspace_write_projection_preserves_symbolic_project_root() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry::skip_missing_path(
                    FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(Some(".rexux".into())),
                    },
                    FileSystemAccessMode::Read,
                ),
            ])
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_dot_rexux_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let explicit_dot_codex = expected_root.join(".rexux");
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_rexux_config = cwd.path().join(".rexux").join("config.toml");
        ...
        assert!(!file_system_policy.can_write_path_with_cwd(&dot_rexux_config, cwd.path()));
        ...
        assert!(!writable_roots[0].is_path_writable(&dot_rexux_config));
    }
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let rexux_dir = real_root.join(".rexux");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&rexux_dir).expect("create .codex");
        ...
        let expected_codex = link_root.join(".rexux");
    ...
    fn project_roots_special_path_preserves_symlinked_root() {
        ...
        let expected_codex = expected_root.join(".rexux");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let rexux_dir = real_tmpdir.join(".rexux");
