...
use rexux_connectors::ConnectorMetadata;
use rexux_connectors::ConnectorMetadataStore;
use rexux_connectors::ConnectorToolSummary;
use rexux_connectors::DirectoryListResponse;
use rexux_connectors::merge::merge_connectors;
use rexux_connectors::merge::merge_plugin_connectors;
use rexux_core::config::Config;
pub use rexux_core::connectors::list_accessible_connectors_from_mcp_tools;
pub use rexux_core::connectors::list_accessible_connectors_from_mcp_tools_with_environment_manager;
pub use rexux_core::connectors::list_accessible_connectors_from_mcp_tools_with_mcp_manager;
pub use rexux_core::connectors::list_accessible_connectors_from_mcp_tools_with_options;
pub use rexux_core::connectors::list_accessible_connectors_from_mcp_tools_with_options_and_status;
pub use rexux_core::connectors::list_cached_accessible_connectors_from_mcp_tools;
...
async fn connector_auth(config: &Config) -> anyhow::Result<RexuxAuth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_rexux_api_key_env*/ false).await?;
    ...
    anyhow::ensure!(
        ...
        "ChatGPT connectors require Rexux backend auth"
    );
    Ok(auth)
}
...
pub async fn list_cached_all_connectors(
    ...
    let connectors = rexux_connectors::cached_directory_connectors(&cache_context)?;
...
pub async fn list_all_connectors_with_options(
    ...
    let cache_context = connector_directory_cache_context(config, &auth);
    let connectors = rexux_connectors::list_all_connectors_with_options(
        cache_context,
...
pub async fn read_connector_metadata(
    ...
    anyhow::ensure!(
        auth.uses_rexux_backend(),
        "ChatGPT backend requests require Rexux backend auth"
    );
    anyhow::ensure!(
...
mod tests {
    use super::*;
    use rexux_connectors::metadata::connector_install_url;
    use rexux_plugin::AppConnectorId;
