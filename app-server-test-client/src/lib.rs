...
use rexux_app_server_protocol::JSONRPCNotification;
use rexux_app_server_protocol::JSONRPCRequest;
...
use rexux_app_server_protocol::SandboxPolicy;
...
use rexux_protocol::dynamic_tools::normalize_dynamic_tool_specs;
...
const OTEL_SERVICE_NAME: &str = "rexux-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $REXUX_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the Rexux app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Rexux", version, about = "Bootstrap Rexux app-server", long_about = None)]
struct Cli {
    /// Path to the `codex` CLI binary. When set, requests use stdio by
    /// spawning `codex app-server` as a child process.
    #[arg(long, env = "REXUX_BIN", global = true)]
    rexux_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--rexux-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "REXUX_APP_SERVER_URL", global = true)]
    url: Option<String>,
...
enum CliCommand {
    ...
    ///
    /// Logs are written to:
    ///   `/tmp/rexux-app-server-test-client/`
    Serve {
    ...
    /// Send a user message through the Rexux app-server.
    SendMessage {
        /// User message to send to Rexux.
        user_message: String,
    },
    ...
    TestLogin {
        ...
        device_code: bool,
        /// Use a Rexux-managed Amazon Bedrock API key.
    ...
    TestLogout,
    /// Fetch the current account rate limits from the Rexux app-server.
    GetAccountRateLimits,
    /// List the available models from the Rexux app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the Rexux app-server.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "REXUX_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let rexux_bin = rexux_bin.unwrap_or_else(|| PathBuf::from("codex"));
            serve(&rexux_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(rexux_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(rexux_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(rexux_bin, url, "thread-decrement-elicitation")?;
        ...
        CliCommand::PluginAnalyticsSmoke {
            ...
            if url.is_some() {
                bail!("plugin-analytics-smoke requires --rexux-bin and does not support --url");
            }
            let rexux_bin = rexux_bin.context("plugin-analytics-smoke requires --rexux-bin")?;
            plugin_analytics_smoke::run(&rexux_bin, &config_overrides, &plugin_id, capture_file)
        }
        CliCommand::PluginAnalyticsMutationSmoke {
            ...
            if url.is_some() {
                bail!(
                    "plugin-analytics-mutation-smoke requires --rexux-bin and does not support --url"
                );
            }
            let rexux_bin =
                rexux_bin.context("plugin-analytics-mutation-smoke requires --rexux-bin")?;
        ...
        CliCommand::PluginRemoteUninstall {
            ...
            if url.is_some() {
                bail!("plugin-remote-uninstall requires --rexux-bin and does not support --url");
            }
            let rexux_bin = rexux_bin.context("plugin-remote-uninstall requires --rexux-bin")?;
...
fn resolve_endpoint(rexux_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if rexux_bin.is_some() && url.is_some() {
        bail!("--rexux-bin and --url are mutually exclusive");
    }
    if let Some(rexux_bin) = rexux_bin {
        return Ok(Endpoint::SpawnRexux(rexux_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if rexux_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --rexux-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(rexux_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(rexux_bin);
        if let Some(rexux_bin_parent) = rexux_bin.parent() {
            let mut path = OsString::from(rexux_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", rexux_bin.display()))?;
...
fn serve(rexux_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/rexux-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,rexux_=trace {}",
        shell_quote(&rexux_bin.display().to_string())
    );
    for override_kv in config_overrides {
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnRexux(rexux_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
            ...
            TestLoginMode::AmazonBedrock { api_key, region } => {
                ...
                let login_response: LoginAccountResponse = client.send_request(
                    ClientRequest::LoginAccount {
                        ...
                        params: rexux_app_server_protocol::LoginAccountParams::AmazonBedrock {
                            api_key,
                            region,
                        },
                    },
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut RexuxClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = RexuxClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = RexuxClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (rexux_bin, url) {
        (Some(_), Some(_)) => bail!("--rexux-bin and --url are mutually exclusive"),
        (Some(rexux_bin), None) => {
            let server = BackgroundAppServer::spawn(&rexux_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve rexux-app-server-test-client binary path")?;
...
impl RexuxClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnRexux(rexux_bin) => Self::spawn_stdio(rexux_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(rexux_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        Self::spawn_stdio_with_env(rexux_bin, config_overrides, &[])
    }
    ...
    fn spawn_stdio_with_env(
        ...
        let rexux_bin_display = rexux_bin.display();
        let mut cmd = Command::new(rexux_bin);
        if let Some(rexux_bin_parent) = rexux_bin.parent() {
            let mut path = OsString::from(rexux_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut rexux_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{rexux_bin_display}` app-server"))?;

        let stdin = rexux_app_server
            .stdin
            .take()
            .context("codex app-server stdin unavailable")?;
        let stdout = rexux_app_server
            .stdout
            .take()
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `rexux-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "rexux-toy-app-server".to_string(),
                    title: Some("Rexux Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: rexux_app_server_protocol::LoginAccountParams::Chatgpt {
                app_brand: None,
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: rexux_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = rexux_core::otel_init::build_provider(
            &config,
