...
//!
//! The history is stored at `~/.rexux/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.rexux`.
...
impl HistoryConfig {
    pub fn new(rexux_home: impl Into<PathBuf>, history: &History) -> Self {
        Self {
            rexux_home: rexux_home.into(),
...
fn history_filepath(config: &HistoryConfig) -> PathBuf {
    config.rexux_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(
    ...
    // Resolve `~/.rexux/history.jsonl` and ensure the parent directory exists.
    ...
    tokio::task::spawn_blocking(move || -> Result<()> {
        ...
        for _ in 0..MAX_RETRIES {
            match rexux_utils_file_lock::try_lock_exclusive(&history_file) {
                Ok(()) => {
...
fn lookup_history_entry(path: &Path, log_id: u64, offset: usize) -> Option<HistoryEntry> {
    ...
    for _ in 0..MAX_RETRIES {
        let lock_result = rexux_utils_file_lock::try_lock_shared(&file);
