...
use rexux_backend_client as backend;
use rexux_backend_client::CodeTaskDetailsResponseExt;
use rexux_git_utils::ApplyGitRequest;
use rexux_git_utils::apply_git_patch;
...
mod api {
    ...
    impl<'a> Tasks<'a> {
        ...
        pub(crate) async fn create(
            ...
            if let Ok(diff) = std::env::var("REXUX_STARTING_DIFF")
                && !diff.is_empty()
            {
                input_items.push(serde_json::json!({
