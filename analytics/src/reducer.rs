...
use crate::events::RexuxCollabAgentToolCallEventParams;
use crate::events::RexuxCollabAgentToolCallEventRequest;
...
use crate::events::RexuxControlToolCallEventParams;
use crate::events::RexuxControlToolCallEventRequest;
use crate::events::RexuxDynamicToolCallEventParams;
use crate::events::RexuxDynamicToolCallEventRequest;
use crate::events::RexuxFileChangeEventParams;
use crate::events::RexuxFileChangeEventRequest;
use crate::events::RexuxGoalEventRequest;
use crate::events::RexuxHookRunEventRequest;
use crate::events::RexuxImageGenerationEventParams;
use crate::events::RexuxImageGenerationEventRequest;
use crate::events::RexuxMcpToolCallEventParams;
use crate::events::RexuxMcpToolCallEventRequest;
use crate::events::RexuxOnboardingExternalAgentImportCompleteEventRequest;
use crate::events::RexuxOnboardingExternalAgentImportCompleteMetadata;
...
use crate::events::RexuxPluginInstallRequestedEventRequest;
use crate::events::RexuxPluginMeasurementEventParams;
use crate::events::RexuxPluginMeasurementEventRequest;
...
use crate::events::RexuxTurnEventParams;
use crate::events::RexuxTurnEventRequest;
use crate::events::RexuxTurnSteerEventParams;
use crate::events::RexuxTurnSteerEventRequest;
use crate::events::RexuxWebSearchEventParams;
use crate::events::RexuxWebSearchEventRequest;
...
use crate::events::rexux_artifact_operation_event_request;
use crate::events::rexux_compaction_event_params;
use crate::events::rexux_goal_event_params;
use crate::events::rexux_hook_run_metadata;
use crate::events::rexux_plugin_install_requested_metadata;
...
use crate::facts::RexuxGoalEvent;
...
use crate::facts::TurnRexuxError;
...
use rexux_app_server_protocol::ClientResponse;
use rexux_app_server_protocol::RexuxErrorInfo;
use rexux_app_server_protocol::CollabAgentStatus;
...
use rexux_app_server_protocol::CommandExecutionApprovalKind;
use rexux_app_server_protocol::CommandExecutionSource;
use rexux_app_server_protocol::CommandExecutionStatus;
use rexux_app_server_protocol::DynamicToolCallOutputContentItem;
...
use rexux_app_server_protocol::NetworkPolicyRuleAction;
use rexux_app_server_protocol::PatchApplyStatus;
use rexux_app_server_protocol::PatchChangeKind;
...
use rexux_app_server_protocol::WebSearchAction;
use rexux_git_utils::SanitizedGitUrl;
use rexux_git_utils::collect_git_info;
...
use rexux_protocol::items::is_safe_plugin_relative_path;
...
impl<'a> AnalyticsDropSite<'a> {
    ...
    fn compaction(input: &'a RexuxCompactionEvent) -> Self {
        Self {
    ...
    fn goal(input: &'a RexuxGoalEvent) -> Self {
        Self {
            event_name: "goal",
            thread_id: &input.thread_id,
    ...
    fn tool_item(
        notification: &'a rexux_app_server_protocol::ItemCompletedNotification,
        item_id: &'a str,
    ) -> Self {
        Self {
...
struct CompletedTurnState {
    ...
    turn_error: Option<RexuxErrorInfo>,
    completed_at: u64,
...
struct TurnState {
    ...
    rexux_error: Option<TurnRexuxError>,
...
impl AnalyticsReducer {
    pub(crate) async fn ingest(&mut self, input: AnalyticsFact, out: &mut Vec<TrackEventRequest>) {
        match input {
            ...
            AnalyticsFact::Custom(input) => match input {
                ...
                CustomAnalyticsFact::GuardianV2(input) => {
                    let event_type = match &input.kind {
                        GuardianV2EventKind::Classification { .. } => {
                            "rexux_guardian_v2_classification"
                        }
                        GuardianV2EventKind::FastDecision { .. } => {
                            "rexux_guardian_v2_fast_decision"
                        }
                    };
                ...
                CustomAnalyticsFact::TurnRexuxError(input) => {
                    self.ingest_turn_rexux_error(*input);
                }
    ...
    fn ingest_artifact_operation(
        ...
        out.push(TrackEventRequest::ArtifactOperation(
            rexux_artifact_operation_event_request(input.tracking, input.operation),
        ));
    }
    ...
    fn ingest_code_mode_tool_call(
        ...
        match input {
            ...
            CodeModeToolCallFact::Completed {
                ...
                let event = TrackEventRequest::DynamicToolCall(RexuxDynamicToolCallEventRequest {
                    event_type: "rexux_dynamic_tool_call_event",
                    event_params: RexuxDynamicToolCallEventParams {
                        base,
    ...
    fn ingest_control_tool_call(
        ...
        base.cell_id = cell_id;
        let event = TrackEventRequest::ControlToolCall(RexuxControlToolCallEventRequest {
            event_type: "rexux_control_tool_call_event",
            event_params: RexuxControlToolCallEventParams {
                base,
    ...
    fn ingest_guardian_review(
        ...
        out.push(TrackEventRequest::GuardianReview(Box::new(
            GuardianReviewEventRequest {
                event_type: "rexux_guardian_review",
    ...
    fn ingest_turn_rexux_error(&mut self, input: TurnRexuxErrorFact) {
        let TurnRexuxErrorFact {
            turn_id,
            thread_id,
            error,
        } = input;
        ...
        turn_state.rexux_error = Some(error);
    }
    ...
    fn ingest_app_mentioned(&mut self, input: AppMentionedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.extend(mentions.into_iter().map(|mention| {
            let event_params = rexux_app_metadata(&tracking, mention);
            TrackEventRequest::AppMentioned(RexuxAppMentionedEventRequest {
    ...
    fn ingest_app_used(&mut self, input: AppUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        let event_params = rexux_app_metadata(&tracking, app);
        out.push(TrackEventRequest::AppUsed(RexuxAppUsedEventRequest {
    ...
    fn ingest_hook_run(&mut self, input: HookRunInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::HookRun(RexuxHookRunEventRequest {
            event_type: "rexux_hook_run",
            event_params: rexux_hook_run_metadata(&tracking, hook),
        }));
    }
    ...
    fn ingest_plugin_used(&mut self, input: PluginUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::PluginUsed(RexuxPluginUsedEventRequest {
            event_type: "rexux_plugin_used",
            event_params: rexux_plugin_used_metadata(&tracking, plugin),
        }));
    }
    ...
    fn ingest_plugin_install_requested(
        ...
        out.push(TrackEventRequest::PluginInstallRequested(
            RexuxPluginInstallRequestedEventRequest {
                event_type: "rexux_plugin_install_requested",
                event_params: rexux_plugin_install_requested_metadata(&tracking, request),
            },
        ));
    }
    ...
    fn ingest_plugin_state_changed(
        ...
        let event = RexuxPluginEventRequest {
    ...
    fn ingest_external_agent_config_import_completed(
        ...
        out.push(TrackEventRequest::ExternalAgentConfigImportCompleted(
            RexuxOnboardingExternalAgentImportCompleteEventRequest {
                event_type: "rexux_onboarding_external_agent_import_complete",
                event_params: RexuxOnboardingExternalAgentImportCompleteMetadata {
            ...
            ServerNotification::TurnCompleted(notification) => {
                ...
                turn_state.completed = Some(CompletedTurnState {
                    ...
                    turn_error: notification
                        .turn
                        .error
                        .and_then(|error| error.rexux_error_info),
    ...
    fn ingest_plugin_measurements(
        ...
        out.extend(
            rows.into_iter()
                ...
                .map(|row| {
                    TrackEventRequest::PluginMeasurement(RexuxPluginMeasurementEventRequest {
                        event_type: "rexux_plugin_measurement_event",
                        event_params: RexuxPluginMeasurementEventParams {
    ...
    fn emit_thread_initialized(
        &mut self,
        connection_id: u64,
        thread: rexux_app_server_protocol::Thread,
        model: String,
        initialization_mode: ThreadInitializationMode,
        thread_originator: Option<String>,
        out: &mut Vec<TrackEventRequest>,
    ...
    fn ingest_compaction(&mut self, input: RexuxCompactionEvent, out: &mut Vec<TrackEventRequest>) {
        let Some((connection_state, thread_state, thread_metadata)) =
        ...
        out.push(TrackEventRequest::Compaction(Box::new(
            RexuxCompactionEventRequest {
                event_type: "rexux_compaction_event",
                event_params: rexux_compaction_event_params(
                    input,
    ...
    fn ingest_goal(&mut self, input: RexuxGoalEvent, out: &mut Vec<TrackEventRequest>) {
        let Some((connection_state, thread_state, thread_metadata)) =
            self.thread_context_or_warn(AnalyticsDropSite::goal(&input))
        else {
            return;
        };
        out.push(TrackEventRequest::Goal(Box::new(RexuxGoalEventRequest {
            event_type: "rexux_goal_event",
            event_params: rexux_goal_event_params(
                input,
                thread_metadata.session_id.clone(),
                thread_state.app_server_client(connection_state),
                connection_state.runtime.clone(),
    ...
    fn ingest_guardian_review_completed(
        &mut self,
        notification: rexux_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification,
        out: &mut Vec<TrackEventRequest>,
    ) {
    ...
    fn emit_turn_steer_event(
        ...
        out.push(TrackEventRequest::TurnSteer(RexuxTurnSteerEventRequest {
            event_type: "rexux_turn_steer_event",
            event_params: RexuxTurnSteerEventParams {
    ...
    fn emit_review_event(
        ...
        out.push(TrackEventRequest::ReviewEvent(RexuxReviewEventRequest {
    ...
    async fn maybe_emit_turn_event(&mut self, turn_id: &str, out: &mut Vec<TrackEventRequest>) {
        ...
        let turn_event = TrackEventRequest::TurnEvent(Box::new(RexuxTurnEventRequest {
            event_type: "rexux_turn_event",
            event_params: rexux_turn_event_params(
                thread_state.app_server_client(connection_state),
                connection_state.runtime.clone(),
                turn_id.to_string(),
                turn_state,
...
fn tool_event_base_mut(event: &mut TrackEventRequest) -> Option<&mut RexuxToolItemEventBase> {
    match event {
    ...
    match item {
        ThreadItem::CommandExecution {
            ...
            Some(TrackEventRequest::CommandExecution(
                RexuxCommandExecutionEventRequest {
        ...
        ThreadItem::FileChange {
            ...
            Some(TrackEventRequest::FileChange(RexuxFileChangeEventRequest {
                event_type: "rexux_file_change_event",
                event_params: RexuxFileChangeEventParams {
                    base,
        ...
        ThreadItem::McpToolCall {
            ...
            Some(TrackEventRequest::McpToolCall(
                RexuxMcpToolCallEventRequest {
                    event_type: "rexux_mcp_tool_call_event",
                    event_params: RexuxMcpToolCallEventParams {
                        base,
        ...
        ThreadItem::DynamicToolCall {
            ...
            Some(TrackEventRequest::DynamicToolCall(
                RexuxDynamicToolCallEventRequest {
                    event_type: "rexux_dynamic_tool_call_event",
                    event_params: RexuxDynamicToolCallEventParams {
                        base,
        ...
        ThreadItem::CollabAgentToolCall {
            ...
            Some(TrackEventRequest::CollabAgentToolCall(
                RexuxCollabAgentToolCallEventRequest {
                    event_type: "rexux_collab_agent_tool_call_event",
                    event_params: RexuxCollabAgentToolCallEventParams {
                        base,
        ...
        ThreadItem::WebSearch(item) => {
            ...
            Some(TrackEventRequest::WebSearch(RexuxWebSearchEventRequest {
                event_type: "rexux_web_search_event",
                event_params: RexuxWebSearchEventParams {
                    base,
        ...
        ThreadItem::ImageGeneration(item) => {
            ...
            Some(TrackEventRequest::ImageGeneration(
                RexuxImageGenerationEventRequest {
                    event_type: "rexux_image_generation_event",
                    event_params: RexuxImageGenerationEventParams {
                        base,
...
fn file_change_counts(changes: &[rexux_app_server_protocol::FileUpdateChange]) -> FileChangeCounts {
...
fn rexux_turn_event_params(
    app_server_client: RexuxAppServerClientMetadata,
    runtime: RexuxRuntimeMetadata,
    turn_id: String,
    turn_state: &TurnState,
    thread_metadata: &ThreadMetadataState,
) -> RexuxTurnEventParams {
    let (
        Some(thread_id),
    ...
    let rexux_error = turn_state.rexux_error.as_ref();
    RexuxTurnEventParams {
        thread_id,
        session_id: thread_metadata.session_id.clone(),
        turn_id,
        ...
        rexux_turn_source: turn_metadata.rexux_turn_source(),
        app_server_client,
        runtime,
        submission_type,
        ephemeral,
        ...
        rexux_error_kind: rexux_error.map(|error| error.kind),
        rexux_error_http_status_code: rexux_error.and_then(|error| error.http_status_code),
...
fn analytics_turn_status(status: rexux_app_server_protocol::TurnStatus) -> Option<TurnStatus> {
    match status {
        rexux_app_server_protocol::TurnStatus::Completed => Some(TurnStatus::Completed),
        rexux_app_server_protocol::TurnStatus::Failed => Some(TurnStatus::Failed),
        rexux_app_server_protocol::TurnStatus::Interrupted => Some(TurnStatus::Interrupted),
        rexux_app_server_protocol::TurnStatus::InProgress => None,
    }
}
    ...
    fn command_execution_script_paths_reject_unsafe_values() {
        assert_eq!(
            safe_plugin_relative_script_path(
                ...
                Some("/home/user/.rexux/plugins/cache/openai-curated/sample/scripts/run.py"),
            ),
            None
        );
        assert_eq!(
