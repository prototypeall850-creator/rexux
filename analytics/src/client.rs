...
impl AnalyticsEventsClient {
    ...
    pub fn track_compaction(&self, event: crate::facts::RexuxCompactionEvent) {
    ...
    pub fn track_goal_event(&self, event: RexuxGoalEvent) {
    ...
    pub fn track_turn_rexux_error(&self, fact: TurnRexuxErrorFact) {
        self.record_fact(AnalyticsFact::Custom(CustomAnalyticsFact::TurnRexuxError(
            Box::new(fact),
        )));
    }
...
async fn send_track_events(
    ...
    } else if !auth.uses_rexux_backend() {
        return;
    }
    if events.is_empty() {
        return;
