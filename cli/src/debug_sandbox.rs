...
#[cfg(target_os = "macos")]
use rexux_core::spawn::REXUX_SANDBOX_ENV_VAR;
...
use rexux_sandboxing::landlock::allow_network_for_proxy;
use rexux_sandboxing::landlock::create_linux_sandbox_command_args_for_permission_profile;
#[cfg(target_os = "macos")]
use rexux_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use rexux_sandboxing::seatbelt::create_seatbelt_command_args;
use rexux_sandboxing::with_managed_mitm_ca_readable_root;
...
pub async fn run_command_under_seatbelt(
    ...
    _rexux_linux_sandbox_exe: Option<PathBuf>,
...
async fn run_command_under_sandbox(
    ...
    let sandbox_state = config_options
        ...
        .sandbox_state_json
        .as_deref()
        .map(serde_json::from_str::<rexux_mcp::SandboxState>)
        .transpose()
    ...
    let rexux_linux_sandbox_exe = match sandbox_state.as_ref() {
        Some(state) => {
            ...
            state
                .rexux_linux_sandbox_exe
                .clone()
                .or(rexux_linux_sandbox_exe)
        }
        None => rexux_linux_sandbox_exe,
    };
    ...
    let network = network_proxy
        .as_ref()
        .map(rexux_core::config::StartedNetworkProxy::proxy);
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Seatbelt => {
            ...
            spawn_debug_sandbox_child(
                ...
                env,
                |env_map| {
                    env_map.insert(REXUX_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
        ...
        SandboxType::Landlock => {
            ...
            let rexux_linux_sandbox_exe = config
                .rexux_linux_sandbox_exe
                .expect("rexux-linux-sandbox executable not found");
...
async fn run_command_under_windows_session(
    ...
    use rexux_windows_sandbox::WindowsSandboxProxySettingsMode;
    use rexux_windows_sandbox::WindowsSandboxSessionRequest;
    use rexux_windows_sandbox::spawn_windows_sandbox_session_for_level;

    let empty_paths: &[AbsolutePathBuf] = &[];
    let spawned = spawn_windows_sandbox_session_for_level(WindowsSandboxSessionRequest {
        permission_profile,
        workspace_roots: workspace_roots.as_slice(),
        rexux_home: config.rexux_home.as_path(),
        command,
        cwd: cwd.as_path(),
        env_map: env,
    ...
    let exit_code = rexux_windows_sandbox::forward_sandbox_session_stdio(spawned).await;
...
async fn spawn_debug_sandbox_child(
    ...
    if !network_sandbox_policy.is_enabled() {
        cmd.env(REXUX_SANDBOX_NETWORK_DISABLED_ENV_VAR, "1");
    }
...
async fn load_debug_sandbox_config(
    ...
    load_debug_sandbox_config_with_rexux_home(
        cli_overrides,
        rexux_linux_sandbox_exe,
        options,
        /*rexux_home*/ None,
...
async fn load_debug_sandbox_config_with_rexux_home(
    cli_overrides: Vec<(String, TomlValue)>,
    rexux_linux_sandbox_exe: Option<PathBuf>,
    options: DebugSandboxConfigOptions,
    ...
    let config = build_debug_sandbox_config_with_loader_overrides(
        ...
        ConfigOverrides {
            cwd: cwd.clone(),
            rexux_linux_sandbox_exe: rexux_linux_sandbox_exe.clone(),
            ..Default::default()
        },
        rexux_home.clone(),
        managed_requirements_mode,
...
async fn build_debug_sandbox_config_with_loader_overrides(
    ...
    if let Some(rexux_home) = rexux_home {
        builder = builder
            .rexux_home(rexux_home.clone())
            .fallback_cwd(Some(rexux_home));
    }
    builder.build().await
}
...
"#;
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_rexux_home(
            Vec::new(),
    ...
    async fn debug_sandbox_honors_config_profile_loader_overrides() -> anyhow::Result<()> {
        ...
        let docs = sandbox_paths.path().join("docs");
        let private = docs.join("private");
        let profile_path = rexux_home.path().join("work.config.toml");
    ...
    async fn debug_sandbox_honors_explicit_builtin_permission_profile() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_rexux_home(
            ...
            Some(rexux_home.path().to_path_buf()),
            CloudConfigBundleLoader::default(),
            /*strict_config*/ false,
        )
        .await?;
        ...
        let actual = config
            ...
            .permission_profile()
            .file_system_sandbox_policy();
        let expected = rexux_protocol::models::PermissionProfile::workspace_write()
            .file_system_sandbox_policy();
        assert!(
            expected
                .entries
