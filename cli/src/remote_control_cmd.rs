...
use rexux_app_server_daemon::LifecycleOutput as AppServerLifecycleOutput;
use rexux_app_server_daemon::LifecycleStatus as AppServerLifecycleStatus;
use rexux_app_server_daemon::RemoteControlReadyOutput as AppServerRemoteControlReadyOutput;
use rexux_app_server_daemon::RemoteControlReadyStatus as AppServerRemoteControlReadyStatus;
use rexux_app_server_daemon::RemoteControlStartOutput as AppServerRemoteControlStartOutput;
...
pub(crate) async fn run(
    ...
    match command.subcommand {
        ...
        Some(RemoteControlSubcommand::Start) => {
            ...
            let output = rexux_app_server_daemon::ensure_remote_control_ready().await?;
        ...
        Some(RemoteControlSubcommand::Stop) => {
            ...
            let output = rexux_app_server_daemon::run(AppServerLifecycleCommand::Stop).await?;
            print_remote_control_stop_output(&output, command.json)?;
        }
        Some(RemoteControlSubcommand::Pair) => {
            let output = rexux_app_server_daemon::start_remote_control_pairing().await?;
...
async fn run_foreground_remote_control(
    ...
    let runtime_options = AppServerRuntimeOptions {
        remote_control_startup_mode: rexux_app_server::RemoteControlStartupMode::EnabledEphemeral,
        install_shutdown_signal_handler: false,
        ..Default::default()
    };
    let (stop_rx, stop_signal_task) = foreground_stop_signal();
    let mut app_server_task = tokio::spawn(rexux_app_server::run_main_with_transport_options(
        arg0_paths,
...
async fn wait_for_foreground_remote_control_ready(
    ...
    rexux_app_server_daemon::enable_remote_control_on_socket(
        socket_path.as_path(),
...
fn daemon_app_server_human_lines(output: &AppServerRemoteControlStartOutput) -> Vec<String> {
    let (managed_rexux_path, managed_rexux_version) = daemon_app_server_identity(output);
    vec![
        "Daemon used app-server:".to_string(),
        format!("  path: {}", managed_rexux_path.display()),
        format!("  version: {}", managed_rexux_version.unwrap_or("unknown")),
    ]
}
...
fn daemon_app_server_identity(
    ...
    match output {
        AppServerRemoteControlStartOutput::Bootstrap(output) => (
            &output.managed_rexux_path,
            output.managed_rexux_version.as_deref(),
        ),
    ...
    fn daemon_ready_output(
        ...
        AppServerRemoteControlReadyOutput {
            daemon: AppServerRemoteControlStartOutput::Start(AppServerLifecycleOutput {
                ...
                backend: None,
                pid: Some(42),
                managed_rexux_path: PathBuf::from("/opt/codex/bin/codex"),
                managed_rexux_version: Some("1.0.0".to_string()),
        ...
        assert_eq!(
            ...
            json!({
                ...
                "daemon": {
                    "status": "started",
                    "pid": 42,
                    "managedRexuxPath": "/opt/codex/bin/codex",
                    "managedRexuxVersion": "1.0.0",
