...
use rexux_thread_store::RolloutMigrationMode;
use rexux_thread_store::RolloutMigrationOptions;
use rexux_thread_store::RolloutMigrationProgress;
use rexux_thread_store::RolloutMigrationReport;
use rexux_thread_store::RolloutMigrationStatus;
...
pub(crate) async fn run(
    ...
    rexux_core::otel_init::record_process_start(otel.as_ref(), "rexux_migrate_rollouts");
    ...
    let state_db = if mode == RolloutMigrationMode::Apply {
        Some(
            rexux_rollout::state_db::try_init(&config)
                .await
...
fn print_outcome(outcome: &rexux_thread_store::RolloutMigrationOutcome) {
...
async fn thread_storage_bytes(rexux_home: &Path, thread_history_db_path: &Path) -> io::Result<u64> {
    let mut bytes = 0_u64;
    let mut directories = vec![
        rexux_home.join(rexux_rollout::SESSIONS_SUBDIR),
        rexux_home.join(rexux_rollout::ARCHIVED_SESSIONS_SUBDIR),
    ];
