...
use rexux_core::config::load_global_mcp_servers;
...
use rexux_mcp::ResolvedMcpOAuthScopes;
use rexux_mcp::apply_http_headers_helper;
use rexux_mcp::compute_auth_statuses;
...
use rexux_rmcp_client::delete_oauth_tokens;
use rexux_rmcp_client::perform_oauth_login;
use rexux_rmcp_client::resolve_mcp_oauth_callback_url;
use rexux_utils_cli::CliConfigOverrides;
use rexux_utils_cli::format_env_display;
...
/// - `add`    — add a server launcher entry to `~/.rexux/config.toml`
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: rexux_config::types::OAuthCredentialsStoreMode,
    keyring_backend_kind: rexux_config::types::AuthKeyringBackendKind,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let mut servers = load_global_mcp_servers(&rexux_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", rexux_home.display()))?;
    ...
    ConfigEditsBuilder::new(&rexux_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", rexux_home.display()))?;
