...
const REXUX_BUNDLE_IDENTIFIER: &str = "com.openai.codex";
const OPENAI_APPLE_TEAM_IDENTIFIER: &str = "2DC432GLL2";
const REXUX_DMG_URL_ARM64: &str = "https://persistent.oaistatic.com/rexux-app-prod/Rexux.dmg";
const REXUX_DMG_URL_X64: &str =
    "https://persistent.oaistatic.com/rexux-app-prod/Rexux-latest-x64.dmg";

pub async fn run_mac_app_open_or_install(
    workspace: PathBuf,
    download_url_override: Option<String>,
) -> anyhow::Result<()> {
    if let Some(app_path) = find_existing_rexux_app_path(&rexux_app_search_dirs()) {
        eprintln!(
            "Opening Desktop app at {app_path}...",
            app_path = app_path.display()
        );
        open_rexux_app(&app_path, &workspace).await?;
        return Ok(());
    }
    ...
    let installed_app = download_and_install_rexux_to_user_applications(&download_url)
        .await
    ...
    open_rexux_app(&installed_app, &workspace).await?;
    Ok(())
}
...
fn find_existing_rexux_app_path(applications_dirs: &[PathBuf]) -> Option<PathBuf> {
    applications_dirs
        .iter()
        .flat_map(|dir| ["ChatGPT.app", "Rexux.app"].map(|app_name| dir.join(app_name)))
        .find(|candidate| is_rexux_app_bundle(candidate))
}

fn rexux_app_search_dirs() -> Vec<PathBuf> {
...
fn is_rexux_app_bundle(app_path: &Path) -> bool {
    if !app_path.is_dir() {
        return false;
    }
    ...
    std::process::Command::new("/usr/bin/plutil")
        ...
        .is_ok_and(|output| {
            output.status.success()
                && String::from_utf8_lossy(&output.stdout).trim() == REXUX_BUNDLE_IDENTIFIER
        })
}

async fn open_rexux_app(app_path: &Path, workspace: &Path) -> anyhow::Result<()> {
    verify_rexux_app_bundle(app_path).await?;
    eprintln!(
        "Opening workspace {workspace}...",
        workspace = workspace.display()
    );
    let url = rexux_new_thread_url(workspace);
...
async fn verify_rexux_app_bundle(app_path: &Path) -> anyhow::Result<()> {
    let requirement = format!(
        "identifier \"{REXUX_BUNDLE_IDENTIFIER}\" and anchor apple generic and certificate leaf[subject.OU] = \"{OPENAI_APPLE_TEAM_IDENTIFIER}\""
    );
    ...
    anyhow::bail!(
        "Desktop app at {} failed OpenAI signature verification (team {OPENAI_APPLE_TEAM_IDENTIFIER}, bundle {REXUX_BUNDLE_IDENTIFIER}): {}",
        app_path.display(),
        String::from_utf8_lossy(&output.stderr).trim()
    );
}

fn rexux_new_thread_url(workspace: &Path) -> String {
...
async fn download_and_install_rexux_to_user_applications(dmg_url: &str) -> anyhow::Result<PathBuf> {
    let temp_dir = Builder::new()
        .prefix("rexux-app-installer-")
        .tempdir()
        .context("failed to create temp dir")?;
    let tmp_root = temp_dir.path().to_path_buf();
    let _temp_dir = temp_dir;

    let dmg_path = tmp_root.join("Rexux.dmg");
    ...
    let result = async {
        let app_in_volume = find_rexux_app_in_mount(&mount_point)
            .context("failed to locate Rexux.app in mounted dmg")?;
        verify_rexux_app_bundle(&app_in_volume)
            .await
            .context("refusing to install an unverified Desktop app")?;
        install_rexux_app_bundle(&app_in_volume).await
    }
    .await;
...
async fn install_rexux_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
    for applications_dir in candidate_applications_dirs()? {
        ...
        let dest_app = applications_dir.join("Rexux.app");
        if dest_app.is_dir() {
            return Ok(dest_app);
        }
        ...
        match copy_app_bundle(app_in_volume, &dest_app).await {
            ...
            Err(err) => {
                eprintln!(
                    "warning: failed to install Rexux.app to {applications_dir}: {err}",
                    applications_dir = applications_dir.display()
                );
            }
        }
    ...
    anyhow::bail!("failed to install Rexux.app to any applications directory");
}
...
fn find_rexux_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
    let direct = mount_point.join("Rexux.app");
    if direct.is_dir() {
        return Ok(direct);
    }
...
#[cfg(test)]
mod tests {
    use super::rexux_new_thread_url;
    use super::find_existing_rexux_app_path;
    use super::open_rexux_app;
    use super::parse_hdiutil_attach_mount_point;
    use super::verify_rexux_app_bundle;
    ...
    #[test]
    fn finds_chatgpt_app_with_rexux_bundle_identifier() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let app_path = temp_dir.path().join("ChatGPT.app");
        write_app_bundle(&app_path, "com.openai.codex");

        assert_eq!(
            find_existing_rexux_app_path(&[temp_dir.path().to_path_buf()]),
            Some(app_path)
        );
    }
    ...
    fn ignores_classic_chatgpt_app() {
        ...
        let rexux_app_path = temp_dir.path().join("Rexux.app");
        write_app_bundle(&rexux_app_path, "com.openai.codex");
    ...
    #[tokio::test]
    async fn rejects_unsigned_app_with_rexux_bundle_identifier() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let app_path = temp_dir.path().join("Rexux.app");
        write_app_bundle(&app_path, "com.openai.codex");

        let err = verify_rexux_app_bundle(&app_path)
            .await
    ...
    async fn rejects_valid_signature_without_openai_signing_identity() {
        ...
        let app_path = temp_dir.path().join("Rexux.app");
        let contents_path = app_path.join("Contents");
        let executable_path = contents_path.join("MacOS/Rexux");
        ...
        fs::write(
            contents_path.join("Info.plist"),
            r#"<?xml version="1.0"?><plist version="1.0"><dict><key>CFBundleIdentifier</key><string>com.openai.codex</string><key>CFBundleExecutable</key><string>Rexux</string></dict></plist>"#,
        )
        .expect("write Info.plist");
    ...
    #[tokio::test]
    async fn refuses_to_launch_unsigned_existing_rexux_app() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let app_path = temp_dir.path().join("Rexux.app");
        write_app_bundle(&app_path, "com.openai.codex");

        let err = open_rexux_app(&app_path, temp_dir.path())
            .await
    ...
    fn parses_mount_point_from_tab_separated_hdiutil_output() {
        let output = "/dev/disk2s1\tApple_HFS\tRexux\t/Volumes/Rexux\n";
        assert_eq!(
    ...
    fn parses_mount_point_with_spaces() {
        let output = "/dev/disk2s1\tApple_HFS\tRexux Installer\t/Volumes/Rexux Installer\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/Rexux Installer")
        );
    }

    #[test]
    fn rexux_new_thread_url_encodes_workspace_path() {
        let url = url::Url::parse(&rexux_new_thread_url(Path::new("/tmp/codex workspace/#1")))
