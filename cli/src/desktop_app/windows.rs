...
const REXUX_WINDOWS_INSTALLER_URL: &str =
    "https://get.microsoft.com/installer/download/9PLM9XGG6VKS?cid=website_cta_psi";
const REXUX_MICROSOFT_STORE_WEB_URL: &str = "https://apps.microsoft.com/detail/9plm9xgg6vks";
...
pub async fn run_windows_app_open_or_install(
    ...
    if rexux_app_is_installed().await? {
        eprintln!("Opening workspace {display_workspace} in the Desktop app...");
        open_url(&rexux_new_thread_url(&workspace_path)).await?;
        return Ok(());
    }
    ...
    let download_url = download_url_override
        .as_deref()
        .unwrap_or(REXUX_WINDOWS_INSTALLER_URL);
    if open_url(download_url).await.is_err() && download_url_override.is_none() {
        open_url(REXUX_MICROSOFT_STORE_WEB_URL).await?;
    }
    eprintln!("After installing the Desktop app, open workspace {display_workspace}.");
    Ok(())
}

async fn rexux_app_is_installed() -> anyhow::Result<bool> {
    // This package identity is stable across Rexux- and ChatGPT-branded builds.
    let output = Command::new("powershell.exe")
        .arg("-NoProfile")
        .arg("-Command")
        .arg(
            "Get-StartApps | Where-Object AppID -Like 'OpenAI.Rexux_*!App' | Select-Object -First 1 -ExpandProperty AppID",
        )
        .output()
        .await
...
fn rexux_new_thread_url(workspace: &str) -> String {
...
mod tests {
    ...
    #[test]
    fn rexux_new_thread_url_encodes_windows_workspace_path() {
        assert_eq!(
            rexux_new_thread_url(r"C:\Users\akuma\repos\koba"),
            r"codex://threads/new?path=C%3A%5CUsers%5Cakuma%5Crepos%5Ckoba"
        );
    }

    #[test]
    fn rexux_new_thread_url_preserves_verbatim_workspace_path() {
        assert_eq!(
            rexux_new_thread_url(r"\\?\C:\Users\akuma\repos\koba"),
