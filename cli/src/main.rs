...
use rexux_app_server_daemon::BootstrapOptions as AppServerBootstrapOptions;
use rexux_app_server_daemon::LifecycleCommand as AppServerLifecycleCommand;
use rexux_app_server_daemon::RemoteControlMode as AppServerRemoteControlMode;
use rexux_arg0::Arg0DispatchPaths;
use rexux_arg0::arg0_dispatch_or_else;
use rexux_chatgpt::apply_command::ApplyCommand;
use rexux_chatgpt::apply_command::run_apply_command;
use rexux_cli::NO_CHATGPT_LOGIN_MESSAGE;
use rexux_cli::read_api_key_from_stdin;
use rexux_cli::run_login_status;
use rexux_cli::run_login_with_api_key;
use rexux_cli::run_logout;
use rexux_cloud_config::cloud_config_bundle_loader_for_storage;
use rexux_cloud_tasks::Cli as CloudTasksCli;
use rexux_exec::Cli as ExecCli;
use rexux_exec::Command as ExecCommand;
use rexux_exec::ReviewArgs;
use rexux_execpolicy::ExecPolicyCheckCommand;
use rexux_responses_api_proxy::Args as ResponsesApiProxyArgs;
use rexux_rollout_trace::REDUCED_STATE_FILE_NAME;
use rexux_rollout_trace::replay_bundle;
use rexux_state::StateRuntime;
use rexux_tui::AppExitInfo;
use rexux_tui::Cli as TuiCli;
use rexux_tui::ExitReason;
use rexux_tui::UpdateAction;
use rexux_utils_absolute_path::AbsolutePathBuf;
use rexux_utils_cli::CliConfigOverrides;
use rexux_utils_cli::ProfileV2Name;
use rexux_utils_cli::SharedCliOptions;
...
use rexux_core::build_models_manager;
use rexux_core::config::Config;
use rexux_core::config::ConfigBuilder;
use rexux_core::config::ConfigLoadOptions;
...
#[clap(
    ...
    // `rexux-x86_64-unknown-linux-musl`, but the help output should always use
...
enum Subcommand {
    ...
    /// Manage external MCP servers for Rexux.
    Mcp(McpCli),

    /// Manage Rexux plugins.
    Plugin(PluginCli),

    /// Start Rexux as an MCP server (stdio).
    ...
    /// Update Rexux to the latest version.
    Update,

    /// Diagnose local Rexux installation, config, auth, and runtime health.
    Doctor(DoctorCommand),

    /// Run commands within a Rexux-provided sandbox.
    ...
    /// Apply the latest diff produced by Rexux agent as a `git apply` to your local working tree.
    ...
    /// [EXPERIMENTAL] Browse tasks from Rexux Cloud and apply changes locally.
...
struct ReviewCommand {
    /// Error out when config.toml contains fields that are not recognized by this version of Rexux.
...
#[cfg(target_os = "macos")]
type HostSandboxArgs = rexux_cli::SeatbeltCommand;
#[cfg(target_os = "linux")]
type HostSandboxArgs = rexux_cli::LandlockCommand;
#[cfg(target_os = "windows")]
type HostSandboxArgs = rexux_cli::WindowsCommand;
...
struct UnsupportedSandboxArgs {
    /// Layer $REXUX_HOME/<name>.config.toml on top of the base user config.
...
struct AppServerCommand {
    ...
    #[command(flatten)]
    code_mode_host: rexux_app_server::AppServerCodeModeHostArgs,
    ...
    #[arg(
        long = "listen",
        value_name = "URL",
        default_value = rexux_app_server::AppServerTransport::DEFAULT_LISTEN_URL
    )]
    listen: rexux_app_server::AppServerTransport,
    ...
    #[command(flatten)]
    auth: rexux_app_server::AppServerWebsocketAuthArgs,
}
...
struct ExecServerCommand {
    ...
    request_dispatch_mode: rexux_exec_server::RequestDispatchMode,
    ...
    /// Use Agent Identity auth from REXUX_ACCESS_TOKEN for remote registration.
    #[arg(
    ...
    #[arg(
        ...
        env = rexux_exec_server::REXUX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for Rexux tooling.
    #[clap(hide = true)]
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating Rexux via `{cmd_str}`...");
    let status = {
        #[cfg(windows)]
        {
    ...
    println!("\n🎉 Update ran successfully! Please restart Rexux.");
    Ok(())
}
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    {
        anyhow::bail!(
            "`codex update` is not available in debug builds. Install a release build of Rexux to use this command."
        );
    }

    #[cfg(not(debug_assertions))]
    {
        let Some(action) = rexux_tui::get_update_action() else {
            anyhow::bail!(
                "Could not detect the Rexux installation method. Please update manually: https://developers.openai.com/codex/cli/"
            );
        };
        run_update_action(action)
    }
...
async fn run_session_archive_cli_command(
    action: rexux_tui::SessionArchiveAction,
    ...
    rexux_tui::run_session_archive_command(
        action,
        target,
        rexux_tui::SessionArchiveCommandOptions {
            cli: interactive,
            arg0_paths,
...
fn delete_action(target: &str, force: bool) -> anyhow::Result<rexux_tui::SessionArchiveAction> {
    if force && rexux_protocol::ThreadId::from_string(target).is_err() {
        anyhow::bail!("--force requires a session UUID; names must be confirmed interactively");
    }
    let confirmation = match force {
        true => rexux_tui::DeleteConfirmation::Skip,
        false => rexux_tui::DeleteConfirmation::Prompt,
    };
    Ok(rexux_tui::SessionArchiveAction::Delete(confirmation))
}

async fn run_debug_app_server_command(cmd: DebugAppServerCommand) -> anyhow::Result<()> {
    match cmd.subcommand {
        DebugAppServerSubcommand::SendMessageV2(cmd) => {
            let rexux_bin = std::env::current_exe()?;
            rexux_app_server_test_client::send_message_v2(&rexux_bin, &[], cmd.user_message, &None)
                .await
        }
    }
}
...
fn main() -> anyhow::Result<()> {
    let remote_control_disabled = rexux_app_server::take_remote_control_disabled_env();
...
async fn cli_main(
    ...
    match subcommand {
        ...
        Some(Subcommand::Exec(mut exec_cli)) => {
            ...
            rexux_exec::run_main(exec_cli, arg0_paths.clone()).await?;
        }
            ...
            match subcommand {
                None => {
                    let transport = if stdio {
                        rexux_app_server::AppServerTransport::Stdio
                    } else {
                        listen
                    };
                    let auth = auth.try_into_settings()?;
                    let runtime_options = rexux_app_server::AppServerRuntimeOptions {
                        code_mode_host_transport: code_mode_host.into(),
                        remote_control_startup_mode: match (remote_control, remote_control_disabled)
                        {
                            (true, _) => {
                                rexux_app_server::RemoteControlStartupMode::EnabledEphemeral
                            }
                            (false, true) => {
                                rexux_app_server::RemoteControlStartupMode::DisabledEphemeral
                            }
                            (false, false) => {
                                rexux_app_server::RemoteControlStartupMode::ResolvePersisted
                            }
                        },
                        ..Default::default()
                    };
                    rexux_app_server::run_main_with_transport_options(
                        arg0_paths.clone(),
                        root_config_overrides,
                        ...
                        analytics_default_enabled,
                        transport,
                        rexux_protocol::protocol::SessionSource::VSCode,
                        auth,
                        runtime_options,
                    )
                    .await?;
                ...
                Some(AppServerSubcommand::Daemon(daemon_cli)) => match daemon_cli.subcommand {
                    ...
                    AppServerDaemonSubcommand::Bootstrap(bootstrap_cli) => {
                        let output =
                            rexux_app_server_daemon::bootstrap(AppServerBootstrapOptions {
                    ...
                    AppServerDaemonSubcommand::PidUpdateLoop => {
                        ...
                        rexux_app_server_daemon::run_pid_update_loop(http_client_factory).await?;
                    }
                },
                Some(AppServerSubcommand::Proxy(proxy_cli)) => {
                    let socket_path = match proxy_cli.socket_path {
                        ...
                        None => {
                            ...
                            rexux_app_server::app_server_control_socket_path(&rexux_home)?
                        }
                    };
                    rexux_stdio_to_uds::run(socket_path.as_path()).await?;
                }
                Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                    let options = rexux_app_server_protocol::GenerateTsOptions {
                        experimental_api: gen_cli.experimental,
                        ..Default::default()
                    };
                    rexux_app_server_protocol::generate_ts_with_options(
                        &gen_cli.out_dir,
                ...
                Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                    rexux_app_server_protocol::generate_json_with_experimental(
                        &gen_cli.out_dir,
                        gen_cli.experimental,
                    )?;
                }
                Some(AppServerSubcommand::GenerateInternalJsonSchema(gen_cli)) => {
                    rexux_app_server_protocol::generate_internal_json_schema(&gen_cli.out_dir)?;
                }
            }
        }
        ...
        Some(Subcommand::Archive(cmd)) => {
            let output = run_session_archive_cli_command(
                rexux_tui::SessionArchiveAction::Archive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
        ...
        Some(Subcommand::Unarchive(cmd)) => {
            let output = run_session_archive_cli_command(
                rexux_tui::SessionArchiveAction::Unarchive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.api_key.is_some() {
                        ...
                        eprintln!("{NO_CHATGPT_LOGIN_MESSAGE}");
                        std::process::exit(1);
                    }
                }
            }
        ...
        Some(Subcommand::Cloud(mut cloud_cli)) => {
            ...
            rexux_cloud_tasks::run_main(cloud_cli, arg0_paths.rexux_linux_sandbox_exe.clone())
                .await?;
        }
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            #[cfg(target_os = "macos")]
            rexux_cli::run_command_under_seatbelt(
                sandbox_cli,
                arg0_paths.rexux_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "linux")]
            rexux_cli::run_command_under_landlock(
                sandbox_cli,
                arg0_paths.rexux_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "windows")]
            rexux_cli::run_command_under_windows_sandbox(
                sandbox_cli,
                arg0_paths.rexux_linux_sandbox_exe.clone(),
                loader_overrides,
            )
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            ...
            tokio::task::spawn_blocking(move || rexux_responses_api_proxy::run_main(args))
                .await??;
        }
...
async fn run_exec_server_command(
    ...
    let rexux_self_exe = arg0_paths
        .rexux_self_exe
        .clone()
        .ok_or_else(|| anyhow::anyhow!("Rexux executable path is not configured"))?;
    let runtime_paths = rexux_exec_server::ExecServerRuntimePaths::new(
        rexux_self_exe,
        arg0_paths.rexux_linux_sandbox_exe.clone(),
    )?;
    if let Some(base_url) = cmd.remote {
        ...
        let auth_provider =
            load_exec_server_remote_auth_provider(&config, &base_url, cmd.use_agent_identity_auth)
                .await?;
        let mut remote_config = rexux_exec_server::RemoteEnvironmentConfig::new(
            base_url,
            environment_id,
            auth_provider,
            ...
            async move {
                ...
                match cmd.command {
                    Some(ExecServerSubcommand::Forward { connect }) => {
                        rexux_exec_server::run_remote_environment_forward_until_shutdown(
                            remote_config,
                            connect,
                            shutdown,
                        )
                    ...
                    None => {
                        rexux_exec_server::run_remote_environment_until_shutdown(
                            remote_config,
                            runtime_paths,
                            shutdown,
                        )
        ...
        let http_client_factory = config
            .as_ref()
            .map(rexux_core::config::Config::http_client_factory)
            .unwrap_or_else(|| {
        ...
        let listen_url = cmd
            .listen
            .unwrap_or_else(|| rexux_exec_server::DEFAULT_LISTEN_URL.to_string());
        exec_server_telemetry::run_until_shutdown(
            async move {
                rexux_exec_server::run_main_with_telemetry(
                    &listen_url,
                    runtime_paths,
                    telemetry,
                    http_client_factory,
...
async fn load_exec_server_remote_auth_provider(
    config: &rexux_core::config::Config,
    base_url: &str,
    use_agent_identity_auth: bool,
) -> anyhow::Result<rexux_api::SharedAuthProvider> {
    if use_agent_identity_auth {
        read_rexux_access_token_from_env().ok_or_else(|| {
            anyhow::anyhow!("REXUX_ACCESS_TOKEN is required when --use-agent-identity-auth is set")
        })?;
        let auth = AuthManager::shared_from_config(config, /*enable_rexux_api_key_env*/ false)
            .await?
            .auth()
            .await
            .ok_or_else(|| anyhow::anyhow!("Agent Identity authentication is unavailable"))?;
        if !matches!(auth, RexuxAuth::AgentIdentity(_)) {
            anyhow::bail!(
                "REXUX_ACCESS_TOKEN did not provide permitted Agent Identity authentication"
            );
        }
        return Ok(rexux_model_provider::auth_provider_from_auth(&auth));
    }

    let (auth_manager, auth) = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires ChatGPT authentication or API key authentication; run `codex login` or set REXUX_API_KEY",
    )
    .await?;
    ...
    if auth_manager.is_workload_identity_selected() {
        Ok(rexux_model_provider::auth_provider_from_auth_manager(
            auth_manager,
            &auth,
        ))
    } else {
        Ok(rexux_model_provider::auth_provider_from_auth(&auth))
    }
}

fn is_supported_exec_server_remote_auth(auth: &RexuxAuth) -> bool {
...
async fn load_exec_server_config(
...
) -> anyhow::Result<rexux_core::config::Config> {
    ...
    if enable_workload_identity && is_workload_identity_selected() {
        ...
        .await?;
        let bootstrap_auth_config = bootstrap_auth_config(&rexux_home, &bootstrap_config)?;
...
) -> anyhow::Result<(Arc<AuthManager>, rexux_login::RexuxAuth)> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_rexux_api_key_env*/ true).await?;
...
async fn enable_feature_in_config(feature: &str) -> anyhow::Result<()> {
    ...
    let rexux_home = find_rexux_home()?;
    ConfigEditsBuilder::new(&rexux_home)
        .set_feature_enabled(feature, /*enabled*/ true)
        .apply()
        .await?;
    println!("Enabled feature `{feature}` in config.toml.");
    maybe_print_under_development_feature_warning(&rexux_home, feature);
    Ok(())
}
...
fn loader_overrides_for_profile(
    ...
    match profile_v2 {
        Some(profile_v2) => {
            let rexux_home = find_rexux_home()?;
            Ok(loader_overrides_for_profile_at_rexux_home(
                Some(profile_v2),
                &rexux_home,
            ))
        }
...
fn loader_overrides_for_profile_at_rexux_home(
    profile_v2: Option<&ProfileV2Name>,
    rexux_home: &std::path::Path,
) -> LoaderOverrides {
    match profile_v2 {
        Some(profile_v2) => LoaderOverrides {
            user_config_path: Some(resolve_profile_v2_config_path(rexux_home, profile_v2)),
...
fn maybe_print_under_development_feature_warning(rexux_home: &std::path::Path, feature: &str) {
    ...
    let config_path = rexux_home.join(rexux_config::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_debug_prompt_input_command(
    ...
    let sandbox_mode = if shared.dangerously_bypass_approvals_and_sandbox {
        Some(rexux_protocol::config_types::SandboxMode::DangerFullAccess)
    } else {
    ...
    let overrides = ConfigOverrides {
        ...
        sandbox_mode,
        cwd: shared.cwd,
        rexux_self_exe: arg0_paths.rexux_self_exe,
        rexux_linux_sandbox_exe: arg0_paths.rexux_linux_sandbox_exe,
    ...
    let user_instructions_provider = Arc::new(RexuxHomeUserInstructionsProvider::new(
        config.rexux_home.clone(),
    ));
    let auth_manager =
        AuthManager::shared_from_config(&config, /*enable_rexux_api_key_env*/ false).await?;
    let mut extensions = rexux_extension_api::ExtensionRegistryBuilder::new();
    rexux_git_attribution::install(
        &mut extensions,
        auth_manager,
        config.chatgpt_base_url.clone(),
        config.http_client_factory(),
    );
    rexux_skills_extension::install(&mut extensions, |config: &Config| {
        rexux_skills_extension::SkillsExtensionConfig {
            ...
            shadow_selection_enabled: config
                .features
                .enabled(rexux_features::Feature::SkillSearch),
        }
    });
    let prompt_input = rexux_core::build_prompt_input(
        config,
        input,
        /*state_db*/ None,
...
async fn run_debug_clear_memories_command(
    ...
    clear_memory_roots_contents(&config.rexux_home).await?;
...
async fn print_app_server_daemon_output(command: AppServerLifecycleCommand) -> anyhow::Result<()> {
    let output = rexux_app_server_daemon::run(command).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}

fn updater_http_client_factory(
    config: anyhow::Result<rexux_core::config::Config>,
) -> rexux_http_client::HttpClientFactory {
    match config {
...
async fn print_app_server_remote_control_output(
    ...
    let output = rexux_app_server_daemon::set_remote_control(mode).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}
...
async fn run_interactive_tui(
    ...
    let terminal_info = rexux_terminal_detection::terminal_info();
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". Rexux's interactive TUI may not work in this terminal."
        );
    ...
    if interactive.agents_overview && remote.is_none() {
        ...
        rexux_app_server_daemon::run(AppServerLifecycleCommand::Start)
            .await
            .map_err(std::io::Error::other)?;
    }
    ...
    let start_tui = || {
        rexux_tui::run_main(
            interactive.clone(),
            arg0_paths.clone(),
            rexux_config::LoaderOverrides::default(),
        ...
        match local_state_db::backup_files_for_fresh_start(startup_error).await {
            ...
            Err(backup_err) => {
                ...
                return Ok(AppExitInfo::fatal(format!(
                    "failed to move damaged Rexux local database files into a backup folder automatically: {backup_err}"
                )));
            }
        }
    }
...
fn resolve_remote_endpoint(
    remote: Option<String>,
    remote_auth_token_env: Option<String>,
) -> std::io::Result<Option<rexux_tui::RemoteAppServerEndpoint>> {
    let mut remote_endpoint = remote
        .as_deref()
        .map(rexux_tui::resolve_remote_addr)
        .transpose()
        .map_err(std::io::Error::other)?;
    if let Some(remote_auth_token_env) = remote_auth_token_env {
        ...
        if !rexux_tui::remote_addr_supports_auth_token(endpoint) {
            return Err(std::io::Error::other(
        ...
        let auth_token = read_remote_auth_token_from_env_var(&remote_auth_token_env)
            .map_err(std::io::Error::other)?;
        let rexux_tui::RemoteAppServerEndpoint::WebSocket {
            auth_token: slot, ..
        } = endpoint
        else {
            return Err(std::io::Error::other(
...
mod tests {
    ...
    async fn updater_http_client_factory_honors_respect_system_proxy() {
        let rexux_home = tempfile::tempdir().expect("temporary Rexux home");
        let config = ConfigBuilder::default()
    ...
    fn exec_server_remote_auth_accepts_api_key_auth() {
        let auth = RexuxAuth::from_api_key("sk-test");
    ...
    #[test]
    fn profile_loader_overrides_use_explicit_rexux_home() -> anyhow::Result<()> {
        let rexux_home = tempfile::tempdir()?;
        let profile: ProfileV2Name = "work".parse()?;

        let overrides =
            loader_overrides_for_profile_at_rexux_home(Some(&profile), rexux_home.path());

        assert_eq!(
            overrides.user_config_path,
            Some(resolve_profile_v2_config_path(rexux_home.path(), &profile))
        );
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        ...
        let Some(rexux_exec::Command::Resume(args)) = exec.command else {
    ...
    fn later_exec_sandbox_partially_overrides_approve_for_me() {
        ...
        assert_matches!(
            exec.sandbox_mode,
            Some(rexux_utils_cli::SandboxModeCliArg::ReadOnly)
        );
        assert_eq!(
            exec.config_overrides.raw_overrides,
            vec![
    ...
    fn later_resume_approval_policy_partially_overrides_approve_for_me() {
        ...
        assert_matches!(
            ...
            Some(rexux_utils_cli::ApprovalModeCliArg::Never)
        );
        assert_eq!(
    ...
    fn default_app_server_socket_path() -> AbsolutePathBuf {
        let rexux_home = find_rexux_home().expect("codex home");
        rexux_app_server::app_server_control_socket_path(&rexux_home)
    ...
    fn sample_exit_info(conversation_id: Option<&str>, thread_name: Option<&str>) -> AppExitInfo {
        ...
        AppExitInfo {
            token_usage,
            thread_id,
            resume_hint: thread_id.map(|thread_id| rexux_tui::ResumableThread {
                thread_id,
    ...
    async fn format_exit_messages_preserves_auth_env_through_tui_runner() {
        let exit_info = run_tui_with_recovery(
            || async {
                ...
                exit_info.disconnect_info = Some(rexux_tui::DisconnectInfo {
                    command: vec![
                        "codex".to_string(),
                        "--remote".to_string(),
        ...
        assert_eq!(
            ...
            vec![
                ...
                "Reconnect: codex --remote wss://example.com:443/ --remote-auth-token-env REXUX_REMOTE_TOKEN resume 123e4567-e89b-12d3-a456-426614174000",
                "Stop the current turn: run codex --remote wss://example.com:443/ --remote-auth-token-env REXUX_REMOTE_TOKEN agents, select this task, and press ctrl + x.",
    ...
    fn resume_merges_option_flags() {
        ...
        assert_matches!(
            ...
            Some(rexux_utils_cli::SandboxModeCliArg::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(rexux_utils_cli::ApprovalModeCliArg::OnRequest)
        );
        assert_eq!(
            interactive.cwd.as_deref(),
    ...
    fn read_remote_auth_token_from_env_var_reports_missing_values() {
        let err = read_remote_auth_token_from_env_var_with("REXUX_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn read_remote_auth_token_from_env_var_trims_values() {
        let auth_token =
            read_remote_auth_token_from_env_var_with("REXUX_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn app_server_listen_websocket_url_parses() {
        ...
        assert_eq!(
            app_server.listen,
            rexux_app_server::AppServerTransport::WebSocket {
    ...
    fn app_server_listen_unix_socket_url_parses() {
        ...
        assert_eq!(
            app_server.listen,
            rexux_app_server::AppServerTransport::UnixSocket {
    ...
    fn app_server_listen_off_parses() {
        ...
        assert_eq!(app_server.listen, rexux_app_server::AppServerTransport::Off);
    }
    ...
    fn app_server_capability_token_flags_parse() {
        ...
        assert_eq!(
            ...
            Some(rexux_app_server::WebsocketAuthCliMode::CapabilityToken)
        );
        assert_eq!(
            app_server.auth.ws_token_file,
            Some(PathBuf::from("/tmp/rexux-token"))
        );
    }
    ...
    fn app_server_signed_bearer_flags_parse() {
        ...
        assert_eq!(
            app_server.auth.ws_auth,
            Some(rexux_app_server::WebsocketAuthCliMode::SignedBearerToken)
        );
        assert_eq!(
            app_server.auth.ws_shared_secret_file,
            Some(PathBuf::from("/tmp/rexux-secret"))
        );
