...
pub(super) async fn background_server_check(config: &Config) -> DoctorCheck {
    ...
    let state_dir = config.rexux_home.join(STATE_DIR_NAME);
    ...
    let socket_path = match rexux_app_server::app_server_control_socket_path(&config.rexux_home) {
...
async fn socket_status(socket_path: &Path) -> SocketStatus {
    ...
    match rexux_app_server_daemon::probe_app_server_version(socket_path).await {
...
mod tests {
    ...
    async fn test_config(rexux_home: PathBuf) -> Config {
    ...
    fn create_socket_placeholder(config: &Config) {
        let socket_path = rexux_app_server::app_server_control_socket_path(&config.rexux_home)
            .expect("socket path");
