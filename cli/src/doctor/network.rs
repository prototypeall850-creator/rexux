...
#[cfg(target_os = "macos")]
use rexux_http_client::MacosSystemProxyConfiguration;
use rexux_http_client::RouteAwareClientPool;
use rexux_http_client::RouteAwareRequestError;
use rexux_http_client::RouteFailureClass;
#[cfg(target_os = "macos")]
use rexux_http_client::macos_system_proxy_configuration;
...
pub(super) fn check(config: Option<&Config>) -> DoctorCheck {
    ...
    for name in ["REXUX_CA_CERTIFICATE", "SSL_CERT_FILE"] {
...
pub(super) async fn probe_status(
    ...
    let response = if env::var("REXUX_SANDBOX").as_deref() == Ok("seatbelt") {
