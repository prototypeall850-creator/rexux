...
fn event_target(value: &str) -> Option<&'static str> {
    ...
    let name = *parts.last()?;
    let package = parts.iter().any(|part| part.starts_with("openai.rexux_"));
    let trusted = package || parts.windows(2).any(|pair| pair == ["openai", "codex"] || pair == ["rexux", "rexux"]);
    match name {
        "rexux-windows-sandbox-setup.exe" => Some("sandbox_setup"),
        "rexux-command-runner.exe" => Some("command_runner"),
        "codex.exe" => Some("codex"),
        "rexux.exe" => Some("rexux"),
        "rexux-desktop.exe" => Some("rexux_desktop"),
        "chatgpt.exe" | "electron.exe" if trusted => Some("rexux_desktop"),
        "rg.exe" if trusted => Some("ripgrep"),
        _ if package => Some("rexux_desktop_package"),
        _ => None,
    }
}
...
fn classify(channels: &[Option<Vec<Evidence>>]) -> DoctorCheck {
    ...
    let summary = match (status, visible) {
        ...
        (CheckStatus::Ok, _) => "no locally visible recent Rexux security enforcement was found",
        (CheckStatus::Warning, false) => "security event channels could not be inspected",
        (CheckStatus::Warning, true) => "recent Rexux security audit or detection requires review",
        (CheckStatus::Fail, _) => "endpoint security blocked or quarantined a Rexux executable",
    };
    let mut check = desktop_check("desktop.security.enforcement", status, summary).details(details);
    if status != CheckStatus::Ok {
        check = check.remediation(
            "ask your organization's security administrator to review endpoint security events and the approved Rexux application policy",
        );
    }
    check
}
