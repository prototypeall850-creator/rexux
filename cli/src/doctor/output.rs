...
fn header_suffix(report: &DoctorReport) -> String {
    let version = format!("v{}", report.rexux_version);
    report
        .checks
        .iter()
...
fn update_note(check: &DoctorCheck, report: &DoctorReport) -> Option<DoctorNote> {
    ...
    let mut parenthetical = format!("current {}", report.rexux_version);
...
",
    ...
    fn render_human_report_snapshot_covers_environment_rows() {
        ...
        *targets = "exclusion targets: verified Rexux app and required helpers".into();
...
",
    ...
    fn redact_detail_preserves_env_var_names() {
        assert_eq!(
            redact_detail("auth env vars present: OPENAI_API_KEY, REXUX_API_KEY"),
            "auth env vars present: OPENAI_API_KEY, REXUX_API_KEY"
        );
    }
