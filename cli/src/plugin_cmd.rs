...
use rexux_core_plugins::ConfiguredMarketplace;
use rexux_core_plugins::OPENAI_BUNDLED_MARKETPLACE_NAME;
use rexux_core_plugins::PluginInstallOutcome;
...
use rexux_core_plugins::allowed_configured_marketplace_names;
use rexux_core_plugins::installed_marketplaces::marketplace_install_root;
use rexux_core_plugins::installed_marketplaces::resolve_configured_marketplace_root;
use rexux_core_plugins::marketplace::MarketplaceListError;
use rexux_core_plugins::marketplace::MarketplacePluginAuthPolicy;
use rexux_core_plugins::marketplace::MarketplacePluginInstallPolicy;
use rexux_core_plugins::marketplace::MarketplacePluginSource;
use rexux_core_plugins::marketplace::find_marketplace_manifest_path;
use rexux_core_plugins::remote;
...
use rexux_core_plugins::remote::RemotePluginSummary;
...
pub async fn run_plugin_list(
    ...
    let marketplace_sources = configured_marketplace_sources(&plugins_input, rexux_home.as_path());
...
pub(crate) fn configured_marketplace_sources(
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names(&plugins_input.config_layer_stack, rexux_home);
...
pub(crate) async fn load_cli_auth_manager(config: &Config) -> Result<Arc<AuthManager>> {
    Ok(AuthManager::shared_from_config(config, /*enable_rexux_api_key_env*/ true).await?)
}
...
async fn fetch_remote_marketplaces(
    ...
    let auth = context.auth.as_ref();
    if !auth.is_some_and(RexuxAuth::uses_rexux_backend) {
        ensure!(
    ...
    let result = if context.plugins_input.remote_plugin_enabled {
        remote::fetch_remote_marketplaces(
            ...
            /*catalog_cache_root*/ Some(context.rexux_home.as_path()),
            cache_mode,
        )
        .await
...
pub(crate) fn configured_marketplace_snapshot_issues(
    ...
    for (configured_name, marketplace) in configured_marketplaces {
        ...
        match find_marketplace_manifest_path(&root) {
            ...
            None => {
                if is_implicit_system_marketplace_root(configured_name, rexux_home, &root) {
                    continue;
                }
                issues.push(ConfiguredMarketplaceSnapshotIssue {
                    marketplace_name: configured_name.clone(),
