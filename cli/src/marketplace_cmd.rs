...
use rexux_core_plugins::PluginMarketplaceUpgradeOutcome;
...
use rexux_core_plugins::marketplace::marketplace_root_dir;
use rexux_core_plugins::marketplace_add::MarketplaceAddOutcome;
use rexux_core_plugins::marketplace_add::MarketplaceAddRequest;
use rexux_core_plugins::marketplace_add::add_marketplace;
use rexux_core_plugins::marketplace_remove::MarketplaceRemoveOutcome;
use rexux_core_plugins::marketplace_remove::MarketplaceRemoveRequest;
...
enum MarketplaceSubcommand {
    ...
    /// List plugin marketplaces Rexux is currently considering and their roots.
...
async fn run_list(config: Config, args: ListMarketplaceArgs) -> Result<()> {
    ...
    if args.json {
        let marketplace_sources =
            configured_marketplace_sources_by_root(config.rexux_home.as_path(), &plugins_input);
        let output =
...
impl JsonMarketplaceListOutput {
    fn from_marketplaces(
        marketplaces: Vec<rexux_core_plugins::marketplace::Marketplace>,
...
fn configured_marketplace_sources_by_root(
    ...
    let marketplace_sources = configured_marketplace_sources(plugins_input, rexux_home);
...
async fn run_upgrade(
    ...
    builder: rexux_core::config::ConfigBuilder,
) -> Result<()> {
    ...
    let reload_config: rexux_core_plugins::ConfigLayerReload = std::sync::Arc::new(move || {
        runtime
