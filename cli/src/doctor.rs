...
use rexux_api::ResponsesWebsocketClient;
use rexux_api::is_azure_responses_provider;
...
use rexux_install_context::RexuxPackageLayout;
use rexux_install_context::InstallContext;
use rexux_install_context::InstallMethod;
use rexux_install_context::StandalonePlatform;
use rexux_login::AuthDotJson;
use rexux_login::AuthManager;
use rexux_login::REXUX_ACCESS_TOKEN_ENV_VAR;
use rexux_login::REXUX_API_KEY_ENV_VAR;
use rexux_login::RexuxAuth;
use rexux_login::OPENAI_API_KEY_ENV_VAR;
use rexux_login::default_client::create_client_without_request_logging;
...
/// Options for building a local Rexux diagnostic report.
///
...
async fn build_report(
    ...
    DoctorReport {
        ...
        overall_status,
        rexux_version: env!("CARGO_PKG_VERSION").to_string(),
        checks,
    }
}
...
async fn load_config(
    ...
    ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load Rexux config")
}
...
fn redacted_json_report(report: &DoctorReport) -> JsonDoctorReport {
    ...
    JsonDoctorReport {
        ...
        rexux_version: report.rexux_version.clone(),
        checks,
    }
}
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    details.push(format!(
        "managed by bun: {}",
        env::var_os("REXUX_MANAGED_BY_BUN").is_some()
    ));
    details.push(format!(
        "managed by Vite+: {}",
        env::var_os("REXUX_MANAGED_BY_VITE_PLUS").is_some()
    ));
    details.push(format!(
        "managed by pnpm: {}",
        env::var_os("REXUX_MANAGED_BY_PNPM").is_some()
    ));
    push_env_path_detail(
        &mut details,
        "managed package root",
        "REXUX_MANAGED_PACKAGE_ROOT",
    );

    let path_entries = rexux_path_entries();
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::MissingPackageRoot => {
                ...
                remediation = Some(
                    "Reinstall or update Rexux so the JS shim provides REXUX_MANAGED_PACKAGE_ROOT."
                        .to_string(),
                );
            }
...
fn doctor_managed_by_npm(current_exe: Option<&Path>) -> bool {
    env::var_os("REXUX_MANAGED_BY_NPM").is_some()
        && !inherited_managed_env_for_cargo_binary(current_exe)
}

fn inherited_managed_env_for_cargo_binary(current_exe: Option<&Path>) -> bool {
    if env::var_os("REXUX_MANAGED_BY_NPM").is_none()
        && env::var_os("REXUX_MANAGED_BY_BUN").is_none()
        && env::var_os("REXUX_MANAGED_BY_VITE_PLUS").is_none()
        && env::var_os("REXUX_MANAGED_BY_PNPM").is_none()
    {
        return false;
    }
...
fn describe_method_with_package_layout(
    method: &str,
    package_layout: Option<&RexuxPackageLayout>,
) -> String {
...
fn npm_global_root_check() -> NpmRootCheck {
    let Some(running_package_root) = env::var_os("REXUX_MANAGED_PACKAGE_ROOT").map(PathBuf::from)
    else {
...
fn rexux_path_entries() -> Vec<String> {
    #[cfg(windows)]
...
fn config_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    details.push(format!("REXUX_HOME: {}", config.rexux_home.display()));
...
fn config_toml_details(config: &Config, details: &mut Vec<String>) {
    let config_path = config.rexux_home.join(rexux_config::CONFIG_TOML_FILE);
...
fn auth_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    let auth_path = config.rexux_home.join("auth.json");
    details.push(format!(
        "auth storage mode: {:?}",
    ...
    let env_auth_vars = [
        ...
        REXUX_API_KEY_ENV_VAR,
        REXUX_ACCESS_TOKEN_ENV_VAR,
    ]
    .into_iter()
    ...
    match load_auth_dot_json(
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no Rexux credentials were found",
        )
        .details(details)
...
fn stored_auth_mode(auth: &rexux_login::AuthDotJson) -> &'static str {
...
fn stored_auth_issues(
    ...
    match stored_auth_mode_value(auth) {
        AuthMode::ApiKey => {
            ...
            let env_key_present =
                env_var_present(OPENAI_API_KEY_ENV_VAR) || env_var_present(REXUX_API_KEY_ENV_VAR);
...
async fn state_check(config: &Config, command: &DoctorCommand) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "REXUX_HOME", &config.rexux_home);
    ...
    rollout_stats_details(&mut details, &config.rexux_home);
...
async fn sqlite_integrity_detail(
    sqlite: &rexux_state::SqliteConfig,
    details: &mut Vec<String>,
    label: &str,
    path: &Path,
    ...
    let (rows, timed_out) = match rexux_state::sqlite_integrity_check(sqlite, path, deadline).await
    {
        Ok(rexux_state::SqliteIntegrityCheck::Complete(rows)) => (rows, false),
        Ok(rexux_state::SqliteIntegrityCheck::TimedOut(rows)) => (rows, true),
        Err(err) => {
...
fn rollout_stats_details(details: &mut Vec<String>, rexux_home: &Path) {
    let active = collect_rollout_stats(&rexux_home.join("sessions"));
    let archived = collect_rollout_stats(&rexux_home.join("archived_sessions"));
...
fn auth_mode_name(auth: &RexuxAuth) -> &'static str {
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match rexux_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "REXUX_HOME was resolved without config",
        )
        .detail(format!("REXUX_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "REXUX_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
...
fn provider_auth_reachability_mode_from_auth(
    ...
    if provider_base_url.is_some_and(|url| !url.trim().is_empty())
        ...
        || env_var_present(REXUX_API_KEY_ENV_VAR)
    {
        return ProviderAuthReachabilityMode::ApiKey;
    }
    if env_var_present(REXUX_ACCESS_TOKEN_ENV_VAR) {
...
mod tests {
    ...
    fn startup_warning_counts_group_known_sources() {
        let warnings = vec![
            ...
            "[features].rexux_hooks is deprecated. Use [features].hooks instead.".to_string(),
    ...
    fn config_overrides_from_interactive_preserves_global_options() {
        ...
        let arg0_paths = Arg0DispatchPaths {
            rexux_self_exe: Some(PathBuf::from("/bin/codex")),
            rexux_linux_sandbox_exe: Some(PathBuf::from("/bin/rexux-linux-sandbox")),
            main_execve_wrapper_exe: Some(PathBuf::from("/bin/rexux-execve-wrapper")),
        };
        ...
        assert_eq!(overrides.rexux_self_exe, arg0_paths.rexux_self_exe);
        assert_eq!(
            overrides.rexux_linux_sandbox_exe,
            arg0_paths.rexux_linux_sandbox_exe
        );
        assert_eq!(
    ...
    async fn mcp_check_ignores_disabled_servers() {
        let disabled_server: McpServerConfig = toml::from_str(
            r#"
                ...
                enabled = false
                required = true
                bearer_token_env_var = "REXUX_DOCTOR_DISABLED_MCP_TOKEN"
            "#,
        )
        ...
        assert!(
            check
                .details
                .iter()
                .all(|detail| !detail.contains("REXUX_DOCTOR_DISABLED_MCP_TOKEN"))
        );
        assert!(
            check
                .details
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running Rexux."),
            Vec::new(),
            |_| false,
        )
        .expect("non-OpenAI provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running Rexux.".to_string())
        );
    }
    ...
    fn provider_reachability_mode_uses_api_key_auth() {
        ...
        assert_eq!(
            provider_auth_reachability_mode_from_auth(
                ...
                /*provider_env_key*/ None,
                /*provider_base_url*/ None,
                |name| name == REXUX_API_KEY_ENV_VAR,
                /*stored_auth*/ None,
            ),
            ProviderAuthReachabilityMode::ApiKey
        );
    ...
    async fn mcp_check_fails_required_missing_stdio_command() {
        let required_server: McpServerConfig = toml::from_str(
            r#"
                command = "definitely-missing-rexux-doctor-mcp"
                required = true
            "#,
        )
        .expect("should deserialize required MCP config");
        ...
        assert!(check.details.iter().any(|detail| {
            detail.contains(
                "required: stdio command \"definitely-missing-rexux-doctor-mcp\" is not resolvable",
            )
        }));
    }
