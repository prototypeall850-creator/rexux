...
#[derive(Debug, Parser)]
#[command(name = "rexux-state-logs")]
#[command(about = "Tail Rexux logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to REXUX_HOME. Defaults to $REXUX_HOME or ~/.rexux.
    #[arg(long, env = "REXUX_HOME")]
    rexux_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --rexux-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn resolve_sqlite_config(args: &Args) -> anyhow::Result<SqliteConfig> {
    ...
    if let Some(rexux_home) = args.rexux_home.as_ref() {
        config_builder = config_builder.rexux_home(rexux_home.clone());
    }
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["rexux-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["rexux-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["rexux-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["rexux-state-logs", "--level", "WARN"])
            .expect("parse uppercase log level");

        assert_eq!(args.level, Some(LogLevelThreshold::Warn));
    }

    /// Explicit database selection must not parse an overridden Rexux home.
    #[tokio::test]
    async fn direct_db_skips_rexux_home_config() {
        let rexux_home = tempfile::tempdir().expect("create Rexux home");
        std::fs::write(rexux_home.path().join("config.toml"), "model = [")
            .expect("write invalid config");
        let sqlite_home = tempfile::tempdir().expect("create SQLite home");
        let db_path = sqlite_home.path().join("logs_2.sqlite");
        let args = Args::try_parse_from([
            OsString::from("rexux-state-logs"),
            OsString::from("--rexux-home"),
            rexux_home.path().as_os_str().to_owned(),
            OsString::from("--db"),
