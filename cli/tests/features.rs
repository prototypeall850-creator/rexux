...
fn interactive_validates_config_before_requiring_terminal() -> Result<()> {
    ...
    for &(args, config_file, contents, expected_error) in cases {
        let rexux_home = TempDir::new()?;
        std::fs::write(rexux_home.path().join(config_file), contents)?;
...
-> Result<()> {
    let rexux_home = TempDir::new()?;
    std::fs::write(rexux_home.path().join("config.toml"), "")?;
    std::fs::write(
...
"#,
    ...
    let remote_only_cwd = rexux_home.path().join("remote-only-working-directory");
