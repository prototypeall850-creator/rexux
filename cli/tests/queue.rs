...
fn queue_rejects_empty_message() -> Result<()> {
    let output = std::process::Command::new(rexux_utils_cargo_bin::cargo_bin("codex")?)
...
async fn run_remote_queue_command(response: QueueResponse) -> Result<(Output, Value)> {
    ...
    let server_home = rexux_home.path().to_path_buf();
    ...
    let output = tokio::process::Command::new(rexux_utils_cargo_bin::cargo_bin("codex")?)
        .env("REXUX_HOME", rexux_home.path())
        .env("REXUX_REMOTE_TOKEN", "test-token")
        .args(remote_args)
...
async fn queue_rejects_local_daemon_that_does_not_support_queueing() -> Result<()> {
    let rexux_home = tempfile::tempdir_in("/tmp")?;
    let socket_path = rexux_app_server::app_server_control_socket_path(rexux_home.path())?;
    ...
    assert!(!rexux_home.path().join("queue_1.sqlite").exists());
    Ok(())
}
