...
use rexux_login::CLIENT_ID;
use rexux_login::REXUX_ACCESS_TOKEN_ENV_VAR;
use rexux_login::REVOKE_TOKEN_URL_OVERRIDE_ENV_VAR;
use rexux_login::login_with_bedrock_access_keys;
...
fn write_file_auth_config(rexux_home: &Path) -> Result<()> {
    std::fs::write(
...
fn read_auth_json(rexux_home: &Path) -> Result<Value> {
    let auth_json = std::fs::read_to_string(rexux_home.join("auth.json"))?;
...
fn login_with_api_key_reads_stdin_and_writes_auth_json() -> Result<()> {
    ...
    write_file_auth_config(rexux_home.path())?;
    ...
    let auth = read_auth_json(rexux_home.path())?;
...
fn login_status_reports_auth_storage_errors() -> Result<()> {
    let rexux_home = TempDir::new()?;
    write_file_auth_config(rexux_home.path())?;
    std::fs::write(rexux_home.path().join("auth.json"), "{invalid json")?;
...
fn login_status_validates_configured_workload_identity() -> Result<()> {
    let rexux_home = TempDir::new()?;
    write_file_auth_config(rexux_home.path())?;
    let missing_assertion = rexux_home.path().join("missing-identity-token");

    rexux_command(rexux_home.path())?
        .env_remove(REXUX_ACCESS_TOKEN_ENV_VAR)
...
async fn debug_prompt_input_follows_authenticated_attribution_setting() -> Result<()> {
    ...
    for enabled in [true, false] {
        let output = rexux_command(rexux_home.path())?
        ...
        assert_eq!(
            prompt.contains("Co-authored-by: Rexux <noreply@openai.com>"),
            enabled
        );
