...
use rexux_exec_server::ExecServerClient;
use rexux_exec_server::NoiseChannelIdentity;
use rexux_exec_server::NoiseChannelPublicKey;
use rexux_exec_server::NoiseRendezvousConnectArgs;
...
fn local_exec_server_ignores_invalid_config_without_strict_config() -> Result<()> {
    let rexux_home = TempDir::new()?;
    std::fs::write(rexux_home.path().join("config.toml"), "not valid toml = [")?;
...
fn local_exec_server_allows_disabled_parent_lifetime_environment_variable() -> Result<()> {
    ...
    let mut cmd = rexux_command(rexux_home.path())?;
    cmd.env(
        rexux_exec_server::REXUX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
        "false",
    )
...
"#
    ...
    let mut command = tokio::process::Command::new(rexux_utils_cargo_bin::cargo_bin("codex")?);
    command
        .env("REXUX_HOME", rexux_home.path())
        .env("REXUX_API_KEY", "test-api-key")
        .env(
            rexux_exec_server::REXUX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
            "true",
        )
    ...
    let argv = vec![
        "cmd.exe",
        "/C",
        "if defined REXUX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE (exit /b 1) else ping -n 61 127.0.0.1",
    ];
    #[cfg(not(windows))]
    let argv = vec![
        "/bin/sh",
        "-c",
        "[ -z \"${REXUX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE+present}\" ] && exec /bin/sleep 60",
    ];
    ...
    client
        .exec(ExecParams {
            ...
            shell_snapshot: None,
            env_policy: Some(rexux_exec_server::ExecEnvPolicy {
                inherit: rexux_protocol::config_types::ShellEnvironmentPolicyInherit::All,
...
"#
    ...
    let rexux_bin = rexux_utils_cargo_bin::cargo_bin("codex")?;
    let rexux_home = rexux_home.path().to_path_buf();
    let subprocess = async move {
        let mut command = tokio::process::Command::new(rexux_bin);
        command
...
fn local_exec_server_exits_successfully_on_sigterm() -> Result<()> {
    let rexux_home = TempDir::new()?;
    let mut child = std::process::Command::new(rexux_utils_cargo_bin::cargo_bin("codex")?)
        .env("REXUX_HOME", rexux_home.path())
