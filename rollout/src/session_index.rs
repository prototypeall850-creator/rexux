...
pub async fn append_thread_name(
    ...
    append_session_index_entry(rexux_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(rexux_home);
...
fn session_index_path(rexux_home: &Path) -> PathBuf {
    rexux_home.join(SESSION_INDEX_FILE)
}
