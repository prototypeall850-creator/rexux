...
//! existing rollout path. They must not do that at the same time for one Rexux home, so they share
...
/// Try to exclude rollout compression and migration for one Rexux home.
pub fn try_acquire_rollout_maintenance_lock(
    rexux_home: &Path,
) -> io::Result<Option<RolloutMaintenanceGuard>> {
    let directory = rexux_home.join(".tmp");
