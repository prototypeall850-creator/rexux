...
use crate::state_db;
use rexux_file_search as file_search;
...
pub struct ThreadItem {
    ...
    pub section: Option<rexux_state::ThreadSection>,
...
impl From<rexux_state::Anchor> for Cursor {
    fn from(anchor: rexux_state::Anchor) -> Self {
        let ts = anchor
            .ts
            .timestamp_nanos_opt()
...
pub async fn get_threads(
    ...
    let root = rexux_home.join(SESSIONS_SUBDIR);
    get_threads_in_root(
        root,
        page_size,
        cursor,
...
///
/// Directory layout: `~/.rexux/sessions/YYYY/MM/DD/rollout-YYYY-MM-DDThh-mm-ss-<uuid>.jsonl`
...
async fn find_thread_path_by_id_str_in_subdir(
    ...
    subdir: &str,
    id_str: &str,
    state_db_ctx: Option<&rexux_state::StateRuntime>,
    ...
    let mut root = rexux_home.to_path_buf();
    root.push(subdir);
    if !root.exists() {
...
pub async fn find_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(rexux_home, SESSIONS_SUBDIR, id_str, state_db_ctx).await
}
...
pub async fn find_archived_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(rexux_home, ARCHIVED_SESSIONS_SUBDIR, id_str, state_db_ctx)
        .await
}
...
pub async fn find_rollout_path_by_rollout_id(
    ...
    for subdir in [SESSIONS_SUBDIR, ARCHIVED_SESSIONS_SUBDIR] {
        let path = find_rollout_path_by_rollout_id_from_filenames(
            rexux_home.join(subdir).as_path(),
            rollout_id,
        )
        .await?;
        if path.is_some() {
