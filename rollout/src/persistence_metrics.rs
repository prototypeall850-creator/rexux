...
fn rollout_item_type(item: &RolloutItem) -> String {
    match item {
        ...
        RolloutItem::RealtimeItem(item) => match &item.content {
            rexux_protocol::realtime::RealtimeItemContent::RealtimeSessionStarted => {
                "realtime.session_started".to_string()
            }
            rexux_protocol::realtime::RealtimeItemContent::TranscriptSegment { .. } => {
                "realtime.transcript_segment".to_string()
            }
            rexux_protocol::realtime::RealtimeItemContent::BemItemPromoted { .. } => {
                "realtime.bem_item_promoted".to_string()
            }
            rexux_protocol::realtime::RealtimeItemContent::RealtimeSessionClosed { .. } => {
