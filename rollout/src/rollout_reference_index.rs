...
impl RolloutReferenceIndex {
    ...
    pub async fn scan(rexux_home: &Path) -> io::Result<Self> {
        Self::scan_paths(vec![
    ...
    pub async fn scan_unarchived(rexux_home: &Path) -> io::Result<Self> {
        Self::scan_paths(vec![rexux_home.join(SESSIONS_SUBDIR)]).await
    }
