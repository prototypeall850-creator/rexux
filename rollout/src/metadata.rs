...
use rexux_state::BackfillState;
use rexux_state::BackfillStats;
use rexux_state::BackfillStatus;
use rexux_state::DB_ERROR_METRIC;
use rexux_state::DB_METRIC_BACKFILL;
use rexux_state::DB_METRIC_BACKFILL_DURATION_MS;
use rexux_state::ExtractionOutcome;
use rexux_state::ThreadMetadataBuilder;
use rexux_state::apply_rollout_item;
use std::path::Path;
...
pub(crate) async fn backfill_sessions_with_lease(
    ...
    let metric_client = rexux_otel::global();
    ...
    let sessions_root = rexux_home.join(SESSIONS_SUBDIR);
    let archived_root = rexux_home.join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(rexux_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(rexux_home: &Path, path: &Path) -> String {
    path.strip_prefix(rexux_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
