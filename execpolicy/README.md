...
- From the Rexux CLI, run `rexux execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
rexux execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
rexux execpolicy check \
...
```bash
cargo run -p rexux-execpolicy -- check --rules path/to/policy.rules git status
```
