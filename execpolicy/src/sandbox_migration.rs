...
///
/// this migration is intentionally one-shot. once complete, a marker in `rexux_home` prevents
...
pub async fn prefix_rule_migration(
    ...
    let marker_path = rexux_home.join(MIGRATION_MARKER_FILENAME);
    if tokio::fs::try_exists(&marker_path).await? {
        return Ok(());
    }
    clean_rules_file(policy_path, banned_prefixes).await?;

    write_migration_marker(rexux_home, &marker_path).await?;
    Ok(())
}

// atomically writes the marker after creating codex home when needed.
async fn write_migration_marker(rexux_home: &Path, marker_path: &Path) -> io::Result<()> {
    tokio::fs::create_dir_all(rexux_home).await?;
    let rexux_home = rexux_home.to_owned();
    let marker_path = marker_path.to_owned();
    tokio::task::spawn_blocking(move || {
        let mut marker = tempfile::NamedTempFile::new_in(rexux_home)?;
