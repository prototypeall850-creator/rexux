...
pub struct ExecApprovalElicitRequestParams {
    ...
    pub rexux_command: Vec<String>,
    pub rexux_cwd: PathBuf,
    pub rexux_parsed_cmd: Vec<ParsedCommand>,
}
...
pub(crate) async fn handle_exec_approval_request(
    ...
    call_id: String,
    approval_id: String,
    rexux_parsed_cmd: Vec<ParsedCommand>,
    thread_id: ThreadId,
) {
    let escaped_command =
        shlex::try_join(command.iter().map(String::as_str)).unwrap_or_else(|_| command.join(" "));
    let message = format!(
        "Allow Rexux to run `{escaped_command}` in `{cwd}`?",
