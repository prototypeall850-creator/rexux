//! Asynchronous worker that executes a **Rexux** tool-call inside a spawned
...
use rexux_core::config::Config as RexuxConfig;
...
/// To adhere to MCP `tools/call` response format, include the Rexux
...
/// Run a complete Rexux session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_rexux_tool_session(
    id: RequestId,
        ...
        .await
    {
        ...
        Err(e) => {
            let result = CallToolResult::error(vec![ContentBlock::text(format!(
                "Failed to start Rexux session: {e}"
            ))]);
    ...
    run_rexux_tool_session_inner(thread_id, thread, outgoing, id, active_turns).await;
}

pub async fn run_rexux_tool_session_reply(
    thread_id: ThreadId,
    ...
    run_rexux_tool_session_inner(thread_id, thread, outgoing, request_id, active_turns).await;
}

async fn run_rexux_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<RexuxThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
            ...
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(rexux_event_to_notification(&event)) above has
