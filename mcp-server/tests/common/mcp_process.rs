...
impl McpProcess {
    pub async fn new(rexux_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(rexux_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = rexux_utils_cargo_bin::cargo_bin("rexux-mcp-server")
            .context("should find binary for rexux-mcp-server")?;
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("rexux-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = rexux_login::default_client::originator().value;
    ...
    pub async fn send_rexux_tool_call(
        &mut self,
        params: RexuxToolCallParam,
    ) -> anyhow::Result<i64> {
        let rexux_tool_call_params = CallToolRequestParams::new("codex").with_arguments(
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(rexux_tool_call_params)?),
        )
        .await
    }
    ...
    pub async fn read_stream_until_legacy_task_complete_notification(
        ...
        self.read_stream_until_rexux_event("task_complete").await
    }

    pub async fn read_stream_until_rexux_event(
        &mut self,
        event_type: &str,
    ) -> anyhow::Result<JsonRpcNotification<CustomNotification>> {
        self.read_stream_until_rexux_event_matching(event_type, |_| true)
            .await
    }

    pub async fn read_stream_until_rexux_event_matching(
        &mut self,
        event_type: &str,
        predicate: impl Fn(&serde_json::Value) -> bool,
    ) -> anyhow::Result<JsonRpcNotification<CustomNotification>> {
        eprintln!("in read_stream_until_rexux_event({event_type})");
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `rexux-mcp-server` child process.
        //
