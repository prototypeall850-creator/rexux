...
use rexux_core::spawn::REXUX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use rexux_mcp_server::RexuxToolCallParam;
use rexux_mcp_server::ExecApprovalElicitRequestParams;
use rexux_mcp_server::ExecApprovalResponse;
use rexux_mcp_server::PatchApprovalElicitRequestParams;
use rexux_mcp_server::PatchApprovalResponse;
use rexux_protocol::protocol::FileChange;
use rexux_protocol::protocol::ReviewDecision;
use rexux_shell_command::parse_command;
...
async fn test_exec_command_approval_triggers_elicitation() {
    if env::var(REXUX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Rexux sandbox."
        );
        return;
    }
...
async fn exec_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // as an elicitation.
    let rexux_request_id = mcp_process
        .send_rexux_tool_call(RexuxToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            rexux_request_id.to_string(),
            params.rexux_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    let rexux_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(rexux_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(rexux_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    rexux_mcp_tool_call_id: String,
    rexux_event_id: String,
    thread_id: rexux_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow Rexux to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let rexux_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        rexux_elicitation: "exec-approval".to_string(),
        rexux_mcp_tool_call_id,
        rexux_event_id,
        rexux_command: command,
        rexux_cwd: workdir.to_path_buf(),
        rexux_call_id: "call1234".to_string(),
        rexux_parsed_cmd,
    })?;
    Ok(params_json)
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_rexux_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    rexux_tool_passes_base_instructions()
        .await
        .expect("codex tool should pass base instructions");
}

async fn rexux_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    create_config_toml(rexux_home.path(), &server.uri())?;
    let skill_dir = rexux_home.path().join("skills").join("demo");
    ...
    let mut mcp_process = McpProcess::new_with_env(
        rexux_home.path(),
        &[("OPENAI_API_KEY", None), ("REXUX_ACCESS_TOKEN", None)],
    )
    .await?;
    ...
    .await??;
    assert_eq!(rexux_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(rexux_response.id, RequestId::Number(rexux_request_id));
    assert_eq!(
        rexux_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
    ...
    assert_eq!(
        developer_text
            .matches("Co-authored-by: Rexux <noreply@openai.com>")
            .count(),
        1
    );
    assert_eq!(
        developer_text
            .matches("Generated with [Rexux](https://openai.com/codex/).")
            .count(),
        1
    );
    assert_eq!(
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_rexux_tool_forwards_skills_extension_warnings() {
    skip_if_no_network!();

    rexux_tool_forwards_skills_extension_warnings()
        .await
        .expect("codex tool should forward skills extension warnings");
}

async fn rexux_tool_forwards_skills_extension_warnings() -> anyhow::Result<()> {
    let server =
        create_mock_responses_server(vec![create_final_assistant_message_sse_response("Enjoy!")?])
            .await;
    let rexux_home = TempDir::new()?;
    create_config_toml(rexux_home.path(), &server.uri())?;
    let skills_dir = rexux_home.path().join("skills");
    for index in 0..200 {
    ...
    let mut mcp_process = McpProcess::new(rexux_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp_process.initialize()).await??;
    ...
    let warning = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_rexux_event_matching("warning", |params| {
            params["msg"]["message"]
                .as_str()
...
fn create_expected_patch_approval_elicitation_request_params(
    ...
    message_lines.push("Allow Rexux to apply proposed code changes?".to_string());
    let params_json = serde_json::to_value(PatchApprovalElicitRequestParams {
        message: message_lines.join("\n"),
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        rexux_elicitation: "patch-approval".to_string(),
        rexux_mcp_tool_call_id,
        rexux_event_id,
        rexux_reason: reason,
        rexux_grant_root: grant_root,
        rexux_changes: changes,
        rexux_call_id: "call1234".to_string(),
    })?;
...
/// Create a Rexux config that uses the mock server as the model provider.
/// The command explicitly requests escalation so that we exercise the
/// elicitation code path.
fn create_config_toml(rexux_home: &Path, server_uri: &str) -> std::io::Result<()> {
