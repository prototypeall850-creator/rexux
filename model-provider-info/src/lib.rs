//! Registry of model providers supported by Rexux.
//!
//! Providers can be defined in two places:
//!   1. Built-in defaults compiled into the binary so Rexux works out-of-the-box.
//!   2. User-defined entries inside `~/.rexux/config.toml` under the `model_providers`
//!      key. These override or extend the defaults at runtime.

use rexux_api::Provider as ApiProvider;
use rexux_api::RetryConfig as ApiRetryConfig;
use rexux_protocol::auth::AuthMode;
use rexux_protocol::config_types::ModelProviderAuthInfo;
use rexux_protocol::error::RexuxErr;
use rexux_protocol::error::EnvVarError;
...
pub const CHATGPT_REXUX_BASE_URL: &str = "https://chatgpt.com/backend-api/codex";
...
impl ModelProviderInfo {
    ...
    fn build_header_map(&self) -> RexuxResult<HeaderMap> {
    ...
    pub fn to_api_provider(&self, auth_mode: Option<AuthMode>) -> RexuxResult<ApiProvider> {
    ...
    pub fn api_key(&self) -> RexuxResult<Option<String>> {
        match &self.env_key {
            Some(env_key) => {
    ...
    pub fn supports_rexux_backend_routes(&self) -> bool {
        self.is_openai()
...
/// BYOK provider entry that authenticates with an API key from the environment.
///
/// Only providers that speak the Responses API are bundled; Chat Completions
/// gateways can be reached through a local LiteLLM proxy instead.
fn create_byok_provider(
    name: &str,
    base_url: &str,
    env_key: Option<&str>,
...
pub fn built_in_model_providers(
    ...
    // providers are bundled with Rexux CLI, so we only include the OpenAI and
    // open source ("oss") providers by default, plus BYOK-friendly providers
    // for this build. Users are encouraged to add to `model_providers` in
    ...
    [
        ...
        (
            ...
            create_byok_provider(
                ...
                Some("Create an API key at https://openrouter.ai/settings/keys and export it as OPENROUTER_API_KEY."),
            ),
        ),
        (
            LITELLM_PROVIDER_ID,
            create_byok_provider(
                ...
                None,
                Some("Run `litellm --model <provider/model>` locally; the proxy routes to 100+ Chat Completions providers."),
            ),
        ),
    ]
    .into_iter()
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    ...
    let default_rexux_oss_base_url = format!(
        "http://localhost:{rexux_oss_port}/v1",
        rexux_oss_port = std::env::var("CODEX_OSS_PORT")
            .ok()
    ...
    let rexux_oss_base_url = std::env::var("CODEX_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or(default_rexux_oss_base_url);
    create_oss_provider_with_base_url(&rexux_oss_base_url, wire_api)
}
