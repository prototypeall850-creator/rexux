...
`rexux-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `rexux-core`.
...
compatibility-sensitive orchestration in `rexux-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `rexux-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `rexux-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `rexux-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `rexux-core`.
3. Leave compatibility-sensitive adapters in `rexux-core` while downstream
...
`rexux-core`, that is a sign to revisit the extraction boundary before adding
more here.
