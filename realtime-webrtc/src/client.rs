...
impl VoiceHost {
    pub async fn connect(package: &RexuxPackageLayout, build_commit: &str) -> Result<Self> {
        let root = package.package_dir.as_path().canonicalize()?;
        let name = if cfg!(windows) {
            "rexux-voice-host.exe"
        } else {
            "rexux-voice-host"
        };
        let path = root.join("rexux-resources/voice/bin").join(name);
        ensure!(
        ...
        let SpawnedProcess {
            ...
            stderr_rx,
            exit_rx,
        } = rexux_utils_pty::spawn_pipe_process(
            &path,
            &[],
            &root,
            &environment,
