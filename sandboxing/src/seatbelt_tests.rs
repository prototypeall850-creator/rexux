...
use rexux_network_proxy::build_config_state;
...
use rexux_protocol::permissions::PROTECTED_METADATA_PATH_NAMES;
...
fn process_platform_defaults_allow_scratch_without_granting_it_to_filesystem_helpers() {
    let workspace = tempfile::Builder::new()
        .prefix("rexux-seatbelt-approved-project-")
    ...
    for scratch_root in ["/private/tmp", "/private/var/tmp"] {
        let scratch = tempfile::Builder::new()
            .prefix("rexux-seatbelt-process-scratch-")
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/rexux-unreadable");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/rexux-readable");
    let unreadable = absolute_path("/tmp/rexux-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
            path: root.into(),
...
fn preferences_access_requires_unrestricted_reads() {
    ...
    denied_path.entries.push(FileSystemSandboxEntry::new(
        absolute_path("/tmp/rexux-private").into(),
        FileSystemAccessMode::Deny,
    ));
...
fn restricted_reads_cannot_read_preferences_outside_allowed_roots() {
    ...
    let marker = "rexux-preferences-read-canary";
...
fn create_seatbelt_args_allowlists_explicit_unix_socket_paths_without_proxy() {
    ...
    let extra_allow_unix_sockets = vec![absolute_path("/tmp/rexux-browser-use")];
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
        command: vec!["/usr/bin/true".to_string()],
        file_system_sandbox_policy: &file_system_policy,
        network_sandbox_policy: NetworkSandboxPolicy::Restricted,
    ...
    let expected_socket_root = normalize_path_for_sandbox(Path::new("/tmp/rexux-browser-use"))
...
async fn create_seatbelt_args_merges_proxy_and_explicit_unix_socket_paths() -> anyhow::Result<()> {
    ...
    let network_socket = "/tmp/rexux-proxy-use";
    let explicit_socket = "/tmp/rexux-browser-use";
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_rexux_subpaths() {
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_rexux_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Seatbelt policy.
    let config_toml = dot_rexux_canonical.join("config.toml");
    let output = Command::new(MACOS_PATH_TO_SEATBELT_EXECUTABLE)
        .args(&args)
        .current_dir(&cwd)
        .output()
...
fn seatbelt_protects_resolved_target_of_symlinked_metadata_directory() {
    ...
    let dot_codex = writable_root.join(".rexux");
...
#[test]
fn create_seatbelt_args_block_first_time_dot_rexux_creation_with_metadata_name_regex() {
    let tmp = TempDir::new().expect("tempdir");
    ...
    let dot_codex = repo_root.join(".rexux");
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".rexux")).expect("create .codex");
    fs::write(
        vulnerable_root.join(".rexux").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    ...
    let dot_rexux_canonical = vulnerable_root_canonical.join(".rexux");
