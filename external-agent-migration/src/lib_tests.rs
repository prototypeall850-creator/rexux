...
#[test]
fn subagent_requires_minimum_rexux_agent_fields() {
...
fn subagent_target_preserves_dotted_file_stem() {
    let target_agents = Path::new("/repo/.rexux/agents");
    let source_file = source_path("agents/security.audit.md");

    assert_eq!(
        subagent_target_file(&source_file, target_agents),
        Some(PathBuf::from("/repo/.rexux/agents/security.audit.toml"))
    );
}
...
fn hook_script_copy_keeps_existing_target_scripts() {
    ...
    let target_config_dir = root.path().join(".rexux");
