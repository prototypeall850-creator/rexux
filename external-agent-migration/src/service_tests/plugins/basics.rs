...
async fn authenticated_plugin_migration_uses_chatgpt_curated_marketplace() {
    ...
    fs::create_dir_all(plugin_root.join(".rexux-plugin")).expect("create curated plugin directory");
    fs::write(
  ...
  "plugins": [{
    ...
    let mut service = service_for_paths(external_agent_home.clone(), rexux_home);
...
fn marketplace_import_sources_prefers_scoped_source_over_registry_name_collision() {
    let (root, external_agent_home, _rexux_home) = fixture_paths();
...
fn marketplace_import_sources_prefers_supported_declaration_over_materialization() {
    let (_root, external_agent_home, _rexux_home) = fixture_paths();
...
async fn import_plugins_requires_details() {
    ...
    let err = service_for_paths(external_agent_home, rexux_home)
...
async fn detect_repo_skips_plugins_only_configured_in_project_codex() {
    ...
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
    fs::create_dir_all(repo_root.join(".rexux")).expect("create repo codex dir");
    fs::create_dir_all(&rexux_home).expect("create codex home");
    fs::write(
        repo_root.join(EXTERNAL_AGENT_DIR).join("settings.json"),
        r#"{
...
async fn detect_repo_skips_plugins_even_with_installed_marketplace() {
    ...
    let rexux_home = root.path().join(".rexux");
    let repo_root = root.path().join("repo");
    let marketplace_root = rexux_home.join(".tmp").join("marketplaces").join("debug");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
