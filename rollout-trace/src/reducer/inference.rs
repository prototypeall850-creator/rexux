...
impl TraceReducer {
    ...
    /// If a call is still `Running`, Rexux stopped observing that provider stream
    ...
    pub(super) fn close_running_inference_calls_for_turn_end(
        ...
        for inference in self.rollout.inference_calls.values_mut() {
            if inference.rexux_turn_id == rexux_turn_id
