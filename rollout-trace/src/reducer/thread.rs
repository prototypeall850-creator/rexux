...
impl TraceReducer {
    ...
    /// Starts a Rexux turn inside an existing thread.
    pub(super) fn start_rexux_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if self.rollout.rexux_turns.contains_key(&rexux_turn_id) {
            bail!("duplicate codex turn start for {rexux_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.rexux_turns.insert(
            rexux_turn_id.clone(),
            RexuxTurn {
                rexux_turn_id,
                thread_id,
    ...
    /// Marks a Rexux turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_rexux_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.rexux_turns.get(&rexux_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "codex turn end for {rexux_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.rexux_turns.get_mut(&rexux_turn_id) else {
            bail!("codex turn end referenced unknown turn {rexux_turn_id}");
        };
