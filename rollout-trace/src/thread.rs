...
use crate::protocol_event::rexux_turn_trace_event;
...
pub const REXUX_ROLLOUT_TRACE_ROOT_ENV: &str = "REXUX_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `REXUX_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the Rexux
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(REXUX_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed Rexux turn lifecycle events from protocol lifecycle events.
    pub fn record_rexux_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            rexux_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what Rexux did while executing that boundary.
    pub fn record_tool_call_event(&self, rexux_turn_id: impl Into<RexuxTurnId>, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        ...
        context.append_with_context_best_effort(rexux_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_rexux_turn_id: impl Into<RexuxTurnId>,
    ...
    pub fn record_rexux_turn_started(&self, rexux_turn_id: impl Into<RexuxTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let rexux_turn_id = rexux_turn_id.into();
        context.append_with_context_best_effort(
            rexux_turn_id.clone(),
            RawTraceEventPayload::RexuxTurnStarted {
                rexux_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
    ...
    /// Builds reusable inference trace context for one Rexux turn.
    ///
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        &self,
        rexux_turn_id: RexuxTurnId,
        payload: RawTraceEventPayload,
    ) {
        let event_context = RawTraceEventContext {
            thread_id: Some(self.thread_id.clone()),
            rexux_turn_id: Some(rexux_turn_id),
        };
