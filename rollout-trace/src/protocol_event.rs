//! Mapping from Rexux protocol events into raw rollout-trace events.
//!
...
//! `rexux-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `rexux-rollout-trace`.
//!
...
use rexux_protocol::protocol::PatchApplyStatus;
use rexux_protocol::protocol::SubAgentActivityEvent;
...
use crate::RexuxTurnId;
use crate::ExecutionStatus;
use crate::RawTraceEventPayload;

pub(crate) struct RexuxTurnTraceEvent {
    pub context_turn_id: RexuxTurnId,
    pub payload: RawTraceEventPayload,
}

pub(crate) fn rexux_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<RexuxTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let rexux_turn_id = event.turn_id.clone();
            Some(RexuxTurnTraceEvent {
                context_turn_id: rexux_turn_id.clone(),
                payload: RawTraceEventPayload::RexuxTurnStarted {
                    rexux_turn_id,
                    thread_id,
                },
            })
        ...
        EventMsg::TurnComplete(event) => {
            ...
            Some(RexuxTurnTraceEvent {
                context_turn_id: rexux_turn_id.clone(),
                payload: RawTraceEventPayload::RexuxTurnEnded {
                    rexux_turn_id,
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a rexux_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a rexux_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a rexux_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a rexux_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a rexux_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a rexux_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a rexux_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a rexux_protocol::protocol::CollabCloseEndEvent),
...
struct ExecCommandBeginTracePayload<'a> {
    ...
    command: &'a [String],
    cwd: String,
    parsed_cmd: &'a [rexux_protocol::parse_command::ParsedCommand],
