...
///
/// A Rexux turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one Rexux turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        rexux_turn_id: RexuxTurnId,
...
impl InferenceTraceAttempt {
    ...
    pub fn record_started(&self, request: &impl Serialize) {
        ...
        append_with_context_best_effort(
            ...
            RawTraceEventPayload::InferenceStarted {
                ...
                rexux_turn_id: attempt.context.rexux_turn_id.clone(),
    ...
    /// trace-specific serialization rules. That keeps rexux-core focused on
    ...
    /// Records a provider stream that Rexux intentionally stopped consuming.
    ///
...
/// Rexux received it, so this helper restores that content in the raw payload.
...
fn append_with_context_best_effort(
    ...
    let event_context = RawTraceEventContext {
        ...
        rexux_turn_id: Some(context.rexux_turn_id.clone()),
    };
...
mod tests {
    ...
    fn enabled_context_records_replayable_inference_attempt() -> anyhow::Result<()> {
        ...
        assert_eq!(inference.rexux_turn_id, "turn-1");
