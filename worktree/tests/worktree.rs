...
use rexux_worktree::WorktreeManager;
...
impl RepositoryFixture {
    fn new() -> Self {
        ...
        let rexux_home = temp_root.join("rexux-home");
        let repository = temp_root.join("project");
        fs::create_dir_all(&rexux_home).expect("create Rexux home");
    ...
    fn manager(&self) -> WorktreeManager {
        let settings =
            WorktreeSettings::from_desktop_config(&self.rexux_home, /*desktop*/ None)
...
fn git_output(repository: &Path, args: &[&str]) -> std::process::Output {
    Command::new("git")
        ...
        .args([
            "-c",
            "user.name=Rexux Worktree Test",
            "-c",
            "user.email=rexux-worktree-test@example.invalid",
            "-c",
...
fn thread_binding_rejects_primary_and_unmanaged_worktrees() {
    ...
    assert!(
        !fixture.repository.join(".git/rexux-thread.json").exists(),
...
fn thread_binding_writes_the_exact_desktop_owner_schema_into_git_metadata() {
    ...
    let metadata_path = checkout.join(run_git(
        &checkout,
        &["rev-parse", "--git-path", "rexux-thread.json"],
    ));
