...
use rexux_login::AuthEnvTelemetry;
...
pub const DOCTOR_REPORT_ATTACHMENT_FILENAME: &str = "rexux-doctor-report.json";
/// Filename used for the raw Rexux Apps MCP tools cache feedback attachment.
pub const REXUX_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME: &str = "rexux-apps-tools-cache.json";
/// Filename used for the raw connector directory cache feedback attachment.
pub const REXUX_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME: &str = "rexux-app-directory-cache.json";
...
pub fn emit_feedback_request_tags_with_auth_env(
    ...
    tracing::info!(
        ...
        auth_env_rexux_api_key_present = tracing::field::debug(auth_env.rexux_api_key_env_present),
        auth_env_rexux_api_key_enabled = tracing::field::debug(auth_env.rexux_api_key_env_enabled),
...
#[derive(Clone)]
pub struct RexuxFeedback {
    inner: Arc<FeedbackInner>,
}

impl Default for RexuxFeedback {
    fn default() -> Self {
        Self::new()
    }
}
        ...
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(
                Targets::new()
                    ...
                    .with_target("rexux_http_client::transport", LevelFilter::DEBUG)
                    .with_target("rexux_api::sse", LevelFilter::DEBUG)
                    ...
                    .with_target("rexux_api::responses_websocket_timing", LevelFilter::OFF)
                    .with_target("rexux_core::post_sampling_token_estimate", LevelFilter::OFF),
            )
    }
...
pub struct FeedbackUploadOptions<'a> {
    ...
    ///
    /// These are included after `rexux-logs.log` and before path-backed rollout
...
impl FeedbackSnapshot {
    ...
    pub fn log_attachment(&self, logs_override: Option<Vec<u8>>) -> FeedbackAttachment {
        FeedbackAttachment {
            filename: "rexux-logs.log".to_string(),
...
mod tests {
    ...
    fn ring_buffer_drops_front_when_full() {
        let fb = RexuxFeedback::with_capacity(/*max_bytes*/ 8);
        {
    ...
    fn logger_layer_filters_noisy_trace_payloads() {
        ...
        tracing::trace!(target: "rexux_api::responses_websocket_timing", payload = "secret");
        tracing::trace!(target: "rexux_http_client::transport", "transport-trace");
        tracing::trace!(target: "rexux_api::sse", "sse-trace");
        tracing::trace!(target: "rexux_api::sse::responses", "nested-sse-trace");
        tracing::debug!(target: "rexux_http_client::transport", "transport-debug");
        tracing::debug!(target: "rexux_api::sse::responses", "sse-debug");
        tracing::trace!(target: "rexux_feedback_test", "unrelated-trace");
        log::trace!(target: "rexux_feedback_test", "unrelated-log-trace");
        log::trace!(
    ...
    async fn feedback_upload_allows_slow_reports() {
        ...
        upload_test_feedback(&RexuxFeedback::new(), &dsn, &[attachment])
            .await
    ...
    async fn feedback_upload_deadline_stops_retries_and_later_attachments() {
        ...
        let snapshot = RexuxFeedback::new()
            .snapshot(/*session_id*/ None)
            .with_feedback_diagnostics(FeedbackDiagnostics::default());
    ...
    async fn feedback_upload_retries_diagnostics_without_replaying_core() {
        ...
        upload_test_feedback(&RexuxFeedback::new(), &dsn, &attachments)
            .await
    ...
    async fn feedback_upload_stops_diagnostics_for_sentry_rate_limits() {
        for (accepted_before_limit, rate_limit) in [
            ...
            upload_test_feedback(&RexuxFeedback::new(), &dsn, &[later])
                .await
    ...
    async fn feedback_upload_reports_rejected_sentry_response_without_exposing_feedback() {
        ...
        let feedback = RexuxFeedback::new();
        let dsn = format!("http://public@{}/42", server.address());
    ...
    async fn feedback_upload_does_not_forward_private_data_to_redirect_target() {
        ...
        let error = upload_test_feedback(&RexuxFeedback::new(), &dsn, &[])
            .await
    ...
    async fn feedback_upload_reports_transport_failures() {
        ...
        assert!(
            error
                .downcast_ref::<rexux_http_client::RouteAwareRequestError>()
                .is_some_and(rexux_http_client::RouteAwareRequestError::is_connect)
        );
    }

    #[test]
    fn feedback_attachments_gate_connectivity_diagnostics() {
        let extra_filename = format!("rexux-feedback-extra-{}.jsonl", ThreadId::new());
        ...
        let snapshot_with_diagnostics = RexuxFeedback::new()
            .snapshot(/*session_id*/ None)
        ...
        let attachments_without_diagnostics = RexuxFeedback::new()
            .snapshot(/*session_id*/ None)
            .with_feedback_diagnostics(FeedbackDiagnostics::default())
    ...
    fn path_backed_attachments_use_binary_content_types() {
        let suffix = ThreadId::new();
        let gzip_filename = format!("rexux-desktop-app-logs-{suffix}.tar.gz");
        let unknown_filename = format!("rexux-feedback-extra-{suffix}.binunknown");
        ...
        let attachments = RexuxFeedback::new()
            .snapshot(/*session_id*/ None)
            .feedback_attachments(
                /*include_logs*/ false,
                &[],
