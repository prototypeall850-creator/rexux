...
//! detailed explanation of what "hermetic" means here lives in `rexux_http_client::custom_ca`; these
...
const REXUX_CA_CERT_ENV: &str = "REXUX_CA_CERTIFICATE";
const PROBE_PROXY_ENV: &str = "REXUX_CUSTOM_CA_PROBE_PROXY";
const PROBE_TLS13_ENV: &str = "REXUX_CUSTOM_CA_PROBE_TLS13";
const PROBE_URL_ENV: &str = "REXUX_CUSTOM_CA_PROBE_URL";
...
fn probe_command() -> Command {
    ...
    cmd.env_remove(REXUX_CA_CERT_ENV);
...
fn spawn_tls13_test_server() -> Tls13TestServer {
    rexux_utils_rustls_provider::ensure_rustls_crypto_provider();
...
#[test]
fn uses_rexux_ca_cert_env() {
    let temp_dir = TempDir::new().expect("tempdir");
    let cert_path = write_cert_file(&temp_dir, "ca.pem", TEST_CERT_1);

    let output = run_probe(&[(REXUX_CA_CERT_ENV, cert_path.as_path())]);
...
#[test]
fn prefers_rexux_ca_cert_over_ssl_cert_file() {
    let temp_dir = TempDir::new().expect("tempdir");
    let cert_path = write_cert_file(&temp_dir, "ca.pem", TEST_CERT_1);
    let bad_path = write_cert_file(&temp_dir, "bad.pem", "");

    let output = run_probe(&[
        (REXUX_CA_CERT_ENV, cert_path.as_path()),
...
fn posts_to_tls13_server_using_custom_ca_bundle() {
    ...
    let output =
        run_probe_posting_to_tls13_server(&[(REXUX_CA_CERT_ENV, cert_path.as_path())], &server.url);
...
fn rejects_empty_pem_file_with_hint() {
    ...
    assert!(stderr.contains("REXUX_CA_CERTIFICATE"));
