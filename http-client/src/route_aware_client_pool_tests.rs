...
async fn legacy_custom_ca_fallback_is_limited_to_reqwest_default() {
    const CHILD_POLICY_ENV: &str = "REXUX_HTTP_CLIENT_POOL_INVALID_CA_TEST_POLICY";
        ...
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(
                tracing_subscriber::filter::Targets::new()
                    .with_target("rexux_http_client", tracing::Level::TRACE),
            ),
    );
    let _guard = tracing::subscriber::set_default(subscriber);
    tracing::debug!(target: "rexux_http_client", "log capture sentinel");
