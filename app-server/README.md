...
`rexux app-server` is the interface Rexux uses to power rich interfaces such as the [Rexux VS Code extension](https://marketplace.visualstudio.com/items?itemName=openai.chatgpt).
...
Similar to [MCP](https://modelcontextprotocol.io/), `rexux app-server` supports bidirectional communication using JSON-RPC 2.0 messages (with the `"jsonrpc":"2.0"` header omitted on the wire).

Supported transports:

- stdio (`--stdio` or `--listen stdio://`, default): newline-delimited JSON (JSONL)
- websocket (`--listen ws://IP:PORT`): one JSON-RPC message per websocket text frame (**experimental / unsupported**)
- unix socket (`--listen unix://` or `--listen unix://PATH`): websocket connections over `$REXUX_HOME/app-server-control/app-server-control.sock` or a custom socket path, using the standard HTTP Upgrade handshake
...
The unix socket transport is intended for local app-server control-plane clients. `rexux app-server proxy`
opens exactly one raw stream connection to `$REXUX_HOME/app-server-control/app-server-control.sock`
...
Currently, you can dump a TypeScript version of the schema using `rexux app-server generate-ts`, or a JSON Schema bundle via `rexux app-server generate-json-schema`. Each output is specific to the version of Rexux you used to run the command, so the generated artifacts are guaranteed to match that version.

```
rexux app-server generate-ts --out DIR
rexux app-server generate-json-schema --out DIR
```

## Core Primitives

The API exposes three top level primitives representing an interaction between a user and Rexux:

- **Thread**: A conversation between a user and the Rexux agent. Each thread contains multiple turns.
...
Clients must send a single `initialize` request per transport connection before invoking any other method on that connection, then acknowledge with an `initialized` notification. The server returns the user agent string it will present to upstream services, `codexHome` for the server's Rexux home directory, and `platformFamily` and `platformOs` strings describing the app-server runtime target; subsequent requests issued before initialization receive a `"Not initialized"` error, and repeated `initialize` calls on the same connection receive an `"Already initialized"` error.
...
when a Rexux session is created by `thread/start`, `thread/resume`, or
`thread/fork`. Rexux advertises that profile in the downstream MCP
...
Applications building on top of `rexux app-server` should identify themselves via the `clientInfo` parameter.

**Important**: `clientInfo.name` is used to identify the client for the OpenAI Compliance Logs Platform. If
you are developing a new Rexux integration that is intended for enterprise use, please contact us to get it
...
- `memory/reset` — experimental; clear the current `REXUX_HOME/memories` directory and reset persisted memory stage data in sqlite while preserving existing thread memory modes; returns `{}` on success.
...
- `thread/realtime/start` — start a thread-scoped realtime session (experimental); pass `outputModality: "text"` or `outputModality: "audio"` to choose model output, optionally pass `model` and `version` to override configured realtime selection for this session only, pass `includeStartupContext: false` to omit Rexux's generated startup context, and optionally pass `initialItems` to seed V3 with complete role-bearing text messages at session creation. Pass `realtimeStartInstructions` and `realtimeEndInstructions` to control the developer instructions given to the backing Rexux model when this session starts and ends. Version `"v1"` uses legacy Bidi `conversation.handoff.*`, `"v2"` uses the Realtime Voice API, and `"v3"` preserves V1 Rexux Voice behavior while using Frameless Bidi `delegation.*`. For V3 automatic Rexux text, `codexResponseHandoffMode` accepts `"thinking"` (the default; all output uses channel-less thinking appends), `"commentary"` (all output uses the commentary channel), or `"bemTags"` (the raw BEM envelope selects the API channel: BEM `analysis` and `commentary` use `commentary`, while BEM `final` and unparsable output use `speakable`). The BEM envelope remains in the appended text for the frontend model to interpret. V1 and V2 ignore this setting. For V3, pass `delegationAckFiller: false` to suppress the Realtime API's delegation acknowledgement filler or `true` to restore it; omitting the field preserves the Realtime API's default. V1 and V2 ignore `delegationAckFiller`. V3 handoffs do not prepend the legacy `"Agent Final Message"` label. Pass `clientManagedHandoffs: true` to disable automatic Rexux response delivery so only the client's explicit append calls produce handoffs. Pass `codexResponsesAsItems: true` to send automatic Rexux responses as realtime conversation items instead, and optionally pass `codexResponseItemPrefix` to prepend experiment instructions to those items. Returns `{}` and streams `thread/realtime/*` notifications. Omit `transport` for the websocket transport, or pass `{ "type": "webrtc", "sdp": "..." }` to create a Bidi WebRTC session from a browser-generated SDP offer; the remote answer SDP is emitted as `thread/realtime/sdp`. Conversation `version: "v2"` requests remain unsupported for WebRTC. Parent-owned Multi-Agent V2 subagents reject this request.
...
- `process/spawn` — experimental; spawn a standalone process without the Rexux sandbox on the host where the app server is running; returns after the process starts and emits `process/outputDelta` and `process/exited` notifications.
...
- `mcpServer/resource/read` — read a resource from a configured MCP server by optional `threadId`, `server`, and `uri`, returning text/blob resource `contents`. Pass `originCallId` with `threadId` to scope a Rexux app widget to the app and account of the completed tool call that produced it; successful scoped reads return the same `originCallId`. Optional `connectorId` restricts other hosted app resources to their originating connector. If `threadId` is omitted, the server reads from the latest MCP config directly.
...
Start a fresh thread when you need a new Rexux conversation.
...
Experimental: use `memory/reset` to clear local memory artifacts and sqlite-backed memory stage data for the current Rexux home. This preserves existing thread memory modes; use `thread/memoryMode/set` separately when a thread's future memory eligibility should change.
...
Turns attach user input (text, images, or audio) to a thread and trigger Rexux generation. The `input` field is a list of discriminated unions:
...
Experimental `cyberAccessProgram` also applies only to the new turn. It accepts `standard`, `daybreakBlue`, or `daybreakRed`; omission preserves automatic backend behavior. For ChatGPT-authenticated requests through the built-in OpenAI provider, Rexux sends the corresponding `standard`, `daybreak_blue`, or `daybreak_red` value in `access_programs.cyber` on Responses and remote-compaction requests. WebSocket `response.create` messages carry the choice per request, so changing it does not require reconnecting. The server still enforces workspace authorization and model restrictions. API-key and custom-provider requests omit this field. This field does not change the saved model or grant access.
...
{ "method": "turn/start", "id": 33, "params": {
    ...
    "input": [
        ...
        { "type": "skill", "name": "skill-creator", "path": "/Users/me/.rexux/skills/skill-creator/SKILL.md" }
    ]
} }
{ "id": 33, "result": { "turn": {
    "id": "turn_457",
...
Then send `offer.sdp` to app-server. Core uses `experimental_realtime_ws_backend_prompt` for the backend instructions and the thread conversation id as the default Realtime API session identifier. This `realtimeSessionId` value refers to the upstream Realtime API session, not a Rexux session/thread-group id. The start response is `{}`; the remote answer SDP arrives later as `thread/realtime/sdp` and should be passed to `setRemoteDescription()`:
...
The existing-call transport attaches Rexux to the call over its sideband WebSocket without creating
another call or emitting `thread/realtime/sdp`. The client owns the SDP negotiation and the initial
realtime session configuration. Rexux startup context is disabled by default for existing calls;
...
Omit `prompt` to use Rexux's default realtime backend prompt. Send `prompt: null` or
...
the backing Rexux model when the thread enters realtime mode, and
`realtimeEndInstructions` to provide its developer instructions when the session
ends. These instructions configure Rexux, not the realtime frontend model. They
...
field preserves Rexux's existing default instructions.
...
Pass `includeStartupContext: false` to skip Rexux's startup context for this
...
Pass `clientManagedHandoffs: true` to suppress automatic Rexux response handoffs
...
Pass `codexResponsesAsItems: true` to inject automatic Rexux responses with
...
experiment instructions to each automatic Rexux response item. Omit
...
Use `review/start` to run Rexux’s reviewer on the currently checked-out project. The request takes the thread id plus a `target` describing what should be reviewed:

- `{"type":"uncommittedChanges"}` — staged, unstaged, and untracked files.
- `{"type":"baseBranch","branch":"main"}` — diff against the provided branch’s upstream (see prompt for the exact `git merge-base`/`git diff` instructions Rexux will run).
...
For an inline review, Rexux streams the usual `turn/started` notification followed by an `item/started`
...
- For clients that are already sandboxed externally, set the legacy `sandboxPolicy` to `{"type":"externalSandbox","networkAccess":"enabled"}` (or omit `networkAccess` to keep it restricted). Rexux will not enforce its own sandbox in this mode; it tells the model it has full file-system access and passes the `networkAccess` state through `environment_context`.
...
- `processId` is optional for buffered execution. When omitted, Rexux generates an internal id for lifecycle tracking, but `tty`, `streamStdin`, and `streamStdoutStderr` must stay disabled and follow-up `command/exec/write` / `command/exec/terminate` calls are not available for that command.
...
Use `process/spawn` to start a standalone argv-based process without the Rexux sandbox on the host where the app server is running. The `process/*` API is experimental and requires `initialize.params.capabilities.experimentalApi: true`. The spawn response means the process has started and the `processHandle` is registered; completion is reported later through `process/exited`.
...
- `thread/realtime/started` — `{ threadId, realtimeSessionId }` once realtime starts for the thread (experimental). `realtimeSessionId` is the upstream Realtime API session identifier, not a Rexux session/thread-group id.
...
- `contextCompaction` — `{id}` emitted when rexux compacts the conversation history. This can happen automatically.
- `compacted` - `{threadId, turnId}` when rexux compacts the conversation history. This can happen automatically. **Deprecated:** Use `contextCompaction` instead.
...
`codexErrorInfo` maps to the `RexuxErrorInfo` enum. Common values:
...
Certain actions (shell commands or modifying files) may require explicit user approval depending on the user's config. When `turn/start` is used, the app-server drives an approval flow by sending a server-initiated JSON-RPC request to the client. The client must respond to tell Rexux whether to proceed. UIs should present these requests inline with the active turn so users can review the proposed command or diff before choosing.
...
`rexux_approval_kind: "mcp_tool_call"` and may include `persist: "session"`,
  ...
  "params": {
    ...
    "input": [
      ...
      },
      {
        "type": "skill",
        "name": "skill-creator",
        "path": "/Users/me/.rexux/skills/skill-creator/SKILL.md"
      }
    ]
  }
}
...
{ "id": 25, "result": {
    "data": [{
        ...
        "skills": [
            {
              "name": "skill-creator",
              "description": "Create or update a Rexux skill",
              "enabled": true,
              "pluginId": null,
              "interface": {
                "displayName": "Skill Creator",
                "shortDescription": "Create or update a Rexux skill",
                "iconSmall": "icon.svg",
...
```json
{
  ...
  "id": 27,
  "params": {
    "path": "/Users/alice/.rexux/skills/skill-creator/SKILL.md",
    "name": null,
    "enabled": false
  }
}
...
For linked Git worktrees, project hook declarations come from the matching `.rexux/` folders in the root checkout rather than from divergent hook declarations stored only in the linked worktree. This keeps each repo on one authoritative project-hook definition and one trust state.
...
```json
{
  ...
  "result": {
    "data": [{
      "cwd": "/Users/me/project",
      "hooks": [{
        "key": "/Users/me/.rexux/config.toml:pre_tool_use:0:0",
        ...
        "sourcePath": "/Users/me/.rexux/config.toml",
        "source": "user",
        "pluginId": null,
        "displayOrder": 0,
        "enabled": true,
...
```json
{
  ...
  "params": {
    "edits": [{
      "keyPath": "hooks.state",
      "value": {
        "/Users/me/.rexux/config.toml:pre_tool_use:0:0": {
          "enabled": false
        }
      },
...
Rexux supports these authentication modes. The current mode is surfaced in `account/updated` (`authMode`), which also includes the current ChatGPT `planType` when available, and can be inferred from `account/read`. Self-serve Business ProLite accounts use the `self_serve_business_prolite` plan type; Enterprise automation accounts use `enterprise_cbp_automation`.

- **API key (`apiKey`)**: Caller supplies an OpenAI API key via `account/login/start` with `type: "apiKey"`. The API key is saved and used for API requests.
- **ChatGPT managed (`chatgpt`)** (recommended): Rexux owns the ChatGPT OAuth flow and refresh tokens. Start via `account/login/start` with `type: "chatgpt"` for the browser flow or `type: "chatgptDeviceCode"` for device code; Rexux persists tokens to disk and refreshes them automatically.
- **Rexux managed Amazon Bedrock auth (experimental)**: Caller supplies an Amazon Bedrock API key using `type: "amazonBedrock"` or AWS access keys using `type: "amazonBedrockAccessKeys"` via `account/login/start`. The client must enable the `experimentalApi` initialization capability. Rexux replaces the current primary auth with the Bedrock credential and writes `model_provider = "amazon-bedrock"` to the user config.
- **Personal access token (`personalAccessToken`)**: Rexux uses a ChatGPT-backed personal access token loaded outside the app-server login RPCs, such as with `rexux login --with-access-token` or `REXUX_ACCESS_TOKEN`.
...
{ "id": 1, "result": { "account": { "type": "amazonBedrock", "usesRexuxManagedCredentials": false }, "requiresOpenaiAuth": false } }
```
...
- `requiresOpenaiAuth` reflects the active provider; when `false`, Rexux can run without OpenAI credentials.
- Amazon Bedrock reports `usesRexuxManagedCredentials: true` when it uses a Bedrock API key or AWS access keys managed by Rexux. It reports `false` for external credential paths, including the AWS credential chain and configured command auth. This identifies whether Rexux-managed credentials are selected; it does not validate that the credential source can resolve credentials.
...
2. Open `authUrl` in a browser; the app-server hosts the local callback.
   ...
   organization setup to the hosted Rexux success page instead. When hosted login success is
   enabled, clients may set `appBrand` to `"rexux"` or `"chatgpt"` to select the matching hosted
   page artwork; omitted or `null` values default to `"rexux"`.
...
active user config. Neither flow changes `$REXUX_HOME/.env`. Existing loaded sessions keep their
...
profile or environment credentials leaves existing keys in `$REXUX_HOME/.env` unchanged.
...
any Rexux-managed credentials, and leaves AWS-managed credentials and `$REXUX_HOME/.env` unchanged.
...
credentials are Rexux-managed or AWS-managed. If the selected model is Bedrock-specific, logout
also clears `model`; `model_reasoning_effort` and other generic settings are preserved.
Rexux-managed credentials are removed; AWS profiles, environment credentials, and
`$REXUX_HOME/.env` are left untouched.
...
`rexux app-server` schema generation defaults to the stable API surface (experimental fields and methods filtered out). Pass `--experimental` to include experimental methods/fields in generated TypeScript or JSON schema:
...
rexux app-server generate-ts --out DIR --experimental
rexux app-server generate-json-schema --out DIR --experimental
```
...
5. Verify the protocol crate:
   ...
   ```bash
   just test -p rexux-app-server-protocol
   ```
