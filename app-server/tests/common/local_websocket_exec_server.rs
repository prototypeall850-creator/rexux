...
impl LocalWebsocketExecServer {
    pub(crate) async fn start(rexux_home: &Path, exec_server_program: &Path) -> Result<Self> {
        let mut command = Command::new(exec_server_program);
        command.stdin(Stdio::null());
        command.stdout(Stdio::piped());
        command.stderr(Stdio::inherit());
        command.current_dir(rexux_home);
        command.env("REXUX_HOME", rexux_home);
        #[cfg(target_os = "linux")]
        command.env(
            REXUX_LINUX_SANDBOX_EXE_ENV_VAR,
