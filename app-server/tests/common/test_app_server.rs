...
use rexux_app_server_protocol::FsCopyParams;
use rexux_app_server_protocol::FsCreateDirectoryParams;
use rexux_app_server_protocol::FsGetMetadataParams;
use rexux_app_server_protocol::FsReadDirectoryParams;
use rexux_app_server_protocol::FsReadFileParams;
use rexux_app_server_protocol::FsRemoveParams;
use rexux_app_server_protocol::FsUnwatchParams;
use rexux_app_server_protocol::FsWatchParams;
use rexux_app_server_protocol::FsWriteFileParams;
...
use rexux_app_server_protocol::ModelProviderCapabilitiesReadParams;
...
use rexux_app_server_protocol::PluginSearchParams;
use rexux_app_server_protocol::PluginSkillReadParams;
use rexux_app_server_protocol::PluginUninstallParams;
use rexux_app_server_protocol::ProcessKillParams;
use rexux_app_server_protocol::ProcessSpawnParams;
use rexux_app_server_protocol::ProjectImportParams;
use rexux_app_server_protocol::ProjectListParams;
use rexux_app_server_protocol::ProjectReadParams;
use rexux_app_server_protocol::RemoteControlClientsListParams;
use rexux_app_server_protocol::RemoteControlClientsRevokeParams;
use rexux_app_server_protocol::RemoteControlPairingStartParams;
use rexux_app_server_protocol::RemoteControlPairingStatusParams;
...
use rexux_app_server_protocol::ThreadRealtimeListVoicesParams;
...
use rexux_exec_server::REXUX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR;
use rexux_exec_server::REXUX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR;
use rexux_exec_server::REXUX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR;
use rexux_exec_server::REXUX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR;
use rexux_exec_server::REXUX_EXEC_SERVER_URL_ENV_VAR;
...
use core_test_support::test_rexux::TestEnv;
...
pub struct TestAppServer {
    ...
    // removing an owned REXUX_HOME that may still be its cwd on Windows.
    ...
    _owned_rexux_home: Option<TempDir>,
}

pub const DEFAULT_CLIENT_NAME: &str = "rexux-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "REXUX_APP_SERVER_DISABLE_MANAGED_CONFIG";
#[cfg(windows)]
...
impl TestAppServer {
    /// Starts building a server with a temporary REXUX_HOME and the standard
    ...
    async fn new_with_program_env_and_args(
        ...
        cmd.env(
            ...
            rexux_home.join("managed_config.toml"),
        );
        cmd.env_remove(REXUX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
        ...
        let mut process = loop {
            ...
            if !process
                ...
                || retries == 2
            {
                break process.context("rexux-mcp-server proc should start")?;
            }
            retries += 1;
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: rexux_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl TestAppServerBuilder {
    /// Uses this existing REXUX_HOME instead of a temporary one.
    pub fn with_rexux_home(mut self, rexux_home: &Path) -> Self {
        self.rexux_home = Some(rexux_home.to_path_buf());
        self
    }
    ...
    /// Builds a server with a temporary REXUX_HOME and automatic environment
    /// by default.
    pub async fn build(self) -> anyhow::Result<TestAppServer> {
        ...
        let (rexux_home, owned_rexux_home) = match rexux_home {
            Some(rexux_home) => (rexux_home, None),
            None => {
                let owned_rexux_home = TempDir::new()?;
                (
                    owned_rexux_home.path().to_path_buf(),
                    Some(owned_rexux_home),
                )
            }
        };
        let attribution_settings_server = if rexux_home.join("auth.json").is_file() {
            let config_path = rexux_home.join("config.toml");
        ...
        let (auto_env, delayed_exec_server) = match environment {
            TestAppServerEnvironment::Auto => {
                let environments_toml = rexux_home.join("environments.toml");
                ensure!(
                ...
                let (auto_env, delayed_exec_server) = match exec_server_delay {
                    Some(added_delay) => {
                        ...
                        let exec_server_program =
                            rexux_utils_cargo_bin::cargo_bin("exec-server")
                        ...
                        let local_websocket_exec_server =
                            LocalWebsocketExecServer::start(&rexux_home, &exec_server_program)
                                .await?;
                ...
                let mut auto_env_overrides = vec![
                    (
                        REXUX_EXEC_SERVER_URL_ENV_VAR.to_string(),
                        auto_env.exec_server_url().map(str::to_string),
                    ),
                    (
                        REXUX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR.to_string(),
                        None,
                    ),
                    (
                        REXUX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                    (REXUX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR.to_string(), None),
                    (
                        REXUX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                ];
        ...
        let mut program = match program {
            ...
            None => rexux_utils_cargo_bin::cargo_bin("rexux-app-server")
                .context("should find binary for rexux-app-server")?,
        };
        let mut owned_install_dir = None;
        if !custom_program
            && rexux_utils_cargo_bin::runfiles_available()
            && let Ok(code_mode_host_program) =
                rexux_utils_cargo_bin::cargo_bin("rexux-code-mode-host")
        {
        ...
        app_server._owned_rexux_home = owned_rexux_home;
...
impl Drop for TestAppServer {
    fn drop(&mut self) {
        // These tests spawn a `rexux-app-server` child process.
        //
