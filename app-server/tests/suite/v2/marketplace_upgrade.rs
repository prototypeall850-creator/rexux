...
fn init_marketplace_repo(root: &Path, marketplace_name: &str, marker: &str) -> Result<String> {
    ...
    run_git(root, &["config", "user.name", "Rexux Tests"])?;
...
fn disable_plugin_startup_tasks(rexux_home: &Path) -> Result<()> {
...
fn marketplace_install_root(rexux_home: &Path) -> std::path::PathBuf {
    rexux_home.join(INSTALLED_MARKETPLACES_DIR)
}

fn expected_installed_root(rexux_home: &Path, marketplace_name: &str) -> Result<AbsolutePathBuf> {
    AbsolutePathBuf::try_from(
        marketplace_install_root(&rexux_home.canonicalize()?).join(marketplace_name),
    )
...
async fn marketplace_upgrade_all_configured_git_marketplaces() -> Result<()> {
    ...
    disable_plugin_startup_tasks(rexux_home.path())?;
    ...
    let debug_root = expected_installed_root(rexux_home.path(), "debug")?;
    let tools_root = expected_installed_root(rexux_home.path(), "tools")?;
    ...
    for (root, expected_revision) in [
        ...
        let metadata: serde_json::Value = serde_json::from_str(&std::fs::read_to_string(
            root.as_path().join(".rexux-marketplace-install.json"),
        )?)?;
        assert_eq!(
...
async fn automatic_upgrade_isolates_git_while_explicit_install_preserves_configuration(
    ...
    let plugin_root = plugin_source.path().join(".rexux-plugin");
    ...
    run_git(rexux_home.path(), &["init", "--quiet"])?;
    let source_path = source.path().display().to_string();
    let missing_path = rexux_home.path().join("missing").display().to_string();
    ...
    for untrusted_source in [&source_path, &plugin_url] {
        ...
        assert!(run_git(rexux_home.path(), &["ls-remote", untrusted_source, "HEAD"]).is_err());
    }
    ...
    let plugin_root = rexux_home
        .path()
        .join("plugins/cache/trusted/toolkit/1.0.0/.rexux-plugin/plugin.json");
    ...
    let marketplace_path = marketplace_install_root(rexux_home.path())
    ...
    std::fs::write(
        plugin_source.path().join(".rexux-plugin/plugin.json"),
    ...
    assert!(
        rexux_home
            .path()
            .join("plugins/cache/trusted/toolkit/1.1.0/.rexux-plugin/plugin.json")
            .is_file(),
