...
async fn review_start_exec_approval_item_id_matches_command_execution_item() -> Result<()> {
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": rexux_protocol::protocol::ReviewDecision::Approved }),
    )
    .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
...
async fn review_start_with_detached_delivery_returns_new_thread_id() -> Result<()> {
    ...
    let rexux_home = TempDir::new()?;
    create_config_toml(rexux_home.path(), &server.uri())?;
    let colliding_skill_dir = rexux_home.path().join("skills/review-agent-collision");
    ...
    let canonical_rexux_home = std::fs::canonicalize(rexux_home.path())?.try_into()?;
    let review_skill_path = system_cache_root_dir(&canonical_rexux_home)
        .join("review-agent")
        .join("SKILL.md");
