...
async fn bedrock_app_server(
    ...
    let aws_config = rexux_home.join("aws-config");
    let aws_credentials = rexux_home.join("aws-credentials");
...
async fn setup_bedrock_rejects_invalid_or_conflicting_credentials() -> Result<()> {
    let rexux_home = TempDir::new()?;
    let mut app_server = bedrock_app_server(rexux_home.path(), &[]).await?;
