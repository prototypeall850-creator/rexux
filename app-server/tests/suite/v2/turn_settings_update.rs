...
-> Result<()> {
    ...
    .await;
    let rexux_home = TempDir::new()?;
    mock_config(rexux_home.path(), &server.uri())?.write(rexux_home.path())?;
    let mut app = TestAppServer::builder()
        .with_rexux_home(rexux_home.path())
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
        .await?;
...
fn mock_config(rexux_home: &Path, server_uri: &str) -> Result<MockResponsesConfig> {
    ...
    let models = [MODEL_A, MODEL_B]
        ...
        .collect();
    let catalog_path = rexux_home.join("step-models.json");
    std::fs::write(
        &catalog_path,
