...
use rexux_login::auth::BedrockAccessKeysAuth;
...
fn create_config_toml(rexux_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
...
fn read_config_toml(rexux_home: &Path) -> Result<toml::Value> {
    Ok(toml::from_str(&std::fs::read_to_string(
        rexux_home.join("config.toml"),
    )?)?)
}

fn load_file_auth(rexux_home: &Path) -> Result<Option<AuthDotJson>> {
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    create_config_toml(rexux_home.path(), CreateConfigTomlParams::default())?;
...
async fn logout_account_succeeds_when_config_reload_fails() -> Result<()> {
    ...
    assert_eq!(load_file_auth(rexux_home.path())?, None);
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request_with_auto_env(rexux_app_server_protocol::ThreadStartParams {
            model: Some("mock-model".to_string()),
            ..Default::default()
        })
        .await?;
    let thread: rexux_app_server_protocol::ThreadStartResponse =
        timeout(DEFAULT_READ_TIMEOUT, mcp.read_response(thread_req)).await??;

    let turn_req = mcp
        .send_turn_start_request(rexux_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            client_user_message_id: None,
            input: vec![rexux_app_server_protocol::UserInput::Text {
    ...
    .await?;
    let _: rexux_app_server_protocol::TurnStartResponse =
...
async fn login_amazon_bedrock_replaces_primary_auth_and_persists_provider(
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
        .await?;
    let mut expected_config = read_config_toml(rexux_home.path())?;
    expected_config
        .as_table_mut()
    ...
    assert_eq!(
        load_file_auth(rexux_home.path())?,
        Some(AuthDotJson {
    ...
    assert_eq!(read_config_toml(rexux_home.path())?, expected_config);
    assert!(!rexux_home.path().join(".env").exists());
    ...
    assert_eq!(
        ...
        GetAccountResponse {
            account: Some(Account::AmazonBedrock {
                uses_rexux_managed_credentials: true,
            }),
    ...
    if managed_access_keys {
        ...
        assert_eq!(load_file_auth(rexux_home.path())?, None);
        assert_eq!(read_config_toml(rexux_home.path())?, expected_logout_config);
        assert!(!rexux_home.path().join(".env").exists());
        assert_account_updated(&mut mcp, /*auth_mode*/ None).await?;
        assert_eq!(
            read_account(&mut mcp).await?,
...
async fn login_amazon_bedrock_rejects_non_bedrock_provider_override_without_changes() -> Result<()>
{
    ...
    let expected_auth = load_file_auth(rexux_home.path())?;
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
        .await?;
    let expected_config = read_config_toml(rexux_home.path())?;
    ...
    assert_eq!(load_file_auth(rexux_home.path())?, expected_auth);
    assert_eq!(read_config_toml(rexux_home.path())?, expected_config);
...
async fn logout_aws_managed_bedrock_clears_provider_and_restores_default_account() -> Result<()> {
    for managed_bedrock_auth in [false, true] {
        let rexux_home = TempDir::new()?;
        create_config_toml(rexux_home.path(), aws_managed_bedrock_config())?;
        let config_path = rexux_home.path().join("config.toml");
        let config = std::fs::read_to_string(&config_path)?
            .replace(
                "model = \"mock-model\"",
        ...
        let dotenv_path = rexux_home.path().join(".env");
        let aws_credentials_path = rexux_home.path().join("aws-credentials");
        let dotenv = "AWS_ACCESS_KEY_ID=environment-id\nAWS_SECRET_ACCESS_KEY=environment-secret\n";
        let aws_credentials = "[rexux-bedrock]\naws_access_key_id = profile-id\naws_secret_access_key = profile-secret\n";
...
#[tokio::test]
async fn set_auth_token_works_without_active_chatgpt_login() -> Result<()> {
    let rexux_home = TempDir::new()?;
    create_config_toml(rexux_home.path(), CreateConfigTomlParams::default())?;
    ...
    // ChatGPT browser/device login is unavailable in this BYOK build; the
    // request errors immediately.
    let request_id = mcp.send_login_account_chatgpt_request().await?;
    let err: JSONRPCError = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_error_message(RequestId::Integer(request_id)),
    ...
    .await??;
    assert!(
        err.error.message.contains("not available in this BYOK build"),
        "unexpected error: {:?}",
        err.error.message
    );
    ...
    // Setting external auth tokens still works and publishes account/updated.
    let set_id = mcp
        .send_chatgpt_auth_tokens_login_request(
            access_token,
            WORKSPACE_ID_EMBEDDED.to_string(),
    ...
    // Cancelling a login id that was never started still returns NotFound.
    let cancel_id = mcp
