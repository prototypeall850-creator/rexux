...
use rexux_app_server::AppServerRuntimeOptions;
use rexux_app_server::AppServerTransport;
use rexux_app_server::AppServerWebsocketAuthSettings;
use rexux_app_server::PluginStartupTasks;
use rexux_app_server::RemoteControlStartupMode;
use rexux_app_server::run_main_with_transport_options;
...
use rexux_app_server_protocol::RemoteControlClient;
use rexux_app_server_protocol::RemoteControlClientsListOrder;
use rexux_app_server_protocol::RemoteControlClientsListParams;
use rexux_app_server_protocol::RemoteControlClientsListResponse;
use rexux_app_server_protocol::RemoteControlClientsRevokeParams;
use rexux_app_server_protocol::RemoteControlClientsRevokeResponse;
use rexux_app_server_protocol::RemoteControlConnectionStatus;
use rexux_app_server_protocol::RemoteControlDisableResponse;
use rexux_app_server_protocol::RemoteControlEnableResponse;
use rexux_app_server_protocol::RemoteControlPairingStartParams;
use rexux_app_server_protocol::RemoteControlPairingStartResponse;
use rexux_app_server_protocol::RemoteControlPairingStatusParams;
use rexux_app_server_protocol::RemoteControlPairingStatusResponse;
use rexux_app_server_protocol::RemoteControlStatusChangedNotification;
use rexux_app_server_protocol::RemoteControlStatusReadResponse;
...
use rexux_state::RemoteControlEnrollmentRecord;
...
async fn explicit_remote_control_startup_fails_when_disabled_by_requirements() -> Result<()> {
    ...
    let managed_config_path = rexux_home.path().join("managed_config.toml");
    let socket_path = rexux_home.path().join("app-server.sock");
    let transport =
        AppServerTransport::from_listen_url(&format!("unix://{}", socket_path.display()))?;
    let _rexux_home_guard = EnvVarGuard::set("REXUX_HOME", rexux_home.path().as_os_str());

    let result = timeout(
        STARTUP_TIMEOUT,
        run_main_with_transport_options(
            Arg0DispatchPaths {
                rexux_self_exe: Some(std::env::current_exe()?),
...
async fn listen_off_honors_persisted_remote_control_enable() -> Result<()> {
    let rexux_home = TempDir::new()?;
    let listener = configured_remote_control_listener(rexux_home.path()).await?;
...
async fn remote_control_disable_returns_disabled_status() -> Result<()> {
    let rexux_home = TempDir::new()?;
    let _listener = configured_remote_control_listener(rexux_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_rexux_home(rexux_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_enable_returns_connecting_status() -> Result<()> {
    let rexux_home = TempDir::new()?;
    let mut backend = BlockingRemoteControlBackend::start(rexux_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_rexux_home(rexux_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn stdio_eof_exits_with_remote_control_connection() -> Result<()> {
    let rexux_home = TempDir::new()?;
    let mut backend = ConnectedRemoteControlBackend::start(rexux_home.path()).await?;
    let mut app_server = TestAppServer::builder()
        .with_rexux_home(rexux_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_pairing_start_returns_pairing_artifacts() -> Result<()> {
    let rexux_home = TempDir::new()?;
    let mut backend = PairingRemoteControlBackend::start(rexux_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_rexux_home(rexux_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_client_management_works_while_disabled() -> Result<()> {
    let rexux_home = TempDir::new()?;
    let mut backend = ClientManagementRemoteControlBackend::start(rexux_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_rexux_home(rexux_home.path())
        .without_auto_env()
        .build_initialized()
...
impl ConnectedRemoteControlBackend {
    async fn start(rexux_home: &std::path::Path) -> Result<Self> {
        let listener = configured_remote_control_listener(rexux_home).await?;
...
async fn configured_remote_control_listener(rexux_home: &std::path::Path) -> Result<TcpListener> {
