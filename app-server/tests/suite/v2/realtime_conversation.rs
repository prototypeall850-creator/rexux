...
use rexux_app_server_protocol::ThreadRealtimeAudioChunk;
...
use rexux_app_server_protocol::ThreadRealtimeInitialItem;
use rexux_app_server_protocol::ThreadRealtimeItem;
...
use rexux_app_server_protocol::ThreadRealtimeItemContent;
...
use rexux_app_server_protocol::ThreadTimelineEntry;
...
use rexux_protocol::protocol::RealtimeConversationVersion;
...
const STARTUP_CONTEXT_HEADER: &str = "Startup context from Rexux.";
...
impl RealtimeE2eHarness {
    ...
    async fn start_webrtc_realtime(&mut self, offer_sdp: &str) -> Result<StartedWebrtcRealtime> {
        self.start_webrtc_realtime_with_rexux_response_routing(
            offer_sdp,
            /*client_managed_handoffs*/ None,
            /*rexux_responses_as_items*/ None,
            /*rexux_response_handoff_mode*/ None,
    ...
    async fn start_webrtc_realtime_with_rexux_response_items(
        &mut self,
        offer_sdp: &str,
        ...
        self.start_webrtc_realtime_with_rexux_response_routing(
            offer_sdp,
            /*client_managed_handoffs*/ None,
            /*rexux_responses_as_items*/ Some(true),
            /*rexux_response_handoff_mode*/ None,
            /*delegation_ack_filler*/ None,
            RealtimeConversationVersion::V1,
        )
    ...
    async fn start_webrtc_realtime_with_rexux_response_routing(
        &mut self,
        offer_sdp: &str,
        client_managed_handoffs: Option<bool>,
        rexux_responses_as_items: Option<bool>,
        rexux_response_handoff_mode: Option<RexuxResponseHandoffMode>,
        ...
        let start_request_id = self
            ...
            .send_thread_realtime_start_request(ThreadRealtimeStartParams {
                ...
                rexux_response_item_prefix: rexux_responses_as_items
                    .unwrap_or(false)
    ...
    async fn start_websocket_realtime(&mut self) -> Result<ThreadRealtimeStartedNotification> {
        self.start_websocket_realtime_with_rexux_responses_as_items(
            /*rexux_responses_as_items*/ None,
        )
        .await
    }

    async fn start_websocket_realtime_with_rexux_response_items(
        &mut self,
    ...
    async fn start_websocket_realtime_with_rexux_responses_as_items(
        &mut self,
        rexux_responses_as_items: Option<bool>,
    ) -> Result<ThreadRealtimeStartedNotification> {
        let start_request_id = self
...
async fn realtime_webrtc_start_emits_sdp_notification() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some("multipart/form-data; boundary=rexux-realtime-call-boundary")
    );
...
async fn webrtc_v3_start_posts_live_session_and_joins_without_session_update() -> Result<()> {
    ...
    let started = harness
        .start_webrtc_realtime_with_rexux_response_routing(
            "v=offer\r\n",
            /*client_managed_handoffs*/ None,
            /*rexux_responses_as_items*/ None,
            /*rexux_response_handoff_mode*/ None,
...
async fn webrtc_v1_client_managed_handoffs_disable_automatic_output() -> Result<()> {
    ...
    assert!(
        ...
        "automatic Rexux output should not reach realtime in client-managed handoff mode"
    );
...
#[tokio::test]
async fn webrtc_v1_ignores_rexux_response_handoff_mode() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let started = harness
        .start_webrtc_realtime_with_rexux_response_routing(
            ...
            /*client_managed_handoffs*/ None,
            /*rexux_responses_as_items*/ None,
            /*rexux_response_handoff_mode*/ Some(RexuxResponseHandoffMode::BemTags),
            /*delegation_ack_filler*/ None,
            RealtimeConversationVersion::V1,
        )
        .await?;
...
async fn webrtc_v1_handoff_request_delegates_context_and_manual_append_speaks() -> Result<()> {
    ...
    let started = harness
        .start_webrtc_realtime_with_rexux_response_items("v=offer\r\n")
        .await?;
    assert_eq!(started.started.version, RealtimeConversationVersion::V1);
    assert_call_create_multipart(
        harness.call_capture.single_request(),
...
async fn realtime_automatic_standalone_output_is_item_and_append_speaks() -> Result<()> {
    ...
    let started = harness
        .start_websocket_realtime_with_rexux_response_items()
        .await?;
    assert_eq!(started.version, RealtimeConversationVersion::V2);
    assert_eq!(
...
async fn websocket_v3_routes_handoffs_by_session_mode() -> Result<()> {
    ...
    for (mode, channel_prefixes, texts, expected_channels) in [
        ...
        (
            Some(RexuxResponseHandoffMode::Commentary),
            None,
            [
                "[ANALYSIS]silent context",
                "[COMMENTARY]still working",
        ...
        (
            Some(RexuxResponseHandoffMode::BemTags),
            None,
            [
                "[ANALYSIS]silent context",
                "[COMMENTARY]still working",
...
async fn websocket_v2_background_agent_steering_ack_requests_response_create() -> Result<()> {
    ...
    // Phase 4: release the gated delegated turn. Rexux should then continue
