...
use rexux_app_server_protocol::AppConfig;
use rexux_app_server_protocol::AppLinkConfig;
use rexux_app_server_protocol::AppLinksConfig;
use rexux_app_server_protocol::AppToolApproval;
use rexux_app_server_protocol::ApprovalsReviewer;
use rexux_app_server_protocol::AppsConfig;
use rexux_app_server_protocol::AppsDefaultConfig;
use rexux_app_server_protocol::AskForApproval;
use rexux_app_server_protocol::BrowserUseAccessApprovalLifetime;
use rexux_app_server_protocol::BrowserUseConfig;
use rexux_app_server_protocol::BrowserUseOriginPolicy;
use rexux_app_server_protocol::BrowserUseOriginPolicyConfig;
use rexux_app_server_protocol::BrowserUseRequirements;
use rexux_app_server_protocol::CliAuthCredentialsStoreMode;
use rexux_app_server_protocol::ComputerUseConfig;
use rexux_app_server_protocol::ComputerUseMacosConfig;
use rexux_app_server_protocol::ComputerUseMacosRequirements;
use rexux_app_server_protocol::ComputerUseRequirements;
use rexux_app_server_protocol::ComputerUseWindowsConfig;
use rexux_app_server_protocol::ComputerUseWindowsExeConfig;
...
use rexux_app_server_protocol::ConfigEdit;
use rexux_app_server_protocol::ConfigLayerSource;
...
use rexux_app_server_protocol::ForcedChatgptWorkspaceIds;
...
use rexux_app_server_protocol::ToolsV2;
...
use rexux_protocol::config_types::WebSearchContextSize;
use rexux_protocol::config_types::WebSearchLocation;
use rexux_protocol::config_types::WebSearchToolConfig;
...
fn write_config(rexux_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
async fn config_read_disables_guardian_v2_when_managed_config_requires_guardian_v1() -> Result<()> {
    let rexux_home = TempDir::new()?;
    write_config(&rexux_home, "[features]\nguardianv2 = true\n")?;
    std::fs::write(
        rexux_home.path().join("requirements.toml"),
...
"#,
    ...
    let rexux_home_path = rexux_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(rexux_home_path.join("config.toml"))?;
...
"#,
    ...
    set_project_trust_level(rexux_home.path(), workspace.path(), TrustLevel::Trusted)?;
    let rexux_home_path = rexux_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(rexux_home_path.join("config.toml"))?;
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_rexux_folder: project_config.clone(),
        }
    );
    assert_eq!(
        origins
    ...
    assert_eq!(
        origins
            ...
            .name,
        ConfigLayerSource::Project {
            dot_rexux_folder: project_config,
        }
    );
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let rexux_home = TempDir::new()?;
    write_config(&rexux_home, r#"model = "gpt-user""#)?;
...
async fn config_read_respects_managed_project_root_markers() -> Result<()> {
    let rexux_home = TempDir::new()?;
    write_config(&rexux_home, "model_context_window = 16384\n")?;
    let workspace = TempDir::new()?;
    let ancestor_config = workspace.path().join(".rexux");
    let child = workspace.path().join("child");
    let child_config = child.join(".rexux");
    for dir in [
    ...
    let mut app_server = TestAppServer::builder()
        .with_rexux_home(rexux_home.path())
        .without_auto_env()
        .with_env_overrides(&[("REXUX_APP_SERVER_MANAGED_CONFIG_PATH", Some(&managed_path))])
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
        .await?;
    ...
    let project_layers = layers
        ...
        .into_iter()
        .filter_map(|layer| {
            if let ConfigLayerSource::Project { dot_rexux_folder } = layer.name {
                Some((dot_rexux_folder, layer.config, layer.disabled_reason))
            } else {
                None
            }
        })
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let rexux_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path = AbsolutePathBuf::resolve_path_against_base("config.toml", rexux_home);
...
"#,
    ...
    let write_id = mcp
        .send_config_value_write_request(ConfigValueWriteParams {
            file_path: Some(rexux_home.path().join("config.toml").display().to_string()),
            key_path: "model".to_string(),
            value: json!("gpt-new"),
            merge_strategy: MergeStrategy::Replace,
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let rexux_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(rexux_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
"#,
    ...
    assert_eq!(
        ...
        AbsolutePathBuf::resolve_path_against_base("config.toml", &rexux_home)
    );
    assert_eq!(batch_write.overridden_metadata, None);

    let persisted: toml::Value =
        toml::from_str(&std::fs::read_to_string(rexux_home.join("config.toml"))?)?;
...
fn assert_layers_user_then_optional_system(
    layers: &[rexux_app_server_protocol::ConfigLayer],
