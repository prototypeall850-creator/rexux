...
use rexux_config::types::OtelHttpProtocol;
use rexux_core::config::ConfigBuilder;
use rexux_core_plugins::loader::curated_plugin_cache_version;
...
async fn guardian_review_turns_and_tools_reach_analytics() -> Result<()> {
    ...
    let turns = children
        .iter()
        .filter(|event| event["event_type"] == "rexux_turn_event")
        .map(|event| {
            let params = &event["event_params"];
    ...
    let tools = children
        .iter()
        .filter(|event| event["event_type"] == "rexux_web_search_event")
        .collect::<Vec<_>>();
...
fn set_metrics_exporter(config: &mut rexux_core::config::Config) {
...
async fn app_server_default_analytics_disabled_without_flag() -> Result<()> {
    ...
    let provider = rexux_core::otel_init::build_provider(
        &config,
        SERVICE_VERSION,
        Some("rexux-app-server"),
...
pub(crate) async fn mount_analytics_capture(server: &MockServer, rexux_home: &Path) -> Result<()> {
    Mock::given(method("POST"))
...
pub(crate) async fn wait_for_goal_event(
    ...
    wait_for_matching_analytics_event(server, read_timeout, |event| {
        event["event_type"] == "rexux_goal_event"
...
pub(crate) fn thread_initialized_event(payload: &Value) -> Result<&Value> {
    ...
    events
        ...
        .ok_or_else(|| anyhow::anyhow!("rexux_thread_initialized event should be present"))
}
...
fn write_curated_metrics_plugin(rexux_home: &Path) -> Result<PathBuf> {
    let plugin_id = PluginId::parse(METRICS_PLUGIN_ID)?;
    let plugin_root = PluginStore::new(rexux_home.to_path_buf()).plugin_root(
        &plugin_id,
...
"#,
    ...
    std::fs::write(
        &script_path,
        r#"test -n "$REXUX_PLUGIN_METRICS_OUTPUT"
sleep "${1:-0.3}"
printf '%s' '{"version":1,"measurements":[{"name":"findings","value":3,"dimensions":{"severity":"high"}},{"name":"files_scanned","value":17}]}' > "$REXUX_PLUGIN_METRICS_OUTPUT"
"#,
    )?;

    let curated_repo = rexux_home.join(".tmp/plugins");
...
async fn assert_plugin_measurement_analytics(remote: bool, background: bool) -> Result<()> {
    ...
    skip_if_remote!(
        Ok(()),
        "trusted plugin metrics fixture uses a local Rexux home cache"
    );
    skip_if_wine_exec!(Ok(()), "plugin metrics fixture is Unix-only");

    let rexux_home = TempDir::new()?;
    let script_path = write_curated_metrics_plugin(rexux_home.path())?.canonicalize()?;
    let mut command = vec![
        "/bin/sh".to_string(),
...
"#,
    ...
    mount_analytics_capture(&analytics_server, rexux_home.path()).await?;
    ...
    if remote {
        assert_eq!(
            ...
            rexux_exec_server::REMOTE_ENVIRONMENT_ID
        );
    }
    ...
    for measurement_name in ["findings", "files_scanned"] {
        wait_for_matching_analytics_event(&analytics_server, Duration::from_secs(10), |event| {
            event["event_type"] == "rexux_plugin_measurement_event"
    ...
    let command_event =
        wait_for_matching_analytics_event(&analytics_server, Duration::from_secs(10), |event| {
            event["event_type"] == "rexux_command_execution_event"
                && event["event_params"]["item_id"] == call_id
        })
        .await?;
    assert_eq!(
