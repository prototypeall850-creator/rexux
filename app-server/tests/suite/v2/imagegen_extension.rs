...
use rexux_app_server_protocol::ImageGenerationFailure;
...
async fn standalone_image_generation_returns_saved_path_hint_to_model() -> Result<()> {
    ...
    create_config_toml(rexux_home.path(), &server.uri(), ImagegenTestMode::Direct)?;
...
async fn standalone_image_edit_uses_attached_model_visible_image() -> Result<()> {
    ...
    let (edit_request, _) = run_image_edit_test(|rexux_home| {
        let image_path = rexux_home.join("attached.png");
...
async fn run_image_edit_test(
    ...
    let rexux_home = TempDir::new()?;
    let (arguments, input) = input(rexux_home.path())?;
    let response_mock = responses::mount_sse_sequence(
        &server,
        vec![
            responses::sse(vec![
