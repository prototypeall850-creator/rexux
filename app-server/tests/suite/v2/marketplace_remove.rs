...
fn write_installed_marketplace(rexux_home: &std::path::Path, marketplace_name: &str) -> Result<()> {
...
async fn marketplace_remove_preserves_project_marketplace(user_entry: bool) -> Result<()> {
    ...
    // TestAppServer starts in REXUX_HOME, so make it a trusted project as well.
    std::fs::create_dir_all(rexux_home.path().join(".git"))?;
    std::fs::create_dir_all(rexux_home.path().join(".rexux"))?;
    let project_config_path = rexux_home.path().join(".codex/config.toml");
