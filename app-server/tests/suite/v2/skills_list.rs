...
use rexux_app_server_protocol::SkillScope;
use rexux_app_server_protocol::SkillsChangedNotification;
...
fn write_cached_remote_plugin_with_skill(
    ...
    let plugin_root = rexux_home.join("plugins/cache/openai-curated-remote/linear/local");
...
fn write_cached_local_curated_plugin_with_skill(
    ...
    let plugin_root = rexux_home.join(format!(
...
async fn skills_list_uses_each_cwds_bundled_skills_configuration() -> Result<()> {
    ...
    for (cwd, enabled) in [(disabled_cwd.path(), false), (enabled_cwd.path(), true)] {
        ...
        std::fs::create_dir_all(cwd.join(".rexux"))?;
        std::fs::write(
            cwd.join(".codex/config.toml"),
            format!("[skills.bundled]\nenabled = {enabled}\n"),
        )?;
        set_project_trust_level(rexux_home.path(), cwd, TrustLevel::Trusted)?;
    }
...
async fn skills_list_runtime_enable_refreshes_shared_system_skill_cache() -> Result<()> {
    let rexux_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    let stale_skill_path = rexux_home
        .path()
...
async fn runtime_remote_plugin_toggle_updates_local_curated_plugin_skills() -> Result<()> {
    ...
    write_cached_local_curated_plugin_with_skill(rexux_home.path(), "openai-curated")?;
    std::fs::write(
        rexux_home.path().join("config.toml"),
        format!(
...
async fn skills_list_loads_remote_installed_plugin_skills_from_cache() -> Result<()> {
    ...
    let expected_skill_path =
        std::fs::canonicalize(write_cached_remote_plugin_with_skill(rexux_home.path())?)?;
...
async fn skills_list_skips_cwd_roots_when_environment_disabled() -> Result<()> {
    let rexux_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    write_skill(&rexux_home, "home-skill")?;
...
async fn skills_list_accepts_relative_cwds() -> Result<()> {
    ...
    std::fs::create_dir_all(rexux_home.path().join(&relative_cwd))?;
...
async fn skills_list_preserves_requested_cwd_order(home_plugins_enabled: bool) -> Result<()> {
    ...
    write_skill(&rexux_home, "shared-skill")?;
    write_cached_local_curated_plugin_with_skill(rexux_home.path(), "openai-api-curated")?;
    std::fs::write(
        rexux_home.path().join("config.toml"),
        format!(
            r#"[features]
