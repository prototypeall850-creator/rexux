...
async fn account_switch_reloads_telemetry_collectors_and_preserves_trace_context() -> Result<()> {
    ...
    write_otel_config(rexux_home.path(), &initial_endpoint)?;
    write_chatgpt_auth(
        rexux_home.path(),
    ...
    let mut app_server = TestAppServer::builder()
        ...
        .with_json_logging("rexux_app_server::otel_reloader=info")
    ...
    write_otel_config(rexux_home.path(), &next_endpoint)?;
...
fn write_otel_config(rexux_home: &Path, collector_endpoint: &str) -> Result<()> {
    std::fs::write(
