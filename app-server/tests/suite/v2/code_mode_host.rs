...
async fn code_mode_model_output_uses_structured_host_timing(
    ...
    let mut host = match transport {
        "grpc" => Some(
            Command::new(rexux_utils_cargo_bin::cargo_bin("rexux-code-mode-host")?)
    ...
    let mut builder = TestAppServer::builder()
        ...
        .with_json_logging("rexux_code_mode::timing=info,rexux_core::tools::parallel=info");
...
async fn app_server_shares_flag_selected_grpc_code_mode_host_across_threads() -> Result<()> {
    let host_program = rexux_utils_cargo_bin::cargo_bin("rexux-code-mode-host")?;
    ...
    MockResponsesConfig::new(&model_server.uri())
        ...
        .write(rexux_home.path())?;
    let original_config = std::fs::read_to_string(rexux_home.path().join("config.toml"))?;
