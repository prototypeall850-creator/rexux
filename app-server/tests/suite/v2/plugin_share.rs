...
async fn plugin_share_save_uploads_local_plugin() -> Result<()> {
    ...
    write_remote_plugin_config(rexux_home.path(), &format!("{}/backend-api", server.uri()))?;
    write_chatgpt_auth(
        rexux_home.path(),
    ...
    write_corrupt_plugin_share_local_path_mapping(rexux_home.path())?;
    ...
    let request_id = mcp
        .send_plugin_list_request(PluginListParams {
            ...
            marketplace_kinds: Some(vec![
                rexux_app_server_protocol::PluginListMarketplaceKind::Local,
            ]),
            force_refetch: false,
        })
        .await?;
...
async fn plugin_share_update_targets_updates_share_targets() -> Result<()> {
    ...
    assert_eq!(
        ...
        PluginShareUpdateTargetsResponse {
            ...
            discoverability: rexux_app_server_protocol::PluginShareDiscoverability::Unlisted,
        }
    );
    Ok(())
}
...
async fn plugin_share_update_targets_publishes_workspace_plugin() -> Result<()> {
    ...
    assert_eq!(
        ...
        PluginShareUpdateTargetsResponse {
            ...
            discoverability: rexux_app_server_protocol::PluginShareDiscoverability::Listed,
        }
    );
    Ok(())
}
...
async fn plugin_share_delete_removes_created_workspace_plugin() -> Result<()> {
    ...
    let local_plugin_path = AbsolutePathBuf::try_from(rexux_home.path().join("local-plugin"))?;
    write_plugin_share_local_path_mapping(rexux_home.path(), "plugins_123", &local_plugin_path)?;
...
fn write_remote_plugin_config(rexux_home: &Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
...
fn write_test_plugin(root: &Path, plugin_name: &str) -> std::io::Result<PathBuf> {
    ...
    write_file(
        &plugin_path.join(".rexux-plugin/plugin.json"),
...
fn write_corrupt_plugin_share_local_path_mapping(rexux_home: &Path) -> std::io::Result<()> {
    write_file(
