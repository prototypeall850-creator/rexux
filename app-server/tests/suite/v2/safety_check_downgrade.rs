...
async fn cyber_policy_response_emits_typed_error_notification_v2() -> Result<()> {
    ...
    assert_eq!(
        error,
        ErrorNotification {
            error: rexux_app_server_protocol::TurnError {
                misalignment: None,
    ...
    loop {
        ...
        match notification.method.as_str() {
            "error" => {
                ...
                if payload.error.rexux_error_info == Some(RexuxErrorInfo::CyberPolicy) {
                    error = Some(payload);
                }
            }
            "model/rerouted" => {
...
fn create_config_toml(rexux_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
