use rexux_app_server_protocol::ConfigLayer as ApiConfigLayer;
use rexux_app_server_protocol::ConfigLayerMetadata as ApiConfigLayerMetadata;
use rexux_app_server_protocol::ConfigLayerSource as ApiConfigLayerSource;
use rexux_config::ConfigLayer;
use rexux_config::ConfigLayerMetadata;
use rexux_config::ConfigLayerSource;

/// Converts a config-layer source owned by `rexux-config` into the app-server wire type owned by
/// `rexux-app-server-protocol`.
///
...
pub(crate) fn config_layer_source_to_api(source: ConfigLayerSource) -> ApiConfigLayerSource {
    match source {
        ...
        ConfigLayerSource::Project { dot_rexux_folder } => {
            ApiConfigLayerSource::Project { dot_rexux_folder }
        }
...
/// Converts config-layer metadata owned by `rexux-config` into the app-server wire type owned by
/// `rexux-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
...
/// Converts a config layer owned by `rexux-config` into the app-server wire type owned by
/// `rexux-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
