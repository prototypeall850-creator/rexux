...
use rexux_app_server_protocol::AllowDenyRequirement;
use rexux_app_server_protocol::AutoReviewRequirements;
use rexux_app_server_protocol::BrowserUseAccessApprovalLifetime;
use rexux_app_server_protocol::BrowserUseOriginPolicy;
use rexux_app_server_protocol::BrowserUseRequirements;
use rexux_app_server_protocol::CliAuthCredentialsStoreMode;
use rexux_app_server_protocol::ClientResponsePayload;
use rexux_app_server_protocol::ComputerUseMacosRequirements;
use rexux_app_server_protocol::ComputerUseRequirements;
use rexux_app_server_protocol::ComputerUseWindowsExeRequirement;
use rexux_app_server_protocol::ComputerUseWindowsRequirements;
use rexux_app_server_protocol::ConfigBatchWriteParams;
use rexux_app_server_protocol::ConfigReadParams;
use rexux_app_server_protocol::ConfigReadResponse;
use rexux_app_server_protocol::ConfigRequirements;
use rexux_app_server_protocol::ConfigRequirementsReadResponse;
use rexux_app_server_protocol::ConfigValueWriteParams;
use rexux_app_server_protocol::ConfigWriteErrorCode;
use rexux_app_server_protocol::ConfigWriteResponse;
use rexux_app_server_protocol::ConfiguredHookHandler;
use rexux_app_server_protocol::ConfiguredHookMatcherGroup;
use rexux_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use rexux_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use rexux_app_server_protocol::FeedbackRequirements;
use rexux_app_server_protocol::InAppBrowserRequirements;
use rexux_app_server_protocol::JSONRPCErrorError;
use rexux_app_server_protocol::ManagedHooksRequirements;
use rexux_app_server_protocol::ModelProviderCapabilitiesReadResponse;
use rexux_app_server_protocol::ModelsRequirements;
use rexux_app_server_protocol::NetworkDomainPermission;
use rexux_app_server_protocol::NetworkRequirements;
use rexux_app_server_protocol::NetworkUnixSocketPermission;
...
use rexux_config::HookEventsToml;
use rexux_config::HookHandlerConfig as CoreHookHandlerConfig;
use rexux_config::ManagedHooksRequirementsToml;
use rexux_config::MatcherGroup as CoreMatcherGroup;
use rexux_config::ResidencyRequirement as CoreResidencyRequirement;
use rexux_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use rexux_core::ThreadManager;
use rexux_features::Feature;
use rexux_features::canonical_feature_for_key;
use rexux_features::feature_for_key;
...
impl ConfigRequestProcessor {
    ...
    ) -> Result<rexux_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes = rexux_core_plugins::toggles::collect_plugin_enabled_candidates(
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ...
    ConfigRequirements {
        cli_auth_credentials_store: requirements.cli_auth_credentials_store.map(
            |mode| match mode {
                rexux_config::types::AuthCredentialsStoreMode::File => {
                    CliAuthCredentialsStoreMode::File
                }
                rexux_config::types::AuthCredentialsStoreMode::Keyring => {
                    CliAuthCredentialsStoreMode::Keyring
                }
                rexux_config::types::AuthCredentialsStoreMode::Auto => {
                    CliAuthCredentialsStoreMode::Auto
                }
                rexux_config::types::AuthCredentialsStoreMode::Ephemeral => {
        ...
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(rexux_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(rexux_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
                ...
                .map(|implementations| {
                    implementations
                        ...
                        .map(|implementation| match implementation {
                            rexux_config::types::WindowsSandboxModeToml::Elevated => {
                                WindowsSandboxSetupMode::Elevated
                            }
                            rexux_config::types::WindowsSandboxModeToml::Unelevated => {
...
fn map_computer_use_requirements_to_api(
    computer_use: rexux_config::ComputerUseRequirementsToml,
...
fn map_browser_use_requirements_to_api(
    browser_use: rexux_config::BrowserUseRequirementsToml,
...
fn map_browser_use_origin_policy_to_api(
    policy: rexux_config::BrowserUseOriginPolicyToml,
...
fn map_allow_deny_requirement_to_api(
    requirement: rexux_config::AllowDenyRequirementToml,
) -> AllowDenyRequirement {
    match requirement {
        rexux_config::AllowDenyRequirementToml::Allow => AllowDenyRequirement::Allow,
        rexux_config::AllowDenyRequirementToml::Deny => AllowDenyRequirement::Deny,
    }
}

fn map_browser_use_access_approval_lifetime_to_api(
    lifetime: rexux_config::BrowserUseAccessApprovalLifetimeToml,
) -> BrowserUseAccessApprovalLifetime {
    match lifetime {
        rexux_config::BrowserUseAccessApprovalLifetimeToml::Turn => {
            BrowserUseAccessApprovalLifetime::Turn
        }
        rexux_config::BrowserUseAccessApprovalLifetimeToml::Thread => {
...
) -> rexux_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => rexux_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: rexux_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(rexux_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(rexux_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(rexux_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: rexux_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        rexux_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        rexux_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}

fn map_network_unix_socket_permission_to_api(
    permission: rexux_config::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        rexux_config::NetworkUnixSocketPermissionToml::Allow => NetworkUnixSocketPermission::Allow,
        rexux_config::NetworkUnixSocketPermissionToml::Deny => NetworkUnixSocketPermission::Deny,
    }
}
...
mod tests {
    ...
    use rexux_app_server_protocol::FeedbackRequirements;
    use rexux_app_server_protocol::WindowsSandboxSetupMode;
    use rexux_config::AllowDenyRequirementToml;
    use rexux_config::AutoReviewRequirementsToml;
    use rexux_config::BrowserUseAccessApprovalLifetimeToml;
    use rexux_config::BrowserUseOriginPolicyToml;
    use rexux_config::BrowserUseRequirementsToml;
    use rexux_config::ComputerUseMacosRequirementsToml;
    use rexux_config::ComputerUseRequirementsToml;
    use rexux_config::ComputerUseWindowsExeRequirementToml;
    use rexux_config::ComputerUseWindowsRequirementsToml;
    use rexux_config::ConfigRequirementsToml;
    use rexux_config::ModelsRequirementsToml;
    use rexux_config::NewThreadModelDefaultsToml;
    use rexux_config::WindowsRequirementsToml;
