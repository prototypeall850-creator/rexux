...
use rexux_login::default_client::SetOriginatorError;
...
const NON_ORIGINATING_CLIENT_NAMES: &[&str] = &["rexux_app_server_daemon", "rexux-backend"];
...
impl InitializeRequestProcessor {
    ...
    pub(crate) async fn initialize(
        ...
        let client_mcp_extensions = rexux_mcp::client_mcp_extensions(
            extensions,
        ...
        let rexux_home = self.config.rexux_home.clone();
        if session
            ...
            if let Err(error) = set_default_originator(originator.clone()) {
                match error {
                    ...
                    SetOriginatorError::AlreadyInitialized => {
                        ...
                        // TODO(owen): Once we remove support for REXUX_INTERNAL_ORIGINATOR_OVERRIDE,
        ...
        let user_agent = get_rexux_user_agent();
