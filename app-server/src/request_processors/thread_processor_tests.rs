...
mod thread_processor_behavior_tests {
    async fn forked_from_id_from_rollout(path: &Path) -> Option<String> {
        rexux_core::read_session_meta_line(path)
            .await
            .ok()
    ...
    fn dynamic_tool(
        ...
        match namespace {
            Some(namespace) => {
                DynamicToolSpec::Namespace(rexux_app_server_protocol::DynamicToolNamespaceSpec {
    ...
    fn validate_dynamic_tools_rejects_duplicate_name_in_same_namespace() {
        ...
        let tools = vec![DynamicToolSpec::Namespace(
            rexux_app_server_protocol::DynamicToolNamespaceSpec {
        ...
        assert!(err.contains("rexux_app"), "unexpected error: {err}");
        assert!(err.contains("my_tool"), "unexpected error: {err}");
    }
    ...
    fn summary_from_stored_thread_preserves_millisecond_precision() {
        ...
        let stored_thread = StoredThread {
            ...
            thread_source: Some(rexux_protocol::protocol::ThreadSource::User),
            agent_nickname: None,
            agent_role: None,
            agent_path: None,
            git_info: None,
    ...
    fn collect_resume_override_mismatches_includes_service_tier() {
        ...
        let config_snapshot = ThreadConfigSnapshot {
            ...
            approval_policy: rexux_protocol::protocol::AskForApproval::OnRequest,
            approvals_reviewer: rexux_protocol::config_types::ApprovalsReviewer::User,
            permission_profile: rexux_protocol::models::PermissionProfile::Disabled,
            full_access: false,
    ...
    fn test_thread_metadata(
        ...
        let mut builder = ThreadMetadataBuilder::new(
            ...
            Utc::now(),
            rexux_protocol::protocol::SessionSource::default(),
        );
    ...
    async fn read_summary_from_rollout_preserves_agent_nickname() -> Result<()> {
        ...
        let session_meta = SessionMeta {
            ...
            thread_source: Some(rexux_protocol::protocol::ThreadSource::Subagent),
    ...
    fn summary_from_state_db_metadata_preserves_agent_nickname() -> Result<()> {
        ...
        let summary = summary_from_state_db_metadata(
            ...
            "0.0.0".to_string(),
            source,
            Some(rexux_protocol::protocol::ThreadSource::Subagent),
