...
//! the configured Rexux executable as a subprocess, accepts only valid JSON from
...
use rexux_feedback::FeedbackAttachment;
...
pub(crate) struct DoctorFeedbackReport {
    /// JSON support report to upload as `rexux-doctor-report.json`.
...
/// feedback attachment.
///
/// Failure to spawn Rexux, finish before the timeout, or parse JSON means the
...
pub(crate) async fn doctor_feedback_report(
    ...
    let mut command = doctor_command(&executable, workspace, config.rexux_home.as_path());
...
fn doctor_command(executable: &Path, cwd: &Path, rexux_home: &Path) -> Command {
...
mod tests {
    ...
    fn doctor_command_keeps_missing_workspace_out_of_process_cwd() {
        let rexux_home = tempdir().expect("codex home");
        let workspace = rexux_home.path().join("deleted-workspace");

        let command = doctor_command(Path::new("codex"), &workspace, rexux_home.path());
        let command = command.as_std();

        assert_eq!(command.get_current_dir(), Some(rexux_home.path()));
        assert_eq!(
        ...
        assert_eq!(
            ...
            [("REXUX_HOME".as_ref(), Some(rexux_home.path().as_os_str()))]
        );
    }
