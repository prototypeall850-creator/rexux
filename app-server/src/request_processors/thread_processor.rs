...
use rexux_app_server_protocol::SelectedCapabilityRoot;
use rexux_app_server_protocol::ThreadHistoryMode as ApiThreadHistoryMode;
use rexux_app_server_protocol::ThreadRevertParams;
use rexux_app_server_protocol::ThreadRevertResponse;
use rexux_app_server_protocol::ThreadRevertedNotification;
use rexux_app_server_protocol::ThreadSection;
use rexux_app_server_protocol::ThreadSectionAppearance;
use rexux_app_server_protocol::ThreadSectionMoveParams;
use rexux_app_server_protocol::ThreadSectionMoveResponse;
use rexux_extension_api::ExtensionDataInit;
use rexux_extension_api::ThreadIdleCause;
use rexux_protocol::SanitizedGitUrl;
...
const REXUX_TUI_CLIENT_NAME: &str = "codex-tui";
...
struct ThreadRevertRuntimeSnapshot {
    config: Config,
    settings: RexuxThreadSettingsOverrides,
...
fn collect_resume_override_mismatches(
    ...
    if let Some(requested_review_policy) = request.approvals_reviewer.as_ref() {
        let active_review_policy: rexux_app_server_protocol::ApprovalsReviewer =
    ...
    if let Some(requested_sandbox) = request.sandbox.as_ref() {
        ...
        let sandbox_matches = matches!(
            ...
            (
                ...
                rexux_protocol::protocol::SandboxPolicy::DangerFullAccess
            ) | (
                SandboxMode::DangerFullAccess,
...
fn validate_dynamic_tools(tools: &[DynamicToolSpec]) -> Result<(), String> {
    ...
    fn validate_dynamic_tool<'a>(
        ...
        if let Err(err) = rexux_tools::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
...
impl ThreadRequestProcessor {
    ...
    pub(crate) async fn thread_rollback(
        ...
        if app_server_client_name != Some(REXUX_TUI_CLIENT_NAME) {
    ...
    async fn load_thread(
        &self,
        thread_id: &str,
    ) -> Result<(ThreadId, Arc<RexuxThread>), JSONRPCErrorError> {
    ...
    fn listener_task_context(&self) -> ListenerTaskContext {
        ListenerTaskContext {
            ...
            rexux_home: self.config.rexux_home.to_path_buf(),
    ...
    async fn thread_start_inner(
        ...
        if matches!(
            history_mode,
            Some(rexux_app_server_protocol::ThreadHistoryMode::Paginated)
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<rexux_protocol::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<rexux_app_server_protocol::ThreadStartSource>,
        ...
        let effective_permissions_trust_project = match &effective_permission_profile {
            rexux_protocol::models::PermissionProfile::Disabled
            | rexux_protocol::models::PermissionProfile::External { .. } => true,
            rexux_protocol::models::PermissionProfile::Managed { .. } => {
            ...
            && effective_permissions_trust_project
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                rexux_core::config::set_project_trust_level(
                    &listener_task_context.rexux_home,
        ...
        if let Ok(Some(err)) =
            rexux_core::check_execpolicy_for_warnings(&config.config_layer_stack).await
        {
        ...
        let new_thread = listener_task_context
            ...
            .start_thread(StartThreadOptions {
                ...
                initial_history: match session_start_source
                    .unwrap_or(rexux_app_server_protocol::ThreadStartSource::Startup)
                {
                    rexux_app_server_protocol::ThreadStartSource::Startup => InitialHistory::New,
                    rexux_app_server_protocol::ThreadStartSource::Clear => InitialHistory::Cleared,
                },
                history_mode,
                thread_source,
                dynamic_tools,
        ...
        let NewThread {
            ...
            Err(err) => {
                ...
                return Err(match err.details() {
                    ...
                    RexuxErrorDetails::UnsupportedOperation(message) => {
    ...
    fn build_thread_config_overrides(
        ...
        approval_policy: Option<rexux_app_server_protocol::AskForApproval>,
        approvals_reviewer: Option<rexux_app_server_protocol::ApprovalsReviewer>,
        ...
        ConfigOverrides {
            ...
            approval_policy: approval_policy
                .map(rexux_app_server_protocol::AskForApproval::to_core),
            approvals_reviewer: approvals_reviewer
                .map(rexux_app_server_protocol::ApprovalsReviewer::to_core),
            sandbox_mode: sandbox.map(SandboxMode::to_core),
            rexux_linux_sandbox_exe: self.arg0_paths.rexux_linux_sandbox_exe.clone(),
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = rexux_core::util::normalize_thread_name(&name) else {
    ...
    async fn memory_reset_response_inner(&self) -> Result<MemoryResetResponse, JSONRPCErrorError> {
        ...
        clear_memory_roots_contents(&self.config.rexux_home)
            .await
            .map_err(|err| {
                internal_error(format!(
                    "failed to clear memory directories under {}: {err}",
                    self.config.rexux_home.display()
                ))
            })?;
    ...
    async fn thread_revert_response(
        ...
        let revert_result = self
            .thread_store
            .revert_thread(rexux_thread_store::RevertThreadParams {
                thread_id,
                before_turn_id,
            })
            .await
    ...
    async fn reload_paginated_thread(
        ...
        let thread_state = self.thread_state_manager.thread_state(thread_id).await;
        self.ensure_listener_task_running(thread_id, Arc::clone(&rexux_thread), thread_state)
            .await?;
        let mut thread = self
    ...
    async fn apply_thread_read_store_fields(
        ...
        if include_turns {
            if matches!(
                thread.history_mode,
                rexux_app_server_protocol::ThreadHistoryMode::Paginated
            ) {
                self.thread_store
    ...
    ) -> Result<rexux_app_server_protocol::TurnsPage, JSONRPCErrorError> {
    ...
    async fn thread_resume_inner(
            ...
            .await
        {
            Ok(NewThread {
                ...
                let instruction_sources = rexux_thread.legacy_instruction_sources().await;
                let SessionConfiguredEvent { rollout_path, .. } = session_configured;
                ...
                thread.thread_source = rexux_thread
                    .config_snapshot()
                    .await
                    .thread_source
                    .map(Into::into);
                ...
                let config_snapshot = rexux_thread.config_snapshot().await;
                let (turns_backwards_cursor, items_backwards_cursor) =
                    if matches!(config_snapshot.history_mode, ThreadHistoryMode::Paginated) {
            ...
            && matches!(stored_thread.history_mode, ThreadHistoryMode::Paginated)
        {
            ...
            if let Some(current_path) = current_thread.rollout_path.as_ref()
                && !path_utils::paths_match_after_normalization(
                    rexux_rollout::plain_rollout_path(requested_path).as_path(),
                    rexux_rollout::plain_rollout_path(current_path).as_path(),
                )
            {
                return Err(invalid_request(format!(
    ...
    async fn thread_fork_inner(
        ...
        let source_thread_name = source_thread
            .name
            .as_deref()
            .and_then(rexux_core::util::normalize_thread_name);
        let prepared_fork = if paginated_source {
            let boundary = match (last_turn_id.as_deref(), before_turn_id.as_deref()) {
                (Some(turn_id), None) => {
                    rexux_thread_store::ForkBoundary::ThroughTurn(turn_id.to_string())
                }
                (None, Some(turn_id)) => {
                    rexux_thread_store::ForkBoundary::BeforeTurn(turn_id.to_string())
                }
                (None, None) => rexux_thread_store::ForkBoundary::Latest,
                (Some(_), Some(_)) => unreachable!("fork boundaries are mutually exclusive"),
            };
            Some(
                self.thread_store
                    .prepare_fork(rexux_thread_store::PrepareForkParams {
        ...
        let NewThread {
            ...
            Err(err) => {
                remove_pending_project_metadata(self.thread_store.as_ref(), reserved_thread_id)
                    .await;
                return Err(match err.details() {
                    RexuxErrorDetails::Io(_) | RexuxErrorDetails::Json(_) => {
...
fn deserialize_stored_thread_item(
    item: rexux_thread_store::StoredThreadItem,
...
fn stored_turn_to_api_turn(
    ...
    let error = turn.error.map(|error| TurnError {
        ...
        rexux_error_info: error.rexux_error_info,
...
pub(super) fn core_thread_write_error(operation: &str, err: RexuxErr) -> JSONRPCErrorError {
    match err.details() {
        RexuxErrorDetails::ThreadNotFound(thread_id) => {
            invalid_request(format!("thread not found: {thread_id}"))
        }
        RexuxErrorDetails::InvalidRequest(message) => invalid_request(message.clone()),
        RexuxErrorDetails::UnsupportedOperation(message) => method_not_found(message.clone()),
...
) -> (Thread, Option<rexux_thread_store::StoredThreadHistory>) {
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<rexux_protocol::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(rexux_protocol::protocol::SessionSource::Unknown);
...
fn preview_from_rollout_items(items: &[RolloutItem]) -> String {
    items
        ...
        .find_map(|item| match item {
            RolloutItem::ResponseItem(item) => match rexux_core::parse_turn_item(&item.item) {
                Some(rexux_protocol::items::TurnItem::UserMessage(user)) => Some(user.message()),
                _ => None,
            },
            _ => None,
        })
...
fn build_thread_from_snapshot(
    thread_id: ThreadId,
    session_id: String,
    multi_agent_version: Option<rexux_protocol::protocol::MultiAgentVersion>,
    config_snapshot: &ThreadConfigSnapshot,
    path: Option<PathBuf>,
) -> Thread {
