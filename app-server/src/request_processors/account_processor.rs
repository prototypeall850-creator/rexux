...
const CHATGPT_LOGIN_UNAVAILABLE_MESSAGE: &str = "ChatGPT sign-in is not available in this BYOK build. Use API key login instead.";
use rexux_login::login_with_bedrock_access_keys;
use rexux_model_provider::is_supported_amazon_bedrock_region;
...
impl AccountRequestProcessor {
    ...
    fn current_account_updated_notification(&self) -> AccountUpdatedNotification {
        ...
        AccountUpdatedNotification {
            auth_mode: auth
                .as_ref()
                .map(RexuxAuth::api_auth_mode)
                .map(auth_mode_to_api),
            plan_type: auth.as_ref().and_then(RexuxAuth::account_plan_type),
        }
    }
    ...
    async fn login_chatgpt_response(&self) -> Result<LoginAccountResponse, JSONRPCErrorError> {
        let _ = self;
        Err(invalid_request(CHATGPT_LOGIN_UNAVAILABLE_MESSAGE))
    }
    ...
    async fn get_account_token_usage_response(
        ...
        if let Some(thread_id) = thread_id {
            ...
            let thread_usage = match usage {
                Ok(usage) => Some(rexux_app_server_protocol::ThreadUsage {
                    ...
                    groups: usage
                        ...
                        .into_iter()
                        .map(
                            |group| rexux_app_server_protocol::ThreadUsageBreakdownGroup {
                                model: group.model,
...
mod tests {
    use super::*;
    use rexux_backend_client::TokenUsageProfileDailyBucket;
    use rexux_backend_client::TokenUsageProfileStats;
    use http::StatusCode;
