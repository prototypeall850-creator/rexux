...
use rexux_connectors::ConnectorRuntimeTool;
use rexux_connectors::connector_runtime_context_key;
use rexux_connectors::connector_tool_is_synthetic;
use rexux_connectors::installed_connector_runtime;
...
use rexux_mcp::host_owned_rexux_apps_enabled;
use rexux_mcp::tool_is_model_visible;
    ...
    pub(crate) async fn apps_installed(
        ...
        let result = async {
            ...
            let previous_snapshot = mcp_manager
                ...
                .current_snapshot(config.rexux_home.to_path_buf(), cache_key.clone());
            let snapshot = if force_refresh && runtime_enabled {
                let refresh_result = async {
                    anyhow::ensure!(
                        !mcp_servers.is_empty(),
                        "host-owned MCP server '{REXUX_APPS_MCP_SERVER_NAME}' is not enabled"
                    );
                    ...
                    let rexux_apps_auth_manager =
                        host_owned_rexux_apps_enabled(&mcp_config, auth.as_ref())
                    ...
                    let runtime = McpRuntime::new(McpRuntimeInput {
                        ...
                        rexux_apps_tools_cache_key: cache_key.clone(),
                        ...
                        .await
                    {
                        ...
                        Err(anyhow::anyhow!(
                            "failed to refresh tools for MCP server '{REXUX_APPS_MCP_SERVER_NAME}'"
                        ))
                    };
