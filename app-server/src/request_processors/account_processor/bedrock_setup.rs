...
use rexux_app_server_protocol::AwsCredentialType;
use rexux_app_server_protocol::BedrockAwsProfile;
use rexux_app_server_protocol::BedrockDiscoverParams;
use rexux_app_server_protocol::BedrockDiscoverResponse;
use rexux_app_server_protocol::BedrockEnvironmentCredential;
use rexux_app_server_protocol::BedrockSetupParams;
use rexux_app_server_protocol::BedrockSetupResponse;
...
impl AccountRequestProcessor {
    pub(crate) async fn bedrock_discover(
        ...
        let profiles = rexux_aws_auth::discover_aws_profiles()
            .await
    ...
    pub(crate) async fn bedrock_setup(
            ...
            )
        {
            return Err(invalid_request(
                "Rexux-managed Bedrock credentials are already configured and take priority over AWS environment credentials. Run `codex logout` and try again.",
            ));
        }
        ...
        let profile = match &params {
            BedrockSetupParams::Profile { profile, .. } => {
                ...
                rexux_aws_auth::validate_aws_profile(profile, region)
                    .await
                    .map_err(|err| {
