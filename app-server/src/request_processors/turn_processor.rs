...
use super::*;
use rexux_agent_extension::AgentInvocation;
use rexux_agent_extension::AgentRun;
use rexux_agent_extension::AgentRunner;
...
use rexux_protocol::protocol::AdditionalContextEntry as CoreAdditionalContextEntry;
use rexux_protocol::protocol::AdditionalContextKind as CoreAdditionalContextKind;
use rexux_protocol::protocol::TurnSettingsUpdate;
use rexux_protocol::protocol::TurnSettingsUpdateOutcome;
use rexux_skills::system_cache_root_dir;
...
struct ThreadSettingsBuildParams {
    ...
    sandbox_policy: Option<rexux_app_server_protocol::SandboxPolicy>,
...
impl TurnRequestProcessor {
    ...
    fn review_request_from_target(
        ...
        let hint = rexux_core::review_prompts::user_facing_hint(&core_target);
    ...
    async fn turn_start_inner(
        ...
        if turn_has_input && started {
            ...
            if config_snapshot.is_primary_environment_configured() {
                rexux_memories_write::start_memories_startup_task(
    ...
    async fn build_thread_settings_overrides(
    ...
    ) -> Result<rexux_protocol::protocol::ThreadSettingsOverrides, JSONRPCErrorError> {
        ...
        let approval_policy =
            approval_policy.map(rexux_app_server_protocol::AskForApproval::to_core);
        let approvals_reviewer =
            approvals_reviewer.map(rexux_app_server_protocol::ApprovalsReviewer::to_core);
        ...
        Ok(rexux_protocol::protocol::ThreadSettingsOverrides {
            environments,
    ...
    async fn thread_settings_update_inner(
        ...
        if thread_settings != rexux_protocol::protocol::ThreadSettingsOverrides::default() {
            self.submit_core_op(
                request_id,
                thread.as_ref(),
                ...
                let (message, data, error_type) = match reason {
                    ...
                    NotSubmittedReason::ActiveTurnNotSteerable { turn_kind } => {
                        let (message, turn_steer_error) = match turn_kind {
                            rexux_protocol::protocol::NonSteerableTurnKind::Review => (
                                "cannot steer a review turn".to_string(),
                                TurnSteerRequestError::NonSteerableReview,
                            ),
                            rexux_protocol::protocol::NonSteerableTurnKind::Compact => (
    ...
    async fn prepare_realtime_conversation_thread(
        ...
        request_id: &ConnectionRequestId,
        thread_id: &str,
    ) -> Result<Option<(ThreadId, Arc<RexuxThread>)>, JSONRPCErrorError> {
        ...
        self.submit_core_op(
            ...
            Op::RealtimeConversationStart(ConversationStartParams {
                ...
                flush_transcript_tail_on_session_end: params
                    ...
                    .unwrap_or(false),
                rexux_responses_as_items: params.rexux_responses_as_items.unwrap_or(false),
                rexux_response_item_prefix: params.rexux_response_item_prefix,
                rexux_response_handoff_mode: params.rexux_response_handoff_mode.unwrap_or_default(),
                rexux_response_handoff_channel_prefixes: params
    ...
    async fn start_detached_review(
        ...
        if matches!(
            ...
            rexux_protocol::protocol::ThreadHistoryMode::Paginated
        ) {
            return Err(invalid_request(
    ...
    async fn review_start_inner(
        ...
        match delivery.unwrap_or(ApiReviewDelivery::Inline).to_core() {
            ...
            CoreReviewDelivery::Detached => {
                let review_skill_path = system_cache_root_dir(&self.config.rexux_home)
                    .join("review-agent")
                    .join("SKILL.md");
                let prompt = format!(
