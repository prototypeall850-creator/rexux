//! Measures local REXUX_HOME storage once at standalone app-server startup.
//!
...
const SIZE_BYTES_METRIC: &str = "codex.app_server.rexux_home.size_bytes";
...
pub(crate) fn spawn(
    ...
    tokio::task::spawn_blocking(move || {
        let sizes = match directory_sizes(&rexux_home, &shutdown) {
            Ok(sizes) => sizes,
            Err(error) => {
                tracing::debug!(error_kind = ?error.kind(), "Skipping REXUX_HOME size metrics");
                return;
            }
        };
        for (directory, bytes) in [
            ("rexux_home", sizes.rexux_home),
...
fn directory_sizes(rexux_home: &Path, shutdown: &CancellationToken) -> io::Result<DirectorySizes> {
    let sessions = rexux_home.join(SESSIONS_SUBDIR);
    let archived_sessions = rexux_home.join(ARCHIVED_SESSIONS_SUBDIR);
    let mut sizes = DirectorySizes::default();
    let mut pending = vec![rexux_home.to_path_buf()];
    while let Some(directory) = pending.pop() {
        ...
        for entry in std::fs::read_dir(directory)? {
            ...
            if metadata.is_dir() {
                ...
                sizes.rexux_home = sizes.rexux_home.saturating_add(bytes);
