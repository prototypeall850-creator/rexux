...
mod tests {
    ...
    use rexux_config::ThreadConfigContext;
    use rexux_config::ThreadConfigLoadError;
    use rexux_config::ThreadConfigLoadErrorCode;
    ...
    use rexux_extension_api::NoopExtensionEventSink;
    ...
    async fn strict_refresh_reports_thread_planning_failures() -> anyhow::Result<()> {
        ...
        std::fs::write(
            temp_dir.path().join(rexux_config::CONFIG_TOML_FILE),
    ...
    async fn refresh_config_preserves_thread_mcp_overrides() -> anyhow::Result<()> {
        ...
        let thread_config = initial_config_manager
            ...
            .await?;
        let thread = thread_manager
            .start_thread(rexux_core::StartThreadOptions::new(thread_config))
            .await?
            .thread;
        std::fs::write(
            temp_dir.path().join(rexux_config::CONFIG_TOML_FILE),
...
"#,
        ...
        actual.remove(rexux_mcp::REXUX_APPS_MCP_SERVER_NAME);
...
"#,
    ...
    async fn refresh_test_state() -> anyhow::Result<(
        ...
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            ThreadManager::new(
                &good_config,
                auth_manager.clone(),
                rexux_core::build_models_manager(&good_config, auth_manager.clone()),
                ...
                thread_extensions(
                    ...
                    ThreadExtensionDependencies {
                        ...
                        analytics_events_client: rexux_analytics::AnalyticsEventsClient::disabled(),
                        thread_manager: thread_manager.clone(),
                        goal_service: Arc::new(rexux_goal_extension::GoalService::new()),
                ...
                Arc::new(RexuxHomeUserInstructionsProvider::new(
                    good_config.rexux_home.clone(),
                )),
                /*analytics_events_client*/ None,
                Arc::clone(&thread_store),
                rexux_core::local_agent_graph_store_from_state_db(Some(&state_db)),
        ...
        thread_manager
            .start_thread(rexux_core::StartThreadOptions::new(good_config))
            .await?;
        thread_manager
            .start_thread(rexux_core::StartThreadOptions::new(bad_config))
            .await?;
    ...
    impl ThreadConfigLoader for CountingThreadConfigLoader {
        fn load(
            &self,
            context: ThreadConfigContext,
        ) -> rexux_config::ThreadConfigLoaderFuture<'_, Vec<ThreadConfigSource>> {
