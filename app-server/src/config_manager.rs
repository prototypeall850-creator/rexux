...
use rexux_cloud_config::cloud_config_bundle_loader;
...
use rexux_utils_json_to_toml::json_to_toml;
...
/// Shared app-server entry point for loading effective Rexux configuration.
#[derive(Clone)]
...
impl ConfigManager {
    ...
    pub(crate) fn rexux_home(&self) -> &Path {
        self.rexux_home.as_path()
    }

    pub(crate) fn user_config_path(&self) -> std::io::Result<AbsolutePathBuf> {
        self.loader_overrides.user_config_path(self.rexux_home())
    }
    ...
    pub(crate) async fn load_default_config(&self) -> std::io::Result<Config> {
        ...
        let mut config = ConfigBuilder::default()
            .rexux_home(self.rexux_home.clone())
            .cli_overrides(self.current_cli_overrides())
            .loader_overrides(loader_overrides)
            .fallback_cwd(Some(self.rexux_home.clone()))
    ...
    pub(crate) async fn load_with_cli_overrides(
        ...
        let merged_cli_overrides = cli_overrides
            ...
            .collect::<Vec<_>>();
        let mut config = rexux_core::config::ConfigBuilder::default()
            .rexux_home(self.rexux_home.clone())
    ...
    fn apply_arg0_paths(&self, config: &mut Config) {
        config.rexux_self_exe = self.arg0_paths.rexux_self_exe.clone();
        config.rexux_linux_sandbox_exe = self.arg0_paths.rexux_linux_sandbox_exe.clone();
    ...
    pub(crate) fn new_for_tests(
        ...
        Self::new(
            ...
            Arc::new(rexux_config::NoopThreadConfigLoader),
        )
    }

    #[cfg(test)]
    pub(crate) fn without_managed_config_for_tests(rexux_home: PathBuf) -> Self {
        Self::new_for_tests(
            rexux_home,
            Vec::new(),
