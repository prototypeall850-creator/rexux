...
use rexux_app_server_protocol::Config as ApiConfig;
...
use rexux_app_server_protocol::OverriddenMetadata;
use rexux_app_server_protocol::WriteStatus;
use rexux_config::CONFIG_TOML_FILE;
use rexux_config::ConfigLayerEntry;
use rexux_config::ConfigLayerMetadata;
use rexux_config::ConfigLayerSource;
use rexux_config::ConfigLayerStack;
use rexux_config::ConfigRequirementsToml;
use rexux_config::ShellEnvironmentPolicyFilterRepresentation;
use rexux_config::config_toml::ConfigToml;
use rexux_config::is_structured_feature_path;
use rexux_config::merge_toml_values;
use rexux_config::shell_environment_filter_entry;
use rexux_config::validate_shell_environment_policy_filter_config;
use rexux_core::config::deserialize_config_toml_with_base;
use rexux_core::config::edit::ConfigEdit;
use rexux_core::config::edit::ConfigEditsBuilder;
use rexux_core::config::validate_feature_requirements_for_config_toml;
use rexux_core::path_utils;
use rexux_core::path_utils::SymlinkWritePaths;
use rexux_core::path_utils::resolve_symlink_write_paths;
use rexux_core::path_utils::write_atomically;
...
impl ConfigManager {
    ...
    async fn apply_edits(
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), self.rexux_home()).map_err(
                |err| {
                    ConfigManagerError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_rexux_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_rexux_folder.display(),
        ),
