...
use rexux_app_server_protocol::ThreadQueueChangedNotification;
...
use rexux_extension_api::AgentSpawnFuture;
use rexux_extension_api::AgentSpawner;
...
use rexux_extension_api::InternalSessionSpawnFuture;
use rexux_extension_api::InternalSessionSpawner;
...
use rexux_queue_extension::QueuedItemService;
...
pub(crate) struct ThreadExtensionDependencies {
    ...
    pub(crate) executor_skill_provider: Arc<dyn rexux_skills_extension::SkillProvider>,
...
where
    S: AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = RexuxErr> + 'static,
{
    ...
    if let Some(queue_service) = queue_service {
        rexux_queue_extension::install(&mut builder, queue_service);
    }
    rexux_history_notes_extension::install(&mut builder, auth_manager.clone());
    if let Some(state_db) = state_db {
        rexux_goal_extension::install_with_backend(
            &mut builder,
            state_db,
            analytics_events_client,
            rexux_otel::global(),
            ...
            |config: &Config| GoalExtensionConfig {
                enabled: config.features.enabled(rexux_features::Feature::Goals),
    ...
    rexux_memories_extension::install(&mut builder, rexux_otel::global());
    rexux_mcp_extension::install(&mut builder);
    rexux_mcp_extension::install_executor_plugins(&mut builder, environment_manager);
    rexux_web_search_extension::install(&mut builder, auth_manager.clone());
    rexux_image_generation_extension::install(&mut builder, auth_manager, |config: &Config| {
    ...
    let skill_providers = rexux_skills_extension::SkillProviders::new()
        ...
        .with_orchestrator_provider(Arc::new(
            rexux_skills_extension::OrchestratorSkillProvider::new(),
        ))
...
) -> impl AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = RexuxErr> {
    move |forked_from_thread_id: ThreadId,
          options: StartThreadOptions|
          -> AgentSpawnFuture<'static, NewThread, RexuxErr> {
        let thread_manager = thread_manager.clone();
        Box::pin(async move {
            let thread_manager = thread_manager.upgrade().ok_or_else(|| {
                RexuxErr::UnsupportedOperation("thread manager dropped".to_string())
            })?;
            thread_manager
...
fn internal_session_spawner(
    thread_manager: Weak<ThreadManager>,
) -> impl InternalSessionSpawner<StartThreadOptions, Spawned = NewThread, Error = RexuxErr> {
    move |parent_thread_id: ThreadId,
          options: StartThreadOptions|
          -> InternalSessionSpawnFuture<'static, NewThread, RexuxErr> {
        let thread_manager = thread_manager.clone();
        Box::pin(async move {
            let thread_manager = thread_manager.upgrade().ok_or_else(|| {
                RexuxErr::UnsupportedOperation("thread manager dropped".to_string())
...
#[cfg(test)]
mod tests {
    use rexux_protocol::protocol::ThreadGoal as CoreThreadGoal;
    use rexux_protocol::protocol::ThreadGoalStatus;
    use rexux_protocol::protocol::ThreadGoalUpdatedEvent;
